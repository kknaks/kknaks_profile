...
발주는 직접 하지 않는다. 코디네이터가 `runbook.md` 절차를 따라 워커를 띄우고, 워커는 브리프 한 장만 받는다. 발주 양식(브리프·재개 노트·작업 요약·PR 본문)도 루트 `templates/orchestration/` 에 있다 — 양식은 전부 루트 `templates/` 한 곳이다.

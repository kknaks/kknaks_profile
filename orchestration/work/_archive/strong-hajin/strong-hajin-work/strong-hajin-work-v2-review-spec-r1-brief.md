...
## 추가 재검수 지시 (이번 발주에서 우선)
원장 review-v2-spec-report.md의 F1~6/W1~9 해소 여부를 최소 diff로 확인. 정정 발주 strong-hajin-work-v2-spec-fix1-brief.md 및 v2-spec-fix1-report.md를 읽고 코디 판단 우선. 기존 검수 본문 보존, 같은 보고서에 재검수1차 절만 추가. 제품문서 쓰기 금지. 최신 SPEC003 v0.2.0 대상 해시 기록.
F1은 기존 tasks+타인지정이 원래 수평요청이므로 입구보존 타당. OQ204는 원문 승인범위상 배정현행유지로 이미 해소, 재승인 요구하지 않음. OQ203은 제출과 최종승인 의미를 나눠 기존 submit허용을 유지한 계약 모순 해소 확인. F4는 실제 actor와 system:meeting 출처를 전수 근거로 구분.
새 OQ205/206이 진짜 사용자질문인지 특히 확인: 기존 승인 없는 완료행의 기존사실 존중은 기술호환으로 해소할 수 있는가? system:meeting이 실제 human 확인자 없는지 O29/30 호출경로·승인자 resolve를 읽기전용 대조. 인간이 승인할 수 없는 상태를 '기본값'으로 확정 금지. 질문이 남으면 영향범위/선택지/추천을 구체적으로 보고. 원문미정 OQ203은 질문으로 남겨도 되며 기존동작보존과 구별. 코드/DB실행 여전히 금지.
W10 index/log는 코디 별도라 writer 실패로 세지 말 것. 이미 통과한 원문전수와 형식을 불필요 재조사하지 않고 새 변경으로 깨진 곳 확인. 마지막 완료에는 WORK002 착수 가능 범위와 실제 막힌 범위를 구분.
...
orca orchestration send \
  --to term_29f98b02-b4b2-4735-8daa-81708cb3dfdc --from term_d418b4c2-c8e6-449e-bbde-3acfe3bed2db \
  --type worker_done \

...
# [frontend] WORK002 v2 frontend 전량 구현
...
- frontend/
- /Users/kknaks/orca/workspaces/kknaks_profile/strong_hajin/orchestration/work/strong-hajin-work/v2-frontend-implementation-report.md (완료리포트만, 코드워커의 문서쓰기 예외)

## 6. 구현 단계

1. WORK Phase7 A부터 실제 .design-sync/screens/MyWork.html·참조 JSX/CSS/JS·docs/design/ 를 읽고 구현. 분석자료는 /Users/kknaks/orca/workspaces/kknaks_profile/strong_hajin/reference/2026-09-10-sc-meeting/design-change-2026-09-17 읽기전용. 생성/표 뼈대/글리프를 먼저 진행하며 BE 준비 기다림으로 작업중단 금지.
2. Phase7 B~E 전량: 3탭·표·필터·기한캘린더·수신레일·2열생성·상세흐름. 기존 기능 보존. 상세 중심+직속하위, 담당교체/요청수락/거절/철회/취소동의/조건동의/재개/완료회차/하위차단이름/재요청/숨김 전부 연결. mock은 테스트/구현중 스토리용이고 최종제품 가짜데이터 금지.
3. api.ts/viewModels.ts/labels.ts FE소유, BE REST shape는 SPEC 및 실제코드와 맞추고 질문은 코디에게. BE생산 전 추정 endpoint/필드로 확정하지 않는다. 상태문자열로 권한추론금지, envelope 소비. completion_submitted 외부타입 제거·derived 소비를 누락하지 않는다.
4. Phase8 FE회귀·UX U1~15 데이터/명령소비와 시안대조 정적점검, make frontend-test + 타입 검증. 브라우저 E2E 실행금지. 보고서에 각 Phase 작업/검증/미완료 및 사용자E2E 대상 분리. WORK파일 수정금지.

...
## 5. allowed_paths
/Users/kknaks/orca/workspaces/kknaks_profile/strong_hajin/orchestration/work/strong-hajin-projects/review-tauri-spec-r2-report.md 하나만 작성. 나머지는 읽기 전용.
...
## 이번 재검수 지시 — 앞 지시와 충돌하면 이 절이 우선

- /Users/kknaks/orca/workspaces/kknaks_profile/strong_hajin/orchestration/work/strong-hajin-projects/tauri-spec-fix1-brief.md의 코디 판단 A~H를 먼저 읽어라.
- /Users/kknaks/orca/workspaces/kknaks_profile/strong_hajin/orchestration/work/strong-hajin-projects/tauri-spec-fix1-report.md와 SPEC-006 v0.2.0, 적용 조사 §10을 읽고 F-1~F-5/W-1~W-10 처분을 독립 검증한다.
- 이전 검수의 TTL 3배·청크 콜백 무지연 보장은 채택 안 했다. 네이티브 소유 수명으로 바꾸는 것은 코디 승인 방향이다. 기존 5커맨드·참조계수를 유지하라고 요구하지 마라. 문제를 닫았는지와 새 빈틈을 본다.
- 이전 W-3는 낡은 WebKit 자료에 기댄 지적이었다. https://webkit.org/blog/16574/webkit-features-in-safari-18-4/ 의 WebM/Opus 추가를 반영. 대상 기기 OS·웹뷰 판 실측은 여전히 필요하다.
- 코디가 먼저 발견: AC-T17은 '앱 안에서 화면만 바뀌면 점유가 풀리지 않는다'인데 L-10은 마이크를 멈추는 앱내 전환에서 웹이 해제한다고 한다. '셸이 자동 해제하지 않음'과 '웹의 실제 녹음 종료 해제'를 구분해야 한다. 이 모순을 판정해라.
- 수동 sleep 복귀 L-14가 실제 녹음 생존을 판정하는 방법을 발명했는지, L-12 웹 정지 한계와 AC-T19가 충돌하는지.
- acquire/release 요청 역전·늦은 acquire·문서 세대교체·취소된 외부 네비게이션·CloseRequested 취소 시 점유 보존. 신규 세션 키가 해결하는 범위와 해결 못하는 범위 구분. 플랫폼 구현 상세 과잉 요구는 피하고 관찰 가능한 계약으로 검수.
- 실패(degraded) 응답과 다른 세션이 남은 release의 on/off 의미, 세션 키 충돌/재사용, 표시할 경고 누락.
- '타이머 만료 경로를 없앰'은 맞지만 '녹음 중 절대 풀리지 않는다'는 더 큰 주장이다. OS/프로세스/사용자 사건까지 포함하는 과장인지 확인.
- OQ/실측 미완은 독립 항목이며 문서 계약 오류와 구분. 문서 승인과 구현/배포 승인을 혼동하지 않는다.
- 기존 검수 보고서는 보존. 이번 보고서 하나만 작성: /Users/kknaks/orca/workspaces/kknaks_profile/strong_hajin/orchestration/work/strong-hajin-projects/review-tauri-spec-r2-report.md

...
- 문서 SSOT 는 brief §1 에 적힌 경로(문서 리포 코디 워크트리 절대경로)만 read-only 로 참조한다
...
## 탐색 경로 (Strong_hajin 루트 기준, 실 파일은 Glob 으로 확인)
```
...
## 금지 사항
- canonical(`/Users/kknaks/git/toy_pr2/Strong_hajin`) 수정 금지 — 워크트리에서만 작업
- `backend/`·`docs/`·문서 리포 수정 금지

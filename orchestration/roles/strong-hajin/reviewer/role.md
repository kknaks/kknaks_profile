...
| 모드 | 대상 | 핵심 질문 |
|---|---|---|
| **planner 리뷰** | 문서 리포 `para/projects/summer-star/strong-hajin/` diff | 린트 0 ERROR 인가? 파이프라인·frontmatter 를 지켰나? 도메인 사실에 출처가 있나? |
| **backend 리뷰** | Strong_hajin `backend/` diff | modules/platform/entrypoints 경계를 지켰나? envelope 계약을 server 가 만드나? 있는 operation 을 썼나? |
| **frontend 리뷰** | Strong_hajin `frontend/` diff | 권한을 envelope 로만 판단하나? `api.ts` 밖 fetch 가 없나? 기존 컴포넌트·viewModel 을 재사용했나? |

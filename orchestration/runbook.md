...
- **코디 세션 1개 = 작업 1개 = `work/<slug>/` 1개 = 레포당 워크트리 1개 = 레포당 브랜치 1개.**
  - **새 작업은 사용자가 orca 에서 새 코디를 띄울 때 생긴다.** 그 외에는 새 작업이 아니다.
  - **WORK 번호는 작업 단위가 아니다.** WORK-001·002·003 은 한 작업 안에서 순서대로 만드는 단계다 —
    WORK 가 늘어도 slug·워크트리·브랜치는 **늘지 않는다.**
  - **`new-work.sh` 는 작업 시작에 딱 한 번 돈다.** 워커를 더 태울 때는 다시 돌리지 말고
    `new-work.sh <p> <같은 slug> --workers <w>` 로 **같은 `work/<slug>/` 에 브리프만 추가**한다
    (`brief_suffix` 가 워커별로 구분해 준다).
  - 문서 워커든 코드 워커든 같다. 코드 워커는 코드 레포에 **워크트리 하나**를 갖고, 그 안에서 WORK 를 이어 쌓는다.
  - (2026-09-05 계기 — WORK 번호마다 slug 를 파서 work 디렉토리 3개·워크트리 2개·브랜치 2개가 생겼고,
    새로 판 워크트리가 `origin/main` 기준이라 앞 WORK 의 코드가 없는 **빈 트리에 워커를 태웠다.**)
...
- **구현은 두 판이다: 1루프(계약대로 세운다) → 사용자 E2E → 2루프(눈으로 보고 고친다).**
  - **자동 검증이 못 보는 층이 있다.** 2026-09-21 캘린더 작업은 1루프가 **검수를 전부 통과**했는데(SPEC 4판·코드 4판·테스트 전부 초록) 사용자가 화면을 열자 **다섯이 나왔다** — 같은 것이 두 번 뜨고, 겹친 것이 밑에 깔려 안 보이고, 막아야 할 것이 안 막혔다.
  - **계약이 맞는지는 테스트가 본다. 쓸 만한지는 못 본다.**
  - 그래서 **1루프에서 픽셀·간격·문구를 완벽히 맞추려 시간 쓰지 않는다** — 2루프가 잡는다.
  - **마지막 검수에 「사용자가 실물에서 만날 자리」를 모으게 한다.** 리뷰어는 코드를 읽으므로 「막히진 않는데 사람 눈에 이상해 보일 자리」를 짚을 수 있다.
...
|---|---|---|
| 문서 | 별도 spec 레포 — planner 워커 발주 | 같은 레포 `para/` — 코디네이터가 직접 쓰거나, **문서 워커를 코디 워크트리에 태워** 발주한다(`workspace: "coordinator"`, 아래) |
...
| allowed_paths | 코드 레포 경로만 | **코드 워커**는 문서 경로(`para/`·`orchestration/`)를 **반드시 배제** — 문서는 코디 소유. 워커가 work 문서의 Phase Status 도 갱신하지 않는다(보고로 대신). **코디 워크트리 탑승 문서 워커**는 브리프가 지정한 문서 파일만 |

단일 레포형에서 코디 브랜치의 문서가 미커밋이면 워커 참조가 코디 워크트리의
**작업 중 상태**를 읽게 된다 — 발주 전에 spec 을 닫고(버전 확정), 발주 후에는
계약 절을 함부로 고치지 않는다. 고치면 워커에게 즉시 전파한다.

#### `workspace: "coordinator"` — 문서 워커를 코디 워크트리에 태운다 (단일 레포형 전용)

회사 작업(분리형)은 문서 레포·코드 레포가 따로 있으므로 **해당 없다.** 개인 단일 레포형에서
문서 작업(baseline·decision·분석 리포트)을 워커에게 발주할 때만 쓴다.

- config `workers.<이름>.workspace: "coordinator"` → `new-work.sh` 가 그 워커의 워크트리를
  만들지 않고, 워커 터미널을 **코디 워크트리에 직접** 띄운다.
- 산출물이 코디 워크트리에 바로 생긴다 — 검증은 `git status`·`git diff` 실물로, 커밋·PR 은 코디 몫
  (워커 커밋·push 금지는 그대로).
- 코디와 같은 트리를 공유하므로 **직렬 발주만** — 같은 워크트리에 워커 둘을 병렬로 태우지 않는다.
- 작업 단위는 **§1 고정 규칙**을 따른다 — coordinator 탑승이면 워크트리가 코디 브랜치 하나뿐이라
  그 브랜치에서 도는 일 전체가 slug 하나다. _RESUME 도 작업당 1개.
  (2026-09-03 계기 — designer/writer 를 slug 2개로 쪼개서 재개점이 갈라졌다.)
- (2026-09-03 계기) 문서 워커가 개별 워크트리에 산출물을 남긴 채 워크트리가 정리되어
  BASE·DEC 문서와 분석 리포트가 통째로 유실됐다. 문서 워커는 산출물이 코디 트리에 바로
  떨어지게 태운다.
...
#### 브리프는 **자립형**으로 쓴다 (2026-09-21 캘린더 작업 — 터미널이 여섯 번 죽었다)

**워커 터미널은 자주 죽는다.** 한 작업 안에서 writer·reviewer·backend·frontend 가 번갈아
나갔고, 새로 띄운 워커는 앞 판의 맥락이 **하나도 없다.**

- **브리프 머리에 「읽을 것」 목록을 둔다** — 역할 문서 · 코드 레포 `AGENTS.md` ·
  **앞 판이 남긴 리포트**. 「너는 맥락이 없다」를 명시한다.
- **워커 리포트를 `work/<slug>/` 에 쌓는다.** 보고를 인박스에만 두면 워커가 죽을 때 같이 죽는다.
  워커가 scratchpad 에 쓴 리포트는 **`work/` 로 옮긴다.**
...
- **⚠ 워커를 띄울 때마다 코디handle 이 살아 있는지 확인한다.** `$ORCA_TERMINAL_HANDLE` 은 세션이 재연결되면 **stale 이 되는데 env 값은 그대로다.** `new-work.sh` 는 **발주 시점에 한 번**만 검사하고 **세션 도중에 바뀌는 것은 아무도 안 본다** — 2026-09-21 캘린더 작업에서 실제로 죽은 핸들로 워커들이 보고하고 있었다. 발주가 `terminal_handle_stale` 로 튕겨서야 알았고, **안 잡았으면 다음 완료 보고부터 통째로 유실됐다.**
  - 찾는 법: `orca terminal list --json` 에서 **이 세션의 제목**으로 고른다(워크트리 경로만으로는 **남의 세션을 집는다** — `new-work.sh` 가 자동 추측을 안 하는 이유다).
  - 바뀌었으면 **브리프 전부와 `_RESUME.md` 를 한 번에 갱신**하고 **살아 있는 워커에게 정정을 보낸다.**
  - 보고가 조용히 유실되면 「워커가 조용하다」로 보인다. 원인이 겉으로 안 드러난다.
- **지시는 파일로, 메시지는 포인터 + 세 줄.** 긴 `terminal send` 는 **앞부분이 잘린다** — 2026-09-21 에 2742·3126 bytes 가 두 번 잘렸고, 한 번은 워커가 지시서 파일로 축을 복원해서 살았다. **그 파일이 없었으면 엉뚱한 것을 검수했다.** 재발주·추가 지시는 `work/<slug>/<이름>-instructions.md` 에 쓰고 메시지에는 경로와 요지만 담는다. **1000 bytes 를 넘기지 않는다**는 감각으로.
...
- **워커 수치를 그대로 쓰지 않는다.** 최소한 **관문이 되는 명령 하나**는 코디가 직접 돌린다.
- **기존 flaky 는 한 번 판정하고 파일로 남긴다** — `work/<slug>/flaky-baseline-evidence.md`. 이후 워커·검수가 그 판정을 인용한다. 안 남기면 **「이번 계약이 초록이다」가 증명이 안 닫힌다** — 2026-09-21 에 검수가 그것을 지적했고 맞는 말이었다. 기준선(착수 전) 수치를 먼저 재 두는 것이 그 시작이다.
- **워커가 `[질문]` 을 보내면 그 판이 끝나기 전에 답한다.** 늦으면 다음 판에 반영 비용이 붙는다 — 2026-09-21 에 두 번(K18·K24) 답이 늦어 검수가 「반영 안 됨」으로 FAIL 을 줬다. **워커 잘못이 아니다.** 답이 늦었으면 **검수 발주에 「이건 내가 뒤에 정했다, 워커 잘못 아니다」를 적는다.**
...
**아카이브 직후 — 회고를 개념으로 소화한다 (회사/분리형 프로젝트의 컨셉 입구).**
회고 §2 의 각 항목을 `para/areas/area.md` §3.4 맵에서 찾아 **기존 노트를 보강**하고(없을 때만 신규 + 맵 등재),
회고 §2 항목에 `[[개념]]` 역링크를 남긴다. 개인/단일 레포형은 이 단계가 앞에 있다 —
**decision 을 닫기 전에** 판단 근거가 된 개념을 같은 방식으로 보강·링크한다(그래서 회고 시점엔 이미 있다).
...
- **E2E 스택은 사용자가 볼 때만 띄운다.** `make local-stack` 이 프로세스 열여덟을 물고, 워커 터미널이 각각 Claude 를 문다. 2026-09-21 에 `load average 19.39` 를 봤고 스택이 **메모리 부족으로 두 번 죽었으며** 그 부하가 **테스트 flaky 도 악화시켰다.** 구현 중에는 안 띄우고, 테스트 수치를 비교할 거면 **내린 뒤** 잰다.
- **스택을 띄우기 전에 포트를 본다.** 워크트리마다 compose 프로젝트명이 달라 **같은 포트로 새 컨테이너**를 띄우려 한다. 앞 작업 컨테이너가 살아 있으면 `COMPOSE_PROJECT_NAME=<앞 작업 프로젝트명> make local-stack` 으로 **재사용**한다. **다른 프로젝트가 쓰는 포트는 건드리지 않는다.**

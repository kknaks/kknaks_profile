...
- **작업 단위 = 워크트리 1개 = slug 1개.** coordinator 탑승에서는 워크트리가 코디 브랜치 하나뿐이므로,
  그 브랜치에서 도는 일 전체가 slug 하나다. **발주 1회 = slug 1개가 아니다** — 워커를 더 태울 때는
  새 slug 를 파지 말고 `new-work.sh <p> <같은 slug> --workers <w>` 로 같은 `work/<slug>/` 에
  브리프를 추가한다(`brief_suffix` 가 워커별로 구분해 준다). _RESUME 도 작업당 1개.

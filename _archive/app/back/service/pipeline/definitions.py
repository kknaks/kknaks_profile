...
DAILY_COMMIT = Pipeline(
    ...
    stages=(
        ...
        #
        # **작성은 이 게이트가 한다.** 종전 설계에는 `compose` auto 스테이지가 초안을
        # 만들고 게이트는 보여 주기만 했는데, 재생성(SPEC-013 S-3)이 "서술만 다시
        # 만든다" 라 게이트도 작성 능력을 가져야 했다. 그러면 auto 쪽 작성은 첫 회에만
        # 쓰이고 재생성마다 게이트가 다시 만드는 중복이 된다. 유튜브의 `summarize`(auto)
        # 도 판단 재료만 만들고 노트 작성은 게이트가 한다 — 그 대칭을 지켰다.
        Stage("daily", "gate"),
    ),
)

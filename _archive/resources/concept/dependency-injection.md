...
- [[autowired]] — 주입할 대상을 타입으로 찾게 하는 표식

## 출처

- [[2024-10-04-Day88]] — 아흐레 뒤. **주입을 「적는 일」 자체가 사라진다** — `<property ref="..."/>` 를 손으로 잇던 것이 `@Autowired` 한 줄이 되고, 컨테이너가 타입으로 후보를 찾는다. 이 노트가 정리한 세 경로(생성자·세터·메서드)가 그대로 세 자리(생성자·세터·**필드**)로 다시 나오는데, 필기가 **필드 주입을 두고 「캡슐화를 위배하는 측면이 있어 "객체지향을 파괴하는 방식"이라는 비난을 받는다」**고 적은 것이 이 노트의 「생성자 주입은 만들어진 순간부터 완전하다」와 이어진다. 그리고 Day64 에서 값을 치렀던 문제 — 컨테이너가 인수 없는 생성자로만 만든다 — 가 여기서 풀린다: 기본 생성자가 없으면 **매개변수 있는 생성자를 찾아 부른다** → [[autowired]]

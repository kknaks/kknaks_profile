...
- [[thymeleaf]] — 같은 문제를 기호로 나눠 푸는 다음 세대

## 출처

- [[2024-10-22-Day99]] — 여섯 주 뒤. **표현식이 한 종류가 아니어도 된다는 것이 보인다.** EL 은 `${...}` 하나로 값·URL·메시지를 다 다루는데, Thymeleaf 는 `${}`(모델)·`#{}`(메시지)·`@{}`(URL)·`*{}`(선택 객체)로 **기호를 갈라** 읽는 즉시 종류가 드러나게 했다. `${a ?: b}` 엘비스 연산자와 `|...|` 리터럴 대체처럼 EL 에 없던 표기도 나온다 → [[thymeleaf]]

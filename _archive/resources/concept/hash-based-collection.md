...
- [[time-complexity]] — 자료구조 선택이 곧 복잡도 선택인 자리
- [[sorting]] — 세는 것과 줄 세우는 것이 갈리는 축
...
- [[2025-05-08-Pr1620]] — 포켓몬 도감. **양방향 조회**(이름→번호, 번호→이름)를 map 으로 풀고, 「map 구조는 탐색 시 **O(logN)** 으로 가능하다」로 왜 배열이 아닌지를 적었다. 입력이 숫자인지 문자인지 판별해 방향을 가르는 처리도 함께 있다 → [[number-parsing]]
- [[2025-05-08-Pr9375]] — 패션왕 신해빈. **종류별로 개수를 세는** 전형 — map 에 「종류 → 개수」를 모은 뒤 (개수+1) 을 곱한다. 곱셈이 답이고 map 은 세는 도구다
- [[2025-05-08-Pr1940]] — 주몽. 합이 M 이 되는 짝을 찾는 문제를 **「필요한 짝이 이미 있었나」를 묻는 것**으로 바꾼다 — 찾기가 O(1)·O(log n) 이라 성립하는 접근이다
- [[2025-07-03-pr2910]] — 빈도 정렬. **세는 것은 map, 줄 세우는 것은 정렬**이라는 역할 분담이 그대로 드러난다 — map 으로 횟수를 모은 뒤 비교 함수로 순서를 만든다 → [[sorting]]

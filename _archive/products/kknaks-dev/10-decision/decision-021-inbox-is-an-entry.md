...
#### 삭제가 커밋되는 시점은 **항목이 종결될 때**다

작업트리에서 지우는 것만으로는 부족하다. **그 삭제가 커밋되지 않으면** `reset --hard` 한 번에 파일이 되살아나고 위 약속이 깨진다. 종결이 셋이라 회수 자리도 셋이다.

| 종결 | 회수 방식 |
|---|---|
| `published` | 산출물과 **같은 커밋**으로 (`remove` 액션) |
| `discarded` | 회수 커밋 하나 |
| `deleted` | 〃 |

발행만 계획 안에 있는 이유는 **원자성**이다 — 노트가 나가는 커밋과 원본이 사라지는 커밋이 갈리면 「발행됐는데 입구에 원본이 남은」 중간 상태가 생긴다. 폐기·삭제는 함께 나갈 산출물이 없어 그 문제가 없다.

실패(`prepare_failed`·`publish_failed`)는 **종결이 아니다.** 같은 노트를 다시 push 하면 기존 항목에 합류하고(`ITEM_PENDING_STATUSES`), 그 항목이 나중에 발행되거나 폐기될 때 회수가 돈다.

**회수는 종결을 되돌리지 않는다.** 폐기는 이미 확정된 사람의 결정이라, git 이 실패해도 예외를 올리지 않고 로그만 남긴다 — 파일 하나가 커밋 안 된 채 남는 것이 항목 상태가 어긋나는 것보다 낫다. 지울 커밋 이력이 아예 없으면(로컬에서 만들고 push 하지 않은 노트) **git 을 부르지도 않는다.**

...
### Phase 6 — 죽은 세션에서 stateless 로 되돌아온다

- **Status**: DONE
- **설명**: P5 가 꺼낸 결함이고 **이 발주가 키운 것**이다. D4 를 문장이 아니라 코드로 참이 되게 한다.
- **작업**:
  - [x] 실행 실패가 「세션이 사라졌다」인지 메시지로 판정한다 (`SESSION_LOST`)
  - [x] 그 참조를 **항목 전체**에서 지운다 — 세션은 실행기 파일시스템 하나에 살아서 한 게이트에서 죽었으면 전부 죽었다
  - [x] **한 번 다시 제출한다.** 지운 뒤라 stateless 로 나가고 같은 사유로 또 실패할 수 없다
  - [x] 메시지에서 뽑은 id 에만 기대지 않는다 — **지금 물게 되어 있는 값**도 함께 지운다
- **검증**:
  - [x] 죽은 세션 → 자동 재제출 → 두 번째 수확에서 검토 대기
  - [x] 실패 기록(`SESSION_LOST`)이 남는다 (S-4 회귀)
  - [x] 파싱한 id 가 실제 세션과 **달라도** 복구된다
  - [x] 무관한 실패(timeout)는 이 경로를 안 탄다 — 평소대로 `failed`
  - [x] **가드를 깨뜨려 본다** — 감지를 끄니 2건이 실패하고 되돌리니 통과했다
- **완료 증거**: 985 passed.

  **되돌이표를 규칙이 아니라 구조로 막았다.** 「최대 한 번만 자동 재시도」라는 카운터를 두지 않았다 — 참조를 지운 뒤에는 물 세션이 없어 **같은 사유로 실패하는 것이 불가능**하다. 카운터는 잊히지만 구조는 안 잊힌다.

  판정은 실행기 메시지 문자열에 기댄다(구조화된 코드가 없다). 문구가 바뀌면 이 감지가 조용히 멎으므로, 그때는 `SESSION_LOST` 가 안 찍히고 `EXECUTION_FAILED` 가 다시 늘어나는 것으로 드러난다.

...
| Blocker | 없음 |
| Next | **코드 검토.** 그 다음이 배포다 — ① compose 볼륨·env 반영(`REPO_CACHE_DIR`·`KNOWN_COMMIT_IDENTITIES`·`GH_TOKEN_COMPANY`) ② 마이그레이션 `0007`·`0008` ③ showcase 시드 1회 — `needs_detail` 이 몇으로 나오는지가 **company 레포에 손으로 채울 목록**이다 ④ 13개 최초 클론(~321MB) ⑤ 하루치 실발행 완주. **⚠ 배포 전 확인**: 공용 dev DB 가 이미 `0008` 이라 main 브랜치 back 을 재시작하면 모르는 리비전에서 막힌다 |
...
- **작업**:
  ...
  - [ ] 배포 — 볼륨·env 반영, 서버에서 13개 최초 클론 (~321MB). **코드 검토 뒤로 미뤘다**
  - [ ] 하루치 실발행 완주
- **검증**:
...
- **완료 증거**: 작업 16 중 14. 남은 둘(배포 · 실발행)은 미작성.
  ...
  - **커밋 `9382d27` — 진짜 커밋 조사 + 구 잔디 경로 제거.** `collect_commits.py`·`collect_git.py`·`collect_common.py`·`review_alert.py`(신설) · `main_job.py`·`llm.py`·`upsert.py`(**삭제**) · `inputs.py`·`scheduler.py`·`driver.py`·`bootstrap.py` · 테스트 둘 신설. 777 → **783 passed**.
    ...
    - **diff 본문을 실어 보낸다.** 상한(레포 32KB·커밋 8KB)이 존재하는 이유가 그것이고, BL-004 이 "병목은 프롬프트가 아니라 **입력**" 이라 한 것도 그 말이다. 넘치면 본문만 버리고 **파일명·증감 라인은 절대 안 버린다** — 무엇을 건드렸는지는 남고 어떻게 고쳤는지만 사라진다. ⚠ SPEC-011 §4 `commits[]` 줄에 `diff` 가 없다 — 상한 계약이 본문을 전제하므로 **스펙 쪽이 생략된 것으로 읽었다.** 다음 개정에서 적는다.

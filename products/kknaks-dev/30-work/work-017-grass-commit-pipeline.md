...
| Owner | kknaks |
| Status | in_progress |
| Progress | 72% (P1·P2·P3 done · **P4·P5 in_progress** — **P4 는 작업 다섯이 전부 들어왔고 타입검사·빌드가 통과했다.** P5 는 작업 16 중 다섯(레지스트리·볼륨·config·클론 관리). **여기서 멈춘 이유 둘**: ① P4 검증 여섯 중 셋이 **떠 있는 화면에서 사람이 눌러 봐야** 확인된다 — 코드가 컴파일되는 것과 사람이 그것으로 승인할 수 있는 것은 다르다 ② P5 의 남은 열하나가 이 발주의 무게 전부다 — 진짜 수집·identity·상한·스케줄러·구 경로 제거·배포·실발행. WORK-015 의 80%("BE·FE 전부 done, 실전 e2e 만 남음")가 눈금인데, 여기는 **BE 가 아직 더미 수집 위에 서 있어** 그보다 앞설 수 없다) |
| Branch/PR | `work-017-p2` |
| Blocker | 없음 |
| Next | ① identity 조회 + drift 판정 + Slack(U-2) ② `collect_commits.py` — `git log --all --numstat` · KST 경계 · tree-hash dedupe · 영역 분해 · `counts` ③ 입력 상한 적용 + `truncated` ④ **더미 `collect` 를 진짜로 교체** — 계약이 같으므로 이 스테이지 밖은 손대지 않는다. **P4 잔여는 전부 배포 뒤 화면 조작이라 P5 실발행이 흡수한다.** **Open Issue 둘을 그전에 못박는다** — 「활동 0 차단 위치」는 스케줄러 교체 전에, 「`detail` 실재 stem 검증」은 `collect_commits.py` 착수 때 |
...
- **완료 증거**: 작업 16 중 다섯(레지스트리 둘 · 볼륨 · config · 클론 관리). 나머지는 미작성.

...
| Owner | kknaks |
| Status | in_progress |
| Progress | 65% (P1·P2·P3 done · **P4·P5 in_progress** — 화면이 들어왔고 한 바퀴가 테스트 경로로 돌았고, P5 의 **레지스트리(유일한 스키마 변경)가 닫혔다**. **+5 만 올린 이유**: P5 작업 16 중 **2** 가 들었을 뿐이고, 남은 열넷이 이 발주의 무게 전부다 — 클론·수집·identity·상한·스케줄러·구 경로 제거·배포·실발행. **P4 가 여전히 열려 있는 이유 둘도 그대로다**: ① 이 환경에 node·npm 이 없어 **FE 를 타입검사도 빌드도 하지 못했다** — 화면은 한 번도 렌더된 적이 없다 ② P4 작업 다섯 중 「조사 진행 표시」가 **미착수**다. WORK-015 의 80%("BE·FE 전부 done, 실전 e2e 만 남음")가 눈금인데, 여기는 "FE 가 들어왔으나 돌려 본 적 없음" 이라 그보다 앞설 수 없다) |
| Branch/PR | `work-017-p2` |
| Blocker | 없음 |
| Next | P5 를 레지스트리 위에서 이어간다 — ① `service/jobs/repos.py`(bare 클론·fetch) ② identity 조회·drift ③ `collect_commits.py` + 더미 `collect` 교체. **P4 잔여 둘은 node 가 있는 환경이 생기는 대로 갚는다**(타입검사·빌드, 「조사 진행 표시」) — 화면은 배포 전에만 서면 되고 배포는 P5 끝이라 순서상 여기서 막히지 않는다. **Open Issue 「활동 0 차단 위치」는 스케줄러 교체 전에 결정한다** — 접수 지점을 건드리는 작업이 거기라, 유일하게 열린 spec-코드 차이를 그때 못박는다 |
...
| FE | kknaks | 게이트 화면 (줄/문장 단위 편집) | doing — 코드는 들어왔으나 **타입검사·빌드 미실행**, 「조사 진행 표시」 미착수 |
...
- **작업**:
  - [ ] 조사 진행 표시 (`investigate` N건 중 진행 수, 실패 레포, 상한 적중) — **미착수.** 게이트 payload 에 `failed_repos`·`truncated` 가 실려 오는데 화면이 아직 그것을 그리지 않는다
...
- **검증**:
  ...
  - [ ] 시나리오 7종이 각각 화면에서 구분돼 보인다 — 화면 미실행
- **완료 증거**:
  - 커밋 셋. `8c2aa7a`(승인 화면) · `2a2483a`(한 바퀴 완주) · `ead2ceb`(concept 규율 통일). 737 → **741 passed**.
  ...
  - **P4 를 `DONE` 으로 올리지 않은 근거.** 완주 조건(위 머리말 ①)은 달성됐지만 **Phase 의 산출물인 화면이 한 번도 돌지 않았다.** 근거 둘: ① node·npm 부재로 **타입검사·빌드 미실행** — 화면 코드의 정합은 사람 눈의 코드 대조가 전부이고, 그 대조가 실제로 결함 하나(`mode` 불일치)를 잡았다는 사실이 **대조로 잡히지 않는 결함이 남아 있을 개연성**을 함께 말한다 ② 작업 다섯 중 「조사 진행 표시」가 미착수다. 검증 여섯 중 넷이 화면 조작이나 떠 있는 사이트를 요구하는데, 그중 하나(잔디 칸)는 성질상 P5 로 넘어간다.

### Phase 5 — 진짜 git 수집 + 외부 연동 + 실운영 (BE/Ops)

- **Status**: IN_PROGRESS — 레지스트리(작업 첫 둘)가 들었다. 나머지 열넷은 미착수다.
...
> **P4 가 열린 채로 P5 를 시작한 근거.** Phase 순서를 어긴 것이 아니다 — P4 의 잔여는 **화면**(타입검사·빌드·「조사 진행 표시」)이고, 레지스트리는 그 화면을 **한 줄도 지나지 않는다**. 두 갈래가 만나는 곳은 배포 하나뿐이고 그건 P5 의 맨 끝이다. 반대로 P4 잔여는 이 환경에 node 가 없어 **지금 갚을 수 없는** 종류의 빚이라, 그것을 기다리면 갚을 수 있는 일까지 멈춘다. **다만 빚이 사라진 것은 아니다** — 배포 전에 화면이 서지 않으면 실발행 당일에 FE 를 디버깅하게 된다는 P4 의 판단은 그대로다.
...
> **지금 열려 있는 spec-코드 차이는 「활동 0 차단 위치」 하나다.** 나머지 넷은 닫혔다 — `investigate` 결과 귀속(SPEC-013 v0.0.2 §4) · 잔디 concept 형식 SoT(`ead2ceb`) · career 차이 표시 방식(`8c2aa7a`) · `chain.enabled_stages` 일반화(범위 밖으로 확정). 아래 첫 항목이 **유일하게 남은 것**이고, 스케줄러 교체 전에 못박는다.
>
> **그 아래 둘째 항목은 성격이 다르다** — 스펙과 코드가 어긋난 것이 아니라 **스펙이 요구한 검증(「`detail` 은 실재하는 career stem」)을 코드가 아직 아무 데서도 하지 않는 것**이다. 차이가 아니라 구멍이라 위 문장과 부딪히지 않는다.

- **활동 0 차단이 스펙과 다른 자리에 있다 — 유일하게 열린 spec-코드 차이. P5 착수 전에 결정한다.** SPEC-013 §4 는 Flow 에 "항목 생성(**활동 0이면 없음**)" 을, State 에 `received: 접수 (활동>0)` 을 적어 두었다. 코드는 접수 전이 아니라 `collect` 스테이지의 `NO_ACTIVITY` 로 막는다(`dac6ad9`). **결과적으로 발행되지 않는 것은 같지만 항목 행 하나가 남는다** — 큐 화면에 실패로 닫힌 항목이 활동 없는 날마다 하나씩 쌓인다는 뜻이다.
  ...
  **선택지 셋.** ① 스펙을 코드에 맞춘다(활동 0인 날은 `NO_ACTIVITY` 로 닫힌 항목이 남는다고 계약에 적고, 큐 화면에서 그 상태를 어떻게 보일지 U-1 에 더한다) · ② `collect` 가 활동 0을 판정하면 항목을 **폐기(soft delete)** 해 행을 지운다 · ③ 스케줄러가 접수 전에 가벼운 조사(커밋 유무만)를 한 번 더 한다. **①·② 가 유력하고 ③ 은 두 번 조사를 되살린다.** 늦어도 P5 실운영 전에는 닫는다 — 실데이터에서는 활동 0인 날이 드물지 않다.
  ...
  ⚠ **원래 "P4 착수 전에 고르는 것이 싸다" 고 적었는데 그러지 못했다.** P4 화면(`8c2aa7a`)이 이 상태를 **그리지 않은 채로 들어왔다.** ①을 고르면 큐 화면에 표시가 하나 붙어야 하므로 **화면을 다시 열어야 한다** — 어차피 P4 잔여(「조사 진행 표시」)가 같은 파일에 남아 있어 한 번에 갚으면 된다. 결정 시점은 **스케줄러 교체 전**으로 옮긴다. 접수 지점을 건드리는 작업이 거기고, 그전에는 활동 0이 실제로 발생하지 않는다(더미 `collect` 는 늘 활동을 낸다)
- **`detail` 이 실재하는 career stem 인지 아무도 안 본다 — 레지스트리가 남긴 구멍.** `cf8ac91` 의 CHECK 는 `type=company` 면 `detail` 이 **비지 않았음**만 강제한다. 오타난 stem(`medisolve-ai` → `medisolveai`)은 그대로 등록되고, 그 뒤 발행부는 **없는 파일을 새로 만든다** — career 는 `upsert` 라 "그런 문서 없음" 이 에러가 아니다. 본인작성 보호도 안 걸린다(새 파일에는 보호할 사람 글이 없다). 즉 **조용히 유령 career 문서가 하나 생긴다.** 막을 자리 둘 — ① 등록 시점에 `persona/career/{detail}.md` 존재를 확인(레포 파일시스템을 DB 계층이 알게 되는 것이 걸린다) ② `investigate` 가 결과를 career 로 묶을 때 확인(실패를 늦게 알지만 검증 위치가 자연스럽고, 이미 파일을 읽는 층이다). **②가 유력하다.** `collect_commits.py` 착수 때 함께 고른다

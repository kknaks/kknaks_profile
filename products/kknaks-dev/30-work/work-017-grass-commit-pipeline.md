...
| Blocker | 없음 |
| Next | **열린 결정은 없다.** ① identity 조회 + drift 판정 + Slack(U-2) ② `collect_commits.py` — `git log --all --numstat` · KST 경계 · tree-hash dedupe · 영역 분해 · `counts` ③ 입력 상한 적용 + `truncated` ④ **더미 `collect` 를 진짜로 교체** — 계약이 같으므로 이 스테이지 밖은 손대지 않는다. ②에 `DETAIL_NOT_FOUND` Slack 알림이 딸려 온다(U-1 과 같은 자리). **P4 잔여는 전부 배포 뒤 화면 조작이라 P5 실발행이 흡수한다** |

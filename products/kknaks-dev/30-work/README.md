...
| WORK-021 | 노트 출력 계약에서 JSON 을 뺀다 — 구분자 레코드 (`source_note`·`concept`·`derived`·`post`, 5 phase) | BE | **in_progress 80%** — P1~P4 done(파서·스테이지 넷). **P5 실전 무재시도 완주만 남았고 배포가 선행한다** | SPEC-008 | — | `work-021-note-output-delimiter.md` |
| WORK-022 | 스테이지 사이 세션 이어받기 — 앞 게이트 승계 · `cancelled` 제외 · 실패 세션 보존 · 원문 재전송 제거 (5 phase) | BE | **in_progress 80%** — P1~P4 done. 역검증으로 **취소된 게이트 세션을 무는 기존 결함이 실재함을 확인**했다. P5 재측정만 남았고 배포가 선행한다 | SPEC-009 | — | `work-022-stage-session-inheritance.md` |
...
| SPEC-009 게이트 피드백 | WORK-014·**022** | 014 P3 에서 공통 계약 구현 done / **022 todo — 세션 승계**(v0.0.3 개정: 앞 게이트 승계 S-6 · `cancelled` 제외 S-7 · 형식 실패 세션 보존 S-8 · 이어받을 때 원문 재전송 제거) |

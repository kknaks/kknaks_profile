...
016 비동기 실행 ✅ ─→ 017 잔디 커밋 파이프라인
                        P1 레지스트리+클론+collect ─┐
                        P2 형식 SoT (템플릿·agent) ─┼→ P3 daily_commit 파이프라인 ─→ P5 화면+실운영
                                                     │   P4 apply 확장 ────────────┘
                                                     └ (P1·P2 병렬, P3·P4 병렬)
```

- **017**은 016이 세운 제출/수확 분리와 드라이버 **위에 정의만 얹는다.** 게이트·큐·API·admin FE 는 손대지 않는다 — 손대는 곳은 `chain`(route 없는 파이프라인 분기)과 `apply/`(경로 2개·`upsert`·그래프 밖 산출물·본인작성 보호).
- **P1이 가장 무겁다** — 마이그레이션 + compose 볼륨 + 서버 최초 클론(~321MB)이 전부 여기다. 배포가 필요하다.
...
| 잔디 파이프라인 (BL-004) | **017 todo** (5 phase) | P1 레지스트리+클론+collect 착수 |
...
| SPEC-008 게이트 체인 | WORK-014·015·**017** | 014 route BE done / 015 나머지 스테이지 todo / **017 P3 — `daily_commit` 정의 + route 없는 체인 분기 todo** |
| SPEC-009 게이트 피드백 | WORK-014 | 014 P3 에서 공통 계약 구현 done |
| SPEC-010 Apply Executor | WORK-015·**017** | 015 구현 done (검증 6종·원자 커밋·전량 롤백·재시도) / **017 P4 — `upsert`·그래프 밖 산출물·본인작성 보호 todo** |
| SPEC-011 커밋 조사 | WORK-**017** | P1 todo — 레지스트리·bare 클론·로컬 git 수집 |
| SPEC-012 잔디 산출물 | WORK-**017** | P2·P3 todo — 템플릿 SoT · daily·career·concept 초안 |
| SPEC-013 잔디 게이트 | WORK-**017** | P3·P4·P5 todo — 파이프라인 정의 · 발행 · 화면 |

...
- **검증**:
  - [ ] 스테이지별 소요와 합
  - [ ] 세션이 실제로 물렸는지 `ai_tasks.session_ref` 로 확인 (같은 값이 체인에서 이어지는지)
  - [ ] 나빠졌으면 **나빠졌다고 적는다.** 숫자를 맞추려고 규칙을 비틀지 않는다
- **완료 증거**:
...
|---|---|---|---|
| OQ-1 | 긴 체인에서 세션이 압축돼 원문 세부가 사라지는지 | kknaks | DEC-024 OQ-2. P5 육안 확인에서 징후를 본다 |

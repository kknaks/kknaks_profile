...
> **v0.0.2 개정 — 작성 주체를 게이트로 모았다.** 종전 정의는 `compose`(자동)가 초안을 만들고 `daily` 게이트는 그것을 보여 주기만 하는 모양이었다. 그런데 §3 S-3 이 "재생성은 조사를 다시 돌리지 않는다" 로 정해 두었다. 즉 **재생성이 다시 만드는 것은 서술**이고, 그러려면 게이트가 작성 능력을 갖고 있어야 한다. 게이트가 그 능력을 갖는 순간 `compose` 의 작성은 첫 회에만 쓰이고 재생성마다 게이트가 다시 만드는 **중복**이 된다 — 매일 LLM 호출 하나를 버리는 셈이다. 유튜브가 이미 같은 대칭을 갖고 있다: `summarize`(자동)는 route 판단 재료만 만들고 노트 **작성**은 게이트 스테이지(`source_note`·`concept`·`derived`)가 한다. 그래서 `compose` 자동 스테이지를 없애고 작성을 `daily` 게이트로 옮겼다. **조사(`collect`·`investigate`)는 그대로다** — 이번 개정이 바꾸는 것은 "누가 쓰는가" 하나이고, 조사 입력(SPEC-011)과 문서 형식(SPEC-012)은 영향을 받지 않는다. 함께 §4 에 `investigate` 부분 실패 시 **결과 귀속** 계약을 넣었다(종전에는 어느 spec 도 적지 않아 구현이 먼저 정한 상태였다).
...
- Domain note: `source_kind = daily_commit`, 스테이지 `collect`·`investigate`(자동) + `daily`(게이트). **자동 스테이지는 재료까지만 만들고 작성은 게이트가 한다**(v0.0.2 개정). 항목 상태기계·게이트 상태·리비전 계약은 승인 큐·게이트 체인·피드백 spec 을 그대로 따른다.
- Open questions: 없음
...
**게이트가 열린 시점에 이미 초안이 있다.** 자동 구간은 조사까지이고, 오른쪽에 보이는 daily·career·concept 는 게이트가 쓴 것이다.

### U-1. 조사 진행 표시

- **상태**: `collect`·`investigate` 각각 진행 중/완료/실패. `investigate` 는 레포 N건 중 진행 수를 보여준다. **자동 스테이지는 이 둘뿐이다** — 작성은 게이트 안에서 일어나므로 진행 표시에 별도 줄이 없다
- **문구**: `조사 중 (3/13)` · `레포 2개 조사 실패` · `입력 상한 적용 — 일부 diff 생략`
- **CTA**: 없음(자동 스테이지)
- **기대 결과**: 조사가 끝나면 게이트가 **초안을 만든 뒤** 검토 대기로 열린다. 열림과 작성이 한 단계다
...
2. 승인 큐에서 그날 항목을 연다. `collect`·`investigate` 가 끝나 있고 게이트가 초안을 만든 채 검토 대기다.
...
4. 다시 도는 것은 **게이트의 작성** 하나다. 같은 조사 결과에 피드백과 직전 초안을 얹어 daily·career·concept 를 다시 쓴다.
5. **이 시나리오가 작성 주체를 정한다.** 재생성마다 게이트가 서술을 만드는 이상 작성 능력은 게이트에 있어야 한다. 그것을 자동 스테이지에도 두면 자동 쪽 작성은 첫 회분만 쓰이고 버려진다 — 그래서 작성은 게이트 한 곳에만 있다(v0.0.2 개정).
...
sequenceDiagram
    ...
    Note over Drv,AI: 자동 구간은 여기서 끝난다 — 재료만 있고 서술은 아직 없다
    Drv->>Q: daily 게이트 열림 (= 작성 시작)
    Q->>AI: 작성 제출 (조사 결과 + 형식 SoT)
    AI-->>Q: daily·career·concept 초안 = 리비전 1 → 검토 대기
    Q->>Slack: 승인 대기 알림
    opt 재생성 (S-3)
        Owner->>Q: 피드백
        Q->>AI: 다시 작성 (조사는 다시 돌지 않는다)
        AI-->>Q: 새 리비전
    end
    Owner->>Q: 편집 후 승인
...
| `investigate` | 자동 | 레포별 상세 조사 — **N 건 fan-out**. 문서가 아니라 **재료**를 만든다 |
| `daily` | **게이트** | **작성 + 검토.** 조사 결과를 daily·career·concept 초안으로 쓰고 사람에게 넘긴다. 하루 1개. 승인 = 발행 |

**자동 스테이지가 서술을 만들지 않는다.** 게이트가 작성 능력을 갖는 것은 재생성(S-3)이 요구하는 바이고, 그러면 자동 쪽에 같은 능력을 두는 것은 중복이다. 유튜브도 같은 모양이다 — `summarize`(자동)는 판단 재료만 만들고 노트 작성은 게이트 스테이지가 한다.
...
### Data Contract — `investigate` 결과 귀속

`investigate` 는 레포마다 하나씩 제출하고 **일부가 실패해도 진행한다**(S-4). 그때 성공한 결과를 **어느 레포에 붙이는가**가 계약이다.

| 항목 | 계약 |
|---|---|
| 대응표 | 제출이 **제출 참조 → 레포** 대응표를 남긴다. 수확은 그 표로 결과를 레포에 붙인다 |
| 순서 금지 | 결과를 **순서 있는 리스트로 받지 않는다.** 부분 실패로 한 건이 빠지면 색인이 밀려 A 레포 조사문이 B 레포 것으로 읽힌다 |
| 빈 결과 | 조사문이 비어 있으면 성공으로 치지 않는다. 빈자리를 지어내지 않는다 |
| 누락 표시 | 제출했는데 결과가 없는 레포는 `missing` 으로 들고 간다. 게이트 작성이 "이 레포는 자료가 없다"를 알아야 없는 레포를 서술하지 않는다 |
| 실패 자리 분리 | 스테이지 **실행** 실패는 `stage_failures` 다. `failures` 는 레포 **fetch** 실패의 자리이고 그 소유자는 [[spec-011-commit-collection|KDEV-SPEC-011]] §4 다 — 겹쳐 쓰면 조사 산출물을 덮는다 |
| 화면 | U-1 의 진행 수와 실패 레포 표시가 이 대응표에서 나온다 |

**계약으로 적는 이유.** 순서 있는 리스트로 되돌려도 **정상 경로 테스트는 전부 통과한다.** 어긋남은 부분 실패일 때만 드러나고, 그것도 "그날 daily 가 조용히 틀린다" 로만 드러난다 — 발행된 뒤에야 사람이 읽고 안다. 스펙이 침묵하면 다음 구현자가 되돌려도 막을 근거가 없어서, "부분 실패는 진행한다"(§5)와 같은 층위의 계약으로 여기에 둔다.
...
- **작성은 게이트에만 둔다.** 자동 스테이지는 재료까지만 만든다. 재생성(S-3)이 서술을 다시 만드는 이상 게이트는 작성 능력을 가져야 하고, 같은 능력을 자동 쪽에도 두면 첫 회분만 쓰이고 버려진다. 형식 명세는 프롬프트에 복사하지 않고 `templates/persona/` 에서 싣는다 — 형식 SoT 는 [[spec-012-grass-artifacts|KDEV-SPEC-012]] 가 소유한다.
- **경로는 시스템이 조립한다.** 산출물의 대상 경로를 모델에게 정하게 두지 않는다 — 허용 경로 밖으로 쓰는 계획이 나온다. 기존 노트 게이트와 같은 규율이다.
- **career 대상은 저장하지 않고 다시 계산한다.** 제출과 수확이 같은 입력에서 같은 결론을 내야 한다. 제출 시점의 목록을 붙잡아 두면 프로세스가 재시작된 뒤 수확이 다른 판단을 한다.
- **fan-out 은 게이트 밖에 둔다.** `investigate` 는 자동 스테이지라 승인 리비전을 만들지 않는다. 게이트 공통 계약(리비전 1:1, 수확 멱등)을 건드리지 않는다.
- **부분 실패는 진행한다.** 레포 하나가 막혀도 나머지로 간다. 전부 실패하면 스테이지 실패. 성공분을 어느 레포에 붙이는지는 §4 「`investigate` 결과 귀속」이 정한다 — **순서로 맞추지 않는다.**

...
- 채택:
  ...
  - `reference/` — 참고노트(자료 정리, 출처 종속). ← `persona/notes` 종착 (contents 잔류 — [[decision-008-contents-retention|KDEV-DEC-008]]).
  - `permanent/` — 영구노트. `permanent/archive/` = 장기기억(cold).
  - `persona/posts/` — 발행물(영구노트가 글이 된 것). 신설.
  - `persona/algorithms/` — 개인 배치 산출물, 그래프 무관, 잔류.
  - `persona/contents/` — YouTube 요약 파이프라인, 그래프 무관, 잔류 ([[decision-008-contents-retention|KDEV-DEC-008]]). C-001 삭제 철회.
...
- In: 루트 디렉토리 층 + persona 재편(projects→products, notes→reference (contents 잔류 — DEC-008), posts 신설).

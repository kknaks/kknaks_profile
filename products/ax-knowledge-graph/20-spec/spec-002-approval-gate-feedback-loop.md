...
| `target_path` | yes | 생성 또는 수정 대상 경로 |
| `filename_candidate` | yes | 파일명 후보 |
...
| `target_path` | yes | 적용 대상 경로 |
| `file_action` | yes | `create_markdown`, `patch_markdown`, `update_frontmatter` |
| `target_document_id` | modify only | 기존 문서 수정 대상 |
| `draft_markdown` | create only | 신규 생성될 markdown |
| `diff_preview` | modify only | 기존 문서 대비 diff |
...
| Field | Required | 설명 |
|---|---|---|
| `action_type` | yes | `create_markdown`, `patch_markdown`, `update_frontmatter` |
| `target_path` | yes | document root 기준 상대 경로 |
| `content` | create only | 신규 파일 전체 markdown |
| `patch` | patch only | 적용할 diff/patch |
| `frontmatter_patch` | update_frontmatter only | frontmatter 변경 patch |
| `diff_preview` | modify only | UI 렌더링용 diff |
...
- 새 revision이 `reviewable`이 되면 이전 active revision은 `superseded`가 된다.
...
없음. 피드백은 빠른 선택지와 자유 입력을 함께 제공한다.

...
> **경계**: 이 spec의 대상은 **ax-graph가 AI로 생성하는 지식 문서(reference/permanent 등)의 템플릿**이다. 코드레포의 `templates/product/**`(product-doc-pipeline 메타문서 템플릿, @product-curator 소유)와 **별개**이며 섞지 않는다. 템플릿 = 문서 뼈대, 문서생성 프롬프트(AXKG-SPEC-009) = 뼈대를 채우는 방법이다.
...
- 경계: 문서생성 프롬프트·출력 스키마는 AXKG-SPEC-009 소관. 이 spec은 템플릿(뼈대) + 버전 + frontmatter 스탬프만 다룬다. 코드레포 `templates/product/**`는 범위 밖.
- MVP scope: `reference`·`permanent`·`project_baseline` 3종(resource→reference, area→permanent, project→project_baseline, AXKG-DEC-005). project의 decision/spec 후보 템플릿 확장은 Open Questions. archive는 문서화 게이트로 넘어가지 않으므로(AXKG-SPEC-004) 템플릿이 없다.
...
- **상태**: 로딩, 목록 있음, 빈 목록, 에러
- **문구**: 템플릿 키(reference/permanent/project_baseline)와 용도, 활성 버전 번호, 마지막 수정 시각
- **CTA**: `템플릿 선택`(목록 항목 클릭)
- **기대 결과**: 항목을 선택하면 우측 편집기와 버전 히스토리가 해당 템플릿으로 채워진다. 템플릿 키는 문서 타입 기반이며 AXKG-SPEC-001의 destination 매핑(`resource→reference`, `area→permanent`, `project→project_baseline`)에서 파생한다. 이 spec은 키를 새로 발명하지 않는다.
...
| 필드 | 규칙 |
|---|---|
| `key` | 존재하는 템플릿 key여야 함(문서 타입 기반, reference/permanent/project_baseline) |
| `body` | 비어 있으면 안 됨 |
...
| Resource | Field | 설명 |
|---|---|---|
| Template | `key` | 템플릿 식별 키(문서 타입 기반, AXKG-SPEC-001 destination 매핑에서 파생) |
...
- 템플릿 키는 문서 타입 기반이며 AXKG-SPEC-001의 destination 매핑에서 파생한다. 이 spec에서 새 키를 발명하지 않는다.

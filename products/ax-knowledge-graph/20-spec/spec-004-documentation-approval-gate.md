...
#### concept → {permanent, feature_spec} stale 연쇄와 재생성 게이트 (AXKG-DEC-005 E — permanent 축 구현 완료 / feature_spec·발굴 축 확장 AXKG-WORK-014)

concept가 갱신되면(기존 개념 보충 **또는** 신규 개념 인입), 그 개념과 관계된 기존 문서(구성 개념으로 참조하는 종합 노트 permanent, 그리고 그 개념을 차용 역량으로 쓸 만한 회사 기능정의서 feature_spec)를 찾아 **영향 가능성을 표시**하되 자동으로 다시 쓰지는 않는다. 감지에는 두 축이 있다 — **① backlink**(기존 개념 성장 시 이미 그 개념을 링크한 문서, E-2) + **② retriever 발굴**(신규 개념 인입 시 아직 링크하지 않았으나 의미적으로 관련된 문서, E-8). **permanent·backlink 축 구현 완료**(2026-07-10 BE PLAN-009-T-030 / FE PLAN-009-T-031·032·033 + 라이브 검증: 감지→배지→재생성 게이트→승인→해제 전 구간). **feature_spec 대상·retriever 발굴 축은 AXKG-WORK-014로 확장**(설계 확정, 구현 착수). 동작 계약(2026-07-09 PLAN-009-T-024, 2026-07-25 T-014 확장):
...
- **재생성 게이트(수동)**: 반영은 자동이 아니다. 사용자가 원할 때 해당 문서의 재생성 게이트를 열어 초안 검토 → 승인으로 갱신한다. 승인 게이트 철학(AI는 제안만, 확정은 사용자)을 따르며 **자동 연쇄 재작성은 하지 않는다**.
- **E-8. 감지 축 2 = 신규 개념 발굴(retriever, LLM 판정 없음) — AXKG-WORK-014**: 기존 E-2(backlink)는 **이미 그 개념을 `[[concept]]`로 참조하는** 문서만 잡으므로, **신규 개념 인입**(`create_new_concept` 승인)은 아직 아무도 링크하지 않아 어떤 문서도 감지하지 못한다. 이 공백을 메우려, 신규 concept 생성 apply 시점에 그 개념(stem·title·본문)을 쿼리로 **2단 Graph RAG retriever**(AXKG-SPEC-011, qmd 하이브리드 BM25+벡터)를 돌려 **의미적으로 관련되나 아직 그 개념을 링크하지 않은** 기존 문서에 배지를 붙인다. 발굴 범위는 **corp 경계(같은 회사 문서만) + relevance score 임계 + top-N 상한**으로 과잉 포함을 억제한다(임계·N은 구현 상수). 이는 LLM 영향 판정(triage, E-7)이 **아니라** 검색 기반 후보 발굴이며 — 배지 의미(E-1: "영향 가능성", 판단은 사용자)와 수동 재생성(E-3~E-6)을 그대로 상속한다. 배지에는 유발 concept stem + "관련 신규 개념 인입" 요지를 동봉한다(E-2와 동형).
- **E-9. 대상 타입 확장 = {permanent, feature_spec} — AXKG-WORK-014**: stale 마킹·재생성 대상을 종합 노트(permanent)에서 **회사 기능정의서(feature_spec, AXKG-SPEC-014)까지** 넓힌다. 근거 대칭 — permanent가 구성 개념 유입마다 성장하듯(§4 개념 성장), feature_spec의 `## 8. 연결`도 관련 기술 개념이 뒤늦게 들어오면 그 개념으로의 **차용 링크를 보강**해야 지식 그래프가 조밀해진다(현행: 스펙당 개념 링크 0~1개로 빈약).
  - **재생성 경로 차이**: feature_spec은 permanent와 달리 **producing 문서화 게이트가 없다**(plan-then-fanout 산출, AXKG-SPEC-014/DEC-008). 따라서 재생성 게이트는 문서화 게이트 재문서화 경로(E 구현부)를 재사용하지 못하고, **기능 dedup의 `supplement_existing_feature` 프리미티브**(기존 기능정의서 전문 주입 + 병합·업그레이드 overwrite, AXKG-SPEC-014)를 재사용해 대상 spec 1장을 재생성한다. 입력 계약(E-3 대칭) = 대상 spec 전문 + 유발 concept 전문 + 변경 요지.
  - **출력 규율(E-6 대칭)**: 재생성 초안은 관련 개념을 `## 8. 연결`에 `[[concept]]`로 엮되 **억지 링크를 만들지 않는다**(무관하면 링크 없이 기존 보존). 지적되지 않은 본문 판단은 보존한다.
  - E-1·E-3·E-4·E-5(과잉 포함·1프롬프트=1문서·1승인=1문서·부분 처리 허용)는 **대상 타입·감지 축과 무관하게** 그대로 적용된다.
  - **백필**: E-8 발굴 축 배포 **이전에 인입된 기존 concept**들은 관련 문서를 소급 발굴한 적이 없다. 배포 시 기존 concept 전수를 대상으로 발굴 트리거를 **1회 소급 실행**(멱등)해 그간 누락된 stale 배지를 채운다(AXKG-WORK-014 backfill, `backfill_company_root` 앱-컨텍스트 CLI 패턴 재사용).
- **구현(완료 — permanent·backlink 축)**: E-1~6 계약 그대로 구현됐다(2026-07-10, BE PLAN-009-T-030 / FE PLAN-009-T-031·032·033 + 라이브 검증). stale 표시는 **`document_stale_marks` 테이블**(alembic 0018)에 저장하고, API 3종 — `GET /documents/stale`(stale 목록), stale dismiss(사용자 해제), stale regenerate(재생성) — 으로 노출한다. regenerate는 대상 permanent의 **producing source 문서화 게이트 재문서화 경로를 재사용**한다(1 프롬프트 = 1 문서, E-3/E-4). 미처리 stale은 배지가 유지되는 가시적 상태로 남는다(E-5).
- **구현(착수 — feature_spec·발굴 축, AXKG-WORK-014)**: E-8(retriever 발굴)·E-9(feature_spec 대상 + `supplement_existing_feature` 재생성 경로) 확장. `document_stale_marks` 테이블은 재사용(대상 `document_id`는 타입 무관이라 스키마 변경 없이 permanent·feature_spec 공용). stale regenerate 진입점은 대상 `document_type`으로 분기 — permanent는 기존 경로, feature_spec은 `supplement_existing_feature` 큐잉. 기존 concept 소급은 backfill 스크립트(E-9 백필).
...
- **concept stale 연쇄를 feature_spec·retriever 발굴 축으로 확장(AXKG-DEC-005 E-8/E-9, AXKG-WORK-014)**: 신규 개념 인입 시 backlink가 못 잡는 관련 문서를 retriever(qmd 하이브리드)로 발굴(E-8)하고, 대상을 `feature_spec`까지 넓혀(E-9) 기능정의서 `## 8. 연결`의 개념 차용 링크 빈약(스펙당 0~1개)을 해소한다. feature_spec 재생성은 producing 문서화 게이트가 없어 `supplement_existing_feature` 프리미티브를 재사용. E-1/E-3~E-6 계약은 상속. 배포 시 기존 concept 소급 backfill 1회(멱등). 설계 확정·구현 착수(2026-07-25).

...
- [[externalized-configuration]] — 프롬프트·템플릿을 코드가 아니라 **DB 에 두고 버전으로 관리**해 즉시 반영한다. 「자주 바꾸는 것을 밖으로 뺀다」가 가이드/프롬프트 경계 재정렬의 판단 기준이 됐다
- [[ai-agent]] — worker 가 프로젝트 컨텍스트(`CLAUDE.md → agent.md`)를 **스스로 읽는다** — api 가 주입하지 않는다. 실행 주체가 지시를 찾아 읽는 구조가 이 개념 그대로다
- [[api-response-envelope]] — `classification.v1`·`documentation.v1` 봉투는 **코드 고정 계약**이고 `output_schema` 는 그 안쪽만 관장한다는 경계. 봉투와 알맹이를 가르는 발상이 같다

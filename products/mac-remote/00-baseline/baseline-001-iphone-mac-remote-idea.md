...
links:
  ...
  decisions:
    ...
    - "[[decision-004-mac-helper-first|MRT-DEC-004]]"
...
- 통신: 실시간 양방향 + 서버→클라이언트 push가 필요 → WebSocket 후보 ([[decision-001-websocket-protocol|MRT-DEC-001]])
- 창 목록: macOS 네이티브 API로 수집 ([[decision-002-cgwindowlist-window-source|MRT-DEC-002]])
- 창 식별: 캡처 대신 앱 아이콘 ([[decision-003-app-icon-only-no-capture|MRT-DEC-003]])
- 개발 순서: Mac 헬퍼 CLI 먼저 검증 후 iOS ([[decision-004-mac-helper-first|MRT-DEC-004]])

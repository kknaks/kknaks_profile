...
여섯 번째 decision인 KAG-DEC-006(tools package 등록·허용·검증·실행 경계)도 2026-08-09 `proposed`로 올라가 **사용자 리뷰 대기** 중이다. KAG-DEC-005 PI7이 “복원한 tool call은 실행 요청이지 실행이 아니다”라며 그 요청의 수신자를 넘겼기 때문에 `providers` 다음이 같은 L2의 `tools`이며, 이번 대상은 `tools/` 하나다. KAG-DEC-006도 KAG-DEC-003·KAG-DEC-005를 `proposed` 입력으로만 참조하고 연결을 범주 수준으로만 둔다. 남은 raw 작성 순서는 `sessions` → `skills` → `context` → `runtime`이다.
...
| (미착수) | session event 계약, Codex CLI 격리 옵션 등 | - | KAG-BL-001 Open Questions 참조 |
...
KAG-DEC-001·KAG-DEC-002는 `accepted` 완료. 현재 게이트는 **KAG-DEC-003(core 계약 경계)·KAG-DEC-004(process 실행 격리 경계)·KAG-DEC-005(providers 변환 경계)·KAG-DEC-006(tools 등록·허용·검증·실행 경계)의 사용자 리뷰** 네 건이다. 공개 계약은 디렉터리 하나씩 의존 그래프 아래에서 위로 내려가고, 지금까지 raw 제안이 올라온 것은 `core`(L0)·`process`(L1)·`providers`(L2)·`tools`(L2)다. 네 문서는 서로를 `proposed` 입력으로만 참조하므로 함께 읽는 편이 낫다.

남은 raw 작성 순서는 **`sessions` → `skills` → `context` → `runtime`**이다. `runtime`이 마지막인 이유는 나머지를 조립하는 자리이기 때문이다. spec은 그 뒤에 연다. 확정 전에는 spec·work·코드로 내려가지 않으며, KAG-DEC-007 이후의 ID를 미리 선점하지 않는다.

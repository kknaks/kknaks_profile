...
KAG-DEC-001이 `sessions`에 배정한 “session event의 손실 없는 저장과 조회”라는 책임을 **어떤 파일에 어떤 종류의 것으로 나눠 담을지**, **한 번의 append와 한 번의 조회가 무엇을 보장하는지**, 그리고 **memory store와 향후 durable store가 어떤 계약을 함께 구현하는지** 정한다 — 이 셋은 2026-08-10 사용자 확정으로 이 제품의 결정이다. 구조 옵션 비교와 파일 후보 트리, 파일별 역할과 타입/Protocol 범주, 내부 의존 방향, session/turn/event 식별과 session 내 ordering, append 원자성·중복·동시 writer·부분 실패의 판정, 조회의 안정성과 되읽기 경계, 직렬화 format/version과 unknown fail-closed, 보안·관측·contract test seam까지가 대상이고 **exact signature·저장 schema·DB 제품·sync/async 형태·retention·암호화·migration 도구는 대상이 아니다.**

> baseline의 날것 입력을 spec으로 내리기 전에 적용 방향을 정하는 문서.
> 기능 계약 상세는 `20-spec/`, 실제 작업 순서는 `30-work/`에 둔다.

> **상태 `accepted` (2026-08-10 사용자 확정).** planner가 `proposed`로 올린 권고안을 사용자가 **Option B**로 확정했다. 아래 §Decision 이하는 이제 이 제품의 결정이며, 바꾸려면 새 decision으로 supersede한다. 이 결정이 명시적으로 Out으로 둔 범위(exact signature·저장 schema와 format·DB 제품·sync/async 형태·retention·암호화·migration 도구 등)는 확정되지 않았고, §Open Questions **15건 중 어느 것도 이번 확정으로 영구 closed가 되지 않았다** — 본문이 현재 축을 답한 것 셋(OQ-1·OQ-4·OQ-7)도 재평가 trigger나 잔여 축이 살아 있다. KAG-BL-001·KAG-DEC-001~006의 `accepted`는 이 문서가 바꾸지 않는다 — 이 결정은 그 위에 쌓일 뿐 되돌리지 않는다.
>
> **2026-08-10 선행 상태 정합화.** 이 문서가 작성될 때 `proposed`였던 KAG-DEC-003·KAG-DEC-004·KAG-DEC-005·KAG-DEC-006이 전부 `accepted`가 되어 “그 넷은 `proposed`”라는 주석이 사실과 어긋났다. 그 주석만 현재 상태로 맞췄고 **연결 방식은 바꾸지 않았다** — `core`와의 연결은 여전히 **범주 수준**이며, 확정됐다는 사실을 파일명 강결합의 새 근거로 확대하지 않는다(§0.2·§3.3).
>
> **확정 직전 감사 기록 (2026-08-10).** 선행 accepted 위반 0건 · 후속 세 문서(KAG-DEC-008~010)의 숨은 전제 0건 · acceptance blocker 0건이다. 중점 확인 다섯 축이 전부 일관됐다 — ① **canonical event의 무손실 저장**(SI10·SR3·SR3′·SR3″가 “무엇에 대해 lossless인가”를 canonical event로 못박고, 자를 것이면 event 구성 전에 잘려 있어야 한다는 경로까지 닫는다) ② **codec 판별과 의미 분기의 분리**(§6.3의 허용/금지 표 + 판별을 `serialization` 하나에 가두는 국소화 + §7.3 여섯 번째 seam이 “존재 금지”가 아니라 “위치”를 보는 검사라는 점) ③ **append 순서와 조회 순서**(O1~O5·SI5·SI6이 도착 순서 기준의 session 내 전순서로 단일화) ④ **secret·진단 경계**(SR2가 “주입도 받지 않는다”로 KAG-DEC-004 R6·005 PR7·006 TR6보다 한 걸음 더 가고, SR4가 model-safe와 진단 전용의 구분을 보존) ⑤ **store protocol 소유권**(KAG-DEC-003 §4.1이 core에서 제외해 넘긴 경계를 §3.1·§7.2가 `store.py` 계약 하나로 받는다). **§Decision 본문은 문면 정합화 외에 수정하지 않았다.**
>
> **KAG-DEC-003 OQ-2에 대한 답이 이 확정으로 결정이 됐다.** KAG-DEC-003 감사는 그 질문을 `proposed answer`(“KAG-DEC-007이 답 후보를 냈다”)로 판정했는데, 이번 확정으로 **store 경계는 `sessions` 안 `store.py`에 계약 하나**가 채택된 결정이다. **KAG-DEC-003 본문은 이 확정이 수정하지 않았고**, 그 문서의 OQ-2 행이 아직 `proposed answer`로 적혀 있는 것은 KAG-DEC-003을 다시 여는 시점에 정리한다(§정합성 확인).
>
> **OQ-15의 trigger가 도달했고, 그 결과를 사실대로 적는다.** OQ-15의 Next는 “`core` 상세 decision이 확정된 뒤”였고 KAG-DEC-003은 2026-08-10 `accepted`가 됐다. 확정된 `core` 배치를 실물로 확인한 결과 **event payload 계약(`events.py`, K6)은 있으나 codec은 없고 `protocols.py`도 필수 1 + 권고 1뿐**이다. 그래서 **Z4가 말한 “아는 판별값의 원천”은 accepted 계약으로 실재하지만 그것을 얻는 방법은 여전히 없다.** 이 문서는 **새 `core` API를 발명하지 않고** OQ-15를 열어 둔 채 routing만 날카롭게 했다 — (가)로 가려면 KAG-DEC-003을 supersede하는 별도 decision이 필요하다.

## Context

- 관련 baseline: [[baseline-001-provider-neutral-llm-runtime|KAG-BL-001]]
- 선행 결정: [[decision-001-runtime-directory-boundaries|KAG-DEC-001]] (accepted), [[decision-002-turn-runtime-flow|KAG-DEC-002]] (accepted), [[decision-003-core-contract-boundaries|KAG-DEC-003]] (accepted, 2026-08-10 확정), [[decision-004-process-boundaries|KAG-DEC-004]] (accepted, 2026-08-10 확정), [[decision-005-provider-boundaries|KAG-DEC-005]] (accepted, 2026-08-10 확정), [[decision-006-tools-boundaries|KAG-DEC-006]] (accepted, 2026-08-10 확정)
- 문제/기회
...
- **“권고”와 “확정”을 구분한다.** 작성 시점에는 이 문서 전체가 `proposed`라 모든 §Decision 항목이 권고였고, 근거가 약해 뒤집힐 수 있는 것은 그렇게 표시하거나 Open Questions로 뺐다. **2026-08-10 사용자 확정 이후 §Decision(§1~§8)은 이 제품의 결정이고, 그때 남긴 미결은 §Open Questions에 그대로 있다.**
...
| KAG-DEC-003 | **accepted** (2026-08-10) | **변경하지 않는다.** event payload(K6)·식별자 원시 타입·오류 표현(K7)·공통 응답 참조를 소비하되, **연결은 여전히 파일 이름이 아니라 범주 수준으로만** 둔다 — 확정됐다고 해서 `sessions`가 `core`의 특정 파일에 강결합하지 않는다. 다만 §4.1의 판정 하나(“session event 저장 경계는 core가 아니라 `sessions` 안”)와 그것이 넘긴 OQ-2는 **이 문서가 답하는 질문 자체**이므로 §3에서 정면으로 다루고, **이번 확정으로 그 답이 결정이 됐다**(§Decision·§7.2). **이 문서의 §3 배치와 §4 lifecycle은 그 연결과 독립적으로 성립한다.** 확정된 배치에 **codec은 없다**는 사실이 OQ-15의 현재 상태를 규정한다(§6.3) |
| KAG-DEC-004 | **accepted** (2026-08-10) | 직접 참조하지 않는다. `sessions`는 `process`를 import하지 않는다(KAG-DEC-001 §4). durable store가 외부 프로세스를 쓸 일은 없고, 파일 기반 store가 생기더라도 그것은 subprocess 격리가 아니다. 확정이 이 관계를 바꾸지 않는다 |
| KAG-DEC-005 | **accepted** (2026-08-10) | 직접 참조하지 않는다. 소비하는 것은 성질 하나 — provider 원문은 **구조를 읽지 않는 불투명 값**이라는 것(PI6). 그 값이 event에 실려 오면 `sessions`도 구조를 읽지 않는다(§8 SR5). 소비하는 것은 여전히 **성질**이지 그 문서의 파일명이 아니다 |
| KAG-DEC-006 | **accepted** (2026-08-10) | **변경하지 않는다.** 소비하는 것은 경계의 성질 둘 — `tools`는 아무것도 기록하지 않고 값만 반환하며(TI6), 거부도 실패도 **결과로 반환되어** 기록 재료가 된다(TI1·TR7). 그 값이 `runtime`을 거쳐 이 package에 도달한다. **그 문서 OQ-14(허용 tool 공개 정의 목록의 순서 소유자)는 이 문서의 순서 규칙과 다른 축이다** — 그쪽은 한 요청에 실리는 tool 정의 목록의 나열 순서이고, 이 문서의 O1~O5는 session 안 event의 append 순서다. 둘은 서로를 규정하지 않는다 |
...
사용자가 2026-08-10에 Option B로 확정했다.

- 채택: **Option B — 관심사별 평면 module 분리 + 계약과 구현의 파일 분리 + module 간 총순서 고정.**
- 비권고: Option A(단일 `store.py`), Option C(백엔드별 하위 package). C는 §6.2의 승격 경로로 남긴다.
- 기각: Option D(event 종류별 module 분리).
- **KAG-DEC-003 OQ-2에 대한 답이 이 확정에 포함된다**: store 경계는 `sessions` 안 **`store.py` 하나에 계약 하나**로 둔다(§3.1·§7.2). append와 조회를 두 계약으로 쪼개지 않는 이유는 §7.2에 적는다. **KAG-DEC-003 감사가 `proposed answer`로 판정했던 그 답이 이번 확정으로 결정이 됐고, KAG-DEC-003 본문은 수정하지 않았다.**
- 보류: sync/async 형태, store 계약을 Protocol로 둘지 추상 기반 클래스로 둘지, append 단위에 event를 여럿 받을지, **session revision이 마지막 위치와 같은 값인지 별개 값인지**, 멱등 키의 발급 주체, memory에도 직렬화 왕복을 강제할지, `reading`을 `store`에 접을지의 재평가, 저장 format과 version 표기, provider 원문 보관, retention·삭제 요구의 거처, 실패의 값/예외 표현, 재수출 목록과 fake store의 거처, **`serialization`이 “아는 판별값”을 어떻게 아는가**(확정된 `core` 배치에 codec이 없으므로 새 `core` API를 발명하지 않는다 — §6.3) (→ §Open Questions).
- **확정 범위에 포함된 것 중 원래 열려 있던 축 둘**: ① **append와 조회를 한 계약으로 둔다**(OQ-1의 한 축 — 근거는 SI2가 두 계약에 걸쳐 있다는 것, §7.2). ② **위치 기대(expected revision)는 선택적 입력이다**(OQ-4의 한 축 — §2.4·§4.1 3국면). 두 OQ의 나머지 축은 그대로 열려 있다.

이하 §1~§8이 Option B의 확정 구조다.
...
- **지금 강제하지 않는 이유**: writer가 하나인데 매 append에 기대값을 요구하면 호출부가 그것을 관리해야 하고, 관리할 이유가 없는 값은 곧 형식적으로 채워진다. 그래서 **선택적 입력이다** — 생략하면 무조건 끝에 붙인다. **이 축은 2026-08-10 확정에 포함됐고**, 남는 미결은 **session revision이 마지막 위치와 같은 값인지 별개 값인지**와 첫 호스트 예제 이후의 필수화 재평가다(→ OQ-4의 잔여 축).
...
**`core`와의 연결은 범주 수준으로만 둔다.** KAG-DEC-003이 2026-08-10 `accepted`가 되어 `core` 파일 배치는 확정됐지만, **확정됐다는 사실이 `sessions`가 특정 `core` 파일을 참조해야 한다는 뜻은 아니다** — 이 문서는 그것을 파일명으로 끌어와 강결합하지 않는다. 확정하는 것은 “event payload와 식별자 원시 타입과 오류 표현을 읽고 참조한다”는 범주뿐이다.

### 4. append와 조회의 lifecycle

#### 4.1 한 번의 append가 지나는 국면

한 append는 **국면 6개**를 지난다. 순서를 바꾸거나 국면을 건너뛰는 구현은 이 결정 위반이다. **1~4국면은 저장을 바꾸지 않고, 저장이 바뀌는 것은 5국면에서만이다.**
...
2. **3국면은 선택이다.** 기대값을 주지 않으면 무조건 끝에 붙인다(§2.4). 선택인 것과 없는 것은 다르다 — 계약에는 있고 지금 호출부가 안 쓸 뿐이다. **이 선택성은 2026-08-10 확정에 포함됐고**, 필수화 여부의 재평가와 revision 값의 정체는 열려 있다(→ OQ-4).
...
**다만 그 원천을 `sessions`가 어떤 방법으로 얻는지는 이 문서가 정하지 않는다.** 2026-08-10 `accepted`가 된 KAG-DEC-003을 실물로 확인한 결과는 이렇다 — **원천은 있다.** `core/events.py`가 event payload 계약을 소유하고 종류 범주(turn 시작·사용자 입력·model 응답·tool 검증·tool 결과·final·종료)를 적어 두었으므로 Z4가 말한 “아는 판별값의 원천”은 **accepted 계약으로 실재한다.** **그런데 그것을 얻는 방법은 없다** — 확정된 `core` 배치에 codec module이 없고 `protocols.py`도 model 호출 경계(필수)와 tool handler 경계(권고) 둘뿐이다. 이 문서는 **새 `core` API를 발명하지 않는다.** `core`가 codec을 제공하는 안(그러려면 KAG-DEC-003을 supersede하는 별도 decision이 필요하다)과 `sessions`가 매핑을 갖되 `core` 계약과 동기화되도록 강제하는 안 중 어느 쪽인지는 미결이다(→ OQ-15). 어느 쪽이든 위 표의 금지 열은 그대로 성립해야 하고, **판별이 `serialization` 하나에 갇힌다는 국소성도 유지된다.**
...
- **확정: 선별 재수출.** 호스트가 store를 만들고 넘기는 데 필요한 것만 `__init__.py`가 재수출하고, 라이브러리 내부는 항상 module 경로로 import한다(KAG-DEC-003 §5.1의 S3, KAG-DEC-006 §7.1과 같은 형태).
...
- 근거 셋.
  1. **호스트가 반드시 직접 만지는 package다.** store를 고르고 만드는 것은 L4의 조립이므로(KAG-DEC-001 §4) 그 경로가 표면에 없으면 호스트가 내부 경로를 외운다. `providers`가 무-재수출을 **확정**한 것과 다른 이유가 여기 있다 — provider는 이름이 import 줄에 남는 것이 이점이었지만 store는 그렇지 않다. **세 문서가 다른 답을 낸 것은 근거가 다르기 때문이지 불일치가 아니다**(`core` S3 선별 · `providers` 무-재수출 · `tools`·`sessions` 선별).
...
- **package-root(`kknaks_agents/__init__.py`)에는 올리지 않는다.** 이 문서가 정한 것은 “**여기에 `sessions`를 올릴 근거가 없다**”까지이고, root 표면 **전체**는 KAG-DEC-003 OQ-4로 계속 미결이다.
...
**append와 조회를 두 계약으로 쪼개지 않는다(2026-08-10 확정).** 쪼개는 안도 검토했다 — 쓰기와 읽기를 나누면 읽기 전용 소비자에게 좁은 표면을 줄 수 있다. 채택하지 않은 이유는 **SI2(read-your-write)가 두 계약에 걸쳐 있기 때문**이다. 쓰기 계약과 읽기 계약이 따로 있으면 “붙인 것이 보인다”를 어느 쪽도 단독으로 보장하지 못하고, 그 보장은 결국 “같은 store 구현이면 그렇다”라는 규약으로 내려앉는다. 좁은 읽기 표면이 필요하면 그것은 계약 분할이 아니라 §7.1의 재수출로 다룬다.
...
- 대안 대비 이유
  ...
  - B는 1·2·3·4를 만족하면서 5를 §1의 책임 목록으로 억제한다. module이 7개인 이유가 미학이 아니라 **§1을 통과한 책임이 그만큼이기 때문**이라는 점이 **B를 고른 근거의 핵심**이다.
- 리스크
  ...
  - **아는 판별값의 원천은 있고 얻는 방법이 없다.** §6.3은 원천이 `core`의 event 계약이라고 정했고, **2026-08-10 KAG-DEC-003 확정으로 그 원천은 accepted 계약(`core/events.py`, K6)으로 실재한다.** 그러나 **확정된 배치에 codec이 없어** 얻는 방법은 여전히 OQ-15다. 그동안 구현이 필요하면 `sessions`가 임시 매핑을 갖게 되는데, 그 매핑이 오래 살면 그것이 곧 종류별 판단이 된다. 완화: 임시 매핑을 두더라도 §6.3의 금지 열을 넘지 않게 하고 판별을 `serialization` 하나에 가둔다. **(가) `core`가 codec을 제공하는 안으로 가려면 KAG-DEC-003을 supersede하는 별도 decision이 필요하다는 사실이 확정으로 분명해졌다** — 이 문서는 그것을 발명하지 않는다.
  - ~~**이 문서가 `proposed` 네 건 위에 서 있다.**~~ **2026-08-10 KAG-DEC-003·004·005·006이 `accepted`가 되어 이 리스크는 해소됐다.** 작성 당시에는 KAG-DEC-003이 뒤집히면 §3.2의 값 참조와 §4.4의 event 대응이, KAG-DEC-006이 뒤집히면 §4.4의 6·8단계 재료가 흔들린다는 위험이 있었고, 완화로 연결을 파일명이 아니라 범주 수준으로만 두었다(§0.2·§3.3 마지막 문단). **그 완화는 지금도 유지한다** — 확정됐다는 이유로 파일명 결합을 새로 들이지 않는다.
...
- [[serialization]] — 저장 format 과 version 을 정하고, 모르는 필드를 만났을 때의 처리를 계약에 넣는다
- [[idempotency]] — append 의 **중복 판정**이 이 성질이다. 재시도된 append 가 event 를 둘로 늘리면 순서도 되읽기도 무의미해진다
- [[fail-closed]] — 그 「모르는 필드」의 처리 방향이 **막는 쪽**이라는 것. 이 제품의 모든 경계가 같은 기본값을 쓴다
...
- In
  ...
  - `sessions/__init__.py` 선별 재수출, 새 store 구현자의 요구 7개와 append/조회를 한 계약으로 두는 결정, contract test seam 6종(그중 하나는 동작이 아니라 구조를 보는 검사이고 그 기준은 “종류 이름의 부재”가 아니라 “종류를 보는 코드의 국소화”다) (§7)
...
- Out
  ...
  - **`core` event codec의 형태와 `sessions`가 “아는 판별값”을 얻는 방법** — **`accepted`가 된 KAG-DEC-003도 payload만 배정하고 codec을 두지 않았으므로** 새 `core` API를 발명하지 않는다 (→ OQ-15)
  - `core`의 어느 파일을 참조할지 — **KAG-DEC-003이 `accepted`가 된 뒤에도 이 문서는 확정하지 않는다.** 연결은 범주 수준으로만 둔다 (§3.3)
  ...
  - KAG-DEC-003·KAG-DEC-004·KAG-DEC-005·KAG-DEC-006의 내용 수정 — 이 문서는 그 `accepted` 결정을 소비할 뿐 바꾸지 않는다. **KAG-DEC-003 OQ-2의 답이 이번 확정으로 결정이 됐지만 그 문서 본문은 수정하지 않았다**
  - KAG-DEC-008~010의 본문·status 변경 — 이 확정이 그 셋을 수정하지 않았다
...
이 절은 두 층으로 읽는다. **§원 질문**은 2026-08-09 작성 시점의 표이고 고치지 않는다. **§감사 결과**는 KAG-DEC-003~006이 전부 `accepted`가 된 뒤 2026-08-10에 “이 문서를 accepted로 올릴 준비가 되었는가”를 기준으로 각 질문을 다시 본 **현재 routing view**다.

판정 규칙은 KAG-DEC-003~006 감사와 같다. **아래 다섯 정의는 accepted 전환 준비 감사 당시(문서가 `proposed`이던 시점)의 기준이며 역사 기록으로 보존한다** — 현재 상태는 §감사 결과 표와 §상태 집계가 말한다.

- **`internally resolved`** — 본문이 질문의 **현재 축**을 이미 답해, OQ에는 미래 재평가 trigger나 부수 축만 남은 경우. **영구 closure가 아니다.**
- **`partial`** — **한 OQ 안에 판정이 다른 축이 섞인 경우.** 집계에서는 **OQ 하나를 1건으로만 센다** — 축별로 쪼개 세지 않으므로 합계는 15이고, 어느 축이 어느 판정인지는 감사 결과 표의 해당 행이 밝힌다.
- **`proposed answer`** — KAG-DEC-008~010이 답 후보나 방향을 낸 경우. 확정으로 세지 않는다.
- **`deferred/open`** — 첫 구현·첫 host 예제·durable store·별도 공개 API/의존 정책 결정까지 미뤄도 Option B accepted 전환에 방해가 없는 경우.
- **`acceptance gate`** — accepted로 올리면 `sessions`의 **파일 배치·계약과 구현의 분리·순서와 원자성 불변식·실패 계열·공개 표면**이 어느 쪽인지 정할 수 없어 반드시 사용자 선택이 필요했을 경우만. **0건이었다.**
- 사용자 소유 OQ를 임의로 고르지 않고, 후속 문서의 제안을 곧바로 확정하지 않는다. **status 전환은 이 감사가 아니라 2026-08-10 사용자 확정이 했다.**

### 원 질문 (2026-08-09 작성 시점 · 수정하지 않는다)
...
### 감사 결과 (2026-08-10 기준 routing view)

| ID | 판정 | 현재 답 / 답 후보 | 남은 축 | accepted 전환 blocker | 소유 | Next |
|---|---|---|---|---|---|---|
| OQ-1 | **partial** | **“append와 조회를 한 계약으로 둔다”는 축은 §3.1·§7.2가 단일 값으로 답했고 2026-08-10 확정에 포함됐다** — 근거는 SI2(read-your-write)가 두 계약에 걸쳐 있어 쪼개면 어느 쪽도 단독으로 보장하지 못한다는 것. **이것이 KAG-DEC-003 OQ-2에 대한 답이기도 하다** | 계약을 **Protocol로 둘지 추상 기반 클래스로 둘지**의 형태 축 | **아니다.** 형태가 무엇이든 §3.1의 module 7개·§3.3의 방향 규칙 넷·§7.2의 요구 일곱이 그대로다. 다만 §3.1이 “`store.py`에 기본 구현·공통 로직을 두지 않는다”고 못박았으므로 **어느 형태를 고르든 상속 기반 공통 로직은 배제된다** | planner · KAG-DEC-003 OQ-2의 잔여분 | sync/async(OQ-2)와 묶여 공개 API 형태 결정 시점에 함께 |
| OQ-2 | **deferred/open** | 없음. §Scope Out이 sync/async를 명시적으로 뺐다 | store 계약을 sync·async 중 무엇으로 둘지 | **아니다.** memory는 sync가 durable은 async가 자연스러워 이 package에서 특히 갈리지만, 어느 쪽이든 §4의 국면과 SI1~SI10이 그대로다 | planner · KAG-BL-001 OQ-2 | `runtime` 상세 decision(KAG-DEC-010 OQ-14)과 함께 |
| OQ-3 | **deferred/open** | 없음. §4.1 5국면이 “한 단위에 event가 여럿이면 전부이거나 없음”까지만 정했다 | append 단위에 event를 하나만 받을지 여럿 받을지 | **아니다. 어느 쪽이든 원자성은 단위 전체에 걸린다**(SG5·SI3)는 것이 이미 정해져 있다 | planner | 첫 vertical slice. tool 검증과 결과처럼 짝으로 남겨야 하는 것이 생기면 한 단위가 필요해진다 |
| OQ-4 | **partial** | **선택성 축은 §2.4·§4.1 3국면이 단일 값으로 답했고 2026-08-10 확정에 포함됐다** — 위치 기대(expected revision)는 **선택적 입력**이고 생략하면 무조건 끝에 붙인다. 감지할 값 자체를 지금 계약에 넣는 판단(§2.4)도 확정에 들어갔다 | **필수화 여부의 재평가**와 **session revision이 마지막 위치와 같은 값인지 별개 값인지**(O5가 명시적으로 열어 둔 축) | **아니다.** 선택이든 필수든 §5.1의 위치 계열과 §2.4의 “조정하지 않는다”가 그대로이고, revision 값의 정체는 O3(조밀하지 않아도 됨)과 함께 첫 예제 뒤에 본다 | 사용자 | 첫 호스트 예제 작성 시. 사용 감각 판단이라 planner가 단독으로 정하지 않는다 |
| OQ-5 | **deferred/open** | 없음. §5.1이 “**키 없는 append는 멱등이 아니다**”까지만 계약에 적었다 | 멱등 키를 **누가 만드는가** | **아니다.** 키가 없으면 중복 판정이 성립하지 않는다는 사실이 계약에 적혀 있고, 그 사실 자체가 SI4와 모순되지 않는다(SI4는 “키가 있으면”의 규칙이다). **KAG-DEC-010 §4.3이 이 미결 때문에 “append를 재시도하지 않는다”를 이미 골랐다** | planner | `runtime` 상세 decision |
| OQ-6 | **deferred/open** | 없음. §6.3 마지막 문단과 §7.3 다섯 번째 seam이 “별도 seam으로 막는다”까지만 정했다 | `memory`에도 직렬화 왕복을 강제할지 | **아니다.** 강제하든 안 하든 §3.3 규칙 3(“`memory`는 `serialization`을 참조하지 않는다”)이 **현재 확정**이고, 강제하는 쪽으로 바뀌면 그 규칙 하나만 열린다 | planner | 첫 vertical slice 직후 |
| OQ-7 | **internally resolved** | **현재 축은 §3.1·§3.2가 답했다** — `reading`은 별도 module이고 근거(“순서 보장의 단일화” — 되읽기를 store마다 쓰면 memory와 durable의 순서 보장이 갈라진다)도 적혀 있다. **module 7개는 확정 구조다** | **첫 slice 뒤 합칠지의 재평가** | **아니다.** 현재 배치는 Option B의 일부로 함께 확정 가능했고, 재평가는 구현을 본 뒤의 별도 판단이다 | planner | 첫 vertical slice 직후. **합쳐질 가능성이 가장 큰 후보이고, 합치려면 새 decision으로 supersede한다**(KAG-DEC-006 OQ-7과 같은 성격) |
| OQ-8 | **deferred/open** | 없음. §6.3이 “구체 format을 근거 없이 고르지 않는다”까지만 정했다 | 저장 format과 format version의 표기 방식 | **아니다.** Z1~Z5는 표기 방식과 무관하게 성립하고, **`memory`가 이 질문에 답하지 않아도 되는 것이 결정을 미룰 수 있게 한다** | planner | durable store를 실제로 붙이는 시점 |
| OQ-9 | **deferred/open** | 없음. Z6·SR5가 “담긴다면 구조를 읽지 않는 불투명 값”까지만 정했다 | provider 원문을 event에 **어디까지 보관할지** | **아니다.** 보관 범위와 무관하게 “구조를 읽지 않는다”가 성립한다 | 사용자 · KAG-BL-001 OQ-8 · KAG-DEC-005 OQ-8 | 저장 정책·민감도 판단과 함께 |
| OQ-10 | **deferred/open** | 없음. SR6이 “**라이브러리 append·조회 표면에는 뚫지 않는다**”까지만 정했다 | retention·법적 삭제 요구를 어디에 둘지, SF6과의 충돌을 어떻게 처리할지 | **아니다 — 다만 이 판정에는 대가가 붙는다.** SF6(원본을 지우는 경로 없음)은 **Option B의 일부로 확정됐으므로**, 삭제 요구가 실재하면 그것은 계약 밖 운영 경로여야 한다. 그 경로가 무엇인지 정해지지 않은 채로 확정한다는 사실을 여기 적어 둔다 | 사용자 | 실제 운영을 고려하는 시점 |
| OQ-11 | **deferred/open** | 없음. §5.3이 “어느 쪽이든 SI1과 §5.1의 계열 구분을 만족해야 한다”까지만 정했다 | 실패를 값으로 표현할지 예외로 표현할지 | **아니다** | planner · KAG-DEC-004 OQ-6 · KAG-DEC-005 OQ-6 계열 · KAG-DEC-006 OQ-9 · KAG-DEC-010 OQ-14 | 공개 API 형태 결정 시점에 함께 |
| OQ-12 | **deferred/open** | 없음. **§7.1의 “선별 재수출” 자체는 확정이지만**, 그 목록에 store 계약을 넣을지와 fake·실패 주입 store의 거처는 열려 있다 | 재수출 목록에 store 계약을 넣을지(“호스트용 표면”과 “구현자용 표면”의 분리) · fake store의 거처 | **아니다.** 재수출 방식(선별)이 확정됐으므로 목록의 한 항목이 열려 있는 것이고, §3.1·§3.3은 이 답에 의존하지 않는다 | planner · KAG-DEC-003 OQ-1 · KAG-DEC-005 OQ-6 · KAG-DEC-006 OQ-12 · KAG-DEC-008 OQ-16 · KAG-DEC-009 OQ-13 · KAG-DEC-010 OQ-14 | contract suite 설계·첫 배포 고려 시점에 함께 |
| OQ-13 | **deferred/open** | 없음. 아홉 문서가 각자 “**도입한다면** 같은 검사에 넣을지”만 물었다 | 정적 검사 도입 여부(KAG-DEC-001 OQ-5)와 대상 범위 | **아니다. 규칙의 존재와 자동 강제는 다르다** — §3.3의 방향 규칙 넷은 **Option B의 일부로 지금 확정됐고**, 린터로 강제할지만 미뤄진다. **규칙 1(`store`가 구현을 참조하지 않는다)은 import 목록 비교만으로 검사된다** | planner | 별도 의존성 정책 decision. KAG-DEC-003 OQ-8 · 004 OQ-8 · 005 OQ-9 · 006 OQ-10 · 008 OQ-12 · 009 OQ-12 · 010 OQ-13과 함께 |
| OQ-14 | **proposed answer** | KAG-DEC-010 §5.4가 **방향**을 냈다 — “기록하는 편이 재현에 유리하다(그 turn에 무엇이 공개됐고 어떤 한도였는가를 원장만으로 답할 수 있게 된다). 다만 **실행 context는 담지 않는 편이 권고**”이고, 그 문서도 **결정하지 않았다** | snapshot을 event로 남길지, 남긴다면 무엇을 담을지 | **아니다.** §4.4가 확인한 대로 **남긴다면 `sessions`에는 append가 하나 늘 뿐 계약은 바뀌지 않는다** | planner · KAG-DEC-002 OQ-7 · KAG-DEC-010 §5.4 | 저장 정책·민감도 판단(KAG-BL-001 OQ-9)과 묶여 있다 |
| OQ-15 | **deferred/open** | **trigger는 도달했고 답은 아직 없다.** Next가 “`core` 상세 decision이 확정된 뒤”였고 KAG-DEC-003은 2026-08-10 `accepted`가 됐다. 확정된 배치를 실물로 확인한 결과 — **원천은 있다**(`core/events.py`가 event payload 계약과 종류 범주를 소유한다, K6) 그러나 **얻는 방법은 없다**(codec module이 없고 `protocols.py`도 필수 1 + 권고 1뿐) | `core`가 codec을 제공할지, `sessions`가 매핑을 갖되 동기화를 강제할지 | **아니다.** §6.3의 허용/금지 표와 판별의 `serialization` 국소화는 어느 쪽으로 풀려도 성립하고, module 7개·tier 4단·SI1~SI10도 열리지 않는다. **다만 (가)로 가려면 KAG-DEC-003을 supersede하는 별도 decision이 필요하다** — 이 문서는 새 `core` API를 발명하지 않는다 | planner | `core`에 codec을 더할지 판단하는 시점. **그때까지 임시 매핑을 두더라도 §6.3의 금지 열을 넘지 않는다** |
...
|---|---|---|
| **`closed`** — 영구 종료 | **0** | — |
| `internally resolved` — 본문이 현재 축을 답했고 미래 trigger만 남음 | **1** | OQ-7 |
| `partial` — 한 OQ 안에 판정이 다른 축이 섞임 (OQ 하나를 1건으로 센다) | **2** | OQ-1 · OQ-4 |
| `proposed answer` — KAG-DEC-008~010이 답 후보·방향을 냄 | **1** | OQ-14 |
| `deferred/open` — 첫 구현·예제·durable·별도 정책까지 미뤄도 무방 | **11** | OQ-2 · OQ-3 · OQ-5 · OQ-6 · OQ-8 · OQ-9 · OQ-10 · OQ-11 · OQ-12 · OQ-13 · OQ-15 |
| 합계 | **15** | OQ-1~OQ-15 |

> **acceptance gate 0건.** OQ 어느 것도 `sessions`의 파일 배치·계약과 구현의 분리·순서와 원자성 불변식·실패 계열·공개 표면을 미정으로 남기지 않았다. 사용자가 승인한 것은 **Option B 전체**이고, 그 안에는 원래 열려 있던 축 둘이 함께 들어갔다 — **append와 조회를 한 계약으로 둔다**(OQ-1의 한 축)와 **위치 기대는 선택적 입력이다**(OQ-4의 한 축).
>
> **확정이 나머지를 닫지 않는다.** 15건 중 **영구 closed는 0건**이다. `internally resolved` 1건(OQ-7)은 현재 배치의 설명이지 재평가 금지가 아니고, `partial` 2건은 위에 적은 축만 확정에 들어갔으며, `proposed answer` 1건(OQ-14)은 KAG-DEC-010의 방향일 뿐 채택이 아니고, `deferred/open` 11건은 그대로다. 특히 **OQ-15는 `core` codec의 부재를 그대로 열어 둔다.**

### 정합성 확인 (2026-08-10)

#### 선행 accepted 계약과의 대조 — 위반 0건

| 선행 계약 | 이 문서의 취급 | 판정 |
|---|---|---|
| KAG-DEC-001 — `sessions → core`만 허용 | §3.3 다이어그램이 바깥 참조를 `core` 하나로만 그리고, 규칙 4가 `runtime`·`context`·`tools`·`providers`·`skills`·`process` 전부를 명시적으로 배제한다. **특히 `context`를 참조하고 싶어지는 경로를 이름으로 지목해 막는다** | 위반 없음 |
| KAG-DEC-001 §2 — 손실 없는 저장과 조회, “무엇을 보낼지의 판단은 제외” | S1~S9가 저장·조회·순서·계약까지만 담고, §1 비소유 13종이 투영·folding·검색·알림·접근 제어를 전부 밖으로 보낸다. SI5가 조회에서 필터·요약·정렬 변경을 금지한다 | 위반 없음 |
| KAG-DEC-001 §4 — 조립은 L4에서만 | §1 비소유 마지막 줄이 store 선택·주입을 L4로 두고, §7.1이 소비자를 “L4·`runtime`·`context`·테스트”로 둔다 | 위반 없음 |
| KAG-DEC-002 I1 — 기록 먼저, 사용 나중 | **SI2(read-your-write)가 그 전제의 저장 쪽 절반**이고, SR3′가 “기록한 값과 쓰는 값을 다르게 둘 수 없다”로 나머지 절반을 못박는다. §4.4가 append 다섯 지점을 12단계에 1:1로 대응시킨다 | 위반 없음 |
| KAG-DEC-002 §4 — 무엇을 언제 남길지는 `runtime` | §1 비소유 첫 줄·§4.4·SI8이 세 곳에서 같은 것을 적고, KAG-DEC-010 R5가 반대편에서 같은 배정을 받는다 | 위반 없음 |
| KAG-DEC-002 §4.3 — 짝·종료·불변식 판정은 `runtime` | SI8이 “짝·종료·상태를 판정하지 않는다”로 받고, §4.4 9단계가 “`sessions`는 종류를 모르므로 짝을 셀 수 없다”로 근거까지 적는다 | 위반 없음 |
| KAG-DEC-002 §5 — 부분 진행은 사라지지 않는다 | §4.4 마지막 줄이 **그것이 SI2와 SI3 때문에 성립한다**고 밝힌다. §5.2의 “turn이 실패·취소로 끝났다” 행이 “종료 사실은 새 event이지 앞 event의 수정이 아니다”로 SG1을 지킨다 | 위반 없음 |
| KAG-DEC-002 §5 — 조회 실패를 “없음”으로 해석하지 않는다 | SF2·§4.2 읽는 법 4가 같은 규칙이고, RG6 계열의 판단이 `tools`·`skills`와 같은 방향이다 | 위반 없음 |
| KAG-DEC-003 K6 — event payload는 `core` | §1 비소유 표와 §3.1의 `records.py` 이름 판단(“`events.py`가 아니다”)이 payload를 다시 그리지 않는다. **파일명은 인용하지 않는다** | 위반 없음 |
| KAG-DEC-003 K7 — 오류·거부 사유 표현 | `outcomes.py`가 계열을 값으로 표현하고 SR4가 model-safe와 진단 전용의 구분을 보존한다 | 위반 없음 |
| KAG-DEC-003 §4.1 — store 경계는 core에 두지 않는다 (→ OQ-2) | §3.1·§7.2가 그 경계를 `sessions` 안 `store.py` 계약 하나로 받는다. **이번 확정으로 그 답이 결정이 됐다** | 수신 완료 |
| KAG-DEC-003 §4.2 — 저장 포맷은 `sessions` | S9와 `serialization.py`가 그 배정을 받고 Z1~Z6이 규칙을 적는다 | 위반 없음 |
| KAG-DEC-003 V3·V3′ — 모르는 판별값에서 fail-closed | SI7·SF3·Z3·Z4가 네 자리에서 같은 방향이고, §6.3이 **그것이 codec의 fail-closed이지 종류별 정책 판정이 아님**을 표로 갈랐다 | 위반 없음 |
| KAG-DEC-004 — `sessions ↛ process` | §0.2와 §3.3 규칙 4가 두 번 못박는다. 파일 기반 store가 생겨도 subprocess 격리가 아니다 | 위반 없음 |
| KAG-DEC-005 PI6 — provider 원문은 구조를 읽지 않는 불투명 값 | SR5가 “실려 오면 구조를 읽지 않는다”로 받고, 보관 범위는 OQ-9로 남긴다 | 위반 없음 |
| KAG-DEC-006 TI6·TR7 — `tools`는 기록하지 않고 값만 반환한다 | §4.4의 6·8단계가 그 값을 받는 자리이고, “거부도 기록된다”가 SI1(거부도 결과)과 §4.4로 실현된다 | 위반 없음 |
| KAG-DEC-006 TR8 — 결과 크기 상한의 소유는 미결 | §5.2의 “큰 tool 결과” 행이 **자르는 쪽이 `sessions`가 되면 lossless가 깨진다**고 적고, 자를 것이면 event 구성 전에 잘려 있어야 함을 SR3와 함께 못박는다. **상한 자체는 열어 둔 채 경계만 정한다** | 위반 없음 |
| KAG-DEC-006 OQ-14 — 허용 tool 공개 정의 목록의 순서 | **다른 축이다.** 그쪽은 한 요청에 실리는 tool 정의 목록의 나열 순서이고 이 문서의 O1~O5는 session 안 event의 append 순서다. 이 문서는 tool 정의 목록을 만들지도 정렬하지도 않으므로 OQ-14가 어느 쪽으로 풀려도 O1~O5는 열리지 않는다 | 무관 (혼동 주의) |

#### 문서 내부 모순 — 0건

| 확인 항목 | 결과 |
|---|---|
| 식별자 개수 | **S1~S9 = 9** · 비소유 **13종** · **O1~O5 = 5** · **SG1~SG8 = 8** · module **7 + `__init__.py`** · tier **v0~v3 = 4단** · 방향 규칙 **4** · append **6국면** · 조회 **5국면** · **SI1~SI10 = 10** · 거부·실패 **6계열** · 상황별 소유권 **15행** · **SF1~SF6 = 6** · **Z1~Z6 = 6** · 새 store 요구 **7** · seam **6종** · 보안·관측 **10개**(SR1~SR8 + SR3′ + SR3″). 전부 중복·누락 없이 §Scope In의 서술과 일치 |
| 파일 트리 ↔ §1 책임 | 일치. S1이 `records`, S2가 `ordering`, S3·S4·S5의 **표현**이 `outcomes`이고 그 **요구**는 `store` 계약이 지며 `memory`가 만족한다, S6이 `reading`, S7이 `store`, S8이 `memory`, S9가 `serialization`. **§3.2 표 8행 = module 7 + `__init__.py`**로 트리와 같다 |
| canonical event 무손실 | 정합. SI10(받은 값을 바꾸지 않는다)·SR3(마스킹은 event 구성 전에)·SR3′(기록한 값과 쓰는 값이 같아야 한다)·SR3″(담기지 않은 것은 잃은 것이 아니다)가 **“무엇에 대해 lossless인가”에 하나의 답**(전달받은 canonical event)을 주고, §5.2의 “큰 tool 결과” 행과 §7.2 요구 7이 같은 답을 반복한다. **§0.1의 canonical 정의가 그 기준점이다** |
| codec 판별 ↔ 의미 분기 | 정합. Z4(모르는 판별값에서 멈춤)와 SG6·SI8(종류로 분기하지 않음)이 그대로는 충돌해 보이는데, §6.3의 **허용/금지 표 + 경계 3줄**이 “판별은 무엇을 만들어야 하는가까지, 해석은 그것이 무슨 뜻인가부터”로 가른다. 그리고 §7.3 여섯 번째 seam이 **“종류 이름의 부재”가 아니라 “종류를 보는 코드의 국소화”**를 본다고 기준을 바꾼 이유까지 문서 안에 적혀 있다 |
| append 순서 ↔ 조회 순서 | 정합. O2(도착 순서 기준)·O4(위치 불변)·SI5(조회는 append 순서 그대로)·SI6(읽은 구간 불변)이 한 방향이고, §5.2의 “조회 중 새 append” 행이 **새것이 이번 조회에 포함되는지는 계약이 정하지 않는다**고 경계를 분명히 한다 |
| lifecycle 순서 ↔ 불변식 | 정합. 2국면(중복)이 3국면(위치 기대)보다 먼저인 이유가 §4.1 읽는 법 1에 있고, 4국면(표현)이 5국면(반영)보다 먼저인 것이 SG5·SI3과 짝이며, 6국면 단일 결과가 SI1이다. 조회 4국면의 fail-closed가 SI7·SF3이다 |
| tier ↔ 방향 규칙 | 정합. v2 `store`가 v3 `memory`를 모른다는 것이 규칙 1이고, `reading`이 v3에서 `store` 계약만 보는 것이 규칙 2이며, `memory ↛ serialization`이 규칙 3(그 대가는 §6.3·§7.3이 받는다), 형제 무참조가 규칙 4다 |
| secret·진단 경계 | 정합. SR2가 “주입도 받지 않는다”로 KAG-DEC-004 R6·005 PR7·006 TR6보다 **한 걸음 더 가고**, 그 근거가 SR3(마스킹은 상류에서 끝난다)이다. SR4가 model-safe와 진단 전용을 합치지 않고, SR7이 접근 제어를 호스트에 남기되 **식별자 추측 가능성도 호스트 몫**임을 §2.1(발급하지 않는다)과 이어 적는다 |
| store protocol 소유권 | 정합. KAG-DEC-003 §4.1이 core에서 제외한 경계를 S7·§3.1·§7.2가 받고, §3.1의 “`store.py`에 기본 구현을 두지 않는다”가 KAG-DEC-005의 base class 기각과 같은 이유로 상속을 배제한다. §7.2의 요구 일곱이 그 계약의 목록이다 |

#### KAG-DEC-008~010의 `proposed input` 규율 — 숨은 전제 0건

세 문서 전부가 §0.2 성격의 표에서 **“KAG-DEC-007 | proposed | 확정 사실로 쓰지 않는다”**를 선언하고, 실제로 **경계의 성질만** 인용한다.

| 문서 | 무엇을 소비하는가 | 숨은 전제 |
|---|---|---|
| KAG-DEC-008 | 거의 없다 — `skills`는 아무것도 기록하지 않고 `sessions`를 import하지 않는다(SKI7). PJ7이 SI10과 같은 방향(“자르면 다른 지시가 된다”)임을 인용하는 정도다 | 없음 |
| KAG-DEC-009 | 성질 넷 — SI5(조회는 순서 그대로) · SI6(읽은 구간 불변) · SI7(모르는 판별값에서 멈춤) · SI10(canonical 그대로). **그 넷이 그 문서 무손실 정의의 입력 전제**이고, SR3″를 인용해 “event에 담기지 않은 것은 `context`가 잃은 것이 아니다”를 세운다 | 없음 |
| KAG-DEC-010 | 성질 여섯 — SI3 · SI2 · SI5 · §5.1(중복은 구분된 결과 · 멱등 키 없는 append는 멱등이 아님) · SI10 · §4.4(무엇을 언제 남길지는 전부 `runtime`). 마지막이 그 문서 R5이고, **§4.3의 “append를 재시도하지 않는다”가 OQ-5 미결 위에 선 판단**임을 밝힌다 | 없음 |

**파일명이나 트리를 전제한 곳은 없다.** 세 문서가 인용한 SI2·SI3·SI5·SI6·SI7·SI10과 §4.4·§5.1은 확정 후에도 문면 그대로이므로 **그쪽 본문을 고칠 필요가 없다.**

#### 조건부 보정 후보의 현재 상태

| # | 내용 | 현재 상태 | 결과 |
|---|---|---|---|
| 1 | **KAG-DEC-003 OQ-2 행이 아직 `proposed answer`로 적혀 있다** | 이번 확정으로 그 답(“store 경계는 `sessions` 안 `store.py`에 계약 하나”)은 **채택된 결정**이 됐다. **이 확정은 KAG-DEC-003 본문을 수정하지 않았다** | KAG-DEC-003을 다시 여는 시점에 그 행을 정정한다. **개수·구조는 바뀌지 않는다** — OQ-2는 그 문서에서 여전히 미결 집계에 포함되고, 바뀌는 것은 판정 라벨뿐이다 |
| 2 | **KAG-DEC-008~010의 “KAG-DEC-007 \| proposed” 주석** | 이 확정이 그 셋을 수정하지 않았다 | 각 문서를 리뷰하는 시점에 함께 정리한다. **성질 인용은 확정 후에도 전부 그대로 정확하다** |
| 3 | **OQ-15가 (가) `core` codec 제공으로 풀리는 경우** | 여전히 열려 있다. 확정된 `core` 배치에 codec이 없다 | **KAG-DEC-003을 supersede하는 별도 decision이 필요하다.** 이 문서 쪽에서는 §6.3의 마지막 문단과 `serialization`의 입력이 바뀌고 §3.1·§3.3·SI1~SI10은 열리지 않는다 |
| 4 | **OQ-7이 “합친다”로 풀리는 경우** | 여전히 열려 있다. 현재 확정은 module 7개다 | §3.1 트리·§3.2 표·§3.3 tier v3·§Scope In의 개수 서술이 함께 열린다. **accepted 구조를 바꾸는 일이므로 새 decision으로 supersede한다** |

## Next

**이 문서는 2026-08-10 사용자 확정으로 `accepted`다.** 확정 범위는 §Decision의 Option B(§1~§8)이고, 원래 열려 있던 축 둘(append와 조회를 한 계약으로 둔다 · 위치 기대는 선택적 입력이다)이 함께 들어갔다.

### 2026-08-10 사용자 확정으로 바뀐 것

1. frontmatter `status: proposed` → `accepted`, `tags`의 `status/proposed` → `status/accepted`, `updated_at`.
2. 상단 상태 blockquote를 KAG-DEC-001~006과 같은 “**상태 `accepted` (2026-08-10 사용자 확정)**” 형태로 교체하고, §Decision 서두를 채택·비권고·기각·보류 형태로 바꿨다.
3. **선행 상태 정합화** — KAG-DEC-003·004·005·006이 `accepted`가 되어 어긋난 주석을 §Context 선행 결정 줄·§0.1·§0.2 네 행·§3.3 마지막 문단·§7.1 근거 1·§Rationale 리스크 둘·§Scope Out 두 행에서 정정했다. **연결은 여전히 범주 수준이다.**
4. **KAG-DEC-003 OQ-2의 답이 결정이 됐다.** §Decision과 §7.2가 그 사실을 적고, **KAG-DEC-003 본문은 수정하지 않았다**(§정합성 확인 조건부 후보 1).
5. **OQ-15의 trigger 도달을 사실대로 기록했다** — 원천(`core/events.py`의 event 계약)은 accepted로 실재하지만 **얻는 방법(codec)이 확정된 `core` 배치에 없다.** 새 `core` API를 발명하지 않고 routing만 날카롭게 했다(§6.3·§Rationale).
6. **§Decision 본문의 상태 용어 정합화** — §2.4·§4.1 3국면의 “권고”를 확정 표현으로, §4.1 도입의 “이 권고 위반”을 “이 결정 위반”으로, §7.1 “권고: 선별 재수출”을 “확정”으로, §7.2 “쪼개지 않는다(권고)”를 “(2026-08-10 확정)”으로. **기술 결정·module 7개·tier 4단·국면 6+5·SI/SG/SF/Z/SR/O 개수는 하나도 바꾸지 않았다.**
7. decision index·제품 README·`log.md` 정합화. **다음 사용자 게이트는 KAG-DEC-008(skills 경계) accepted 준비 리뷰다.**

### 확정으로 바뀌지 않은 것

1. **OQ 15건 중 영구 closed는 0건이다.** `internally resolved` 1건(OQ-7)은 현재 배치의 설명이고, `partial` 2건(OQ-1·OQ-4)은 한 축만 확정에 들어갔으며, `proposed answer` 1건(OQ-14)은 KAG-DEC-010의 방향일 뿐 채택이 아니고, `deferred/open` 11건은 그대로다.
2. **§Scope Out은 그대로다.** exact signature·저장 schema와 format·DB 제품·연결과 migration·sync/async·retention·암호화·법적 삭제 절차·queue와 lease·event 종류 확정 목록은 여전히 범위 밖이다.
3. **`sessions`는 event의 의미를 읽지 않는다.** 표현 복원을 위한 구조적 판별만 `serialization` 안에서 허용되고, 종류별 저장·조회·필터·상태 판정은 계속 금지다(§6.3).
4. **삭제 경로는 여전히 계약에 없다.** SF6·SI9가 확정됐으므로 retention 요구가 실재하면 그것은 append·조회 표면 밖의 운영 경로여야 하고, 그 경로는 아직 정해지지 않았다(OQ-10).
5. **KAG-BL-001·KAG-DEC-001~006의 `accepted`와 KAG-DEC-008~010의 `proposed`는 이 확정이 바꾸지 않는다.**

**다음 게이트는 KAG-DEC-008(skills 등록·선택·prompt 투영 경계)의 사용자 리뷰다.** KAG-DEC-008~010은 이제 이 문서를 `proposed input`이 아니라 **`accepted` 선행 결정으로** 참조할 수 있다. 다만 그 세 문서 본문의 “KAG-DEC-007 | proposed | 확정 사실로 쓰지 않는다” 주석은 **각 문서를 리뷰하는 시점에 함께 정리한다** — 이 확정이 그 문서들을 수정하지 않았다.

## Resulting Spec

| Spec | Action | Notes |
|---|---|---|
| (없음) | - | 이 decision은 spec을 만들지 않는다. `sessions` 상세만 확정하며, 형제 package 상세가 정리된 뒤에 첫 spec을 연다. 미래 decision/spec ID를 미리 선점하지 않는다 |

    ...
    <div style={{ marginTop: 10, display: "grid", gap: 14 }}>
      ...
      <section>
        ...
        <div style={{ marginBottom: 8 }}>
          ...
          {(draft.daily.summary ?? []).length === 0 && (
            <p style={{ fontSize: 12, color: "var(--fg-4)" }}>줄이 없습니다.</p>
          )}
          {(draft.daily.summary ?? []).map((line, i) => (
            <div key={i} style={{ display: "flex", gap: 6, marginBottom: 4 }}>
              <input
                value={line}
                disabled={disabled}
                onChange={(e) => {
                  const next = [...(draft.daily.summary ?? [])];
                  next[i] = e.target.value;
                  setDaily({ summary: next });
                }}
                className="mono"
                style={{
                  flex: 1,
              ...
              <button
                ...
                title="이 줄을 지웁니다"
                onClick={() => {
                  const next = (draft.daily.summary ?? []).filter((_, j) => j !== i);
                  setDaily({ summary: next });
                }}

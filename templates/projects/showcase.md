...
코드나 프롬프트에 규칙을 복사하지 않는다. 복사하는 순간 양식 원천이 둘이 되고 한쪽만 고쳐지는
날 조용히 어긋난다.
...
두고 있어 DB 가 값을 막지 않는다. 옛 구조는 `persona/_meta.yaml` 이 소유하고 로더가
검증했는데, 그 자리가 새 구조에 없다.

## 반드시 지킬 것

- 필수 필드 7종: `type` · `id` · `title` · `summary` · `category` · `status` · `stack`
- 파일명은 반드시 `showcase.md` — 제품 디렉토리 깊이 1

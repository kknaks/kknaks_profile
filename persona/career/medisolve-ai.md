...
- 스펙을 구현 전에 완벽하게 쓰기보다, 구현·실측 중 드러난 괴리(배정 알림 종수, round_no 축 오류, DnD 삽입선 좌표계 불일치, MCP 토큰 만료·스트림 유실 위험 지점, 도메인 스펙과 공용 카드 골격 간 필드명 불일치로 인한 값 누락 등)를 "소정합"이라는 이름으로 스펙에 계속 환류하는 방식으로 운영한다.

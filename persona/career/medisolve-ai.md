...
피부과 전용 CRM과 MSO(Multi-Site Operation) 백엔드를 맡고 있으며, 최근에는 레거시 의사결정 흐름의 Action Runtime 이식(위임전결표 SoT화 포함)을 마무리하고 랜딩 페이지 에이전트 채팅 기능의 스펙과 구현을 새로 주도하고 있다.

## 담당 영역

backend 중심으로 도메인 모델과 API를 설계하고, 제품 스펙(SPEC)과 작업계획(WP) 문서로 워크플로 계약을 먼저 정의한 뒤 별도 구현 레포의 실제 구현과 짝을 맞추는 방식으로 일한다. 그 과정에서 알림·권한·태스크 라이프사이클·프론트 데이터 로딩 경계 같은 크로스커팅 관심사를 스펙 레벨에서 조율하고, 위임전결표 같은 조직 거버넌스 문서도 SoT 관점에서 함께 정리한다.

## 챌린지

- 스펙을 구현 전에 완벽하게 쓰기보다, 실측에서 드러난 전제 오류(설정값 부재, 승인 게이트 툴 부재, 배정 알림 종수, round_no 축 오류 등)를 스펙에 계속 환류하는 방식으로 운영한다.
...
- 운영 중 발생한 사고(프롬프트 스토어 미등록으로 평가가 구버전으로 회귀)를 원인까지 추적해, 재발 방지 규칙을 그 자리에서 스펙으로 신설하는 방식으로 대응한다.
...
- 조직 규칙 문서(위임전결표)를 SoT로 선언하고 사람 정보 조회를 별도 서비스로 위임하면, 문서 하나가 여러 목적을 겸하지 않아도 돼 유지비용이 낮아진다.

...
- [[aop]] — 봉투 처리를 컨트롤러에서 걷어내는 장치

## 출처

- [[2025-01-21-Day16]] — 보름 뒤. **봉투를 쓰는 대가가 드러나고 그것을 걷어낸다.** 모든 컨트롤러가 `response.setStatus(rsData.getStatusCode())` 를 반복해야 했는데 — 봉투 안의 코드와 HTTP 상태 코드를 **맞춰 주는 일**이 자리마다 생긴 것이다 — AOP 로 그 줄을 한 곳으로 옮긴다. 「봉투는 상태 코드를 대신하는 것이 아니다」라는 이 노트의 원칙이 **자동으로 지켜지게** 만든 형태다 → [[aop]]

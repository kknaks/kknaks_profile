...
Day66 의 문제(실패 분기에 롤백이 없어 다음 요청 커밋에 얹힌다)가 Day67 에서 **절반 고쳐진다.**
...
try {
  ...
  sqlSessionFactory.openSession(false).rollback();      // ← 이 줄이 생겼다
  req.setAttribute("exception", e);
  req.getRequestDispatcher("/error.jsp").forward(req, res);
}
```

**예외로 끝나는 길이 닫혔다.** 두 문장 중 뒤가 실패하면 앞의 것도 취소된다.

**남은 절반은 「예외 없이 실패하는 길」이다** — Day66 의 삭제처럼 `if (dao.delete(...))` 가 `false` 를 돌려주는 경우는 예외가 아니므로 `catch` 에 걸리지 않는다. 그 분기에 롤백이 있는지는 회차마다 다르다.

그리고 **성공 뒤 `sendRedirect`** 가 붙었다 — 처리한 URL 에 머무르지 않으므로 새로고침이 등록을 반복하지 않는다 → [[redirect]] · [[http-method]]
...
- [[database-lock]] — 이 경계 안에서 잡히고 풀리는 것
...
- [[2025-01-07-Day06]] — **경계를 눈으로 본다.** MariaDB 의 `general_log` 를 켜고 `tail -f` 로 들여다보면 JPA 가 보내는 것이 그대로 찍힌다 — `set autocommit=0` → 조회 → `COMMIT` → `set autocommit=1`. **트랜잭션이 열리고 닫히는 자리가 로그의 네 줄로 나타나는 것**이 이 회차의 가장 값진 확인이고, 「추측하지 말고 실제로 나가는 쿼리를 보라」는 방법이 함께 남는다. 그리고 락이 **이 경계와 같은 수명**을 갖는다는 것도 여기서 드러난다 → [[database-lock]]
...
- [[2024-08-30-Day67]] — 하루 뒤. `catch` 에 `rollback()` 이 생겨 **예외로 끝나는 길이 닫힌다.** 성공 뒤 `sendRedirect` 로 목록으로 보내는 것까지 붙어, 처리 URL 에 머물러 새로고침이 재등록하는 문제도 사라진다. 다만 예외 없이 `false` 를 돌려주는 실패는 여전히 `catch` 에 걸리지 않는다

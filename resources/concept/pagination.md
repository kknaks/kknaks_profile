...
- [[elasticsearch]] — 페이지네이션을 엔진에 맡기는 자리

## 출처

- [[2025-10-01-insight]] — **DB 레벨로 안 내렸을 때 무슨 일이 나는지**가 실무 기록으로 남았다. 문제 원인 둘째가 「전체 데이터를 메모리에서 분할하는 방식 사용 / DB 레벨에서 LIMIT/OFFSET 미적용 / 공시 데이터 조회 시 100개 제한으로 인한 데이터 불완전성」이다 — **다 읽어 놓고 잘라 쓰면** 응답 크기만 줄고 부하는 그대로이고, 그것을 막으려 건 100개 제한이 이번에는 **데이터가 빠지는 문제**가 됐다. 개선은 정렬·페이지네이션을 검색 엔진 레벨로 내려 둘을 함께 없앤다 → [[elasticsearch]]

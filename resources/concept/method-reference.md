...
### 36일 뒤 Day61 — 쓰는 이유가 「짧게」가 아니라 「길어서」다

Day61 의 `DaoFactory` 가 `Proxy.newProxyInstance` 의 세 번째 인수에 이것을 넘긴다.
...
**`this::invoke` 는 Day37 의 `보통예금::year` 와 같은 형태다** — 왼쪽이 타입이 아니라 객체이고, 그 객체가 이번에는 `this` 다. 잡아 나가는 상태도 있다(`DaoFactory` 의 `sqlSession` 필드) → [[object-reference]]

**그런데 쓰는 이유가 Day37 의 사다리 은유와 반대다.** Day37 은 로컬 클래스 → 익명 클래스 → 람다 → 메서드 레퍼런스를 「점점 줄어드는」 순서로 배웠고, 그래서 마지막 칸이 「가장 짧은 표기」로 읽힌다. Day61 이 적은 이유는 **「invoke의 함수가 길기 때문에」**다(문장이 거기서 끊겨 있다) — 같은 노트의 첫 코드 블록은 익명 `InvocationHandler` 를 인수 자리에 그대로 쓰는 형태이고, 몸통이 길어지면 **인수 자리에 코드 스무 줄이 들어가 `newProxyInstance` 의 세 인수가 안 보인다.** 그래서 몸통을 밖으로 빼 이름을 주고 그 이름만 넘긴다.

| | 무엇이 인수 자리에 있나 |
|---|---|
| 익명 클래스 · 람다 | **몸통 전체** — 길면 호출문이 묻힌다 |
| 메서드 레퍼런스 | **이름 하나** — 몸통은 딴 곳에 있고 이름이 붙는다 |

**즉 이 표기가 하는 일은 「짧게 쓰기」가 아니라 「몸통을 인수 자리에서 떼어 내기」다** — 그리고 그때 그 메서드에 이름이 생긴다. 사다리의 마지막 칸이 위 칸의 축약이 아니라 **다른 목적으로 골라지는** 자리다 → [[anonymous-class]] · [[cohesion]]

`private` 메서드를 가리키는 것도 Day37 에 없던 형태다. Day37 의 대상은 전부 `public` 이었는데, **같은 클래스 안에서는 `private` 도 된다** — 접근 검사가 표기가 쓰인 자리를 기준으로 일어나기 때문이다 → [[access-modifier]]
...
- **시그니처를 맞춰 보는 축이 셋이 아니라 넷이다 — 빠진 것이 예외이고, Day61 이 그 자리에 걸린다** — Day37 의 표는 개수·매개변수 타입·반환 타입 셋을 세웠는데 **던지는 예외**도 맞춰야 한다. 규칙의 방향은 반환 타입 쪽과 같다 — **참조된 메서드가 더 좁게(적게) 던져야** 하고, 추상메서드가 선언하지 않은 검사 예외를 던지면 컴파일 오류다. Day61 의 `InvocationHandler.invoke` 는 `throws Throwable` 인데 `DaoFactory.invoke` 에는 `throws` 가 없다 — **좁은 쪽이라 끼워지는 것 자체는 통한다.** 대가는 뒤에 온다: 그 몸통에서 `sqlSession` 이 검사 예외를 던지면 **잡거나 감싸야** 하고, 시그니처를 넓힐 수는 있지만 그러려면 그 메서드 선언을 고쳐야 한다. **넓게 선언된 자리에 좁게 끼우는 것이 지금은 편하고 나중에 제약이 되는** 형태이고, Day37 의 예제들은 아무것도 던지지 않아 이 축이 보이지 않았다 → [[exception-handling]] · [[dynamic-proxy]]
- **「리턴타입과 매개변수가 같으면 메서드 레퍼런스 사용가능」에는 조건이 하나 더 있다 (Day61 의 주석)** — 대입받는 타입이 **추상 메서드가 하나뿐인 인터페이스**여야 한다. `InvocationHandler` 가 그래서 성립한다. 시그니처만 보고 「모양이 맞으면 된다」로 읽으면 추상 메서드가 둘인 인터페이스에 왜 못 끼우는지가 설명되지 않고, 이 노트의 마지막 항목(「추상메서드가 하나일 때만 성립한다」)과 같은 이야기다. 그리고 Day37 의 스태틱 장이 「매개변수와 동일해야」로 적은 것을 **36일 뒤 주석이 그대로 반복한다** — 실제로는 매개변수 타입은 넓어도 되고 「동일」이어야 하는 것은 개수뿐이다 → [[functional-interface]]
...
- [[dynamic-proxy]] — 이 표기로 호출 핸들러를 넘기는 자리
- [[exception-handling]] — 표에 빠져 있던 네 번째 축
...
- [[2024-08-22-Day61]] — 36일 뒤. **이 표기를 실습 코드에서 쓰는 첫 자리**다 — `Proxy.newProxyInstance` 의 세 번째 인수(`InvocationHandler`)로 `this::invoke` 를 넘기고, 주석으로 성립 조건을 적었다(「리턴타입과 매개변수가 같으면 메서드 레퍼런스 사용가능」 — 추상 메서드가 하나뿐인 인터페이스여야 한다는 조건이 빠졌고, Day37 스태틱 장의 「동일해야」를 그대로 반복한다). 형태는 Day37 의 `보통예금::year` 와 같은 **`객체::인스턴스메서드`** 인데 왼쪽이 `this` 이고 대상이 **`private` 메서드**라, Day37 에 없던 두 변형이 함께 나온다. **가장 값이 큰 것은 쓰는 이유다** — 「invoke의 함수가 길기 때문에」(문장이 끊겨 있다)이고, 같은 노트의 첫 코드 블록이 익명 `InvocationHandler` 를 인수 자리에 그대로 쓴 형태여서 대비가 된다. 즉 이 표기가 골라진 이유가 **「짧게 쓰기」가 아니라 「긴 몸통을 인수 자리에서 떼어 내 이름을 주기」**이며, Day37 의 「점점 줄어드는 네 칸 사다리」로는 설명되지 않는 동기다. 그리고 `InvocationHandler.invoke` 가 `throws Throwable` 인데 `DaoFactory.invoke` 에는 `throws` 가 없어 **Day37 의 시그니처 표에 없던 네 번째 축(예외)**이 드러난다 — 좁게 던지는 것은 허용되므로 끼워지지만, 그 몸통에서 검사 예외를 던질 수 없게 된다. 다만 같은 블록의 `return new Proxy.newProxyInstance(…)` 가 정적 메서드에 `new` 를 붙인 문법 오류라 이 코드는 실행되지 않는다 → [[dynamic-proxy]]

...
- **데이터가 두 곳에 있으면 어긋난다** — DB 에는 지웠는데 색인에는 남는 식이다. **동기화가 이 구조의 항상적인 비용**이고, 「실시간 검색」도 색인이 반영된 뒤의 이야기다 → [[transaction]] · [[data-pipeline]]
...
- [[data-pipeline]] — 원본과 색인을 맞추는 장치

## 출처

- [[2025-01-22-Day17]] — 하루 뒤. **「둘을 맞춰 주는 일」이 실물이 된다** — Logstash 가 30초마다 MySQL 을 읽어 `app1_posts` 인덱스에 밀어 넣고, `document_id` 를 원본의 기본키에 맞춰 같은 행이 겹치지 않게 한다. Day15 가 ELK 의 Logstash 를 「데이터동기화, 파이프라인」 한 줄로만 적었던 것이 여기서 설정 파일로 채워진다. 다만 **삭제가 전해지지 않는다**는 이 방식의 큰 구멍은 다뤄지지 않았다 → [[data-pipeline]]

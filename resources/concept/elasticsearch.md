...
- [[async-io]] — 검색을 비동기로 부를 때의 이점
- [[performance-testing]] — 도입 효과를 재는 방법

## 출처

- [[2025-10-01-insight]] — 아홉 달 뒤. **학습이 실무 판단으로 쓰인 자리**다. 관심 그룹의 뉴스·공시 조회를 MySQL 에서 Elasticsearch 로 옮기며 얻은 것이 셋으로 정리됐다 — ① **집계(Aggregation)로 질의를 합쳤다**: 국가별로 나뉘어 US 1회 + KR 1회 + 그룹 확인 1회였던 **3회 호출이 1회 검색**이 된다 ② **정렬·페이지네이션을 엔진 레벨에서** 처리해 메모리에서 자르던 것을 없앴다 ③ **파라미터 바인딩**으로 f-string 이 만들던 SQL 인젝션 위험이 사라졌다. 그리고 k6 로 잰 결과가 **고부하일수록 격차가 커진다**는 것을 보인다(50 VUs 90% ↔ 300 VUs 215% 향상) → [[async-io]] · [[performance-testing]]

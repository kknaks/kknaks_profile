...
- [[sorting]] — 짝짓기 전에 같은 것을 붙여 두는 전처리
...
- [[2025-05-08-Pr3986]] — 좋은 단어 판별. **이 자료구조가 딱 맞는 문제**다 — 글자를 하나씩 보며 스택의 맨 위와 같으면 빼고 다르면 넣는다. 끝에 비어 있으면 좋은 단어다. 「stack 자료 구조를 사용하여 짝짓기」 한 줄이 풀이 전부이고, 괄호 짝 맞추기와 같은 골격이다
- [[2025-05-08-Pr1213]] — 팰린드롬 만들기. **정렬을 먼저 해서** 같은 글자를 붙여 놓은 뒤 스택으로 짝을 짓고, **짝이 안 맞는 글자가 둘 이상이면 불가능**하다고 판정한다 — 스택이 「짝을 세는 도구」로 쓰이는 변형이다 → [[sorting]]

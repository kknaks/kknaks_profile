...
summary: 요청 핸들러를 고르는 조건(params·headers·produces·consumes)과 아규먼트로 받을 수 있는 것들(@RequestParam·도메인 객체·@RequestHeader·@CookieValue·@InitBinder), 그리고 리턴값으로 응답을 만드는 방법(@ResponseBody·HttpEntity·ResponseEntity)을 정리한 회차
author: 이건학
date: 2024.10.16
tags:
  - bitcamp

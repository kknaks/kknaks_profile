...
summary: MySQL 서버에 접속해 사용자를 만들고 데이터베이스 권한을 준 뒤, DDL 로 데이터베이스와 테이블을 만들며 컬럼 타입·NULL·기본값·PK·UK·인덱스·자동증가와 alter table 까지 다룬다
author: 이건학
date: 2024.08.06
tags:
- bitcamp
stack: [MySQL, SQL]
links: [2024-08-05-Day48, 2024-08-07-Day52]
---
...
### 테이블 생성
  ...
  - 테이블 정보 보기 
    ...
    ![image](https://github.com/user-attachments/assets/9bfbc4fe-8a07-4275-a798-b4aa92d50e50)
...
- not null 옵션을 주면, insert into 시에 해당 컬럼을 생략 할 수 없다.
  ...
  ![image](https://github.com/user-attachments/assets/93a9d33c-ceae-44e7-a479-89fc1d7484e9)
...
- 값을 입력시에 null로 입력값을 주면 NULL값이 들어간다
  ...
  ```
  ![image](https://github.com/user-attachments/assets/8661bcc6-1a7c-4e91-abd9-1b023832d77b)
...
- **key** : 데이터를 구분 할 수 있는 컬럼들의 집합
  - ex) {email}, {jumin}, {id}, {name, tel}, {tel, basic_addr, gender, name}
...
- pk를 적용하기 위해서는 옵션에 primary key라고 선언한다.
    ...
    ```
    ![image](https://github.com/user-attachments/assets/38b32506-c897-4672-b8f8-ced8382527d3)
...
- pk가 그룹으로 지정되어야 한다면, constraint 제약조건이름 primary key (컬럼명, 컬럼명, ...) 으로 선언한다.
  ...
  -> constraint primary key (name,age));
    ```
  ![image](https://github.com/user-attachments/assets/64581b70-8542-42bf-9063-bbe7dc64fdb4)
...
- primay key는 아니지만 다른 값들이 중복되면 안된느 컬럼을 지정 할 때 사용한다. 
  ...
  insert into test1(no,name,age,kor,eng,math) values(4,'c',20,81,81,81);
   ...
   ```
  ![image](https://github.com/user-attachments/assets/7460f70d-fecb-496f-83b7-bc73a02f277c)
...
### index 적용
 ...
 - 입력/변경/삭제 시에는 속도가 느려지지만, 조회에서는 속도가 빠르다.
   ...
   ```
   ![image](https://github.com/user-attachments/assets/28bf6d9f-20a1-44fe-bdf0-12352ec9ac0c)
...
- 한 테이블 당 하나만 지정 가능하다.
  ...
  ![image](https://github.com/user-attachments/assets/59167873-31e3-481e-840c-c074798a4d55)

...
summary: 멀티쓰레드 서버가 SqlSession 하나를 공유하면 캐시와 커밋 경계가 함께 섞인다는 문제를 짚고, SqlSessionFactory 에 GoF 프록시(SqlSessionFactoryProxy)를 씌워 ThreadLocal 로 쓰레드마다 같은 SqlSession 을 돌려주게 했다. DaoFactory 와 Command 구현체는 SqlSession 대신 SqlSessionFactory 를 받아 그때그때 세션을 얻어 쓰고 commit/rollback 한다
author: 이건학
date: 2024.08.23
tags:
- bitcamp

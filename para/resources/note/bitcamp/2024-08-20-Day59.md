...
summary: Reflection API 로 클래스를 문자열 이름으로 로딩하고(Class.forName) 이름·수퍼클래스·중첩클래스·인터페이스 정보를 꺼낸 뒤 메서드·필드·생성자를 찾아 invoke·set·newInstance 로 직접 부리는 것까지 훑었다. 후반은 MyBatis 로 넘어가 SQL Mapper 와 OR Mapper 를 가르고 mybatis-config.xml 과 매퍼 XML 을 준비해 SqlSessionFactoryBuilder → SqlSessionFactory → SqlSession 사다리를 세우고 selectList·insert·selectOne·update·delete 다섯 메서드로 DAO 의 JDBC 코드를 XML 로 옮겼다
author: 이건학
date: 2024.08.20
tags:
- bitcamp
...
- Mybatis를 사용해서 JDBC API의 역할을 이전하고 소스에서는 자바코드만 작성한다.
    ...
    ![image](https://github.com/user-attachments/assets/b936cbb4-ab91-47e5-8669-7eef8468c3cf)
...
- SqlSessionFactoryBuilder가 xml파일을 읽어 Factory 객체를 인스턴스해야한다.
    ...
    ![image](https://github.com/user-attachments/assets/de9e5a2b-798c-46ec-8063-b0ea47da9ecc)
  ㅌ
    ```java
...
- User객체들을 List에 add한고 List를 리턴한다.
    ...
    ```
    ![image](https://github.com/user-attachments/assets/a6bc431d-8821-45e5-828f-e943c67924c3)
...
- 완성된 sql구문을 sql서버에 전송한다.
    ...
    ```
    ![image](https://github.com/user-attachments/assets/e55276ab-8a09-45c3-9916-a5058de3525e)
...
- 완성된 객체를 java에 넘겨준다.
    ...
    ```
    ![image](https://github.com/user-attachments/assets/052833ab-90cf-4811-8c6d-f1362d9f3188)
...
- user의 property name으로 getProperty() 메서드를 통해 값을 얻어 sql구문을 call한다.
  ...
  ```
  ![image](https://github.com/user-attachments/assets/61063af5-f662-470c-bb46-b4cc814a0add)
...
- primitive 타입의 property name은 없기때문 사용자가 임의로 작성 가능하다.
  ...
  ```
  ![image](https://github.com/user-attachments/assets/c3f0ca3d-86fc-41df-a321-f2328042d620)

...
flowchart LR
    ...
    NEWD["한국인 신환 수"] == "산출(0)" ==> NEW
    NEWF["외국인 추정 신환 수"] == "산출(0)" ==> NEW
    SF["외국인 추정 매출"] == "산출(0)" ==> SALES
    SF == "산출(0)" ==> FSS["외국인 매출 비중<br/>(의존 리스크 KPI)"]
    CH["외국인 유입 채널<br/>(미관측)"] -. "보류 — 데이터 부재" .-> NEWF
    NEWF -. "보류 — 구축 효과 미확정" .-> NEWD
    WD -.-> RSV
    SE -.-> NEW
    HO -.-> VST
```
...
노드와 엣지는 **데이터 자산으로 산출**했다 — 관계를 Agent Instruction 에 넣지
않는다는 방법론(S-001)의 구현이다. 최초 구성은 노드 19종 · 엣지 20행이었고,
개정 1(4b장 — 내·외국인 분해)로 **노드 25종 · 엣지 27행**이 현재 상태다
(할인율 attribute 노드 포함 — 기각 엣지의 참조 무결성용).
...
## 4b. 개정 1 — 내·외국인 분해 반영 (2026-09-01, 담당자 요청)

글로서리 개정 3(외국인 추정)과 05 개정 4(KPI 승격 2종)를 Meta 에 반영한다.

**노드 5종 추가** (nodes.csv):

| 노드 | 유형 | 비고 |
| --- | --- | --- |
| 한국인 신환 수 | kpi | 정식 KPI (상태 부여) — 현 시점 핵심 위기 지표 |
| 외국인 추정 신환 수 | kpi | 분리 원천 |
| 외국인 추정 매출 | kpi | 분리 원천 |
| 외국인 매출 비중 | kpi | 정식 KPI — 높을수록 의존 리스크 |
| 외국인 유입 채널 | **unobserved** | 새 노드 유형 — 조작 가능하나 **데이터에 없음**(담당자 확인 항목). 미관측 원인 후보를 그래프에 자리만 만들어 두는 장치 |

**엣지 7행 추가** (edges.csv):

- 산출(0) 5행 — 분해 항등식: 한국인·외국인 신환 →(0) 신환 / 외국인 매출 →(0) 매출 /
  외국인 매출·매출 →(0) 외국인 매출 비중. 정의이므로 자동 확정.
- **보류 2행** — ① 외국인 신환 → 한국인 신환 (구축 효과): 일 잔차 r=−0.25 임계 미달,
  월 r=−0.92 는 추세 혼입으로 기각(기록 06 부기) — **미확정**. ② 외국인 유입 채널 →
  외국인 신환: 미관측 — 채널 기록 수집이 선결 과제. 매출 57%가 걸린 경로가 그래프에서
  물음표로 남아 있다는 사실 자체가 이 개정의 핵심 정보다.
  **[채널 단서, 2026-09-01 추가]** 중화권 대상 페이스북 페이지 「Ceramique Clinic
  韓國江南店」(profile.php?id=61582724153639)의 실존을 확인했다 — nexus 카탈로그의
  8개 언어(zh-CN·zh-TW 포함) 운영과 정합하는 유입 채널 후보. 게시물은 로그인 벽으로
  미수집 상태이며, 게시 일자 시계열을 확보하면 네이버 리뷰와 같은 개입 신호로
  쓸 수 있다 — 수집 과제로 등록.

노드 합계 25종(kpi 17 + 개입 2 + 유기 1 + 외생 3 + 미관측 1 + 속성 1), 엣지 27행
(채택 4 · 산출 14 · 외생 3 · 보류 3 · 기각 3). 참조 무결성 검사 통과 — 고아 엣지 0.

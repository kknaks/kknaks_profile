...
6. **명세 없는 추출을 하지 않는다 (2026-08-31 추가 — 1차 실버·골드 폐기의 교훈).**
   실버·골드의 계산 명세는 [[hospital-revenue-semantic-view]] 가 단일 원천이다.
   추출은 층마다 검증 게이트를 통과하고 **사용자 승인을 받은 뒤에만** 다음 층으로
   간다. 「뽑고 → 검증하고 → 수정하고 → 다시 뽑는다」 — 검증 없이 EDA 로 넘어가지
   않는다. 폐기 경위는 정의서 §0.
7. **데이터는 [[medallion-architecture]] 원칙으로 계층화한다.** 원본(원천 덤프 · 매출
...
| 단계 | 상태 |
| --- | --- |
| 0~1. 반입 · 인벤토리 | ✅ 브론즈 3종 (Vegas 235개 · NEXUS 참조 14개 CSV · 리뷰 1,962건) |
| 2. 용어 정의 | ✅ [[hospital-revenue-glossary]] — 확인 목록 6건 전부 종결 |
| 3. 실버 · 골드 | ❌ **1차 산출물 폐기** (2026-08-31). 명세를 먼저 세우고 재작업 — [[hospital-revenue-semantic-view]] 작성 완료, **재추출은 사용자 승인 대기** |
| 4. EDA | 미착수 취급 — 1차 EDA 결과는 폐기된 데이터 기반이라 무효 |
| 5~6. Meta · Node | 대기 |

## 5. 아직 안 정한 것

- 프로모션 노출 플래그를 시작 판정에 쓸지 — 정의서 §4, 재추출 승인 전에 정한다
- 리뷰 시술 태그 폐쇄 목록 — 정의서 §2, 사람 승인 필요
- Node 인스턴스 레벨 — 6단계에서

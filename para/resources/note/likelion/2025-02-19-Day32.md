...
type: reference
id: 2025-02-19-Day32
title: Day32 스프링 이벤트로 종속성 제거
summary: 모놀리식을 서비스→모듈→종속성 분리→마이크로서비스 순으로 쪼개며, 같은 테이블을 도메인마다 다른 엔티티(Author)로 보는 역할 기반 설계와 그 한계를 짚고, ApplicationEventPublisher·@EventListener로 모듈 간 직접 호출을 끊은 회차
author: 이건학
date: 2025.02.19
tags:
  - 멋쟁이사자처럼

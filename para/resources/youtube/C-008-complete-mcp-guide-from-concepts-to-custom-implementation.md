...
summary:
  en: 'Master connecting LLMs with external tools: understand MCP fundamentals, integrate existing protocols, and build custom MCP servers with FastMCP'
...
- stdio(로컬 명령어 기반)와 streamable HTTP(원격 웹 서비스)의 두 가지 연결 방식으로 로컬 또는 클라우드 MCP 서버를 구성할 수 있다.

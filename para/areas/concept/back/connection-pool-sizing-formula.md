...
- [[osiv]] — 커넥션을 요청 내내 붙드는 설정
- [[performance-testing]] — 풀이 실제로 마르는지 확인하는 방법
...
- [[2025-02-10-Day25]] — **계산한 것을 확인할 방법**이 생긴다. 풀 크기를 공식으로 정해도 그것이 맞는지는 부하를 걸어 봐야 알고, K6 의 `http_req_waiting`(원격 응답을 기다린 시간)과 `http_req_blocked`(연결 슬롯을 기다린 시간)이 **대기가 어디서 생기는지**를 갈라 보여 준다 → [[performance-testing]]
- [[2025-01-02-Day03]] — **풀이 마르는 새로운 경로**가 나온다. 지금까지는 「쿼리가 느려서」 커넥션이 오래 잡혔다면, 여기서는 **요청 자체가 안 끝나서** 잡힌다 — SSE 연결 하나가 몇 분씩 열려 있는데 OSIV 가 요청 내내 커넥션을 붙들므로, **동시 접속자 수가 곧 필요한 커넥션 수**가 된다. 「다수의 SSE 클라이언트가 연결되면 커넥션 풀이 빠르게 소진될 수 있다」가 그 결과이고, **풀을 키워서 풀 문제가 아니라는 것**을 보여 주는 사례다 → [[osiv]]
- [[C-024-db-connection-pool-sizing-principles-and-production-pitfalls]] — PostgreSQL 위키 기원 공식 소개; NVMe 환경에서 스핀들 = 0으로 사실상 `코어 × 2`; HikariCP 공식 가이드와 Oracle RWP 문서에서 동일 공식 인용; 멀티 WAS 배분 주의사항 및 현행 시스템 적용 시 측정 우선 원칙 제시

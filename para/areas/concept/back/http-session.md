...
- [[jwt]] — 서버가 상태를 안 들고 같은 일을 하는 방식
- [[spring-security]] — 세션을 끄기로 선택하는 자리
- [[distributed-session]] — 서버가 여럿일 때 이 보관소를 밖으로 빼는 답

## 출처

- [[2025-01-17-Day14]] — 이 보관소가 **서버 안에 있다는 것 자체가 문제가 되는 자리**다. 쿠버네티스에서 요청이 파드들에 라운드로빈으로 나뉘면 「두 번째 요청이 첫 번째의 세션을 못 찾는다」 — Day65 부터 「같은 사용자를 알아본다」로 배운 이 장치가 **한 대일 때만 성립했다**는 것이 드러난다. 답은 세션을 서버 밖(Redis)으로 옮기는 것이다 → [[distributed-session]]
- [[2025-01-10-Day09]] — 넉 달 뒤. **이 보관소를 아예 안 쓰기로 하는 선택**이 나온다 — `sessionCreationPolicy(SessionCreationPolicy.STATELESS)` 로 세션을 만들지 않고, 「누가 로그인했는지」를 서버 메모리 대신 **클라이언트가 들고 다니는 토큰**에 담는다. Day65~71 이 세션으로 풀었던 문제를 정반대 방향으로 푸는 것이고, 얻는 것(서버가 여러 대여도 된다)과 잃는 것(발급한 것을 취소할 수 없다)이 맞바꿈으로 드러난다 → [[jwt]] · [[spring-security]]

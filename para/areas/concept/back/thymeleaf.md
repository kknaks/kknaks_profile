...
- [[template-fragment]] — 화면 조각을 이름으로 재사용하는 문법

## 출처

- [[2024-10-23-Day100]] — 하루 뒤. **실제로 붙여 쓰는 회차**다. 부트에서는 `spring.thymeleaf.prefix` 만 정하면 되고 **접미사는 `.html` 이 기본**이라는 것, 즉 [[view-resolver]] 설정이 속성 한 줄로 끝난다는 것이 나온다. 문법 쪽에서는 `th:*` 대신 **`data-th-*` 표기**를 쓰는데, HTML5 의 `data-` 속성 규칙을 따르므로 유효성 검사를 통과한다. `data-th-checked="${project.members.contains(user)}"` 처럼 **속성값 자체를 조건으로 계산**하는 예가 이 엔진의 성격을 잘 보인다 — 태그를 새로 만들지 않고 기존 속성을 채운다 → [[template-fragment]]

...
- [[2025-01-14-Day11]] — 하루 뒤. **값을 코드에서 뺀다** — `variables.tf` 에 `prefix`·`region` 을 선언해 두고 쓰는 쪽은 `var.region` 으로 참조하므로, **같은 정의를 사람마다 다른 이름으로** 적용할 수 있다(실습에서 각자 `dev-1`·`dev-2` 로 접두어를 바꿔 쓴다). 애플리케이션에서 설정을 파일로 뺐던 것과 같은 이유이고, 여기서는 **인프라 정의를 남의 저장소에서 받아 쓸 수 있게** 만드는 장치이기도 하다 → [[externalized-configuration]]

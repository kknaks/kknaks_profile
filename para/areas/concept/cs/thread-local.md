...
### 엿새 뒤 — 그 답을 적을 자리가 만들어졌는데 비어 있다

Day66 의 마지막 章 제목이 **「쓰레드풀 관리하기」**다. 하위 소제목이 넷 있다.
...
**넷 다 본문이 한 줄도 없다.** Day64 가 「풀을 넣는 순간 최악의 버그」라고 적은 그 문제를 다룰 자리가 정확히 이것인데, 제목만 서 있다.

그리고 **컨테이너는 이미 풀을 쓰고 있다** — 제목이 「추가하기」인 것이 그 사실을 놓친 신호로 읽힌다. 내가 만드는 것이 아니라 **이미 그 위에서 돌고 있는데 `remove()` 가 없다**는 것이 이 시점의 실제 상태다 → [[servlet-container]] · [[transaction]]
...
- [[2024-08-29-Day66]] — 엿새 뒤. 마지막 章이 「쓰레드풀 관리하기」이고 하위 소제목이 넷인데 **본문이 전부 비어 있다.** Day64 가 남긴 「쓰레드 풀 + `remove()` 없음」 문제를 적을 자리가 만들어졌으나 채워지지 않았고, 같은 회차의 서블릿들이 커밋을 분기 안에 두어 그 위험을 오히려 키운다

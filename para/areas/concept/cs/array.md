...
- [[time-complexity]] — 인덱스 접근이 O(1) 이라는 것의 값
- [[grid-traversal]] — 2차원 배열을 훑는 정형
...
- [[2025-05-01-Pr10808]] — 알파벳 개수 세기. **인덱스를 값으로 쓰는** 전형이다 — 26칸을 잡아 두고 `s[i] - 'a'` 를 인덱스로 삼아 `cnt[n]++` 한다. 찾을 필요 없이 **바로 그 자리로 가는 것**이 배열의 성질이고, 알파벳처럼 **범위가 작고 고정**일 때 map 보다 낫다 → [[character-encoding]]
- [[2025-05-04-Pr1159]] — 농구경기. 같은 카운팅 배열을 **첫 글자별 인원 세기**에 쓴다. 문제가 달라도 「범위가 작은 값을 센다」는 모양이 같다

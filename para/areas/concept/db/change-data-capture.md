...
## 로그를 읽는다고 다 CDC 는 아니다

같은 binlog·WAL 을 읽어도 **디코딩하느냐**에서 갈린다.

| | CDC (Debezium 등) | 로그 아카이빙 |
|---|---|---|
| 처리 | 로그를 **해석** → 행 단위 변경 이벤트 | 해석 없이 **파일 그대로** 보관 |
| 타겟 | 이종 시스템 가능 (Kafka · 검색엔진 · 다른 DB) | 같은 엔진·버전에서 재생 |
| 산출물 | 변경 스트림 | 원본과 동일한 DB → [[point-in-time-recovery]] |
| 운영 | 커넥터·오프셋·스키마 레지스트리 | 프로세스 하나 |

**백업이 목적이면 CDC 가 아니다.** CDC 스트림만으로는 복원되지 않고(초기 스냅샷이 따로 필요하다), 타겟이 조용히 어긋나도 원본과 대조할 방법이 없다. 반대로 이종 시스템에 흘려보내는 것이 목적이면 파일 보관으로는 할 수 없다.

**부르는 이름이 논의를 바꾼다.** 같은 일을 「CDC 로 하겠다」고 하면 Debezium+Kafka 운영 부담을 떠올리게 되고, 「WAL 아카이빙」이라고 하면 표준 백업 기법으로 읽힌다.
...
- **스키마 변경은 따로 다뤄야 한다** — 컬럼이 추가·삭제되면 대상 쪽 구조도 함께 바뀌어야 하는데, 변경 스트림만으로는 그 순서를 보장하기 어렵다 → [[database-schema]]
  - PostgreSQL 논리 복제는 **DDL 을 전파하지 않는다.** 소스에서 `ALTER TABLE` 하면 구독이 깨지고 사람이 손을 대야 한다. 신규 테이블도 publication 에 수동 등록해야 하고, PK 없는 테이블은 `REPLICA IDENTITY` 설정이 필요하다. **MySQL binlog 는 DDL 이 같이 실려 전파된다** — 같은 「CDC」인데 운영 부담이 갈리는 지점
- **받는 쪽이 멈추면 원본이 위험해진다** — 슬롯을 쓰면 원본이 로그를 붙들고 있어 디스크가 찬다 → [[replication-slot]]
...
- [[write-ahead-logging]] — 읽는 대상이 되는 그 로그
- [[point-in-time-recovery]] — 같은 로그를 디코딩하지 않고 쓰는 쪽
- [[replication-slot]] — 로그 보존을 원본에 요구하는 장치
...
- [[2026-09-21-data-backup]] — 사내 DB 백업 설계를 「CDC 로 하자」로 시작했다가, 목적이 **원본 보관**이라 디코딩이 필요 없다는 것이 드러나 WAL 아카이빙으로 착지했다. 논리 복제를 대안으로 검토했으나 PG 의 **DDL 미전파**가 8개 서비스 × 1명 운영에서 가장 큰 비용이라 기각됐다. 같은 로그를 읽는 두 기술의 경계가 여기서 선명해졌다

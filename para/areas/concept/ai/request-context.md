...
4. 하위 구성 요소는 같은 컨텍스트에서 값을 읽어 지시문, 모델, 도구, 저장소, 응답 형식 또는 워크플로 단계의 실행 조건을 선택한다.
...
워크플로에서는 요청 컨텍스트와 워크플로 상태의 수명을 구분한다. 요청 컨텍스트는 현재 실행을 시작한 요청의 조건을 전달하고, 워크플로 상태는 단계 사이의 진행 값이나 누적 결과를 저장하며 중단·재개 과정에서도 이어질 수 있다.
...
워크플로 단계도 같은 방식으로 요청별 값을 읽어 처리 한도를 바꿀 수 있다.
...
const limitStep = createStep({
  execute: async ({ requestContext }) => {
    const tier = requestContext.get('user-tier') as UserTier['user-tier']
...
워크플로 단계가 사용자 등급이나 요청 정책을 일반 입력 데이터에 섞지 않고 읽을 수 있으므로, 업무 데이터의 입출력 스키마를 유지한 채 요청별 실행 한도와 동작을 바꿀 수 있다.

## 경계와 오해

- **요청 컨텍스트 ≠ 에이전트 메모리** — 요청 컨텍스트는 현재 호출의 실행 조건을 전달하고, 메모리는 여러 호출에 걸친 대화 기록과 지속 상태를 보존한다.
- **요청 컨텍스트 ≠ 워크플로 상태** — 요청 컨텍스트는 요청별 조건을 전달하고, 워크플로 상태는 여러 단계가 공유하는 진행 값과 누적 결과를 보관한다.
...
- [[workflow-orchestration]] — 요청 컨텍스트는 워크플로 단계에 실행 조건을 전달하고, 워크플로 상태는 단계 사이의 진행 데이터를 맡는다.

## 출처

- [[mastra-request-context]] — Mastra에서 요청별 값을 에이전트·도구·워크플로로 전파하고 타입, 스키마, 예약 키로 검증·격리하는 방법을 설명한다.
- [[mastra-workflows-overview]] — 워크플로 단계가 요청 컨텍스트의 사용자 등급을 읽어 처리 한도를 바꾸는 예시와 워크플로 상태와의 역할 차이를 보여 준다.

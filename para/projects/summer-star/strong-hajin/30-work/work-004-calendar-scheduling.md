...
- Decision: **DEC-003** — 채택 `§A`~`§J` · 증보 **`K1`~`K24`** · 기각 넷 · 보류 다섯.
  **2루프는 증보 8 의 `K19`~`K23` 과 `K24`** 를 내린다 — `K15` 는 **`K19` 가 뒤집었다**
- Covers spec: **SPEC-004 전절** — §2 UX 계약 · §3 S-1~S-8(+S-1b·S-1c·S-4b) ·
  ...
  §6 인수조건 **148줄**(v0.3.1). **SPEC-003·SPEC-001** 은 대체되지 않은 절이 **회귀 기준**이다
...
| Owner | kknaks |
| Status | in_progress |
| Progress | **1루프 넷 DONE** · 2루프 넷 TODO |
...
### 2루프(`K19`~`K24`)에서 만질 자리

| # | 파일 | 무엇을 | Phase |
|---|---|---|---|
| 24 | `backend/src/ax_workspace/modules/work/schedule.py` **또는 새 순수 도메인 파일** | **반열림 `[시작, 끝)` 판정 · 자정 넘는 구간 분할.** 새 파일이면 **`PURE_DOMAIN_MODULES` 에 등재** | BE-3 |
| 25 | `backend/src/ax_workspace/platform/task_schedules.py` · `platform/meetings.py` | **`overlapping_blocks(member_ids, 구간)` 이 두 표를 조회**한다. **조회 한 자리** | BE-3 |
| 26 | `backend/src/ax_workspace/modules/work/application.py` | 배정 생성·시각 변경에 겹침 검사. **멱등 키 조회 다음, 나머지 검사 앞** | BE-3 |
| 27 | `backend/src/ax_workspace/modules/meetings/application.py` | **회의 생성·시각 변경에 겹침 검사.** `quick-start` 경로는 **안 태운다** | BE-4 |
| 28 | `bootstrap/application.py` | 새 함수의 조립. **여기서만** | BE-3 |
| 29 | `docs/unified-operations-inventory.json` | **새 라우트 없음.** `approval` 이 합본 조회 행의 `http_signature` 에 드는지만 확인해 **그 행만** 패치 | BE-4 |
| 30 | `frontend/src/features/calendar/MonthGrid.tsx` | **시간 배정을 안 그린다 · 시간순 정렬** | FE-3 |
| 31 | `frontend/src/features/calendar/WeekGrid.tsx` | **겹치는 블록을 나란히** — 시간 격자에 줄 배치 | FE-3 |
| 32 | `frontend/src/features/calendar/ScheduleCard.tsx` | **상태 배지 + 승인 배지 둘** | FE-3 |
| 33 | `frontend/src/lib/viewModels.ts` | 업무 행에 `approval` 타입 한 줄 | FE-3 |
| 34 | `frontend/src/features/work/` 의 **업무 탭 우측 레일** | **회의를 싣는다.** 업무는 **페이지가 주는 것 그대로** | FE-4 |

**회의 도메인에서 닿는 자리 — 세어서 적는다**

**1루프에서 「`my_meetings`·`readable_rows` 를 건드리지 않는다」가 걸렸던 것과 같은 자리다.**
**공유 함수의 다른 소비처**를 먼저 확인한다.

| 확인할 것 | 왜 |
|---|---|
| **회의 생성(예약) 경로가 몇 개인가** — 일반 예약 · `quick-start` · 그 밖(승격 등) | **`quick-start` 만 검증을 안 탄다.** 경로를 안 세면 **엉뚱한 것이 막히거나 새 경로가 샌다** |
| **시각 변경 경로가 몇 개인가** — 회의 정보 수정이 시각을 함께 받는다 | 시각이 바뀌는 자리를 **전부** 태워야 한다. `be-survey-report.md` §6 이 그 표면을 갖는다 |
| **`overlapping_blocks` 가 쓰는 회의 조회의 다른 소비처** | 그 조회를 고치면 **회의 화면·MCP·캘린더가 같이 움직인다.** **고치지 말고 새로 읽는다** |
| **`quick-start` 가 일반 예약과 코드를 공유하는가** | 공유하면 **분기 한 줄**로 갈라야 한다. 공유 지점을 모르고 걸면 둘 다 막힌다 |
| **회의 쪽 기존 계약 회귀** | 요청 필드·응답 행 모양·권한·구획이 **안 바뀌어야** 한다(§6) |

> **이 다섯은 BE-4 착수 전에 `grep` 으로 센다.** 「목록 N개」를 받아 시작하면 **목록 밖에서 FAIL 이 난다.**
...
### 2루프(`K19`~`K24`)는 **스키마를 바꾸지 않는다**

**새 표도, 새 컬럼도, 새 인덱스도 없다.** 명시한다 — 이 자리에서 흔들리기 쉽다.

| 오해하기 쉬운 것 | 실제 |
|---|---|
| `approval` 을 `task_schedules` 에 넣는다? | **아니다.** **합본 조회의 응답 필드**다(`K19`). 업무 쪽 기존 값을 **투영**할 뿐 저장하지 않는다 |
| 겹침을 부분 unique 로 막는다? | **못 한다.** 겹침은 `task_schedules` 와 `meetings` **두 표에 걸쳐** 있다 |
| 회의 쪽에 열을 더한다? | **아니다.** 회의에서 바뀌는 것은 **검증 한 겹**뿐이고 행 모양·요청 필드가 그대로다 |

**겹침의 한계 — WP 도 적는다 (SPEC §2.9 와 같은 말).**

| 불변식 | 누가 막나 | 동시 요청 둘이 오면 |
|---|---|---|
| **하루 한 칸** (같은 업무·같은 날) | **데이터베이스** — 부분 unique | **둘째가 거절된다.** 틈이 없다 |
| **겹침 금지** | **application** — 검사 후 저장 | **둘 다 설 수 있다** |

- **DB 제약으로 올리려는 시도를 하지 마라.** 두 표 사이에는 걸 수 없다.
  검사와 저장을 **한 트랜잭션**에 두는 것이 우리가 할 수 있는 전부이고,
  **그래도 남는 틈이 있다.** **「하루 한 칸」과 같은 세기의 보장이 아니다.**
- **이 차이를 감추지 않는다** — SPEC §2.9 가 계약으로 적었고 이 WP 가 같은 말을 한다.
...
```text
1루프 (DONE)   Phase BE-1 ──▶ Phase BE-2 ──▶ Phase FE-1 ──▶ Phase FE-2
                                                                │
2루프 (TODO)   Phase BE-3 ──▶ Phase BE-4 ──▶ Phase FE-3 ──▶ Phase FE-4
                    ▲
                    └── 1루프가 전부 닫힌 뒤에 시작한다
```

**2루프는 사용자가 실물로 쓰면서 찾은 다섯**(SPEC-004 v0.3.0~v0.3.1 의 `K19`~`K24`)을 내린다.
1루프가 계약대로 섰고, **계약이 덜 정해진 자리와 시안을 그대로 옮겨서 생긴 자리**가 드러났다.

- **BE 가 닫히기 전에 FE 를 태우지 않는다.** 같은 코드 워크트리를 공유하므로 병행하면 서로를 덮는다.
- **BE 를 둘로 가른 이유(2루프)** — **회의 도메인을 건드리는 것(BE-4)이 `§1 Scope Out` 을
  뒤집는 자리**다. **배정 쪽(BE-3)만 먼저 서도 그것만으로 쓸 수 있고**, 회의 쪽에서 막히면
  **배정 쪽이 인질이 되지 않는다.**
...
8. **캘린더는 상태를 말하지 않는다** — **격자도 좌측 카드도** 유형 둘로만 **(증보 K15)**.
   상태는 카드를 열어 **상세**에서 읽는다.
9. **좌측 레일과 격자는 같은 범위를 본다** **(증보 K18)**. 월 뷰면 **그 달**, 주 뷰면 **그 주**.
   격자가 **달 밖 칸을 비워 두므로**(시안) **레일도 달 밖 일정을 내지 않는다.**
   둘이 같은 것을 다르게 보여주면 사람이 **「하나가 틀렸다」**고 읽는다.
...
| G-CAL-03 | `TaskCreateModal` | **부품은 만들지 않는다.** 업무 만들기는 **기존 모달**을 쓴다(§J 정정). 시안의 필드 구성을 따라가지 않는다. **다만 시안 머리의 「업무 만들기」 단추는 세운다** — 누르면 기존 모달을 연다 **(증보 K17)** | — (단추만, 부품 없음) | **FE-2**(단추) |
...
| G-CAL-08 | 상태 색 tint | **만들지 않는다.** 격자도 **좌측 카드도 상태를 내지 않는다** — 배지는 **유형(업무/회의)에만** **(확정 — 증보 K15)**. 상태는 카드를 열어 **상세**에서 읽는다 | — | — |
...
- [ ] **레일에 뜬 일정과 격자에 그려진 일정이 같다** (증보 K18) — 월 뷰면 그 달, 주 뷰면 그 주.
      **레일에만 있고 격자에 없는 일정이 0건**이다.
- [ ] **격자에도 좌측 카드에도 상태가 없다** — 배지는 **유형(업무/회의)에만** (증보 K15).
      **`COMPLETION_SUBMITTED` 인 업무가 카드에서 「완료」로 읽히는 자리가 0개**다.
...
8. **시안 머리의 「업무 만들기」 단추를 세운다** **(증보 K17)**.
   누르면 **기존 업무 생성 모달**을 연다 — 시안의 `TaskCreateModal` **필드 구성을 따라가지 않는다**
   (G-CAL-03 · §J 정정).
   **단추는 레이아웃이라 시안 정본, 모달은 기능이라 우리 것**이다.
   **FE-1 에서 뺀 것은 옳다** — 그 Phase 는 「읽기만」이다.
9. **WARN-A 를 닫는다 — 한 줄로 정한다.**
...
## Execution — 2루프 (`K19`~`K24`)

**SPEC-004 v0.3.1 이 검수 PASS 후 커밋됐다. 계약은 닫혔다 — 다시 열지 않는다.**
아래 넷은 **1루프가 전부 닫힌 뒤** 같은 직렬 규칙으로 돈다.

### Phase BE-3 — 겹침 검증의 **뼈대와 배정 쪽** (`K22`)

- **Status**: TODO
- **설명**: 겹침을 **읽는 문 하나**를 만들고 **배정 쪽에 먼저 건다.**
  **여기까지가 「내 시간표 안에서 겹치지 않는다」**다. 회의 쪽은 BE-4 다.

**작업**

1. **`overlapping_blocks(member_ids, 구간)` 한 함수** — **두 표(`task_schedules`·`meetings`)를
   조회**한다 **(확정 — SPEC §2.9 · §5)**. **표는 둘이어도 문은 하나다** —
   배정 쪽과 회의 쪽이 각자 조회를 쓰면 **두 규칙이 된다.**
   **반열림 `[시작, 끝)` 판정도 그 한 자리에 있다.**
2. **반열림** — 경계가 닿는 것은 겹침이 **아니다.** 10:00–11:00 과 11:00–12:00 은 **둘 다 선다.**
   **새로 정한 규칙이 아니다** — 저장소의 회의 조회 docstring 이 이미 같은 규칙을 쓴다
   (`be-survey-report.md` §6).
3. **자정 넘는 회의를 날짜별로 쪼개** 비교한다 — 이 함수 안에서 한다.
4. **배정 생성·시각 변경에 건다.** **나의 시간만** 본다. 거부는 `TASK_SCHEDULE_OVERLAP` **409**.
5. **영수증이 먼저다 (증보 K12 · SPEC v0.3.1 의 F-2 정정).**
   **멱등 키 조회 → (같은 키면 `200` 영수증) → 그 다음에 겹침 검사.**
   **이 순서를 뒤집으면 드래그 연타가 `409` 를 맞는다** — 같은 키의 재전송은
   **자기가 방금 만든 배정과 같은 구간**이라 **자기 자신과 겹친다.**
   `TASK_SCHEDULE_DAY_TAKEN`·`TASK_SCHEDULE_TASK_CLOSED` 와 **같은 자리에 넣는다.**
6. **검사와 저장을 한 트랜잭션**에.
7. **기존 데이터는 그대로 둔다** — 이미 겹친 것을 소급해 닫거나 고치는 쓰기를 **만들지 않는다.**

**하지 말 것**

- **DB 제약으로 올리려 하지 마라.** 겹침은 `task_schedules` 와 `meetings` **두 표에 걸쳐** 있어
  **제약을 걸 수 없다**(SPEC §2.9). 「하루 한 칸」처럼 부분 unique 로 막으려는 시도가
  **이 Phase 의 가장 흔한 오답**이다.
- **남는 틈을 감추지 마라** — 동시 요청 둘이 **각각 검사를 통과해 둘 다 설 수 있다.**
  **「하루 한 칸」과 같은 세기의 보장이 아니다.**

**완료 판정 — 무엇이 돌면 끝인가**

- [ ] `make test-unit` · `make test-contract` **통과**.
- [ ] 겹치는 배정 생성이 **`TASK_SCHEDULE_OVERLAP` 409** 다. 시각 변경으로 겹치게 만들어도 같다.
- [ ] **경계가 닿는 것은 통과한다** — 11:00 에 끝나는 일정이 있어도 **11:00 시작 배정이 선다.**
- [ ] **같은 멱등 키의 재전송이 `200` 영수증**이다 — **`409` 가 아니다.**
      **이 한 줄이 이 Phase 에서 가장 틀리기 쉬운 자리다.**
- [ ] 겹침을 읽는 **함수가 하나**다 — 배정 쪽이 자기 조회를 따로 쓰지 않는다.
- [ ] 검사와 저장이 **한 트랜잭션**에 있다.
- [ ] **기존 데이터를 건드리는 쓰기가 0건**이다.
- [ ] `make test-postgres` 재통과 (1루프 회귀).
- [ ] **커밋 후 API 를 다시 띄웠다.**

---

### Phase BE-4 — **회의 쪽 검증** + `approval` + 운영 대장 (`K22`·`K19`)

- **Status**: TODO
- **설명**: **`§1 Scope Out` 을 뒤집는 자리다** — 회의 생성·시각 변경에 검증이 붙는다.
  그리고 카드가 승인을 말할 수 있게 **합본 조회에 한 줄을 더한다.**

**작업**

1. **회의 생성·시각 변경에 겹침 검증을 건다** — **주최자 + 참석자 전원**의 시간.
   거부는 `MEETING_TIME_OVERLAP` **409**. **BE-3 이 만든 같은 함수를 쓴다.**
2. **`quick-start` 회의는 이 검사를 지나지 않는다** **(확정 — SPEC §2.9)** —
   지금 당장 시작하는 것이라 막으면 못 쓴다.
3. **사외 참석자는 검사 대상이 아니다** — 계정이 없어 일정이 없다.
4. **남의 시간은 busy/free 만 읽는다.** 제목·내용을 읽지 않고 **응답에도 싣지 않는다.**
   거절 문구는 **이름까지만** — 「민아 님의 일정과 겹칩니다.」
5. **`K19` — 합본 조회 `kind:'task'` 행에 `approval` 을 더한다.**
   값은 저장소 어휘 그대로(`null`·`awaiting_review`·`awaiting_revision`·`approved`).
   **`derived` 묶음 전체를 싣지 않는다 — 이 한 값만이다.**
6. **운영 대장** — **새 라우트가 없다.** `http_count` 는 **159 그대로**다.
   **`approval` 이 합본 조회 행의 `http_signature` 에 드는지 확인하고**, 들면 그 행만 패치한다.
   **전체 재작성 금지.**

**완료 판정 — 무엇이 돌면 끝인가**

- [ ] `make test-unit`(`test_operation_inventory` 포함) · `make test-contract` **통과**.
- [ ] 참석자의 일정과 겹치는 **회의 생성·시각 변경이 `MEETING_TIME_OVERLAP` 409** 다.
- [ ] **`quick-start` 가 막히지 않는다.**
- [ ] **사외 참석자가 있어도 회의가 선다.**
- [ ] **응답에 남의 일정 제목·내용이 없다** — 이름까지만.
- [ ] **회의 쪽 기존 계약이 안 바뀌었다** (회귀) — 요청 필드·응답 행 모양·권한·구획 그대로.
- [ ] 합본 조회 업무 행이 **`approval`** 을 낸다.
- [ ] `http_count` 가 **159 그대로**다. 대장 diff 가 **그 항목만**이고 `acceptance_evidence` 가 보존됐다.
- [ ] `make test-postgres` 재통과.
- [ ] **커밋 후 API 를 다시 띄웠다. 실제 API 계약을 FE 브리프에 박았다.**

---

### Phase FE-3 — 월 뷰 · 겹침 그리기 · 카드 배지 (`K20`·`K21`·`K19`)

- **Status**: TODO
- **설명**: **보이는 것을 고친다.** 쓰기는 안 는다.

**작업**

1. **`K20` — 월 뷰는 업무의 시간 배정을 그리지 않는다.** 종일 띠만.
   **회의는 계속 그린다** — 회의는 **그 자체가 일정**이지 업무의 배정이 아니다.
   **셀 안은 시간순으로 정렬**한다.
   **⚠ 시안과 다르게 간다** — 시안 `calEntries` 는 월 뷰에 둘 다 넣는다.
   **레이아웃이 아니라 기능의 문제**라 우리 것이 정본이다(SPEC §2.1).
2. **주 뷰는 건드리지 않는다** — 종일 칸과 시간 격자가 이미 나뉘어 있다. **회귀로 지킨다.**
3. **`K21` — 시간 격자에서 겹치는 블록을 나란히** 그린다. **밑에 깔지 않는다.**
   줄 배치가 **종일 띠에만** 있고 시간 격자엔 없다(`fe-survey-report.md` §1-4·§1-5 f).
   **막는 것(§2.9)과 다른 물음이다** — **이미 겹쳐 있는 것은 여전히 그려져야 한다.**
4. **`K19` — 좌측 카드가 상태 배지 + 승인 배지 둘**을 낸다.
   **업무 화면이 이미 그렇게 한다** — 새로 만들 것이 없다.
   **`approval` 은 BE-4 가 실어 준다.**
   **상태 배지 하나로는 `COMPLETION_SUBMITTED` 인 업무가 「완료」로 읽힌다.**

**완료 판정 — 무엇이 돌면 끝인가**

- [ ] `make frontend-test`(**Node 20**) · `npx tsc --noEmit` **통과**.
- [ ] **월 뷰에서 같은 업무가 두 번 뜨는 자리가 0개**다. **회의는 월 뷰에도 그려진다.**
- [ ] **월 뷰 셀 안이 시간순**이다.
- [ ] **주 뷰는 그대로다** (회귀) — 종일 칸과 시간 격자가 나뉜 채 둘 다 그린다.
- [ ] **09:30–12:30 안에 든 10:00–11:00 회의가 밑에 깔리지 않는다** — 나란히 앉는다.
- [ ] **좌측 카드가 배지 둘**을 낸다.
      **`COMPLETION_SUBMITTED` 인 업무가 「완료」로만 읽히는 자리가 0개**다.

---

### Phase FE-4 — 거절 문구 · 업무 탭 레일 (`K22`·`K23`·`K24`)

- **Status**: TODO
- **설명**: 2루프의 마지막. **말과 자리**를 붙인다.

**작업**

1. **`K22` 거절 문구** — `TASK_SCHEDULE_OVERLAP` 은 「이미 다른 일정이 있는 시간입니다.」,
   `MEETING_TIME_OVERLAP` 은 **「민아 님의 일정과 겹칩니다.」**
   **이름은 말하고 내용은 말하지 않는다.** **조용한 거절 0개**(§I) 규칙이 여기에도 걸린다.
2. **`K23` — 업무 탭 우측 캘린더에 회의가 선다.** **새 표면을 만들지 않는다.**
3. **`K24` — 그 레일의 축은 둘이 다르다.**
   - **업무**: **그 탭이 들고 있는 것을 그대로** — **지금 동작 유지.**
     그 레일은 **자기 질의가 없고** 페이지의 업무 목록을 받는다.
   - **회의**: **탭과 무관하게 항상 「내 회의」**(`board` 축).
   - **그래서 합본 조회는 회의 절반에만 쓴다.** 업무는 다시 묻지 않는다.
   - **⚠ 업무까지 합본 조회로 바꾸면 「보낸 업무」 탭에서 보낸 업무가 사라진다.**

**완료 판정 — 무엇이 돌면 끝인가**

- [ ] `make frontend-test` · `npx tsc --noEmit` **통과**.
- [ ] 겹침 거절 둘이 **문구를 낸다.** 회의 쪽은 **이름까지만** 말한다.
- [ ] **드래그 연타가 겹침 오류를 띄우지 않는다** — 같은 멱등 키로 보내 영수증을 받는다.
- [ ] **업무 탭 우측 캘린더에 회의가 선다** — 합본 조회의 **회의 절반**을 쓴다.
- [ ] **탭을 바꿔도 그 레일의 업무 목록이 그 탭 것 그대로**다 —
      **「보낸 업무」 탭에서 보낸 업무가 사라지지 않는다** (회귀).
- [ ] 그 레일의 **회의는 어느 탭에서도 같다.**
- [ ] **최종 `make verify` 통과** + `make test-postgres` 재통과.
...
| 단계 | 명령 | 언제 |
|---|---|---|
| 도메인·구조 | `make test-unit` | BE-1 · BE-2 · **BE-3 · BE-4** |
| 외부 계약 | `make test-contract` | BE-1 · BE-2 · **BE-3 · BE-4** |
| DB·제약·동시성 | `make test-postgres POSTGRES_TEST_URL=postgresql+psycopg://ax:ax@127.0.0.1:54329/ax_test_calendar` | **BE-1**(부분 unique 증명) · BE-2(회귀) · FE-2(1루프 최종) · **BE-3·BE-4(회귀)** · **FE-4(2루프 최종)** |
| 인벤토리 | `make test-unit` 안의 `tests/architecture/test_operation_inventory.py` | **BE-2** · **BE-4** |
| 화면 | `make frontend-test` (**Node 20**) | FE-1 · FE-2 · **FE-3 · FE-4** |
| 타입 | `cd frontend && npx tsc --noEmit` | FE-1 · FE-2 · **FE-3 · FE-4** |
| 전체 | `make verify` | **FE-2 에서 한 번**(1루프) · **FE-4 에서 한 번**(2루프) |
...
**2루프(`K19`~`K24`) 인계 — FE-4 뒤에 넘길 것**

| 시나리오 | 조작 | 기대 결과(눈으로) |
|---|---|---|
| E-12 | 이미 일정이 있는 시간에 배정을 놓는다 | **「이미 다른 일정이 있는 시간입니다.」** 가 뜬다 |
| E-13 | **11:00 에 끝나는 일정** 바로 뒤 **11:00** 에 놓는다 | **선다** — 경계가 닿는 것은 겹침이 아니다 |
| E-14 | 드래그를 **연타**한다 | **겹침 오류가 안 뜬다** — 배정이 하나만 선다 |
| E-15 | 참석자가 바쁜 시간에 **회의를 잡는다** | **「민아 님의 일정과 겹칩니다.」** — **무슨 일정인지는 안 나온다** |
| E-16 | **「바로 시작」** 회의를 만든다 | **막히지 않는다** |
| E-17 | 월 뷰를 본다 | 같은 업무가 **띠와 시간 조각으로 두 번 뜨지 않는다.** **회의는 보인다.** 셀 안이 **시간순** |
| E-18 | 09:30–12:30 배정 안에 10:00–11:00 회의가 있는 주를 본다 | **나란히** 보인다 — 밑에 안 깔린다 |
| E-19 | 승인 대기(완료 보고 냄) 업무의 좌측 카드를 본다 | **「완료」로만 읽히지 않는다** — **승인 배지**가 「확인 대기」를 말한다 |
| E-20 | **업무 탭**에서 우측 캘린더를 본다. 탭을 **「보낸 업무」** 로 바꾼다 | **회의가 보인다.** 그리고 **보낸 업무가 사라지지 않는다** |

## 인수조건 추적 — 어느 Phase 가 SPEC §6 의 어느 줄을 닫나

**SPEC-004 v0.3.1 §6 인수조건은 15묶음 · 148줄이다. 전수 매핑한다 — 닫는 Phase 가 없는 줄은 없다.**

> **1루프(v0.2.4)는 14묶음 124줄이었다.** 2루프가 **+24줄** 을 더했다 —
> **새 묶음 「겹침 금지」 14줄** + **「화면」 묶음이 12 → 22 로 +10줄**.
...
| **O** | **겹침 금지 (증보 K22)** | **14** | **BE-3**(4줄 — 배정 409 · 경계 통과 · **멱등 영수증** · 시각 변경 409) · **BE-4**(6줄 — 회의 409 · `quick-start` · 사외 · 응답에 제목·내용 없음 · 자정 분할 · 회의 계약 회귀) · **BE-3·BE-4 공유**(3줄 — 기존 데이터 · 읽는 문 하나 · 한 트랜잭션) · **문서**(1줄 — 「한계가 문서에 적혀 있다」) | `make test-contract` · `make test-postgres` · **한계 줄은 SPEC §2.9 와 이 WP §Domain / Schema 가 닫는다** |
| K | 화면 (§E·§J) | **22** | **FE-1**(10줄) · **FE-2**(2줄) · **FE-3**(6줄 — K19 배지 둘·「완료」로만 0개 / K20 월 뷰 셋 / K21 나란히) · **FE-4**(3줄 — K23 회의가 선다 / K24 업무 목록 그대로 · 회의는 탭 무관) · **BE-4**(1줄 — 합본 조회가 `approval` 을 낸다) | `make frontend-test` · `npx tsc --noEmit` · `make test-contract`(`approval`) · E2E |
...
> **`O` 가 `J` 와 `K` 사이에 있는 이유** — 2루프가 더한 **새 묶음**이라 알파벳 순서가 아니라
> **SPEC §6 의 등장 순서**에 맞춰 넣었다. 1루프 ID `A`~`N` 을 **밀지 않는다** —
...
| 루프 | Phase | 대략 |
|---|---|---|
| 1루프 | BE-1 · BE-2 · FE-1 · FE-2 | 60 · 26 · 14 · 24 = **124** |
| 2루프 | **BE-3** | 4 + 공유 3 = **약 7** |
| 2루프 | **BE-4** | 6 + `approval` 1 = **약 7** |
| 2루프 | **FE-3** | **6** |
| 2루프 | **FE-4** | **3** |
| 2루프 | **문서** | **1** (겹침의 한계) |
| | | 2루프 합 **24** |

> **「한계가 문서에 적혀 있다」 한 줄은 구현이 닫지 않는다.** SPEC §2.9 가 계약으로 적었고
> 이 WP 의 §Domain / Schema 가 같은 말을 한다. **Phase 에 매달 자리가 없어 여기 명시한다** —
> 「닫는 Phase 가 없다」로 읽히지 않게.
...
### SPEC 밖에서 이 work 가 여는 인수조건 — **WARN-A · K17 · K18**

**아래 넷은 SPEC-004 §6 의 124줄에 없다.** 위 표의 합계에도 들어가지 않는다.
**여기서 정하고 여기서 증명한다.**

**WARN-A** (검수 4차가 WP 로 넘긴 것) — **FE-2**
...
**K17** (DEC-003 증보 7) — **FE-2**

- [ ] 시안 머리의 **「업무 만들기」 단추**가 있고, 누르면 **기존 업무 생성 모달**이 열린다.
      **시안 `TaskCreateModal` 의 필드 구성을 따라가지 않았다.**

**K18** (DEC-003 증보 7) — **FE-1**

- [ ] **레일에 뜬 일정과 격자에 그려진 일정이 같다** — 월 뷰면 그 달, 주 뷰면 그 주.
      **레일에만 있고 격자에 없는 일정이 0건**이다.

> **왜 SPEC 에 없나.** WARN-A 는 **계약이 갈리지 않아 FAIL 이 아니었고**(서버는 한 값),
> K17·K18 은 **레이아웃과 화면 범위**라 SPEC §1 Scope Out 이 그 층을 `30-work/` 로 보냈다.
...
- [ ] **SPEC-004 v0.3.1 §6 인수조건 148줄 전항**이 「데이터: 에이전트」 칸까지 자동검증으로 확인됐다
      (「한계가 문서에 적혀 있다」 한 줄은 **문서가 닫는다**).
- [ ] **2루프 `K19`~`K24` 여섯**이 전부 Phase 에 닫혔다.
- [ ] **겹침의 한계**(application 이 막고 DB 가 아니다)가 **SPEC 과 WP 둘 다**에 적혀 있고,
      **DB 제약으로 올리려는 구현이 없다.**
...
- **회의 규칙을 뒤집는다** — 「공유받은 회의는 캘린더에 서지 않는다」가 **다른 SPEC 의 계약**인데
  ...
  회의 화면 경로는 그대로다.
- **문서 정정 (2026-09-21 · DEC-003 증보 6 `K15` · 증보 7)** — 이 WP 의 **G-CAL-08 초판**은
  「격자에는 만들지 않는다. **좌측 카드는 기존 상태 톤**을 쓴다」였고, SPEC §2.6 도 같은 말이었다.
  **BE-1 이 「합본 조회의 `state` 가 `COMPLETION_SUBMITTED` 를 `"done"` 으로 투영한다」를 드러내
  그 둘이 옛말이 됐다** — 카드가 상태를 내면 **승인 대기 업무를 「완료」로 읽는다.**
  **`K15` 가 「좌측 카드는 유형만」으로 뒤집었고 FE-1 이 그것을 따른 것이 옳다**(증보 7).
  **이 판에서 G-CAL-08 행 · FE-1 작업 8 · FE-1 완료 판정과 SPEC §2.6 을 그 말로 맞췄다.**
- **검수 이월 (SPEC-004 v0.3.1) — 「`409` 가 둘이던 때의 지도 셋」.**
  `K22` 가 생성 경로에 **세 번째 409**(`TASK_SCHEDULE_OVERLAP`)를 만들었는데,
  **그것을 가리키는 지도 셋이 아직 「둘」로 적혀 있다** — 추적표와 1차 이력표의
  「Case Matrix **두** 409 행」, 그리고 §4 의 「그 409」.
  **계약은 안 갈린다** — `TASK_SCHEDULE_OVERLAP` 행 자체에 「재전송이면 영수증이 먼저다」가
  붙어 있고 §5 가 「닫힌 열거가 아니다」를 못 박았다. **읽는 지도의 숫자만 낡았다.**
  **그 표를 손댈 때 숫자를 맞춘다.** 이 WP 가 SPEC 을 고치지 않는다.

...
| **(증보)** | **DEC-003 § Decision 의 증보 K1~K24** — 초판이 미결로 올린 넷(K1~K4) · **검수 1~3차가 연 아홉**(K5~K14) · **BE-1 구현이 드러낸 하나**(K15) · **2루프에서 사용자가 찾은 다섯**(K19~K23) · **v0.3.0 이 올린 `OQ-501` 의 답 하나**(K24)를 코디네이터가 정한 것. 확정과 같은 무게다. **`K15` 는 `K19` 가 뒤집었다** |
...
- Decision reference: `DEC-003` (frontmatter `links.decisions` 와 일치).
  ...
  **검수 2차 FAIL 이 연 셋**(K11~K13) · **검수 3차 FAIL 이 연 하나**(K14) ·
  **BE-1 구현이 드러낸 하나**(K15) · **2루프에서 사용자가 실물로 쓰며 찾은 다섯**(K19~K23) ·
  **v0.3.0 이 올린 `OQ-501` 의 답**(K24, 2026-09-21)을 닫은 자리다(§7).
  **`K19` 가 `K15` 를 뒤집었다.**
...
- **겹침 금지** — 배정·회의가 시간으로 겹치면 **뒤에 오는 것을 막는다** **(확정 — 증보 K22)**.
  **회의 생성·시각 변경에도 건다** — 그래서 아래 Scope Out 한 줄이 뒤집혔다 (§2.9)
- **업무 탭 우측 캘린더에 회의를 싣는다** **(확정 — 증보 K23)**. **새 표면을 만들지 않는다** (§2.10)
...
- ~~**회의 도메인의 변경** — 기간 파라미터 외에는 손대지 않는다~~
  → **이 줄은 뒤집혔다 (확정 — 증보 K22, 2026-09-21).**
  **회의 생성·시각 변경에 겹침 검증이 붙는다** (§2.9). **초판 문장을 지우지 않는다** —
  그때는 겹침 계약 자체가 없었고, **사용자가 알고 고른 변경**이다.
  회의 도메인에서 **바뀌는 것은 그 검증 한 겹뿐**이고 조회·구획·행 모양은 그대로다.
- **공유받은 회의를 캘린더에 세우는 것**도 기존 규칙을 뒤집는다 **(확정 — 증보 K9)** —
  그쪽은 회의 도메인 코드를 건드리지 않고 **합본 조회가 자기 규칙으로 읽을 뿐**이며
...
**월 뷰는 업무의 시간 배정을 그리지 않는다 (확정 — 증보 K20).**

| 무엇 | 월 뷰 | 주 뷰 |
|---|---|---|
| 업무의 **기간 띠** | **그린다** | 종일 칸에 그린다 |
| 업무의 **시간 배정** | **그리지 않는다** **(확정 — K20)** | **시간 격자에 그린다** |
| **회의** | **그린다** — 회의는 **그 자체가 일정**이지 업무의 배정이 아니다 | 시간 격자에 그린다 |

- **왜** — 월 뷰에서 둘을 다 그리면 **같은 업무가 띠와 시간 조각으로 두 번** 뜬다.
- **셀 안은 시간순으로 정렬한다** **(확정 — K20)**.
- **주 뷰는 그대로다** — 종일 칸과 시간 격자가 **이미 나뉘어 있어** 두 번 뜨지 않는다.
- **⚠ 시안과 다르게 간다.** 시안의 `calEntries`(`calendar.v1.jsx:33-38`)는 월 뷰에 **둘 다** 넣는다.
  **레이아웃이 아니라 기능의 문제**이므로 우리 것이 정본이다(머리 ⚠) — 시안은 목데이터라
  같은 업무가 두 번 뜨는 것이 눈에 띄지 않았다.

**겹치는 시간 블록은 나란히 그린다 (확정 — 증보 K21).**

- 시간 격자에서 두 블록의 시간이 겹치면 **좌우로 나눠 나란히** 앉는다. **밑에 깔지 않는다.**
- 지금은 줄 배치(lane)가 **종일 띠에만** 있고 시간 격자엔 없어서,
  09:30–12:30 안에 든 10:00–11:00 회의가 **밑에 깔려 안 보인다**(`fe-survey-report.md` §1-4·§1-5 f).
- **이것은 화면 계약이다** — 겹침을 **막는 것**(§2.9)과 다른 물음이다.
  **이미 겹쳐 있는 것**(기존 데이터 · 서로 다른 사람의 회의)은 여전히 그려져야 한다.
...
- 캘린더는 회의에 **쓰기 명령을 내지 않는다** **(도출)**. 회의 시각·정보 변경은
  ...
  **그 명령에 겹침 검증이 붙는 것은 이 절과 어긋나지 않는다** **(증보 K22 · §2.9)** —
  **캘린더가 부르는 것이 아니라 회의 화면이 부르는 명령**이 한 겹 더 검사할 뿐이다.
...
### 2.6 **격자는 상태를 말하지 않고, 좌측 카드는 배지 둘로 말한다** (확정 — §J · 증보 K19)

| 자리 | 계약 | 무엇이 정했나 |
|---|---|---|
| **캘린더 격자**(띠·시간 블록) | **상태를 말하지 않는다.** 색은 **유형 둘(업무/회의)로만** 칠한다 | **(레이아웃)** 격자에 상태를 얹을 자리가 배치에 없다 — 시안 `calendar.css:45-47` |
| **좌측 카드** | **상태 배지 + 승인 배지 둘**을 낸다. 업무 화면과 **같은 방식** | **(확정 — 증보 K19)** |
| **상세** | **우리 기존 어휘로 상태를 낸다** — 기존 업무 화면과 **같은 표시** | **(확정 — §J 정정)** |

**카드는 배지 둘을 낸다 (확정 — 증보 K19).**

- **상태만 내면 거짓말이 된다** — 합본 조회의 `state` 는 기존 투영을 그대로 쓰는데
  `COMPLETION_SUBMITTED` 인 업무가 거기서 **`"done"` 으로 나간다**(프론트 어휘 5종 유지).
  **상태 배지 하나로는 승인 대기 업무가 「완료」로 읽힌다.**
- **그래서 합본 조회 업무 행에 승인 여부를 싣는다** — `approval` **(제안 — 이름)**.
  값은 저장소 어휘 그대로 `null` · `awaiting_review` · `awaiting_revision` · `approved` 다
  **(확정 — 증보 K19 · SPEC-003 의 `derived.approval` 계승)**. §4 합본 조회가 그 필드를 갖는다.
- **카드가 그 둘을 따로 그린다** — **상태 배지**(`open`·`in_progress`·`blocked`·`done`·`cancelled`)와
  **승인 배지**(「확인 대기」 등). **업무 화면이 이미 그렇게 한다** — 새로 만들 것이 없다.
- **시안 목데이터의 `badge:'결정 요청'`·`actions:'accept'` 를 계약으로 올리지 않는다** —
  시안의 필드 구성은 기능 정본이 아니다. **말은 저장소에 모여 있는 것**을 쓴다
  **(확정 — 우리 것이 정본 · `fe-survey-report.md` §7-1)**.
- 저장소가 상태를 **색**으로 바꾸는 자리 셋 중 **캘린더 칩에 상태를 클래스로 붙이던 자리**는
  **격자에서 안 쓰게 된다** **(도출 — 격자는 유형 둘로만 칠한다)**.

> **이 절은 두 번 정정됐다. 두 문장을 다 남긴다 — 이 SPEC 의 관례다.**
>
> **① 초판(v0.1.0)** — 「**좌측 카드 · 상세** | **우리 기존 어휘로 상태를 낸다**」.
>
> **② 증보 6 `K15`(2026-09-21)** 가 「**좌측 카드와**」를 뺐다 — 「좌측 카드는 유형만 낸다.
> 상태는 카드를 열어 상세에서 읽는다」. 근거는 **BE-1 이 드러낸 투영 사실**이었다.
>
> **③ 증보 8 `K19`(2026-09-21, 사용자 확인)가 `K15` 를 다시 뒤집었다 — 지금 이 절이다.**
>
> **`K15` 가 왜 틀렸나.** `K15` 는 「상태를 **보여줄 방법이 없어서**」 뺀 것이지
> 「보여주면 **안 돼서**」가 아니었다. 그때 세 길 중 **「필드를 더한다」를 비용 때문에 안 골랐는데
> 그 비용 추정이 틀렸다** — 프론트에 **승인 라벨과 승인 배지가 이미 있고**, 업무 화면은
> **상태 배지와 승인 배지를 따로** 낸다(`fe-survey-report.md` §7-1). **새로 만들 것이 없다.**
> 서버가 한 줄 더 실으면 카드가 **같은 방식**을 쓴다.
> **투영 사실은 그대로다** — 달라진 것은 **그 사실을 어떻게 다루느냐**다.
> `K15` 는 **감췄고**, `K19` 는 **승인 배지로 말한다.**
...
| G-CAL-08 | 상태 색 tint | **요구하지 않는다** — **격자도 좌측 카드도 상태를 내지 않는다**(§2.6 · 증보 K15) |
...
### 2.9 **시간이 겹치면 뒤에 오는 것을 막는다** (확정 — 증보 K22)

**이번 판의 본론이다.** 「하루 한 칸」은 **같은 업무의 같은 날**만 막았다 —
**다른 업무와의 겹침도, 회의와의 겹침도 검증이 없었다.**

**무엇이 겹침인가**

| 항목 | 계약 |
|---|---|
| 겹침의 정의 | **반열림 `[시작, 끝)`.** **경계가 닿는 것은 겹침이 아니다** — 10:00–11:00 과 11:00–12:00 은 **둘 다 선다** |
| 시간 블록 | **둘** — 업무 시간 배정 · 회의 |
| 자정 넘는 회의 | **날짜별로 쪼개** 비교한다 |

> **반열림은 새로 정한 규칙이 아니다.** 저장소의 회의 조회가 이미
> 「창은 반열림 `[시작, 끝)` 이다 — 그 창이 끝나는 순간에 시작하는 회의는 겹치지 않는다」를
> docstring 으로 적어 뒀다(`be-survey-report.md` §6). **겹침 검증이 같은 규칙을 쓴다.**
...
|---|---|
| **배정 생성 · 시각 변경** | **나의 시간만** |
| **회의 생성 · 시각 변경** | **주최자 + 참석자 전원**의 시간 |
| `quick-start` 회의 | **막지 않는다** — 지금 당장 시작하는 것이라 막으면 못 쓴다 |
| **사외 참석자** | **검사 대상이 아니다** — 계정이 없어 일정이 없다 |

> **참석자를 막는 것이 정당한 이유.** 참석 회의는 **이미 그 사람 캘린더에 선다** —
> §D 가 회의 가시성을 `board`(참석·공유) 축으로 정했고 참석 관계를 직접 잡는다.
> **안 보이는 것을 근거로 막는 것이 아니다.**

**남의 시간을 어떻게 읽나**

- **busy / free 만 읽는다.** **제목·내용을 읽지 않는다** **(확정 — K22)**.
- **거절 문구는 이름만 말한다** — 「**민아 님의 일정과 겹칩니다.**」 **(제안 — 문구)**.
  **무슨 일정인지 말하지 않는다.**

**무엇을 막고 무엇을 안 막나**

- **기존 데이터는 그대로 둔다.** 이미 겹쳐 있는 것을 소급해 닫거나 고치지 않는다.
  **검증은 새 쓰기에만** 건다 **(확정 — K22)**.
- **에러 둘** — `TASK_SCHEDULE_OVERLAP` 409 · `MEETING_TIME_OVERLAP` 409 (§4 Case Matrix).

**한계를 숨기지 않는다 (확정 — K22).**

| 불변식 | 누가 막나 | 동시 요청 둘이 오면 |
|---|---|---|
| **하루 한 칸** (같은 업무·같은 날) | **데이터베이스** — 부분 unique (§A) | **둘째가 거절된다.** 틈이 없다 |
| **겹침 금지** | **application** — 검사 후 저장 | **둘 다 설 수 있다** |

- **두 표 사이에는 제약을 걸 수 없다.** 겹침은 `task_schedules` 와 `meetings` **두 표에 걸쳐** 있어
  DB 가 답할 수 있는 모양이 아니다.
- **그래서 검사와 저장이 한 트랜잭션에 있어야 한다** **(확정 — K22)**. 그래도 **남는 틈이 있다** —
  **동시 요청 둘이 각각 검사를 통과해 둘 다 설 수 있다.**
  **이 차이를 감추지 않는다** — 「하루 한 칸」과 **같은 세기의 보장이 아니다.**
- **왜 통합 시간 표로 가지 않았나** — 회의 시각의 **정본이 둘이 되는 것이 겹침 경합보다 나쁘다**
  (DEC-003 증보 8). §A 가 「제목·담당은 조인 — 복사하면 조용히 낡는다」로 이미 같은 판단을 했다.
  **유지보수가 아파지면 그때 옮긴다**(사용자 지시).

**읽는 문은 하나다 (확정 — K22).**

- `overlapping_blocks(member_ids, 구간)` **한 문**이 **두 표를 조회**한다 **(제안 — 이름)**.
  **표는 둘이어도 문은 하나다** — 갈리면 배정 쪽과 회의 쪽이 다른 규칙을 쓰게 된다.

**이것이 §1 Scope Out 한 줄을 뒤집는다** — 「회의 도메인의 변경 — 기간 파라미터 외에는 손대지
않는다」. **회의 생성·시각 변경에 이 검증이 붙는다.** §1 이 그 사실을 적는다.

### 2.10 업무 탭 우측 캘린더에도 회의가 선다 (확정 — 증보 K23·K24)

- 그 화면은 지금까지 **이 SPEC 의 범위 밖**이었다. **범위로 들인다.**
- **새 표면을 만들지 않는다** **(확정 — K23)**.
...
| **회의** | **탭과 무관하게 항상 「내 회의」** — §D 의 `board` 축 |

- **그 레일은 자기 질의가 없다** — 페이지가 이미 들고 있는 업무 목록을 **그대로 받는다**.
  그래서 **업무 축을 이 SPEC 이 정할 자리가 아니다.**
- **회의는 업무의 분류 축에 속하지 않는다** — 내 업무·보낸 업무·완료 업무·참조.
  **「보낸 업무」 탭이라고 해서 「내가 보낸 회의」 같은 것이 있는 게 아니다.**
- **그래서 합본 조회는 회의 절반에만 쓴다** **(확정 — K24)**.
  업무 절반은 **그 화면이 이미 들고 있으므로 다시 묻지 않는다.**
- **§D 를 넓히지 않는다** — `K23` 이 요구한 것은 **「회의도 보이게」**이지 업무 축의 변경이 아니다.

> **캘린더 화면과 다른 점은 업무 축 하나다.** 캘린더 화면은 업무도 합본 조회로 받고
> 그 축이 `my_work` 다(§2.7). **업무 탭 레일은 업무를 받지 않는다** —
...
**회의 생성·시각 변경 — 검증 한 겹이 붙는다 (확정 — 증보 K22)**

| 무엇 | 이번에 |
|---|---|
| 회의 **생성**(예약) · **시각 변경** | **겹침 검증이 붙는다.** 주최자 + 참석자 전원의 시간을 본다. 겹치면 `MEETING_TIME_OVERLAP` **409** |
| `quick-start` 회의 | **막지 않는다** |
| 그 밖의 회의 계약 | **바뀌지 않는다** — 요청 필드·응답 행 모양·권한·구획이 전부 그대로다 |

> **새 라우트를 만들지 않는다.** 기존 회의 명령에 **검증 한 겹**이 더해질 뿐이므로
> **운영 대장에 행이 늘지 않는다** — 그 행들의 `http_signature` 도 인자가 안 바뀌면 그대로다
> **(도출 — §J 의 등록 규칙)**.
...
| `kind` | 무엇 | 실리는 것 |
|---|---|---|
| `'task'` | 내가 맡은 업무 | `task_id` · `title` · `state` · **`approval`** · `start_date` · `due_date` · **`span_from` · `span_to`** · `version` · `schedules[]` |
...
- **`approval` 은 승인 여부다** **(확정 — 증보 K19)**. 값은 저장소 어휘 그대로
  `null` · `awaiting_review` · `awaiting_revision` · `approved` (SPEC-003 의 `derived.approval` 계승).
  **카드가 상태 배지와 승인 배지를 따로 내기 위해 필요하다**(§2.6) —
  `state` 만으로는 `COMPLETION_SUBMITTED` 인 업무가 **「완료」로 읽힌다.**
  **`derived` 묶음 전체를 싣지 않는다** — 이 한 값만이다 **(제안 — 최소)**.
...
| **시간 겹침 — 배정** | **나의 시간과 겹치면 거부** **(확정 — 증보 K22)**. 반열림 `[시작, 끝)`. `TASK_SCHEDULE_OVERLAP` 409 |
| **시간 겹침 — 회의** | **주최자 + 참석자 전원**의 시간과 겹치면 거부 **(확정 — 증보 K22)**. `quick-start` 는 **막지 않는다** · 사외 참석자는 **대상이 아니다**. `MEETING_TIME_OVERLAP` 409 |
| 이미 겹쳐 있는 것 | **그대로 둔다** **(확정 — 증보 K22)**. 검증은 **새 쓰기에만** 건다 |
| 회의 | **캘린더에서 쓰기 명령이 없다** **(확정 — §F)**. 회의 시각 변경은 **회의 화면**의 기존 명령이고, 거기에 위 겹침 검증이 붙는다 |
...
| `TASK_SCHEDULE_OVERLAP` | **409** | 배정 생성·시각 변경이 **내 다른 일정과 시간이 겹침** **(확정 — 증보 K22)** | 「이미 다른 일정이 있는 시간입니다.」 **(제안 — 문구)**. **재전송이면 영수증이 먼저다** **(확정 — 증보 K12)** — 같은 키의 재전송은 **자기가 방금 만든 배정과 겹치므로** 이 409 에 걸리면 안 된다. **경계가 닿는 것은 겹침이 아니다** — 11:00 에 끝나는 일정과 11:00 에 시작하는 배정은 **둘 다 선다** |
| `MEETING_TIME_OVERLAP` | **409** | 회의 생성·시각 변경이 **주최자 또는 참석자의 일정과 겹침** **(확정 — 증보 K22)** | 「**민아 님의 일정과 겹칩니다.**」 **(제안 — 문구)**. **이름은 말하고 내용은 말하지 않는다** — 남의 시간은 **busy/free 만** 읽는다. `quick-start` 는 이 검사를 지나지 않는다 |
...
| 합본 조회 업무 행의 승인 | **`approval`** = `null` · `awaiting_review` · `awaiting_revision` · `approved` | **(확정 — 증보 K19)** |
| 겹침의 정의 | **반열림 `[시작, 끝)`.** 경계가 닿는 것은 겹침이 아니다 | **(확정 — 증보 K22)** |
| 겹침을 막는 층 | **application.** DB 제약이 아니다 — 두 표에 걸쳐 있다 | **(확정 — 증보 K22)** |
| 월 뷰가 그리는 것 | 업무 **기간 띠** + **회의**. **업무의 시간 배정은 안 그린다** | **(확정 — 증보 K20)** |
...
| 캘린더 **격자·좌측 카드**가 내지 않는 것 | **업무 상태의 색·배지** | **(확정 — §J · 증보 K15 · §2.6)** |
| **상세**의 상태 어휘 | **우리 기존 어휘 그대로** (`fe-survey-report.md` §7-1) | **(확정 — §J 정정 · §2.6)** |
...
- **영수증이 409 보다 먼저다 (확정 — 증보 K12).** 생성이 내는 **모든 409**
  (`TASK_SCHEDULE_DAY_TAKEN` · `TASK_SCHEDULE_TASK_CLOSED` · **`TASK_SCHEDULE_OVERLAP`**)에
  **같은 순서가 적용된다** — **멱등 키 조회 → (같은 키면 영수증) → 아니면 나머지 검사.**
  SPEC-003 의 선례 그대로다. **이 목록은 닫힌 열거가 아니다** — 생성 경로에 409 가 더 생기면
  **그것도 같은 순서를 탄다.**
- **`TASK_SCHEDULE_OVERLAP` 이 특히 이 순서를 탄다 (확정 — 증보 K22 가 K12 를 좁히지 않는다).**
  같은 멱등 키의 재전송은 **자기가 방금 만든 배정과 같은 구간**이라 **자기 자신과 겹친다.**
  영수증을 먼저 안 보면 **드래그 연타가 `409` 를 맞는다** — 그것이 **멱등 키가 존재하는 이유
  그 자체**다(§4 「멱등 키가 존재하는 이유가 드래그 연타」).
  **`MEETING_TIME_OVERLAP` 은 이 순서와 무관하다** — 회의 명령이라 K12 의 사정권 밖이다.
...
- **겹침은 application 이 답한다 — 그리고 그 틈이 남는다** **(확정 — 증보 K22)**.
  두 표에 걸쳐 있어 **DB 제약을 걸 수 없다.** 검사와 저장을 **한 트랜잭션**에 두지만,
  **동시 요청 둘이 각각 검사를 통과해 둘 다 설 수 있다.**
  **「하루 한 칸」과 같은 세기의 보장이 아니다** — §2.9 가 그 차이를 표로 적는다.
- **겹침을 읽는 문은 하나다** **(확정 — 증보 K22)**. `overlapping_blocks(member_ids, 구간)`
  **(제안 — 이름)** 이 **두 표를 조회**한다. **배정 쪽과 회의 쪽이 각자 조회를 쓰지 않는다** —
  갈리면 두 규칙이 된다. **반열림 판정도 그 한 자리에 있다.**
- **남의 시간은 busy/free 만 읽는다** **(확정 — 증보 K22)**. 제목·내용을 읽지 않고,
  **거절 문구도 이름까지만** 낸다. **그 문을 통과한 값이 화면으로 새지 않는다.**
...
- **업무 조회는 기존 한 자리를 재사용한다** **(제안 — `be-survey-report.md` §7 해석 1)**.
  ...
  깨진다.
- **업무 탭 우측 캘린더는 합본 조회를 회의 절반에만 쓴다** **(확정 — 증보 K23·K24)**.
  **업무는 그 화면이 이미 들고 있으므로 다시 묻지 않는다** — 그 레일은 **자기 질의가 없고**
  페이지의 업무 목록을 그대로 받는다. **그 화면을 위한 새 조회를 만들지 않는다** —
  만들면 「한 자리에서만 답한다」가 또 깨진다.
...
- [ ] 내 다른 일정과 **시간이 겹치는 배정**을 만들면 **`TASK_SCHEDULE_OVERLAP` 409** 다.
- [ ] **경계가 닿는 것은 통과한다** — 11:00 에 끝나는 일정이 있어도 **11:00 시작 배정이 선다**
      (반열림 `[시작, 끝)`).
- [ ] **같은 멱등 키의 재전송은 자기 배정과 겹쳐도 `200` 영수증이다** —
      **`TASK_SCHEDULE_OVERLAP` 이 아니다** (증보 K12 가 K22 보다 먼저다).
      **드래그 연타가 오류 문구를 띄우지 않는다.**
- [ ] 시각 변경으로 겹치게 만들어도 **같은 409** 다.
- [ ] **회의 생성·시각 변경**이 **주최자 또는 참석자**의 일정과 겹치면 **`MEETING_TIME_OVERLAP` 409** 다.
- [ ] **`quick-start` 회의는 막히지 않는다.**
- [ ] **사외 참석자는 검사 대상이 아니다** — 계정이 없는 참석자가 있어도 회의가 선다.
- [ ] **거절 문구가 이름까지만 말한다** — 남의 일정 **제목·내용이 응답에 없다.**
- [ ] **자정을 넘는 회의가 날짜별로 쪼개져** 비교된다.
- [ ] **이미 겹쳐 있는 기존 데이터를 건드리지 않는다** — 소급해 닫거나 고치는 쓰기가 0건이다.
- [ ] 겹침을 **읽는 문이 하나**다 — 배정 쪽과 회의 쪽이 **같은 판정**을 쓴다.
- [ ] 검사와 저장이 **한 트랜잭션**에 있다.
- [ ] **한계가 문서에 적혀 있다** — 겹침은 **application 이** 막고 **DB 제약이 아니며**,
      동시 요청 둘이 **각각 검사를 통과해 둘 다 설 수 있다**(§2.9).
      **이 줄은 구현이 아니라 문서가 닫는다.**
...
- [ ] **좌측 카드가 상태 배지와 승인 배지 둘**을 낸다 (증보 K19) — 업무 화면과 **같은 방식**이다.
- [ ] 합본 조회 업무 행이 **`approval`** 을 낸다 — 값은 저장소 어휘 그대로다.
- [ ] **`COMPLETION_SUBMITTED` 인 업무가 카드에서 「완료」로만 읽히는 자리가 0개**다 —
      **승인 배지가 「확인 대기」를 말한다.**
- [ ] **상세**는 **기존 업무 화면과 같은 상태 어휘**로 낸다 —
      캘린더 전용 상태 어휘를 새로 만들지 않았다.
- [ ] **월 뷰에 업무의 시간 배정이 그려지지 않는다** (증보 K20) —
      **같은 업무가 띠와 시간 조각으로 두 번 뜨는 자리가 0개**다. **회의는 월 뷰에도 그려진다.**
- [ ] **월 뷰 셀 안이 시간순으로 정렬**된다.
- [ ] **주 뷰는 그대로다** — 종일 칸과 시간 격자가 나뉜 채 둘 다 그린다 (회귀).
- [ ] **시간 격자에서 겹치는 블록이 나란히** 그려진다 (증보 K21) —
      09:30–12:30 안에 든 10:00–11:00 회의가 **밑에 깔리지 않는다.**
- [ ] **업무 탭 우측 캘린더에 회의가 선다** (증보 K23) — **새 표면 없이** 합본 조회의
      **회의 절반**을 쓴다.
- [ ] 그 레일의 **업무 목록이 그 탭이 들고 있는 것 그대로**다 (증보 K24) —
      **「보낸 업무」 탭에서 보낸 업무가 사라지지 않는다** (회귀).
- [ ] 그 레일의 **회의는 탭과 무관하게 같다** — 어느 탭에서도 「내 회의」다.
...
| 캘린더에서 상태를 어디서 읽나 | **상세에서 — 우리 기존 어휘로.** 좌측 카드는 유형만 낸다 **(증보 K15 로 「좌측 카드와」가 빠졌다)** | §2.6 · §4 Data Contract |
...
### DEC-003 증보 추적 — 전수 (K1~K15 · K19~K24)

**증보 1(K1~K4)** 은 이 SPEC 초판이 OQ-401~404 로 올린 것을, **증보 2(K5~K8)** 와
**증보 3(K9)** 는 **검수 1차 FAIL** 이 연 것을, **K10** 은 **수정 1차가 주의점으로 올린 것**을,
**증보 4(K11~K13)** 는 **검수 2차 FAIL** 이, **증보 5(K14)** 는 **검수 3차 FAIL** 이,
**증보 6(K15)** 은 **BE-1 구현**이, **증보 8(K19~K23)** 은 **2루프에서 사용자가 실물로 쓰며 찾은 것**을,
**`K24`** 는 **v0.3.0 이 올린 `OQ-501`** 을 코디네이터가 닫았다.
**`K19` 가 `K15` 를 뒤집었고 두 문장을 다 남겼다**(§2.6).
...
| ~~**K15**~~ | ~~좌측 카드는 유형만 낸다~~ → **`K19` 가 뒤집었다** (2026-09-21) | §2.6 의 정정 기록 ②가 문장을 보존한다 | — |
| **K19** | **좌측 카드가 상태 배지 + 승인 배지 둘**을 낸다. 합본 조회에 **`approval`** 을 싣는다. **K15 를 뒤집는다** | §2.6(정정 ③ · **K15 가 왜 틀렸나**까지) · §4 합본 조회 업무 행·bullet · §4 Data Contract | §6 화면 — **배지 둘 · `approval` 을 낸다 · 「완료」로만 읽히는 자리 0개** |
| **K20** | **월 뷰는 업무의 시간 배정을 그리지 않는다.** 회의는 그린다. 셀 안은 **시간순** | §2.1 「월 뷰는 …」 표 + 시안과 다르게 가는 이유 | §6 화면 — **두 번 뜨는 자리 0개 · 회의는 그려진다 · 시간순 · 주 뷰 회귀** |
| **K21** | **겹치는 시간 블록을 나란히** 그린다 | §2.1 「겹치는 시간 블록은 …」 | §6 화면 — **밑에 깔리지 않는다** |
| **K22** | **겹치면 뒤에 오는 것을 막는다.** 배정·회의 둘 다. 반열림 `[시작, 끝)` | **§2.9 전체**(정의·대상·읽는 법·막는 범위·**한계**·읽는 문) · §1 Scope Out 정정 · §4 API 회의 표 · §4 Validation 세 행 · §4 Case Matrix 두 행 · §4 Data Contract · §5 네 bullet | §6 **겹침 금지 13줄** — 409 둘 · 경계 통과 · quick-start · 사외 · 문구 · 자정 · 기존 데이터 · 문 하나 · 트랜잭션 · **한계** · 회의 회귀 |
| **K23** | **업무 탭 우측 캘린더에도 회의가 선다.** 새 표면을 만들지 않는다 | §1 Scope In · §2.10 · §5 | §6 화면 — **회의가 선다 · 합본 조회를 쓴다** |
| **K24** | 그 레일의 축 — **업무는 그 탭이 들고 있는 것 그대로**(지금 동작 유지) · **회의는 항상 「내 회의」**. 합본 조회는 **회의 절반에만** | §2.10 축 표 + 인용 · §5 · §7(OQ-501 닫힘) | §6 화면 — **탭을 바꿔도 업무 목록이 그 탭 것 그대로 · 회의는 탭과 무관하게 같다** |
...
### 드러난 것은 **K1~K24 로 닫혔다 (DEC-003 증보)**

**이 SPEC 에 미결로 남은 것은 없다.** 일곱 판에 걸쳐 스물하나가 닫혔다
(K1~K15 · K19~K24 — K16~K18 은 WORK·구현 쪽이라 이 SPEC 의 항목이 아니다).
**v0.3.0 이 올린 `OQ-501` 은 `K24` 가 닫았다**(아래).
...
**증보 6 — 구현이 드러낸 자리 하나 (SPEC 이 옛말이었다)**

**BE-1 이 사실 하나를 드러냈고 그것이 §2.6 을 뒤집었다.** 합본 조회의 `state` 는 기존 투영을
그대로 쓰는데 **`COMPLETION_SUBMITTED` 인 업무가 거기서 `"done"` 으로 나간다** —
프론트 어휘 5종을 유지하고(§J) K13 으로 행에 `derived` 가 없기 때문이다.
**그래서 좌측 카드가 상태를 내면 승인 대기 업무를 「완료」로 읽는다.**

| 무엇 | 닫은 결정 | 어디 |
|---|---|---|
| §2.6 초판이 「좌측 카드·상세는 우리 기존 어휘로 상태를 낸다」였다 | **K15** — **좌측 카드는 유형만 낸다. 상태는 카드를 열어 상세에서 읽는다** | §2.6(초판 문장을 인용으로 남겼다) · §4 Data Contract · §6 화면 · §6 §J 추적표 |

> **이 SPEC 이 스스로 틀렸던 자리다.** §2.6 은 **BE-1 이 그 투영 사실을 드러내기 전에 쓰였다.**
> **합본 조회 행에 `derived` 를 더하는 길도 있었지만 고르지 않았다** — 시안 `ItemCard` 도
> 배지를 유형에만 쓰고, §J 의 「격자는 상태를 말하지 않는다」를 **카드까지 밀면** 필드를 더할
> 이유가 사라진다(DEC-003 증보 6).
...
**사용자가 실물로 쓰면서 찾은 것들이다.** 1루프는 계약대로 섰는데,
**계약이 덜 정해진 자리**와 **시안을 그대로 옮겨서 생긴 자리**가 드러났다.

| # | 무엇 | 닫은 결정 | 성격 | 이 SPEC 의 어디 |
|---|---|---|---|---|
| **K19** | 좌측 카드에 상태가 없다 | **상태 배지 + 승인 배지 둘.** 합본 조회에 `approval` | **`K15` 번복** | §2.6 · §4 · §6 |
| **K20** | 월 뷰에서 같은 업무가 두 번 뜬다 | **월 뷰는 업무의 시간 배정을 안 그린다.** 회의는 그린다 · 시간순 | **시안과 다르게 감** | §2.1 · §6 |
| **K21** | 겹치는 시간 블록이 밑에 깔린다 | **나란히 그린다** | 누락 | §2.1 · §6 |
| **K22** | 시간이 겹쳐도 안 막는다 | **겹치면 뒤에 오는 것을 막는다.** 회의에도 건다 | **새 계약** | **§2.9** · §1 · §4 · §5 · §6 |
| **K23** | 업무 탭 우측 캘린더에 회의가 없다 | **회의도 싣는다.** 새 표면 없이 | 범위 밖이었음 | §1 · §2.10 · §5 · §6 |
| **K24** | 그 레일의 **축**이 캘린더 화면과 같은가 (v0.3.0 의 `OQ-501`) | **업무는 그 탭 것 그대로 · 회의는 항상 「내 회의」** | **OQ 의 답** | §2.10 · §5 · §6 · §7 |

> **`K22` 가 이 판의 본론이고 `§1 Scope Out` 한 줄을 뒤집는다** —
> 「회의 도메인의 변경 — 기간 파라미터 외에는 손대지 않는다」.
> **사용자가 알고 고른 변경**이고, 회의 도메인에서 바뀌는 것은 **검증 한 겹**뿐이다.
> 초판 문장은 §1 에 남겼다.

> **`K19` 는 이 SPEC 이 같은 자리를 두 번 뒤집은 기록이다.**
> 초판 → `K15` → `K19`. **세 문장을 다 남겼다**(§2.6). `K15` 가 틀렸던 이유 —
> **「보여줄 방법이 없어서」 뺐는데 방법이 이미 있었다.**
...
| 옛 OQ | 닫은 결정 | 무엇으로 | 이 SPEC 의 어디 |
|---|---|---|---|
| **OQ-501** — 업무 탭 우측 캘린더의 축이 캘린더 화면과 같은가 | **K24** | **업무는 그 탭이 들고 있는 것을 그대로**(지금 동작 유지) · **회의는 탭과 무관하게 항상 「내 회의」**(`board` 축) | §2.10 · §5 |

> **v0.3.0 이 이것을 미결로 올렸고 코디네이터가 `K24` 로 닫았다**(DEC-003 증보 8 끝).
> **v0.3.0 은 「축이 같다」고 적었는데 그것이 틀렸다** — 그 레일은 **자기 질의가 없어**
> 페이지의 업무 목록을 그대로 받는다. 같은 축으로 구현하면 **「보낸 업무」 탭에서
> 보낸 업무가 사라진다.** **회의만 같고 업무는 다르다.**

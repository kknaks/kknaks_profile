...
| **(증보)** | **DEC-003 § Decision 의 증보** — SPEC 초판이 미결로 올린 넷(K1~K4) · **검수 1차가 연 다섯**(K5~K9) · **수정 1차가 남긴 주의점 하나**(K10) · **검수 2차가 연 셋**(K11~K13) · **검수 3차가 연 하나**(K14) · **BE-1 구현이 드러낸 하나**(K15)를 코디네이터가 정한 것. 확정과 같은 무게다 |
...
- Decision reference: `DEC-003` (frontmatter `links.decisions` 와 일치).
  ...
  **BE-1 구현이 드러낸 하나**(K15, 2026-09-21)를 닫은 자리다(§7).
...
### 2.6 **캘린더는 상태를 말하지 않는다** — 격자도 좌측 카드도 (확정 — §J · 증보 K15)

| 자리 | 계약 | 무엇이 정했나 |
|---|---|---|
| **캘린더 격자**(띠·시간 블록) | **상태를 말하지 않는다.** 색은 **유형 둘(업무/회의)로만** 칠한다 | **(레이아웃)** 격자에 상태를 얹을 자리가 배치에 없다 — 시안 `calendar.css:45-47` |
| **좌측 카드** | **상태를 내지 않는다.** 배지는 **유형(업무/회의)에만** 쓴다 | **(확정 — 증보 K15)** |
| **상세** | **우리 기존 어휘로 상태를 낸다** — 기존 업무 화면과 **같은 표시** | **(확정 — §J 정정)** |

**상태는 카드를 열어 상세에서 읽는다 (확정 — 증보 K15).**

- **카드가 상태를 내면 거짓말을 한다** — 합본 조회의 `state` 는 기존 투영을 그대로 쓰는데
  `COMPLETION_SUBMITTED` 인 업무가 거기서 **`"done"` 으로 나간다**(프론트 어휘 5종 유지 ·
  K13 으로 행에 `derived` 가 없다). 그러면 **승인 대기인 업무를 카드가 「완료」로 읽는다.**
- **합본 조회 행에 `derived` 를 더하지 않는다** — 시안 `ItemCard` 도 배지를 **유형에만** 쓰므로,
  §J 의 「격자는 상태를 말하지 않는다」를 **카드까지 밀면** 필드를 더할 이유가 사라진다.
  캘린더의 일은 **「이번 주 내가 맡은 업무에 시간을 배분하는 것」**이지 상태를 읽는 것이 아니다.
- **시안 목데이터의 `badge:'결정 요청'`·`actions:'accept'` 를 계약으로 올리지 않는다** —
  시안의 필드 구성은 기능 정본이 아니다.
- 상태의 **말**은 저장소에 **한 곳으로 모여 있고 그것이 정본**이다 — **상세가 그것을 쓴다**
  **(확정 — 우리 것이 정본 · `fe-survey-report.md` §7-1)**.
- 저장소가 상태를 **색**으로 바꾸는 자리 셋 중 **캘린더 칩에 상태를 클래스로 붙이던 자리**는
  **캘린더에서 안 쓰게 된다** **(도출 — `fe-survey-report.md` §7-1)**.

> **초판 문장 — 2026-09-21 증보 6 `K15` 로 정정됐다.**
> 초판은 「**좌측 카드 · 상세** | **우리 기존 어휘로 상태를 낸다** — 기존 업무 화면과 **같은 표시**를
> 쓴다」였고, 마지막 bullet 이 「좌측 카드의 tone 은 **기존 상태 톤**을 쓴다 **(제안 — 우리 어휘)**」였다.
> **BE-1 이 「합본 조회의 `state` 가 `COMPLETION_SUBMITTED` 를 `"done"` 으로 투영한다」는 사실을
> 드러내기 전에 쓰인 문장**이다. **`K15` 가 「좌측 카드와」를 뺐다.**
...
- [ ] **좌측 카드**가 **상태를 내지 않는다** — 배지는 **유형(업무/회의)에만** 쓴다 (증보 K15).
...
**증보 6(K15)** 은 **BE-1 구현**이 연 것을 코디네이터가 닫았다.
...
| **K15** | **좌측 카드는 유형만 낸다.** 상태는 카드를 열어 **상세**에서 읽는다 | §2.6(초판 문장을 인용으로 남겼다) · §4 Data Contract 두 행 · §6 §J 추적표 | §6 화면 — **좌측 카드가 상태를 내지 않는다 · `COMPLETION_SUBMITTED` 가 「완료」로 읽히는 자리 0개 · 상세는 기존 어휘** |
...
### 드러난 것은 **K1~K15 로 전부 닫혔다 (DEC-003 증보)**

**이 SPEC 에 미결로 남은 것은 없다.** 여섯 판에 걸쳐 열다섯이 닫혔다.

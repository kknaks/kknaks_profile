...
| `gold_kpi_monthly` | **8** | 달력 월 1행. **빌드가 일별에서 집계해 생성한다**(2026-09-02 확정). 부분 월 플래그 보유 |
...
**부분 월 판정식 (2026-09-03 확정 — 주별 규칙 준용)**

- **`is_partial_month`** = 그 달의 **관측일 수(`days_observed`)가 달력 일수보다 적으면 참**.
  `days_observed` 는 `gold_kpi_daily` 에 그 달 행이 존재하는 날의 수다.
- 기간 경계가 걸린 달(첫 달 2026-01 · 마지막 달 2026-08)뿐 아니라 **결측일이 있는 달도
  부분 월로 잡힌다** — 2026-02 는 2026-02-17 이 행 자체가 없으므로(0 채움 없음) 28 > 27 로
  부분 월이다.
- **응답에 `is_partial_month` 와 `days_observed` 를 함께 노출**한다. 플래그만 주면 「얼마나
  모자란 달인지」를 소비자가 알 수 없다.
- 부분 월도 **행은 만든다.** 걸러 쓸지는 소비자가 정한다(주별 `is_partial_week` 와 같은 태도).
...
- 실버의 `patientName`·`phone` 은 **미반입**이고 `birthday` 는 `age_band`(10세 단위, 결측
  ...
  같은 진입점을 쓰도록 뷰를 둔다.
- **뷰는 PII 컬럼명을 그대로 유지하고 값만 마스킹본으로 바꾼다.** 컬럼을 숨기지 않는 이유는
  드릴다운 화면이 「이 자리에 실명이 있었고 가려졌다」를 보여야 하기 때문이다 — 컬럼이 사라지면
  마스킹 사실 자체가 안 보인다. 원값이 못 나오는 근거는 **뷰의 SELECT 목록**이다.
- `chart_no` 는 **해시하지 않고 그대로** 반입·노출한다(기록 03 확정 — 내부 데모 범위에서
  조인·검증 추적성 우선). 빈 값은 「환자 미식별」 그룹이다.
- **`chart_no` 방어 (2026-09-03 — WORK-005 실측)**: 추적용으로 유지하되 **숫자 패턴이 아닌
  값은 뷰에서 마스킹 표기로 치환**한다. 원천 오염으로 차트번호 자리에 이름이 앉은 행이
  **1건 실측**됐다 — 컬럼 이름이 `chart_no` 라는 이유로 통과시키면 노출 0건 게이트가
  그 자리에서 뚫린다. DEC-002 의 **노출 0건 게이트 연장**이며, 판정은 컬럼이 아니라
  **값의 형태**로 한다.
...
- [ ] **AC-7 (게이트 3 — PII 마스킹)** 마스킹 뷰 산출 어디에도 `patientName`·`phone`·
      `birthday`·`authorName` **원값 검색 0건**. **`chart_no` 도 숫자 패턴이 아닌 값은
      마스킹 표기로 나간다**(원천 오염 1건 실측). 이름 `김○○` · 전화 `010-****-1234` ·
      생년월일 `1990-**-**` 표기로만 노출.
      **뷰는 PII 컬럼명을 유지한다** — 가리는 것은 이름이 아니라 값이다. 따라서 어떤
      조회·필터·정렬로도 원값이 나올 수 없다. **노출 0건 보증의 주체는 「뷰의 SELECT 목록」과
      AC-8(뷰 경유 강제)이지 컬럼명 숨김이 아니다.**

...
**층 rank는 층간 방향만 강제한다.** 실행층 내부의 순서(`baseline → decision → spec → work`)는 현행 `_TYPE_RANK`가 `baseline 4 / decision·spec 3 / work 2`로 인코딩하고 있는데, 이는 `para/projects/project.md`의 매핑 규칙이 이미 소유한 영역이다. 층 모델은 이를 흡수하지 않고 실행층을 하나로 묶으며, 제품 문서 내부 정합은 `product_doc_pipeline.py` 검증기가 계속 담당한다.

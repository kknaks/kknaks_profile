...
> **부분 supersede (2026-08-11) — [[decision-021-inbox-is-an-entry|KDEV-DEC-021]]**
>
> 아래 D1 중 **`inbox/` 를 「보류함(목적지)」으로 둔 것은 폐기**됐다. `inbox/` 는
> **공부 노트의 입구**이고 서버에서는 항상 비어 있다. route 의 `inbox_hold` 배타
> 옵션도 없앴다(0건 사용). **큐를 DB 에 두는 것과 승인 전 파일을 안 만드는 것은 그대로다.**

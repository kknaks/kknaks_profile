...
- 2026-08-03 **KDEV-BL-006 작성 (raw)** — PARA 정렬과 SoT 지도. 발단은 *"디렉토리가 정리가 안 된다, SoT 가 사방에 흩어져 있다"*. 진단 셋 — ① **`permanent` 가 두 뜻을 겸한다**(R 버킷의 그릇 + synthesis 층 그 자체). `reference` 를 그 안에 넣으면 4층 위계가 뒤집힌다(source 가 synthesis 아래) ② **SoT 라는 말도 두 뜻**이다 — 형식 SoT(`templates/**` 10개)와 데이터 SoT(`40-arch/database`). 선언 문서가 20개인데 같은 단어라 매번 어느 축인지 다시 읽어야 한다. **`permanent` 겸직과 같은 병** ③ **주인 없는 디렉토리 둘** — `downloads/`(바이너리가 git 에 커밋, gitignore 에도 없음) · `reports/`(폐기된 PLAN 체계 잔재). 실측이 판을 바꿨다 — **지식층 전체가 14 파일이고 옮길 둘 중 `permanent/archive/` 는 비어 있으며 `permanent/*.md`(synthesis)도 0건**이라 데이터 이동 위험이 거의 없다. 그리고 **`layer` 는 디렉토리가 아니라 `type` 에서 도출**하므로(DEC-010 D3, `graph.py:78`) 폴더를 옮겨도 그래프 층 판정·L1~L6 이 안 흔들린다 — 코드 접점이 셋뿐이다. 세 안 중 **B 채택**: `resources/{reference,concept,notes}` 신설 + `archive/` 최상위 승격. PARA 와 4층은 겹치는 게 아니라 **직교**한다. Area(`persona/`)는 사용자가 *"최종으로"* 미뤘다. 미결 6 — persona 의 A 판정(지속책임 vs 공개표면) · synthesis 폴더명 · inbox 위치 · downloads/reports 처분 · SoT 두 뜻의 명칭 · P(428 파일)는 이번에 안 건드린다는 명시 여부.
- 2026-08-03 **KDEV-WORK-018 P1~P4 done + DEC-017 D18 추가** — 제품 레지스트리가 화면까지 섰다. **P2 계층 신설**(`repository/` · `service/products/dto.py`, ORM 이 계층 밖으로 안 나간다. `sync_all` 의 ORM 직접 대입을 `mark_synced`/`mark_failed` 로 끊었다) · **P3 service+API**(엔드포인트 8, 사전 검증 7종, 스캐폴드 화이트리스트, 커밋 1개+롤백) · **P4 화면**(배너·표·폼·토글·필터 탭). **로컬 e2e 가 결함 4건을 꺼냈고 `next build` 로는 하나도 안 잡혔다.** ① **`TopNav` 훅 순서 위반으로 `/admin` 전 경로가 죽어 있었다** — 조건부 반환이 `useEffect` 앞에 있어 훅 4개만 실행. **내 변경이 아니라 WORK-011 부터 잠복**했고 dev 엄격검사에서만 드러난다 ② 네이티브 `<select>` 팝업을 OS 가 그려 어두운 화면에서 튄다 → **공용 `Select` 컴포넌트로 대체**(표가 `overflow-x:auto` 라 `position: fixed` 로 띄운다) + `:root` 에 `color-scheme: dark`(머리말은 "Dark-first" 인데 선언이 없었다) ③ **노출을 못 바꾸는 것이 실제로 걸렸다 → D18** — D14 가 기각한 건 *DB 이관*이었고 "화면이 파일을 고친다" 를 빠뜨렸던 것. 실측 **1 insertion/1 deletion**, 주석·키순서 보존, reload 후 공개 6→5장. WORK-017 결함 ⑩(42/38) 회피 ④ **카드 없는 제품에 카드를 만들 경로가 없었다** — `register()` 가 `PRODUCT_EXISTS` 로 거부해 **BL-005 가 진단한 "카드 없는 5개" 가 영영 못 뜨는 상태**였다. `POST /{id}/card` 신설, `mac-remote` 에 P-16 생성. **미등록 발견 창을 실측으로 30일 확정** — 57건 중 29건이 1년 넘게 안 민 죽은 레포라 배너가 소음이었다. 감춘 수(52)를 배너에 함께 싣는다. 887 passed · tsc · next build · doc pipeline 0 errors.

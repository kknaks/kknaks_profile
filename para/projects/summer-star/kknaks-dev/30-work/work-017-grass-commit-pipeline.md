...
| Owner | kknaks |
| Status | done |
| Progress | 100% (**2026-08-03 서버에서 하루치가 실 push 로 나갔다** — `a8c44b4`. 발주 시점의 목표였던 "사람 승인 없이 레포에 쓰이는 경로를 없앤다" 가 실제로 닫혔다: 스케줄러가 `run_daily_intake_job` 으로 교체돼 구 잔디 잡이 더는 돌지 않는다. 결함 10건 발견·9건 수정, 818 passed. **100 으로 올리되 남는 것 셋을 Open Issue 로 세웠다** — 눈금을 깎는 것보다 목록으로 남기는 편이 정직하다. ① `permanent/concept/` 발행 경로가 어디서도 안 탔다 ② cron 09:05 자동 발화가 관측 전이다 ③ 배포 후 유튜브를 한 건도 안 던져 봤다. 셋 다 "코드가 없다" 가 아니라 **"그런 날이 아직 안 왔다"** 라서 이 발주를 열어 둘 이유가 되지 않는다) |
| Branch/PR | `work-017-p2` |
| Blocker | 없음 |
| Next | 없음 — 이 발주는 닫혔다. 관측 항목 셋은 Open Issues 에 있고, 후속 대상(`algorithms`·`content_enrich` 게이트 편입 · studio 누적처)은 Scope 「제외」 그대로 후속 baseline 이다 |
...
| FE | kknaks | 게이트 화면 (줄/문장 단위 편집) | done — 사람이 로컬·운영 양쪽에서 눌렀고 **결함 ⑧⑨를 그 과정에서 잡았다.** 이전 서술: 작업 전부 들어왔고 **`tsc --noEmit`·`next build` 통과**(`75d2605`). 사람이 눌러 보는 경로만 남았다 |
| BE | kknaks | 준비부 일반화·스테이지·발행부·클론 | **done — 코드 전부.** 남은 것은 배포와 실운영 관측이다 |
| QA | kknaks | 검증과 완료 판단 | done — 테스트 경로·로컬 화면·운영 실발행 셋 다 돌았다. 818 passed |
| Ops | kknaks | 볼륨·env·배포·첫 클론 | done — 2026-08-03 배포. 사람이 한 것은 `ENV_FILE` 시크릿에 `WORKER_CONCURRENCY=1` 추가 + 시드 1회뿐, 나머지는 Actions 가 했다 |
...
- **Status**: DONE — 사람이 화면에서 실제로 눌렀다(2026-08-02 로컬 · 2026-08-03 운영). 그 과정에서 **결함 ⑧⑨가 나왔다** — 화면을 안 눌렀으면 못 잡았을 것들이다(아래 「로컬 e2e 결함 10건」).
...
- **Status**: DONE — 배포와 하루치 실발행까지 완주했다(2026-08-03). PR #6 머지 → Actions 자동 배포 → 레지스트리 13건 시드 → 첫 클론 290M → 접수 → 게이트 → 사람 승인 → **실 push**(`a8c44b4`).
...
- **작업**:
  ...
  - [x] 배포 — **Actions 가 거의 다 한다**(`git reset --hard` · `.env` 를 `ENV_FILE` 시크릿으로 덮어씀 · `compose up --build` · 마이그레이션은 back 엔트리포인트). 사람이 한 것은 시크릿에 `WORKER_CONCURRENCY=1` 추가 하나뿐. 첫 클론 **290M/13개, 약 20초**
  - [x] 하루치 실발행 완주 — 항목 `#3867`(2026-08-02), 커밋 `a8c44b4`
- **검증**:
...
- [x] SPEC-011·012·013 의 Acceptance Criteria 가 검증됐다 — 실데이터 항목도 닫혔다: **실클론 13개(290M)** · **identity 3종 실측**(`benesia93@naver.com`·`keonhak33@naver.com`·`kknaks@medisolveai.com` — BL-004 예측과 셋째가 달랐다) · 실측 건수(`commit 4`)
- [x] SPEC-010 개정분이 코드에 반영됐고 **실발행에서 실제로 탔다** — `daily` 가 `upsert` 로 나갔고(구 잡 산출물을 덮었다), 그래프 검증을 건너뛰었고(daily 는 그래프 노드가 아니다), `user_authored` 차단이 접수 단계에 서 있다
...
- [x] **진짜 데이터로 하루치 실발행**(P5) — 커밋 `a8c44b4 knowledge: publish item #3867 (daily:1)`, author `kknaks <benesia93@naver.com>`, `origin/main` 반영 확인.

  ⚠ **나간 것은 `daily` 하나다.** 그날 career 는 `changed:false`(회사 레포 커밋 2건이 자동 브리핑 재업로드라 경력 서술을 바꿀 것이 없었다), concept 는 0건(모델이 "`up:` 이 가리킬 reference stem 이 없다" 고 판단 — 억지로 만들지 않는 규율대로다). **셋이 한 커밋으로 나가는 것은 아직 관측되지 않았다.** career 목적지는 로컬 dry-run 에서 탔고 거기서 결함 ⑩을 찾았지만, **`permanent/concept/` 는 어디서도 한 번도 안 탔다** — 아래 Open Issue
- [x] 구 잔디 경로가 제거되어 이중 실행이 없다 — `main_job.py`·`llm.py`·`upsert.py` 파일째, `inputs.py` 의 세 함수(`9382d27`)
- [~] 기존 유튜브 파이프라인 회귀 없음 — **818 passed**. ⑥에서 JSON 추출기를 셋에서 하나로 합칠 때 유튜브 게이트도 같이 지났다(같은 함수를 쓴다). **실사용 확인은 아직**이다 — 배포 후 유튜브를 한 건도 안 던져 봤다
...
- **닫으면서 남긴 관측 항목 셋 (2026-08-03).** 코드가 없는 것이 아니라 **그런 날이 아직 안 온 것**이라 발주를 열어 두지 않는다.

  ① **`permanent/concept/` 발행 경로가 어디서도 안 탔다.** 로컬 두 판·운영 한 판 모두 concept 0건이었다 — 모델이 "`up:` 이 가리킬 reference stem 이 없다" 며 만들지 않았고 그것이 규율대로다. `daily`·`career` 와 달리 이 목적지는 **allowlist·그래프 검증·발행이 한 번도 실행되지 않았다.** concept 가 나오는 날 처음 돈다.

  ② **cron 09:05 자동 발화가 관측 전이다.** 잡이 `follow()` 를 부르는 것은 단위 테스트가 덮고 접수→발행 경로는 손으로 태웠다. 남은 것은 APScheduler 가 그 시각에 실제로 부르는가 하나다.

  ③ **배포 후 유튜브를 한 건도 안 던져 봤다.** ⑥에서 JSON 추출기를 셋에서 하나로 합쳐 유튜브 게이트도 같은 함수를 쓴다. 818 passed 로 테스트 회귀는 없지만 실사용은 미확인이다.

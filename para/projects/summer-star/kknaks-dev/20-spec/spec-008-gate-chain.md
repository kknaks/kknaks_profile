...
노트를 만드는 게이트(`source_note`·`concept`·`derived`·`post`)가 모델에게 요구하는 출력 형식이다. **JSON 이 아니다.**

**JSON 을 쓰지 않는 이유.** markdown 전문을 JSON 문자열 값으로 넣게 하면, 본문의 따옴표·줄바꿈·백슬래시를 수천 자에 걸쳐 하나도 틀리지 않고 이스케이프해야 통과한다. 실제로 막혔다 — 항목 #3880 의 `source_note` 1차가 `Expecting ',' delimiter: line 3 column 840` 로 죽었고(95초), 재시도(309초)와 사람의 개입이 한 번씩 더 들었다. **확률이 본문 길이에 비례**하므로 본문이 가장 긴 교안(`derived`)에서 가장 자주 난다.

구조는 **짧은 단일행 값들의 헤더 + 손대지 않은 본문**이다. 이스케이프가 필요한 자리를 없앤다.
...
본문을 그대로 쓴다. "따옴표"도 \백슬래시도 줄바꿈도 손대지 않는다.
---8<--- end
```

마커는 `---8<---` 한 종류이고 뒤에 역할이 붙는다.

| 마커 | 역할 |
|---|---|
| `---8<--- note` | 레코드 시작. 레코드가 여럿인 스테이지(`concept`)만 쓴다 |
| `---8<---` (또는 `---8<--- content`) | 헤더/본문 경계. 이 줄 **다음 줄부터** 본문이다 |
| `---8<--- end` | 출력 끝 |
| `---8<--- none` | 레코드 0건. 개념을 하나도 뽑지 않은 경우 |

헤더는 `key: value` 한 줄씩이고 **값은 전부 짧은 단일행**이다. 같은 키를 여러 줄 쓰면 목록이 된다(`derived` 의 `concept`). 스테이지별 키는 아래와 같다.

| 스테이지 | 레코드 수 | 헤더 키 | 본문 |
|---|---|---|---|
| `source_note` | 1 | `filename_stem` | reference 전문 |
| `post` | 1 | `filename_stem` | post 전문 |
| `concept` | 0..N | `filename_stem` · `mode` · `names` | concept 전문 |
| `derived` | 1 | `title_ko` · `title_en` · `summary_ko` · `summary_en` · `tags` · `concept`(반복) · `kind` | `## 개요` 부터의 교안 본문 |

**관용성은 JSON 파서가 갖고 있던 것과 같은 급이어야 한다.** 종전 `extract_json_object` 가 코드펜스와 머리말을 벗겨야 했던 이유는 프롬프트를 조여도 모델이 앞에 설명을 붙이는 일이 실제로 있었기 때문이다. 새 형식도 같은 것을 받는다.

- 출력 전체를 감싼 코드펜스를 벗긴다
- 첫 마커 또는 **아는 헤더 키**가 나오기 전의 산문을 버린다
- `---8<--- end` 뒤의 산문을 버린다
- 본문의 앞뒤 빈 줄을 다듬는다 — frontmatter 는 첫 줄에서 시작해야 한다
- 헤더 자리의 **모르는 키**는 버린다. 필수 키가 없으면 그때 실패하고, 오류 메시지가 **무엇을 봤는지** 함께 알린다

**관용은 여기까지다.** 마커가 아예 없거나 필수 키가 없거나 본문이 비면 게이트를 통과시키지 않는다. 조용히 통과시키면 형식이 깨진 노트가 사람의 승인 화면까지 올라간다.

본문 안에 나온 **역할 없는 마커(`---8<---`)는 글자로 읽는다.** 경계를 여는 일은 이미 끝났고, 그것을 구조로 읽으면 노트가 소리 없이 잘린다 — 잘리는 쪽이 형식이 조금 이상한 쪽보다 나쁘다. `note`·`end` 는 본문 안에서도 구조로 남는다(그 둘이 본문에 나오는 것은 형식 위반이다).

| 에러 코드 | 백엔드 출력 | 프론트 출력 | 표시 위치 |
|---|---|---|---|
| `INVALID_NOTE_OUTPUT` | 마커 없음 · 필수 키 없음 · 본문 빔 · 레코드 수 불일치 | 형식에 맞지 않는 결과입니다. 다시 시도해 주세요. | 게이트 카드 |
| `INVALID_NOTE_STEM` | 파일명 규약 위반 | 파일명이 규약에 맞지 않습니다. | 게이트 카드 |
| `INVALID_CONCEPT_OUTPUT` | 개념 레코드의 `mode`·`content` 이상 | 개념 결과가 형식에 맞지 않습니다. | concept 게이트 |
| `INVALID_DERIVED_OUTPUT` | 교안 헤더 키 누락 · `kind` 미상 · 본문에 「개요」 없음 | 교안 결과가 형식에 맞지 않습니다. | derived 게이트 |

**route 는 이 형식을 쓰지 않는다.** 목적지 판정은 짧은 불리언 조합이라 본문이 없고, 이스케이프가 문제 되는 자리가 아니다. 잔디(`daily`)도 아직 JSON 이다 — 산출물이 넷으로 중첩돼 형태가 다르고, 옮기는 것은 별도 범위다(§7).
...
- **노트를 만드는 게이트의 출력은 구분자 레코드다.** markdown 전문을 JSON 문자열 값에 넣지 않는다 — 이스케이프가 필요한 자리를 만들지 않는 것이 규칙이고, 형식 위반은 관용의 범위 밖에서 게이트를 열지 않는다(§4).
...
- [ ] 승인된 게이트는 재승인되지 않는다.
- [ ] 노트 게이트 넷이 구분자 레코드를 내고, 본문의 따옴표·줄바꿈·백슬래시가 손상 없이 보존된다.
- [ ] 코드펜스로 감싸거나 머리말·꼬리말을 붙인 출력도 파싱된다.
- [ ] 마커가 없거나 필수 키가 빠진 출력은 게이트를 통과하지 못하고 사유가 표시된다.
- [ ] `concept` 가 개념 0건일 때 `---8<--- none` 으로 빈 목록이 표현된다.
- [ ] 실전 자료 하나가 **재시도 없이** 체인을 완주한다.
...
- **(OPEN)** 잔디(`daily`) 게이트를 구분자 레코드로 옮길지. 같은 이스케이프 위험을 그대로 갖고 있고(daily 본문·career 전문·current 표·concept 전문이 전부 JSON 문자열 값이다) 매일 돈다. 다만 산출물이 넷으로 중첩돼 레코드 하나로 떨어지지 않아 형태를 먼저 정해야 한다.

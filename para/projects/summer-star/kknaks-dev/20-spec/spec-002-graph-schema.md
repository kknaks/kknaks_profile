...
실행층 내부 순서(`baseline → decision → spec → work`)는 이 rank가 관리하지 않는다. 층 rank는 **층간 방향만** 강제하고, 제품 문서 내부 정합은 `para/projects/project.md`와 `product_doc_pipeline.py`가 계속 담당한다.
...
- 기존 평문 `links: [id]`는 폐기 → 본문 `[[]]` 또는 `up:`으로 흡수. 단 `products/**` 문서의 frontmatter `links:`는 제품 문서 파이프라인이 소유하는 별개 계약이다(`para/projects/project.md`).

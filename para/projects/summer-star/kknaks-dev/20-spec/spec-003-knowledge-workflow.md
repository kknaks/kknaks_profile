...
- Domain note: 3층 = `source`(reference) → `concept` → `execution`(products). `idea`는 휘발.
...
`concept` 노트 작성은 단순 이동이 아니라 **자료를 내 언어로 다시 쓰며 기존 개념과 잇는 사고 행위**다.

1. 관련 concept들을 본문 `[[]]`로 인용하고, 기반이면 `up:`에도 넣는다.
2. **개념 내용을 재서술하지 않는다.** 엮은 판단만 소유한다. 다만 판단 문장 안에 개념 요지가 인용되는 것은 허용하며 필연적이다.
3. 기존 concept와 `[[]]`로 잇는다 — 여기서 관련·충돌·확장이 발견된다.
4. concept와 products는 별개 SSOT로 평행하게 살아 있고, `up:`은 인용일 뿐 중복이 아니다.
5. **이 층의 판단은 AI가 대신하지 않는다.** 개념까지는 AI 초안 + 승인으로 자동화되지만, "그래서 우리 제품에 이걸 어떻게 쓸 것인가"는 사람이 쓴다.

### S-5. 실행 — 제품으로 내린다

1. concept가 쌓이면 그중 하나를 근거로 제품 작업을 시작한다.
2. `products/{제품}/00-baseline`에 baseline을 만들고 `up:`으로 concept를 가리킨다.
3. 이후는 제품 문서 파이프라인(`para/projects/project.md`)이 소유한다 — baseline → decision → spec → work.

### S-6. 망각 — 아카이브

1. 안 쓰게 된 `concept`는 `archive/`로 옮긴다 (파일명 stem 유지 → 링크 보존).
...
stateDiagram-v2
    ...
    concept --> execution : 개념을 제품 결정의 근거로
...
- **SoT 위임**: 개념 상세는 concept 한 곳. `reference`와 제품 문서는 재서술하지 않고 `[[concept]]`로 위임한다.
...
- [ ] `reference`와 제품 문서 본문에 개념 상세가 복사되지 않고 `[[concept]]`로 위임된다.
- [ ] concept와 products가 같은 내용을 중복 보유하지 않는다.

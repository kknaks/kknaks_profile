...
- **검증**:
  ...
  - [ ] 기본 유형의 **이름 변경과 삭제가 409 `work_type_locked`** 로 거부되고, **색 변경은 된다**
  - [ ] **`kind` 는 기본이든 커스텀이든 PATCH 로 받지 않는다 — 422 `validation_error`**(SPEC-002 §4 가 아예 필드를 두지 않는다. 「잠금」과 다른 층의 거부다)
  - [ ] 같은 이름으로 또 만들면 **409 `duplicate_name`**
  - [ ] 팔레트 밖 토큰명은 **422 `invalid_color_token`**
  - [ ] 커스텀 유형을 삭제하면 목록에서 빠지고, **DB 에는 행이 남아 있다**(소프트 딜리트)
  - [ ] 생성 후 `kind` 를 PATCH 로 바꾸려 하면 **422 `validation_error`** 다(잠금 409 가 아니다)
  - [ ] `pytest` 통과
- **완료 증거**: 미작성

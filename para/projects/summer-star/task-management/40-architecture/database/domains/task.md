...
|---|---|---|
| `task` | 업무 본체 | 소프트 딜리트. `work_type_id` 필수, `project_id` 는 0..1 |
| `task_todo` | 할일 | 진행률 = `done` 개수 / 전체 |
| `task_memo` | 메모 | 사람 기록. 작성자 표기 없음(단일 사용자) |
| `task_log` | 시스템 로그 | **사용자 입력·수정 불가.** 서비스만 쓴다 |
| `task_attachment` | 참고자료 · 결과자료 | `role ∈ {reference, deliverable}` |
| `task_relation` | 연관업무 | 무방향 — `low_task_id < high_task_id` 로 한 행만 |

## Invariants

- **T-1** **`task` 에 시간 컬럼이 없다.** `start_date`·`end_date` 는 `schedule` 로 이관됐다(DEC-005 §3 · §A-4). 리스트의 종료일 정렬·D-day 는 조인으로 만든다.
...
- **T-9** `task_attachment.document_id` 는 **자료함 문서**를 가리킨다. v1 은 **md 만**이다(DEC-004 §8). 로컬 파일 업로드 경로가 v1 에 없다.

...
- **L-9-a** **`document.origin` 은 예외로 둔다**(2026-09-05 확정). v1 문서 생성 경로가 업로드뿐이라 값이 항상 `human` 이고 **「AI 생성」 배지는 v1 에 뜨지 않는다**(§A-15). v2 에서 회의·채팅이 문서를 만들면 스키마 변경 없이 그때 나타난다 — 값이 v1 에도 실제로 채워지므로 「v2 컬럼」이 아니다.

...
>
> **2026-09-07 저녁 추가 — MF-68 · 69 · 70(SYS-OQ-4 닫힘).** **MCP 서버는 별도 컨테이너**다(`app/mcp/` — compose 에 back · worker · **mcp** 셋) ·
> **단명 토큰은 `auth_session` 의 `kind='meeting'` 행**이다(`/start` INSERT → 헤더 → ② 종결 시 행 DELETE) · **웜스타트 실패 처리를 만들지 않는다**(MF-70).
...
4. **worker 가 우리 데이터를 읽는 길은 MCP 도구 7개뿐**이고, 그 도구는 **우리 REST API 의 얇은 래퍼**다(MF-2). DB 직결이 아니다. 권한 판정은 API 가 한다 — 도구는 회의별 단명 토큰(MF-4 · MF-69)으로 그 회의·그 계정 범위만 본다. **MCP 서버는 별도 컨테이너**(`app/mcp/`)이고 FastAPI 안에 있지 않다(MF-68).

## 런타임 배치 — 백엔드는 서버 1대에 산다

| 결정 | 내용 | 근거 |
|---|---|---|
| 배치 | 백엔드 일체(FastAPI · PostgreSQL · Redis · open-kknaks worker · **MCP 서버** · 파일 저장소)를 **사용자가 관리하는 서버 1대**에 docker compose 로 띄운다. 클라이언트는 그 API 를 붙는다. **compose 의 우리 서비스는 셋이다 — back · worker · mcp**(MF-68) | 아래 4가지 |
...
| **MCP 서버** (`app/mcp/` — **별도 컨테이너**, MF-68) | codex 가 회의 데이터를 읽는 유일한 길. 도구 7개 — `get_meeting` · `get_account` · `list_agendas`(사람 + AI 안건) · `get_agenda` · `list_tasks` · `get_task` · `list_work_types`(이름 · 종류 · 설명). **전부 조회.** 우리 REST API 를 **회의별 단명 토큰**(`auth_session(kind='meeting')` — MF-69)으로 부르는 얇은 래퍼다(MF-2). **compose 의 세 서비스 중 하나**(back · worker · mcp) | 쓰기. 권한 판정(API 가 한다). **DB 접근** — back 의 REST 만 부른다. FastAPI 안에 들어가지 않는다 |
...
**권한 경계는 백엔드가 진다.** MCP 는 API 의 얇은 래퍼이고 두 번째 게이트가 아니다. 사용자 세션 JWT 를 그대로 주지 않고 **회의(turn)마다 본인 계정으로 단명 토큰을 새로 발급**해 헤더로 주며, 회의가 끝나면 best-effort 로 폐기한다 — 실패는 자연 만료로 흡수(MF-4 · mediness landing-chat 방식). `--bearer-token-env-var` · OAuth · 설정 파일은 쓰지 않는다.

**토큰의 실체는 `auth_session` 행이다**(**MF-69** · SYS-OQ-4 닫힘 · `../database/domains/account.md` A-13). `kind='meeting'` 행을 하나 더한다 — **발급**은 `/start` 가 본인 계정으로 INSERT(**원문을 `meeting_token` 컬럼에** — A-13 · 2026-09-07 사용자 확정. refresh 처럼 해시만 두지 않는다), **검증**은 mcp 가 부르는 REST 가 그 행으로 계정 · 회의 범위를 보는 것, **폐기는 행 삭제**(② 종결 시 · best-effort). **무상태 JWT 짧은 TTL 은 쓰지 않는다** — 폐기 표면이 없어 MF-4 의 「best-effort 폐기」가 성립하지 않는다.
...
| **MCP 서버**(`app/mcp/` 별도 컨테이너) | codex 가 우리 데이터를 읽는 길 — 도구 7개 | In (worker → **mcp** → back REST) | 위 §codex 설정. **조회만.** compose 서비스 하나이고 FastAPI 안이 아니다(MF-68). 인증은 `auth_session(kind='meeting')` 토큰(MF-69) |
...
sequenceDiagram
  ...
  participant M as MCP 서버 (별도 컨테이너 · 도구 7개)

  Note over W,C: ── 회의 시작 (MF-1 · MF-69) ──
  W->>A: POST /api/meetings/{id}/start
  A->>D: status='recording' · recording_started_at=now() · auth_session(kind='meeting') INSERT  (한 트랜잭션)
  A-->>W: 200 MeetingDetail  ← 즉시. 기다리지 않는다
  A->>Q: (백그라운드) 웜스타트 제출 — 역할 · 규칙 · 용어 다섯 · 도구 목록. 컨텍스트 없음 (MF-50 · 55)
  Q->>C: codex — 새 세션 (allow list · auth_session 행의 단명 토큰 헤더)
  C-->>Q: 「준비됨」 (본문 무시)
  Q-->>A: session_id
  A->>D: meeting.ai_session_id UPDATE (새 세션)  ← 그전까지 배치는 돌지 않는다
  Note over A,C: 웜스타트가 실패하면 예외 전파 로그뿐 — 기록·재시도·재웜스타트 갈래를 만들지 않는다 (MF-70)
  ...
  loop 확정 발화 1000자 · 안건 전환 flush(80자 미만 생략) · 180초 상한 — ai_session_id 가 있을 때만
    ...
    M->>A: REST (auth_session kind='meeting' 행의 단명 토큰)
  ...
  A->>D: 단명 토큰 폐기 = auth_session(kind='meeting') 행 DELETE (best-effort — MF-69)
...
| 13 | **MCP 서버는 별도 컨테이너다.** compose 의 우리 서비스는 back · worker · **mcp** 셋이고, FastAPI 안에 MCP 를 두지 않는다. `worker 의 codex → mcp → back REST` 한 방향이다 | **MF-68** |
| 14 | **단명 토큰은 `auth_session(kind='meeting')` 행이다.** `/start` 가 INSERT 하고 ② 종결이 **행을 DELETE** 한다(best-effort). 무상태 JWT 를 쓰지 않는다 — 폐기 표면이 없다 | **MF-69** |
| 15 | **웜스타트 실패에 처리를 만들지 않는다.** 예외 전파 로그뿐이고, 회의 중이면 배치 미제출 · 종료 후면 ② 가 `final_failed` 로 간다 — 이미 있는 경로다 | **MF-70** |
...
| SYS-14 | **`/start` 는 즉시 응답.** 웜스타트는 큐 — 워커가 죽어 있어도 회의는 시작된다. **실패 처리는 만들지 않는다**(로그뿐) | MF-1 · **MF-70** |
| SYS-15 | **MCP 서버는 `app/mcp/` 별도 컨테이너.** compose 에 back · worker · mcp 셋 | **MF-68** (2026-09-07 저녁) |
| SYS-16 | **회의 단명 토큰은 `auth_session(kind='meeting')` 행.** 발급 = `/start` INSERT · 폐기 = ② 종결 시 행 DELETE | **MF-69** (2026-09-07 저녁) |
...
| ~~SYS-OQ-4~~ | **해소: MF-68 · MF-69(2026-09-07 저녁 · DEC-003 OQ-9 와 같은 항목).** ① MCP 서버는 **`app/mcp/` 별도 컨테이너** — FastAPI 안에 두지 않고 compose 에 back · worker · mcp 셋을 묶는다 ② 단명 토큰은 **`auth_session` 의 `kind='meeting'` 행** — `/start` INSERT · MCP 헤더 · ② 종결 시 행 DELETE(best-effort). 무상태 JWT 는 폐기 표면이 없어 쓰지 않는다. ~~원문: MCP 를 FastAPI 안에 두나 별도 프로세스로 두나 · 토큰을 어느 축으로~~ | — | 닫힘 |

...
| 2026-09-03 | **아키텍처 ① 완료**(architect) — 40-architecture/ system·database(+domains 5)·backend 8문서 1,294줄. schedule 시간 단독소유·회의 줄 3트랙(track human\|ai\|merged)·에러 규약(except Exception 금지). **DEC-001 개정: 인증 전송 쿠키 → Bearer**(+OS 키체인), 웹 우선 개발·Tauri 래핑 마지막 확정 | [system](40-architecture/system/README.md) · [backend](40-architecture/backend/README.md) |
| 2026-09-03 | **BASE-005+DEC-005(캘린더) · BASE-006+DEC-006(메시지함) 작성 — Phase 1 문서 6영역 완비.** 캘린더: **schedule 테이블 분리**(업무 startDate/endDate 이관), 뷰별 드래그 규격, 고스트는 목적지 크기, **시간 겹침 차단**(종일·기간 제외·경계 접촉 허용·드롭 단계에서 차단), 회의 상세 드로어는 회의록 소유·캘린더 재사용, 유형 4색 폐기. 메시지함: **v1 목록 UI만, 기능 전부 v2**(수신이 연동에 걸림) — Q-35 해소 | [BASE-005](00-baseline/baseline-005-calendar.md) · [DEC-005](10-decision/decision-005-calendar.md) · [DEC-006](10-decision/decision-006-messages.md) |

...
- **상태**
  ...
  - 칩 3 은 §4 `scope` 파라미터와 **1:1**(`project` / `recent30` / `all`)이고, 카운트의 `n` 은 응답 `total`·`m` 은 `items.length` 다. **기준 프로젝트가 없으면 「이 프로젝트」는 비활성이고 기본 선택이 「전체」로 내려간다**(무소속 업무 · 생성 드로어에서 프로젝트 미선택)

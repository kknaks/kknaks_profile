...
- **회의록은 이 드로어를 열지 않는다**(2026-09-07 저녁 · **MF-67** — 직전 판의 「슬롯 · 프리필 · 푸터 prop 을 받아 재사용」은 폐기). 회의록에는 **자기 payload 드로어**가 따로 있다(SPEC-008 U-10). 이 드로어는 **회의록 때문에 달라지는 것이 없다** — 안건 칸도, 「저장/넣기」 푸터도, 프리필 prop 도 없다. SPEC-008 U-10 이 이 드로어의 **부품 규격(입력 · 셀렉터 · 일정 · 할일 · 설명 블록)만 시각 참조로 가리킨다**
...
- **회의록은 이 드로어를 열지 않는다**(2026-09-07 저녁 · **MF-67** — 직전 판의 「헤더 업무 셀렉터 · 프리필 · 푸터 prop 을 받아 재사용」은 폐기). 회의록에는 **자기 payload 드로어**가 따로 있다(SPEC-008 U-9). 이 드로어는 **회의록 때문에 달라지는 것이 없다** — 헤더 제목은 그대로 제목이고, 인라인 자동 저장도 그대로다. SPEC-008 U-9 가 이 드로어의 **부품 규격만 시각 참조로 가리키고**, 헤더 셀렉터에는 U-8 의 `RelationPopover` 를 쓴다

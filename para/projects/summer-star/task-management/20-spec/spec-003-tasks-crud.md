...
- Domain note: 외부에 드러나는 것은 **업무**(제목·유형·프로젝트·**계획 시작/종료**·**설명**·완료 결과·상태)와 그 자식 **할일 · 메모 · 첨부(참고자료/결과자료) · 연관업무 · 로그**다
- Open questions: §7

### Business Requirement

- 업무가 이 제품의 중심 기록이다. **유형은 필수**(DEC-002 §3 「업무인지 미팅인지는 필수」)이고 SPEC-002 가 만든 동적 유형을 참조한다 — 고정 enum 3종은 폐기됐다(§A-1).
- **일정은 업무가 소유하고 필드가 4개다**(2026-09-06 개정 — **§A-4 번복** · DEC-002 §「일정 필드 4개 확정」 · ERD T-1).

  | 필드 | 뜻 | 화면 |
  |---|---|---|
  | `startDate` | **계획 시작** | 생성·상세의 일정 **첫째 칸** |
  | `dueDate` | **계획 종료 = 기한** | 일정 **둘째 칸**. D-day·지연 판정의 기준 |
  | `startedAt` | 실적 시작(`진행중` 전이) | **입력하지 않는다.** 로그·상세 메타에만 |
  | `completedAt` | 실적 종료(`완료` 전이) | **입력하지 않는다.** 「오늘 완료」 조회의 기준 |

  **디자인 원본(시작일·종료일 2칸 · 칸반 카드 「08.26 – 08.27」)이 도로 맞다.** 「기한」을 계획 종료와 따로 두지 않는다.
  캘린더의 `schedule` 은 **계획 기간의 파생**이고 이 spec 은 파생 사실만 참조한다(database/README §3).
...
- 업무 **생성 드로어**(840) — 제목·유형·프로젝트·**일정(계획 시작–종료)**·**설명**·할일·참고자료·연관업무를 한 번에 저장
- 업무 **상세 드로어**(840)와 **전체 페이지 승격**(⤢)
- **인라인 편집** — 제목·**설명**·완료 결과, 포커스 해제 시 저장
...
- **문구·필드 순서**
  1. **제목** — 플레이스홀더 「업무 제목」
  2. **유형(필수) · 프로젝트(선택) · 일정** 한 줄 — 유형/프로젝트는 팝오버 셀렉터(SPEC-002 가 만든 목록, 삭제된 것은 목록에 없다).
     **일정은 두 칸이다 — 「계획 시작」 – 「계획 종료」**(디자인 원본 588~594줄 그대로). 둘 다 선택 입력이다.
     「시간 지정」 토글을 켜면 **계획 종료일 안의 시작–종료 시각**을 받는다(`dueStartTime`·`dueEndTime`)
  3. **설명** 한 칸 — 플레이스홀더 「이 업무에 대한 설명」. **`background`·`goal` 2열은 폐기**(2026-09-06 · DEC-002)
...
- **회의록에서 열 때 — 슬롯 하나 · 프리필 · 푸터**(2026-09-07 · MF-13 · 65 · SPEC-008 U-10 · FE §2 규칙 8): 같은 드로어가 `agendaSlot`(본문 **맨 위** 안건 셀렉터 — 그 줄의 안건으로 고정) · `prefill`(= 줄의 `payload` — 제목 · 유형 · 프로젝트 · 계획 시작–종료 · 설명 · 할일. 채워진 필드는 테두리 `#7181F8` + 캡션 「회의에서 반영」) · `submitMode`(`"save"` | `"insert"` — MF-66) 를 prop 으로 받는다. 푸터는 모드별 하나씩 — `"save"`(회의록 편집 모드) 는 「취소 · **저장**」: 값을 회의록 줄의 `payload` 로 보낸다(업무 없음 · 유형이 비어도 된다). `"insert"`(회의록 보기 모드) 는 「취소 · **넣기**」: SPEC-008 §4 의 줄 표면으로 업무 생성 + 줄 갱신 한 트랜잭션(조건은 「업무 만들기」와 같다). 한 푸터에 둘을 같이 두지 않는다. **화면·필드는 그대로다.** 회의록 전용 업무 드로어는 없다

### U-2. 인라인 편집 (P-24)

제목·**설명**·완료 결과에 적용한다. **별도 편집 모드가 없다**(04-task-detail §인라인 편집).

- **상태**
  - *보기*: 그냥 텍스트
  - *편집 중*: 배경 `#F5F6F8` + 내부 테두리 `#E1E3E8` + 캐럿 `#7181F8`. 드로어 우상단에 안내 바 「제목 · 설명은 클릭해 수정하고, 벗어나면 자동 저장됩니다」
  - *저장 중*: 그 필드 우측에 작은 진행 표시
  - *저장 실패*: **SPEC-002 U-7 규격 그대로** — 토스트 「저장하지 못했습니다 · <필드 이름>」 + 필드 테두리 `#E2685B` + 「저장되지 않았습니다 · 다시 저장」. 자동 재시도 없음
- **문구**: 빈 값일 때 플레이스홀더 — 설명 「이 업무에 대한 설명」 · 완료 결과 「무엇을 끝냈는지 적습니다」
- **CTA**: 없다. **저장·취소 버튼을 두지 않는다**([10] 자동 저장). `Esc` 는 편집 전 값으로 되돌리고 저장하지 않는다
- **기대 결과**: 포커스를 벗어나면 저장된다. **제목·설명·완료 결과 수정은 로그에 남지 않는다**(04-task-detail §인라인 편집)
...
- **문구·구성**
  ...
  - 본문 블록 6: ① 설명 ② 할일(U-5) ③ 참고자료·연관업무 2열(U-7·U-8) ④ 결과자료·완료 결과(U-6) ⑤ 메모(U-9) ⑥ 로그(U-10)
...
- **회의록에서 열 때 — 헤더 업무 셀렉터 · 프리필 · 푸터**(2026-09-07 · MF-14 · 65 · SPEC-008 U-9): 회의록이 열면 **항상** 이 모드다 — AI 줄/사람 줄을 가르지 않는다. 같은 드로어가 `taskSelectorSlot`(헤더 **제목 자리가 셀렉터** — 누르면 U-8 의 `RelationPopover` 단일 선택 · 업무를 바꾸면 본문이 그 업무의 현재 값으로 바뀐다 · 아직 안 골랐으면 본문 비활성) · `prefill`(= 줄의 `payload` — 기한 · 상태 · 진행 메모 · 할일 · 연관 · 프로젝트 · 완료 결과. 반영된 필드는 테두리 `#7181F8` + 「회의에서 반영」) · `submitMode`(`"save"` | `"insert"` — MF-66) 를 받는다. **인라인 자동 저장이 없고** 푸터는 모드별 하나씩 — `"save"`(회의록 편집 모드) 는 「취소 · **저장**」: 고른 업무와 변경분을 회의록 줄에 붙인다(업무 안 바뀜). `"insert"`(회의록 보기 모드) 는 「취소 · **넣기**」: SPEC-008 §4 의 줄 표면으로 **한 요청**. 한 푸터에 둘을 같이 두지 않는다. 이 모드의 상태 드롭다운에는 **「완료」 · 「취소」가 없다**(MF-59)

### U-4. 전체 페이지 승격 (P-23)

- **상태**: 드로어가 닫히고 업무 상세 페이지로 이동한다. 뒤 화면·스크림이 없다. 좌(유동) 본문 + 우(528) 메모·로그 2단
- **문구**: 「←」 + breadcrumb 「홈 › 내 업무 › <제목>」 — 「홈」·「내 업무」는 **링크**, 제목만 링크 아님(2026-09-07 · MF-7 · FE §6-3 전 화면 공통. 실측에서 trail 이 전부 `<span>` 이라 목록으로 돌아갈 길이 없었다 — F-5)
- **CTA**: 「←」(`/tasks/` 로 한 단계 뒤로) · breadcrumb 「내 업무」로 목록 복귀. **드로어로 되돌리는 버튼은 없다**(승격은 한 방향 — F-5). 헤더 순서(배지 줄 → 제목)는 **이 화면이 정본**이고 회의록이 따라간다(MF-8)
...
- **상태**
  ...
  - 칩 3 은 §4 `scope` 파라미터와 **1:1**(`project` / `recent30` / `all`)이고, 카운트의 `n` 은 응답 `total`·`m` 은 `items.length` 다. **기준 프로젝트가 없거나 그 프로젝트에 후보가 0건이면 「이 프로젝트」는 비활성이고 기본 선택이 「전체」로 내려간다**(무소속 업무 · 생성 드로어에서 프로젝트 미선택 · **2026-09-07 MF-14 — 회의록 실측에서 프로젝트 0건이라 연결할 업무가 하나도 안 나왔다**). **회의록도 이 팝오버를 그대로 쓴다**(SPEC-008 U-9 — 단일 선택 모드 prop 하나만 더한다)
...
- **해제** — 2026-09-06 추가(구현 중 드러난 공백). 카드의 각 행에 **「해제」**(호버로 드러나는 고스트 버튼, 참고자료 행의 「제거」와 **같은 규격**). `DELETE /api/tasks/{id}/relations/{otherTaskId}` 를 부르고 **연결과 마찬가지로 양방향**이다. **확인 모달을 두지 않는다** — 되돌리는 비용이 다시 거는 것뿐이다
  - 팝오버는 **이미 연결된 업무를 후보에서 제외**하므로(§4 `excludeId`) 체크를 풀어 해제하는 경로는 **없다.** 해제는 **카드 행에서만** 한다 — 연결과 해제를 두 표면에 두면 어느 쪽이 정본인지 갈린다
  - **연결은 걸 수 있는데 뗄 수 없으면 잘못 건 연관이 영구히 남는다.** 서버 표면(§4)은 처음부터 있었고 화면 문구만 비어 있었다
...
| `scope` | ✖ | **필터**. `project`(기본) · `recent30` · `all` — U-8 의 필터 칩 3 이 이것과 1:1 이다 |

정렬은 그대로 — **같은 프로젝트 → 기한 ±7일 → 최근 수정**, 최대 20건. 삭제된 업무는 제외.

**`scope` 는 정렬 힌트가 아니라 후보를 잘라내는 필터다** — 위 `projectId`·`dueDate` 와 역할이 다르다.
칩이 필터인데 서버에 필터가 없으면 화면이 「전체 정렬본」을 세 번 똑같이 보여주게 된다.
...
**기준 프로젝트가 없으면**(무소속 업무이거나 생성 드로어에서 프로젝트를 아직 안 골랐을 때)
「이 프로젝트」 칩은 **비활성**이고 기본 선택이 **「전체」로 내려간다.** 서버는 `scope=project` 인데
기준이 없으면 **`all` 과 같게 답한다** — 빈 목록을 주지 않는다(고를 게 없는 팝오버가 뜨는 게 더 나쁘다).

응답에 **`total`** 을 함께 낸다 — U-8 우측 카운트 「n건 중 m」의 `n` 이다.
`total` 은 **`scope` 를 적용한 뒤의 총계**이고 `items` 는 거기서 상위 20건이다(`m = items.length`).
...
```json
{
  ...
  "startDate": "2026-08-27", "dueDate": "2026-08-29", "dueStartTime": null, "dueEndTime": null,
  "startedAt": "2026-08-27T01:12:00Z", "completedAt": null,
  "dDay": 0, "isOverdue": false,
  "description": "...", "completionResult": null,
  "cancelReason": null,
...
- **`isOverdue`·`overdueDays` 를 상세에도 담는다**(2026-09-06 추가). SPEC-004 U-10 이 「상세: 헤더 기한 옆에 같은 문구」를 요구하는데 상세 계약에 `overdueDays` 가 없어 상세가 「n일 지남」을 못 그렸다. **목록과 같은 파생 규칙**이고(T-4 — 저장하지 않는다) **화면이 다시 계산하지 않는다.** 구현은 **WORK-005 후속 수정** 몫이다

**`PATCH /api/tasks/{id}`** — 보낸 필드만 바꾼다.

```json
{ "title": "..." }            // 또는 description / completionResult / workTypeId / projectId
...
**`POST /api/tasks/{id}/relations`** — `{ "taskIds": [45, 46] }`

**자식 컬렉션의 쓰기 표면은 전부 갱신된 `TaskDetail` 을 돌려준다** — 2026-09-06 확정(구현 중 드러난 공백)

할일·메모·첨부·연관 넷 다 같다. **부분 응답을 주지 않는다**: 자식 하나를 건드리면 로그 한 줄이 늘고
`todoProgress`·파생값이 함께 바뀌므로, 부분 응답이면 **화면이 나머지를 다시 조립**해야 하고 그 조립 규칙이
화면마다 갈린다. **삭제만 `204`** 다.

> 이 문장이 없어서 구현이 넷 중 셋만 `TaskDetail` 을 주고 **할일만 `TodoItem`** 을 줬다.
> 프론트가 그걸 `TaskDetail` 로 받아 **상세 캐시(`['tasks','detail', id]`)에 써 넣어** 캐시가 오염됐다
> (WORK-004 검수 F-4). **응답 형태는 계약이지 구현 재량이 아니다.**

**없는 자식을 지우면 `404`** 다 — 할일·첨부·연관 셋 다 같다. 멱등 삭제로 두지 않는다:
지우려는 것이 이미 없다는 건 **화면이 낡았다는 뜻**이고, 조용히 204 를 주면 그 사실이 묻힌다.
(로그아웃은 「이미 무효여도 성공」이지만 그건 **세션을 끝낸다는 목적이 이미 달성된** 경우다 — 결이 다르다.)
...
| `startDate` | 선택(`YYYY-MM-DD`). **계획 시작.** 달력 날짜다 |
| `dueDate` | 선택(`YYYY-MM-DD`). **계획 종료 = 기한.** **달력 날짜**이고 순간이 아니다(G-2-e) |
| `startDate` ↔ `dueDate` | 둘 다 있으면 **`startDate ≤ dueDate`**. 한쪽만 있어도 된다 |
| `dueStartTime` · `dueEndTime` | **둘 다 있거나 둘 다 없다.** 있으면 `dueDate` 도 있어야 하고 `end > start`(T-1-b) |
| `startedAt` · `completedAt` | **읽기 전용.** 요청으로 받지 않는다 — 상태 전이 시점에 서버가 쓴다(ERD T-1-c) |
| `description` · `completionResult` | 선택. 각 4000자 이하. **`background`·`goal` 은 제거됐다**(2026-09-06 · DEC-002) |
| 할일 `text` | 필수. 1~200자 |
| 메모 `text` | 필수. 1~2000자 |
...
| `invalid_project` | `422 {detail:"사용할 수 없는 프로젝트입니다", code:"invalid_project"}` | 프로젝트 셀렉터 옆 인라인 「삭제된 프로젝트입니다. 다시 골라 주세요」 + 목록 갱신 | 프로젝트 셀렉터 |
...
- **`invalid_project` 는 2026-09-06 신설**이다. 그전엔 프로젝트 실패가 `validation_error` 로 합류해 **화면이 유형과 프로젝트를 구분할 수 없었다** — 상세에 두 셀렉터가 나란히 생기면서 어디가 틀렸는지 못 짚는 문제가 실제가 됐다
- **첨부 `kind="doc"` 은 문서함 work 전까지 `422`** 다(서버가 거부하고 팝오버는 「자료함이 아직 없습니다」 스텁). **`unsupported_file_type` 이 아니라 `validation_error`** 로 낸다 — 파일 종류가 문제가 아니라 **가리킬 문서함이 아직 없기** 때문이다. 문서함 work 가 이 각주와 §6 Acceptance 의 「첨부한 문서를 누르면 문서함 상세로」를 **자기 검증 절에 옮겨 담는다**
...
| 리소스 | 외부에 드러나는 필드 |
|---|---|
| 업무 | `id`(number) · `title` · `status` · `workType`(요약) · `project`(요약 · null 가능) · **`startDate`** · `dueDate` · `dueStartTime` · `dueEndTime` · **`startedAt`** · **`completedAt`** · `dDay`(파생) · `isOverdue`(파생) · **`description`** · `completionResult` · `cancelReason` · `createdAt` · `updatedAt` |
...
| ~~**§A-4 정정의 화면 반영**(`startDate`/`endDate` → 기한)~~ | **2026-09-06 번복.** 일정은 **「계획 시작」–「계획 종료」 두 칸**으로 돌아간다(디자인 원본 그대로). 「시간 지정」 토글은 **계획 종료일 안의 시각**을 받는 용도로 남는다. 근거 DEC-002 §「일정 필드 4개 확정」 · ERD T-1 |

...
회의를 **끝내면** 스피너가 돌고, 서버가 **녹음 원본을 다시 전사**해 화자 분리를 바로잡은 뒤 **AI 가 최종 회의록을 한 번에 쓴다** — 사람이 적은 줄도 다듬고, 액션·업무 줄에는 DB 에 바로 넣을 페이로드가 실린다. 그 회의록을 **편집**하고(줄 고치기 · 줄 추가 · 줄 지우기 · 안건 이름 고치기), 액션 줄로 **업무를 만들고**, 업무 줄로 **업무를 고친다** — **회의록 payload 드로어**를 열어 확인하고 「넣기」를 눌러야 요청이 나간다(MF-67 — 업무 탭 드로어를 재사용하지 않는다).
...
> **MF-13 · 14 · 61 · 21**(업무 생성·갱신 드로어 · 사람 줄은 빈 드로어 · `work_type.description`) · **MF-25 · 7 · 8 · 9**(카운트 다섯 · breadcrumb 링크 + 「←」 · 헤더 순서 · 줄 시각 없음).
>
> **2026-09-07 저녁 정정 — MF-64(정정) · 66 · 67 · 68 · 69 · 70.**
> **MF-67** — 「업무 탭 드로어에 슬롯을 얹어 재사용」은 **폐기**. **회의록 payload 드로어를 따로 만든다**(액션 · 업무 각각 하나) — 필드가 `payload` 키 그대로이고 업무 드로어는 손대지 않는다.
> **MF-64(정정)** — 「칩 넷이 같다」 폐기. **논의·결정은 줄 추가 드로어(U-8), 액션·업무는 payload 드로어가 바로 뜬다** — 「저장」에 줄과 `payload` 가 한 요청으로 생긴다.
> **MF-66** — 푸터는 모드별(편집 「취소 · 저장」 / 보기 「취소 · 넣기」). **MF-68 · 69 · 70** — MCP 별도 컨테이너 · 단명 토큰 = `auth_session` 행 · 웜스타트 실패 처리 없음(DEC-003 OQ-8 · OQ-9 닫힘).

## 1. Context

### Meta

- Decision reference: DEC-003 §1(줄 4종 · 일시정지 · 종료 후 편집 범위 다섯 · AI 한 줄 요약 바 · 확인 모달 크기) · §2(AI 데이터 접근) · §3(status 4종 · 트랙 · `payload` · 용어 보정 표) · §4(종료 파이프라인 — 재전사 → 최종 회의록 · 최종 회의록 작성 · 업무 갱신 허용 필드 일곱 · 재시도) · §5(편집 다섯 · 업무 연동은 **회의록 payload 드로어** — MF-67) · §6(재전사 · 용어 보정 · AI 탭은 로그) · §7(열거된 실패만 — 재전사 실패 · 최종 회의록 실패) · §8(두 표면 · 일시 소유 · AI 도구 연결) · §STT(재전사 모델) · OQ-7(폐기) · **OQ-8 · OQ-9(2026-09-07 저녁 닫힘 — MF-68 · 69 · 70)** · OQ-10 · OQ-12
...
- Architecture reference: `system/README.md` §③ · 불변식 2·3·7·11·12·13·14·15 · `backend/README.md` §5-2 · §5-3 · §6 · §7 · §8-2 · §8-3 · §10 · `database/README.md` §1 · §4 · `database/domains/meeting.md` M-3 ~ M-9-b · M-11 · M-14 ~ M-15 · **M-19 · M-20** · `database/domains/task.md` T-5 · T-6 · `frontend/README.md` §2 규칙 8 · §6(모달 두 크기) · §6-3(상세 헤더) · §8
...
- **업무를 만들고 고치는 입구는 회의록 payload 드로어 둘이다**(**MF-67** — 업무 탭 드로어를 재사용하지 않고 손대지도 않는다). 「업무 생성」은 **액션 payload 드로어**(U-10), 「업무 갱신」은 **업무 payload 드로어**(U-9)를 연다 — 필드가 `payload` 키 그대로이고, 시각 규격만 이미 만들어진 새 업무 드로어 · 업무 상세 드로어를 가리킨다. 사람이 적은 액션·업무 줄은 페이로드가 없어 **빈 드로어**로 연다(MF-61) — 그 줄 하나만 AI 에게 다시 묻지 않는다. **「넣기」를 눌러야 요청이 나간다**(DEC-003 §5 · 편집 모드의 「저장」은 `payload` 만 붙인다 — MF-66). 상태 변경은 **내 업무의 전이 판정**을 그대로 지나고 **`done` 은 회의록이 보내지 않는다**(MF-59 — 완료 결과만 채워 두고 게이트는 업무 화면에서 사람이 연다).
...
- **편집 모드** — 줄 인라인 수정 · 추가(**칩은 둘로 갈린다** — 논의·결정은 줄 추가 드로어, 액션·업무는 payload 드로어가 바로 뜨고 「저장」이 줄과 `payload` 를 한 요청으로 만든다. MF-64 정정) · **줄 삭제(확인 모달 420 · 자리 유지)** · **안건 이름 인라인 수정** · 포커스 해제 자동 저장(줄·안건 이름에만 — **payload 드로어는 모드별 푸터 「저장」/「넣기」**, MF-66)
- **업무 생성 · 업무 갱신** — 줄 버튼 규격, **회의록 payload 드로어 둘**(MF-67), 페이로드 프리필, 모드별 푸터(MF-66), 서버 계약(한 요청 · 한 트랜잭션), 완료 게이트 우회 없음(`done` 미전송), **줄을 지워도 업무는 남는 경계**
- 이 영역 반응형
...
- **문구 · 구성**
  ...
  - **좌 패널 1160**(L1456~1547): 탭 「**회의록**」(dot `#7181F8` — 완료, L1458) | 「**AI 요약**」(dot `#B3B3B3`, L1459) + 우측 「**편집**」(30px r8 테두리 `#D9D9D9` 12/600, L1461)
    ...
    - **AI 요약 탭 = `agendas.ai`** — **SPEC-007 U-4 규격 그대로**(트리 · 「AI 안건」 캡션 · 펼침 · 줄 시각 없음 — 미러 안건 배지는 MF-71 로 없어졌다). 안내 바 우측은 **회의 중 마지막 값 그대로** 「배치 n회 반영 · HH:MM」(배치가 한 번도 없었으면 「AI 요약 없음」). 탭 푸터(SPEC-007 U-4)는 종료 후 **없다**. **버튼이 없다** — 로그성 기록이다(DEC-003 §6). 종료 후 배치가 다시 돌지 않는다(MF-56)
...
### U-6. 줄 버튼 — 액션 줄 「업무 생성」 · 업무 줄 「업무 갱신」 (디자인 시스템 [09] L668~677 · 시안 L1484~1486 · L1508~1510 · DEC-003 §5 · MF-13 · 14 · 59 · 61 · 65 · 67)

**줄을 적는 것만으로는 업무가 생기지 않는다.** 이 버튼을 눌러야 드로어가 열리고, **보기 모드**에서 연 드로어의 **「넣기」**를 눌러야 업무가 생기거나 바뀐다. 보기 모드 · 편집 모드 모두 같은 자리에 같은 버튼이지만 **여는 드로어의 푸터가 모드별로 다르다**(MF-66) — 편집 모드 = 「취소 · 저장」(`payload` 만 줄에 붙인다 · 업무 없음), 보기 모드 = 「취소 · 넣기」(업무를 만들거나 고친다). 한 푸터에 버튼 셋을 두지 않는다. **회의록 탭에만** 있다(AI 탭 · 회의 중 화면에는 없다 — SPEC-007 U-4). **여는 것은 회의록 payload 드로어이고, 액션·업무 각각 하나뿐이다**(MF-65 · **MF-67** — 업무 탭 드로어를 재사용하지 않는다). AI 줄과 사람 줄로 드로어를 나누지 않는다 — 차이는 **`payload` 가 차 있나 비어 있나** 하나뿐이다.

- **상태**
  - *액션 줄*(`kind='action'`): 우측에 「**업무 생성**」(30px r8 테두리 `#D9D9D9` 12/600 `#1E1E1E`, L1486). 누르면 **U-10**(액션 payload 드로어)이 열린다 — **`payload` 가 있으면 그 값이 채워진 채로**(AI 줄 · 또는 사람이 앞서 「저장」한 줄), **없으면 제목 = 줄 본문 · 프로젝트 = 회의의 프로젝트만 채운 빈 드로어**(MF-61). 그 줄 하나만 AI 에게 다시 묻지 않는다. `payload` 가 있는 줄은 버튼 옆에 6px dot `#7181F8`(「채워진 값이 있다」 표시 — 시안 없음, 디자인 시스템 [09] 탭 dot 재사용)
  - *업무 줄 · 넣기 전*(`kind='task'` · `payload` 유무 무관 · `taskId` 유무 무관): 유형 배지(`taskId` 있을 때만) + 「**업무 갱신**」(30px r8 배경 `#F1F2F5` · 테두리 `#E1E3E8` 12/600 `#757575`, [09] L672). 누르면 **U-9**(업무 payload 드로어)가 열린다 — 헤더 셀렉터에 `taskId` 의 업무(없으면 빈 셀렉터), 본문에 `payload` 의 변경분(없으면 빈 값). 툴팁에 `payload` 의 내용 — 「기한 → 09.02 · 상태 → 진행중 · 메모 1건 · 할일 2건 · 완료 결과」 중 있는 것(없으면 툴팁 없음). **「업무 연결」 같은 별도 버튼·앞 단계는 없다**(MF-65 — 업무는 드로어 헤더에서 고른다)
...
- **상태**
  ...
  - *안건별 추가 행*(L1913~1916): 각 안건의 줄 목록 아래 칩 4 — 「+ 논의」 「+ 결정」 「+ 연관 업무」 「+ 액션 아이템」(28px r6 테두리 `#E1E3E8` 12/`#5F6470`). **칩은 둘로 갈린다**(MF-64 — 「칩 넷이 같다」는 폐기)
    - **「+ 논의」 · 「+ 결정」** → **U-8 줄 추가 드로어**가 뜬다. 「**추가**」를 누르면 줄이 생긴다(`POST …/lines` — `payload` 없이). 닫으면 줄이 없다
    - **「+ 연관 업무」 · 「+ 액션 아이템」** → **payload 드로어가 바로 뜬다**(업무 줄이면 U-9, 액션 줄이면 U-10). 편집 모드라 푸터는 「취소 · **저장**」이고, 「저장」이 **줄과 `payload` 를 한 요청으로** 만든다(`POST …/lines {agendaId, kind, content, payload(, taskId)}` — §4). 줄 본문은 드로어의 제목(액션) 또는 내용 칸(업무)이 된다. **닫으면 줄이 없다.** 업무는 여전히 안 생긴다 — 「넣기」는 보기 모드의 것이다(MF-66)
    - 왜 「줄 먼저」를 접었나 — 줄 본문이 필수라 빈 줄을 먼저 만들 수 없고, 본문만 받는 드로어를 하나 더 거치면 액션·업무는 드로어가 연달아 둘 뜬다(MF-64 · 사용자 판정). 「액션만 적어두기」는 그대로 된다 — 「저장」이 줄을 만들고 업무는 안 만든다
    - 줄 추가는 **종류를 고르고 새로 적는 것**이라 MF-60 에 안 걸린다. **하단 프롬프트 바는 없다**([09] L752)
- **문구**
...
- **기대 결과**
  ...
  - 「+ 연관 업무」·「+ 액션 아이템」으로 만든 줄은 **처음부터 `payload` 를 갖고 태어난다** — 그 시점에 AI 가 만든 줄과 완전히 같은 상태다(dot 이 켜져 있고 툴팁에 값이 있다)
  - **AI 요약 탭은 편집 모드가 없다**(DEC-003 §6 · ERD M-6). 편집 중에도 AI 탭은 보기 그대로다
  - 편집 이력이 없다 — 되돌릴 것은 편집 전 값이 아니라 **다음 편집**이다

### U-8. 줄 추가 드로어 — 논의 · 결정 **전용** (시안 L2042~2305 · 디자인 시스템 [09] L740~741 · MF-64 정정)

드로어 840 · 스크림 `rgba(30,30,30,0.32)`(FE §6). 헤더 72 · 푸터 76. 회의록이 가진 **논의·결정 줄을 적는 드로어**다 — 업무가 아니라 회의록 줄을 적는 것이라 업무 탭에 상대가 없다. **「+ 논의」·「+ 결정」 둘만 이 드로어를 연다**(MF-64 정정 — 「칩 넷이 전부 여기로」는 폐기). 「+ 연관 업무」·「+ 액션 아이템」은 **payload 드로어**(U-9 · U-10)가 바로 뜨고 거기서 줄과 `payload` 가 함께 생긴다.

- **상태**
  - *열림*: 헤더 「**논의 추가**」 / 「**결정 추가**」(칩에 따라) 18/700 + 부제 「<제목> · <일시 날짜>」 12/`#9EA2AE`(L2258~2259) + ×. 열자마자 **내용 입력에 포커스**
  - *제출 가능*: 내용 1자 이상. 아니면 「추가」 비활성
  - *제출 중*: 푸터 비활성 + 진행 표시. *실패*: 드로어 유지 + 인라인 안내(§4 Case Matrix)
- **문구 · 필드 순서**
  1. **종류** 세그먼트(36px r8 — 선택 `#F1F2FE` + 테두리 `#7181F8` + `#4B52A8`, L2266~2267) — 「논의」 | 「결정」 둘 사이만 오간다. **「업무」·「액션」 값이 이 세그먼트에 없다**(그 둘은 여기로 오지 않는다 — MF-64 정정. 「종류 하나로 고정되는 갈래」는 삭제). **만든 뒤에는 바꿀 수 없다**(MF-60)
  2. **안건** 셀렉터(44px, L2271~2272) — 기본값은 칩을 누른 안건. 같은 트랙의 다른 안건으로 바꿀 수 있다
  3. **내용**(44px, 포커스 테두리 `#7181F8`, L2276~2277) — 필수. 줄 본문 한 줄
...
- **기대 결과**: 그 안건의 **맨 아래**(마지막 번호 + 1)에 줄이 붙고 드로어가 닫힌다(`POST …/lines` — §4. `payload` · `taskId` 없이 — 논의·결정 줄에 `payload` 를 실으면 `422`). 새 줄은 근거가 없다(화살표는 상세 설명이 있을 때만). **닫으면 줄이 없다**

### U-9. 업무 payload 드로어 — 업무 줄이 여는 회의록 드로어 (MF-14 · MF-59 · MF-64 · MF-65 · MF-66 · **MF-67** · 디자인 시스템 [09] L744~745)

**회의록 것이다**(MF-67). 업무 탭의 업무 상세 드로어를 열지 않고, 그 드로어에 슬롯·prop 을 얹지도 않는다 — 성격이 다르다. **업무 드로어**는 업무를 만들고 고치는 자리(인라인 자동 저장 · 상태 전이 · 첨부 · 연관 · 로그)이고, **payload 드로어**는 **줄에 붙일 값을 적는 자리**다(자동 저장 없음 · 푸터 버튼 하나 · 필드는 `payload` 키만). ⛔ **업무 탭 드로어를 수정하지 않는다.** 회의록 전용 「연관 업무 연결」 드로어 시안(L2306~2599)도 참고하지 않는다 — `payload` 라는 개념이 없던 시안이다(§7 제외).

- **시각 규격의 참조는 이미 만들어진 업무 상세 드로어(SPEC-003 U-3 · `TaskDetailDrawer`)다.** 드로어 840 · 스크림 · 헤더 72 · 푸터 76, 헤더 셀렉터 · 배지 줄 · 입력 · 체크리스트 · 메모 블록의 **규격을 그대로 가리킨다.** 새 시안은 없다 — 같은 규격의 부품으로 조립한다
- **드로어는 하나다**(MF-65). AI 가 만든 줄이든 사람이 적은 줄이든 **같은 드로어 · 같은 필드 · 같은 저장 경로**이고, 다른 것은 **`payload` 가 차 있나 비어 있나**뿐이다. ⛔ 「AI 줄은 넣기 모드 · 사람 줄은 자동 저장」으로 가르지 않는다
- **진입점 둘** — ① 업무 줄의 「업무 갱신」(U-6) ② 편집 모드의 **「+ 연관 업무」 칩**(U-7 — 줄이 아직 없다. 「저장」이 줄과 `payload` 를 함께 만든다 — MF-64 정정)
- **헤더 — 업무 셀렉터**(MF-14 · MF-65 「헤더에서 업무를 바꾼다」): 헤더의 **제목 자리가 셀렉터**다 — `[ <업무 제목> ˅ ]`. 누르면 **업무 화면의 `RelationPopover`(SPEC-003 U-8 팝오버 382) 그대로** 뜬다 — 검색 입력 + 필터 칩 3 + 결과 행, **단일 선택**. `RelationPopover` 는 업무 화면의 부품이고 드로어가 아니라 **그대로 재사용한다**(단일 선택 prop 하나만 더한다 — SPEC-003 U-8). 데이터는 `GET /api/tasks/relations/candidates`(`projectId` = 회의의 프로젝트). 기본 칩은 「이 프로젝트」, **기준 프로젝트에 업무가 0건이거나 회의가 무소속이면 「전체」로 내려간다**(MF-14). `taskId` 가 있으면 그 업무가 골라져 있고(**AI 가 엉뚱한 업무를 골랐으면 여기서 바꾼다**), 없으면(사람이 적은 줄 · 칩으로 연 드로어) 빈 셀렉터 「업무 고르기」 — **앞 단계가 따로 없다.** 그 아래 유형 배지 · 프로젝트 칩은 고른 업무의 것(읽기 전용)
- **본문 — 필드는 변경분 일곱뿐이다**(MF-14 · MF-59 · `payload` 키 그대로)

  | 필드 | 출발값 | 비고 |
  |---|---|---|
  | **내용**(줄 본문) | `payload` 없이 칩으로 열었으면 빈 값 · 줄에서 열었으면 그 줄의 본문(읽기 전용) | 칩 진입에서만 입력이다 — 「저장」 때 줄 본문이 된다(`content` 1~2000 필수) |
  | **기한** | 고른 업무의 현재 `dueDate` · `payload.dueDate` 가 있으면 그 값 | |
  | **상태** | 〃 · **셀렉터에 「완료」 · 「취소」가 없다**(MF-59 · `todo`\|`in_progress` 만) | 완료 게이트에 뒷문을 만들지 않는다 |
  | **진행 메모** | 항상 빈 입력 — **추가분**이다(업무 메모에 새 항목) | 현재 메모 목록을 그리지 않는다 |
  | **할일 추가** | 〃 — 추가분 목록 | |
  | **연관 업무** | 〃 — 추가분(`RelationPopover` 다중 선택 — SPEC-003 U-8) | |
  | **프로젝트** | 고른 업무의 현재 프로젝트 · `payload.projectId` 가 있으면 그 값 | |
  | **완료 결과** | 고른 업무의 현재 `completionResult` · `payload` 값이 있으면 그 값 | 채워 두면 업무 화면의 완료 게이트가 열려 있다 |

  - **제목 · 유형 · 설명 · 시작일은 없다** — 회의록에서 못 바꾼다(MF-14). 참고자료 · 결과자료 · 로그 · 첨부 블록도 **그리지 않는다**(`payload` 키가 아니다)
  - `payload` 로 채워진 필드는 **테두리 `#7181F8` + 캡션 「회의에서 반영」**(디자인 시스템 [07] 포커스 규격 재사용 — 시안 없음). 사람이 고칠 수 있다
  - **업무를 아직 안 골랐으면 본문이 비활성**(캡션 「업무를 고르면 현재 값이 보입니다」) — 변경분은 고른 뒤 적는다
- **푸터 — 모드별 하나씩**(MF-66 · prop `submitMode:"save"|"insert"`. ⛔ 한 푸터에 셋을 두지 않는다). **인라인 자동 저장이 없다** — 포커스를 벗어나도 요청이 나가지 않는다
  - **편집 모드(U-7)에서 열면 「취소」 · 「저장」**(44px `#7181F8`). **「저장」** = **`payload` 를 줄에 붙인다.** 업무는 바뀌지 않는다(회의록을 고치는 자리다). 헤더에서 고른 업무는 `taskId` 로 함께 저장된다. 줄에서 열었으면 `PATCH …/lines/{lineId} { taskId, payload }`, **칩으로 열었으면 `POST …/lines { agendaId, kind:"task", content, taskId, payload }` — 줄과 `payload` 가 한 요청**(§4 · MF-64 정정). 드로어가 닫히고 줄 버튼은 「업무 갱신」 그대로
  - **보기 모드(U-3)에서 열면 「취소」 · 「넣기」**(44px `#7181F8`). **「넣기」** = 헤더의 업무에 변경분을 **한 요청으로 반영**한다 — 확인하고 바로 넣는다. `PATCH …/lines/{lineId}/task { taskId, …변경분 }`(§4). 조건 — 업무가 골라져 있어야 한다(없으면 비활성)
  - 「취소」 · × · `Esc` · 스크림 = 아무것도 저장하지 않는다. 줄에서 열었으면 줄은 그대로, **칩으로 열었으면 줄이 안 생긴다**(MF-64 정정)
...
### U-10. 액션 payload 드로어 — 액션 줄이 여는 회의록 드로어 (MF-13 · MF-59 · MF-61 · MF-64 · MF-65 · MF-66 · **MF-67** · MF-21 · BASE-003 #10 · 디자인 시스템 [09] L748~749)

**회의록 것이다**(MF-67). 업무 탭의 새 업무 드로어를 열지 않고, 그 드로어에 안건 슬롯·prop 을 얹지도 않는다 — U-9 와 같은 이유다(값을 적는 자리 vs 업무를 만드는 자리). ⛔ **업무 탭 드로어를 수정하지 않는다.** 회의록 전용 「업무 생성」 드로어 시안(L2600~2882)도 참고하지 않는다 — 기한 하나뿐이고 `payload` 개념이 없던 시안이다(§7 제외. 시작일이 비던 F-2 는 이 드로어가 일정을 **시작 ~ 종료 둘**로 두므로 다시 생기지 않는다).

- **시각 규격의 참조는 이미 만들어진 새 업무 드로어(SPEC-003 U-1 · `TaskCreateDrawer`)다.** 드로어 840 · 스크림 · 헤더 72 · 푸터 76, 입력 · 셀렉터 · 일정(계획 시작–종료) · 할일 체크리스트 · 설명 textarea 블록의 **규격을 그대로 가리킨다.** 새 시안은 없다
- **드로어는 하나다**(MF-65) — AI 줄과 사람 줄이 같은 드로어를 열고, 다른 것은 `payload` 가 차 있나 비어 있나뿐이다
- **진입점 둘** — ① 액션 줄의 「업무 생성」(U-6) ② 편집 모드의 **「+ 액션 아이템」 칩**(U-7 — 줄이 아직 없다. 「저장」이 줄과 `payload` 를 함께 만든다 — MF-64 정정)
- **본문 — 필드는 `payload` 키 그대로 일곱**(MF-13)

  | 필드 | `payload` 있음(AI 줄 · 사람이 앞서 「저장」한 줄) | `payload` 없음(사람이 회의 중 적은 줄 · 칩으로 막 연 드로어) |
  |---|---|---|
  | **안건**(맨 위 · 고정) | 그 줄의 안건 — 읽기 전용. 칩으로 열었으면 칩을 누른 안건 | 〃 |
  | **제목** | `payload.title` | 줄 본문(칩 진입이면 빈 값) — **이 값이 줄 본문이 된다**(칩 진입) |
  | **유형**(필수) | `payload.workTypeId`(AI 가 `list_work_types()` 의 설명을 보고 고른 것 — MF-21). **`null` 이면 빈 채로** — 제출 조건이 사람에게 고르게 한다(DEC-003 OQ-12) | 빈 값 |
  | **프로젝트** | `payload.projectId` | 회의의 프로젝트(MF-61) |
  | **계획 시작 ~ 종료** | `payload.startDate` · `payload.dueDate` — 말에 나왔으면, 없으면 비운다 | 빈 값 |
  | **설명** | `payload.description` | 빈 값 |
  | **할일** | `payload.todos[]` — 「A 하고 B 하고」가 말에 나왔으면 항목으로 쪼갠 것 | 빈 값 |

  - **상태 필드가 없다** — 만들면 항상 「시작전」이다(DEC-002 §5 · `payload` 에 상태 키가 없다). 시안 L2850~2851 의 「시작 상태」는 제외(§7)
  - 참고자료 · 연관 업무 · 결과자료 · 첨부 · 로그 블록은 **그리지 않는다**(`payload` 키가 아니다)
  - `payload` 로 채워진 필드는 **테두리 `#7181F8` + 캡션 「회의에서 반영」**(U-9 와 같은 표시). 사람이 고칠 수 있다
- **푸터 — 모드별 하나씩**(MF-66 · prop `submitMode:"save"|"insert"`. ⛔ 한 푸터에 셋을 두지 않는다). **인라인 자동 저장이 없다**
  - **편집 모드(U-7 — 「+ 액션 아이템」 칩 · 편집 중 「업무 생성」)에서 열면 「취소」 · 「저장」.** **「저장」** = **`payload` 를 줄에 붙인다. 업무는 아직 없다.** 줄에서 열었으면 `PATCH …/lines/{lineId} { payload }`, **칩으로 열었으면 `POST …/lines { agendaId, kind:"action", content, payload }` — 줄과 `payload` 가 한 요청**(§4 · MF-64 정정. `content` = 제목). 조건 — 제목 1자 이상(유형은 비어 있어도 저장된다 — 넣기 때 고른다). 드로어가 닫히고 줄 버튼은 「업무 생성」 그대로 + dot. **이 시점에 AI 가 채운 줄과 사람이 채운 줄은 완전히 같다**
  - **보기 모드(U-3 — 「업무 생성」)에서 열면 「취소」 · 「넣기」.** **「넣기」** = 업무를 만든다 — 확인하고 바로 넣는다. `POST …/lines/{lineId}/task { …새 업무 필드 }`(§4). 조건은 SPEC-003 `POST /api/tasks` 와 같다(제목 1자 이상 + 유형 선택). **업무 생성과 줄 갱신은 한 요청 · 한 트랜잭션**이다
  - 「취소」 · × · `Esc` · 스크림 = 아무것도 저장하지 않는다. 줄에서 열었으면 줄은 그대로, **칩으로 열었으면 줄이 안 생긴다**(MF-64 정정)
- **기대 결과**
...
- 회의록의 줄 추가 드로어(U-8)와 payload 드로어(U-9 · U-10)도 같은 드로어 규칙이다. 확인 모달 420 은 전 구간 같은 크기. 회의 중 · 시작 전 화면의 반응형은 SPEC-006 U-6 · SPEC-007 U-8
...
1. 액션 줄 「도입 사례 페이지 분리 초안」의 「업무 생성」을 누른다 → **액션 payload 드로어**가 열린다. 맨 위에 「안건 1」이 고정돼 있고, 제목 · 유형 「문서·보고」 · 프로젝트 · 계획 시작 09.01 – 계획 종료 09.05 · 설명 · 할일 「초안 작성」「내부 검토」가 채워져 있다(파란 테두리 + 「회의에서 반영」). 보기 모드라 푸터는 「취소 · 넣기」.
...
### S-6-a. 사용자 — 편집 중에 액션을 하나 더 적는다 (MF-64 정정)

1. 편집 모드에서 안건 2 아래 「**+ 액션 아이템**」을 누른다 → **액션 payload 드로어가 바로 뜬다**(줄 추가 드로어를 거치지 않는다). 안건은 「안건 2」로 고정, 나머지는 비어 있고 프로젝트만 회의 프로젝트다. 푸터는 「취소 · 저장」.
2. 제목 「요금제 표 다시 정리」 · 유형 · 기한을 채우고 「**저장**」 → **요청 하나**(`POST …/lines`)로 그 안건 맨 아래에 액션 줄이 생기고 `payload` 가 붙는다. 줄 옆에 점이 켜져 있다. **내 업무에는 아무것도 안 생겼다.**
3. 「+ 연관 업무」도 같다 — **업무 payload 드로어**가 바로 뜨고 헤더에서 업무를 고른 뒤 「저장」하면 줄과 `payload` 가 함께 생긴다. **어느 쪽이든 드로어를 「취소」로 닫으면 줄이 생기지 않는다.**
4. 「+ 논의」·「+ 결정」만 **줄 추가 드로어**(U-8)를 거친다 — 거기서는 「추가」가 줄을 만든다.

### S-7. 사용자 — 내가 적은 액션을 채워 두고 나중에 넣는다

1. 편집 모드에서, 회의 중 `/액션 요금제 표 다시 정리` 로 적었던 줄의 「업무 생성」을 누른다 → **같은 액션 payload 드로어**가 **제목 「요금제 표 다시 정리」 · 프로젝트만 채워진 채** 열린다. 유형 · 일정 · 설명 · 할일은 비어 있다. AI 를 다시 부르지 않았다.
...
2. 누른다 → **업무 payload 드로어**가 열린다. 헤더 제목 자리가 셀렉터 「제품 소개서 내용 업데이트 ˅」이고, 본문의 변경분 일곱 중 기한 09.02, 진행 메모 「검수 일정이 8/29 오후로 잡혀 기한을 9/2로 옮김」, 완료 결과 「소개서 v2 배포 완료」가 **이미 들어가 있다**(파란 테두리 + 「회의에서 반영」). 상태 셀렉터에 「완료」가 없다. 보기 모드라 푸터는 「취소 · 넣기」.
3. AI 가 업무를 잘못 골랐다 — 헤더 셀렉터를 눌러 **업무 화면과 같은 후보 고르기(`RelationPopover`)** 에서 「제품 소개서 v2 내용 업데이트」로 바꾼다. 기한 · 프로젝트 · 완료 결과의 출발값이 그 업무의 현재 값으로 바뀌고 변경분은 그대로다.
...
2. 편집 모드에서 누르면 **같은 업무 payload 드로어**가 헤더 셀렉터 「업무 고르기」가 빈 채로 열리고 본문은 비활성이며 푸터는 「취소 · 저장」이다. 셀렉터를 눌러 후보 고르기 — 회의 프로젝트에 업무가 0건이라 기본이 「전체」다. 고르면 본문이 그 업무의 현재 값으로 채워진다.
...
| POST | `/api/meetings/{id}/lines` | **줄 추가** — SPEC-007 의 표면을 `ended` 편집으로 **확장**. 논의·결정 줄(U-8)은 줄만, **액션·업무 줄(U-9 · U-10 칩 진입)은 `payload`(업무 줄은 `taskId` 도)를 함께 받는다 — 줄과 `payload` 가 한 요청**(MF-64 정정). **`newTask` 갈래는 없다 · 업무는 안 생긴다**(`task_service` 미호출) | 세션 | SPEC-007(회의 중) · **이 spec**(종료 후) |
...
- **`payload` 저장과 업무 생성은 다른 요청이다**(MF-59 · MF-66). 드로어 「저장」은 줄에 `payload` 만 붙이고(줄에서 열었으면 `PATCH …/lines/{lineId}`, **칩으로 열었으면 `POST …/lines` 가 줄과 `payload` 를 함께 만든다** — MF-64 정정), 「넣기」는 `…/lines/{lineId}/task` 로 업무를 만들거나 바꾼다. **어느 쪽이든 `POST/PATCH …/lines` 는 `task_service` 를 부르지 않는다.** AI 가 채운 `payload` 와 사람이 「저장」한 `payload` 는 **같은 컬럼 · 같은 모양**이다 — 서버는 출처를 구분하지 않는다
...
- **사전 조건은 `status='recording'` 하나다.** 스트림 상태를 보지 않는다 — **`paused/stream`(WS 없음)에서도 받는다**(BE §8-2 · SPEC-007 U-1). WS 가 열려 있으면 서버가 닫고, 없으면 그냥 진행한다. **`ai_session_id` 가 없어도 받는다**(MF-1 — 웜스타트가 실패한 회의). **그 회의는 ② 에서 세션이 없어 `final_failed` 로 간다**(Case Matrix ② 행 「세션 없음」) — 별도 기록 · 재웜스타트 갈래를 만들지 않는다(**MF-70** · DEC-003 OQ-8 닫힘)
...
**`POST /api/meetings/{id}/lines`** → `201 MeetingDetail`(자식 쓰기 응답은 상세 전체 — SPEC-006 §4 · SPEC-007 §4) — SPEC-007 의 본문(`agendaId` · `kind` · `content`)에 종료 후 편집이 더하는 것은 **`detail` · `payload` · `taskId`** 다

```json
{ "agendaId": 71, "kind": "decision", "content": "…", "detail": "…" }   // U-8 — 논의 · 결정. payload · taskId 를 실으면 422
{ "agendaId": 71, "kind": "action", "content": "요금제 표 다시 정리",
  "payload": { "title": "요금제 표 다시 정리", "workTypeId": 3, "projectId": 7, "startDate": null, "dueDate": "2026-09-12", "description": null, "todos": [] } }   // U-10 칩 진입 「저장」 — 줄 + payload 한 요청
{ "agendaId": 71, "kind": "task", "content": "소개서 v2 문구 정리 — 기한 미루기",
  "taskId": 101, "payload": { "dueDate": "2026-09-10" } }               // U-9 칩 진입 「저장」 — 줄 + taskId + payload 한 요청
```

- `agendaId` 의 트랙이 줄의 트랙이다(`human` 또는 `merged`). 회의 중(`recording`)은 SPEC-007 규칙 그대로 — `detail` · `payload` · `taskId` 는 **`ended` 에서만** 받는다
- **`payload`(와 업무 줄의 `taskId`)는 `kind ∈ {action, task}` 에만**(MF-64 정정 — 액션·업무 칩은 payload 드로어가 바로 뜬다). **논의·결정 줄에 오면 `422 validation_error`.** 둘 다 선택이다 — 안 실으면 `payload:null` · `taskId:null` 로 생긴다
- **업무는 생기지 않는다** — 이 표면은 `task_service` 를 부르지 않는다. 업무를 만들거나 바꾸는 것은 「넣기」(`…/lines/{lineId}/task`)뿐이다
- **`newTask` 갈래는 없다**(폐기). 새 업무의 값은 `payload` 로 들어오고, 업무가 되는 것은 보기 모드의 「넣기」다
...
| `content` | 추가 시 **필수 — 칩 넷 어느 쪽이든**(서버가 채우는 갈래가 없다). 논의·결정은 U-8 의 「내용」, 액션은 U-10 의 「제목」, 업무는 U-9 의 「내용」이 이 값이 된다. 공백 제거 후 **1~2000자, 줄바꿈 불가**(SPEC-007 §4 와 같다) |
| `detail` | 선택. 4000자 이하 |
| `kind` | 추가 시 `discussion` · `decision` · `task` · `action` 넷 다. **`POST lines` 의 `payload`(와 `taskId`)는 `action` · `task` 줄에만** — `discussion` · `decision` 에 실리면 `422`(MF-64 정정). **`newTask` 는 어느 줄에서도 받지 않는다.** **`PATCH …/lines/{lineId}` 에 `kind` 가 오면 거부**(MF-60) |
| `taskId`(`POST lines` · `PATCH lines` · `PATCH …/task`) | 본인의 **삭제되지 않은** 업무 또는 `null`(「저장」 때만). `kind='task'` 줄에만. 이미 값이 있어도 바꿀 수 있다(MF-65) |
| `payload`(`POST lines` · `PATCH lines` — 둘 다 「저장」) | 액션 줄 = 생성분 `{title, workTypeId?, projectId?, startDate?, dueDate?, description?, todos[]}` — `title` 1~200 필수, **`workTypeId` 는 `null` 허용**(넣기 때 고른다) · 업무 줄 = 변경분(아래 「업무 갱신 본문」과 같은 키·규칙). `null` 허용. 참조(유형 · 프로젝트 · 연관 업무)는 본인의 삭제되지 않은 것 |
...
| `validation_error` | `422 {detail:"입력값을 확인해 주세요", code:"validation_error"}` — 본문 · 안건 이름 길이 · `kind` 밖 · **`PATCH lines` 에 `kind`**(MF-60) · 갱신 본문에 일곱 키 밖의 것 · **`status:"done"`/`"cancelled"`**(MF-59) · **`POST lines` 에 `newTask`** · **`POST lines` 의 `payload`/`taskId` 가 논의·결정 줄에**(MF-64 정정) · `taskId` 를 `task` 줄이 아닌 줄에 · `PATCH …/task` 에 `taskId` 없음 · 회의 중에 `detail`/`payload` · **편집 대상 트랙이 아닌 줄 삭제 · `ai` 줄 삭제** · 안건 이름과 `state` 동시 전송 | 드로어 안 인라인(그 필드) 또는 인라인 편집 필드 실패 표시 | 그 필드 | 판정 — DEC-003 §4 · §1 표 · MF-59 · MF-60 · ERD M-20 · BE §8-3 |
...
sequenceDiagram
    ...
    alt ① 실패 · 타임아웃
        ...
        M->>BE: REST (auth_session kind='meeting' 행의 단명 토큰 — MF-69)
    ...
    BE->>BE: 단명 토큰 폐기 = auth_session 행 DELETE (best-effort — MF-4 · MF-69)
    ...
    User->>FE: 편집 모드 「+ 결정」 → U-8 줄 추가 드로어 → 「추가」
    FE->>BE: POST /api/meetings/{id}/lines { agendaId, kind:"decision", content, detail }
    BE-->>FE: 201 MeetingDetail (줄만 — payload 없음)

    User->>FE: 편집 모드 「+ 액션 아이템」 → 액션 payload 드로어가 바로 (빈 값 · 푸터 「취소 · 저장」) → 채우고 「저장」
    FE->>BE: POST /api/meetings/{id}/lines { agendaId, kind:"action", content, payload }
    BE-->>FE: 201 MeetingDetail (줄 + payload 한 요청 — 업무 없음 · MF-64 정정)
    Note over FE,BE: 「+ 연관 업무」도 같다 — kind:"task" · taskId · payload. 드로어를 닫으면 줄이 안 생긴다

    User->>FE: 편집 모드 · 기존 액션 줄 「업무 생성」 → 「저장」
    FE->>BE: PATCH /api/meetings/{id}/lines/{lineId} { payload }
    BE-->>FE: 200 MeetingDetail (payload 만 — 업무 없음)

    User->>FE: 보기 모드 · 액션 줄 「업무 생성」 → 액션 payload 드로어(payload 채워짐 · 푸터 「취소 · 넣기」) → 「넣기」
    FE->>BE: POST /api/meetings/{id}/lines/{lineId}/task { title, workTypeId, … todos }
    BE-->>FE: 201 MeetingDetail (업무 생성 + 줄 → task 한 트랜잭션)

    User->>FE: 보기 모드 · 업무 줄 「업무 갱신」 → 업무 payload 드로어(헤더 업무 셀렉터 · 변경분 일곱 · 푸터 「취소 · 넣기」) → 「넣기」
...
- **payload 드로어는 회의록 영역 것이다**(MF-67 · FE §2 규칙 8). `features/meetings` 안에 **액션 payload 드로어 · 업무 payload 드로어 둘**을 만들고, **`features/tasks` 의 새 업무 드로어 · 업무 상세 드로어는 고치지 않는다** — 그 둘에 회의록용 슬롯 prop 을 더하는 코드가 있으면 **반려**. 시각 규격은 그 두 드로어(SPEC-003 U-1 · U-3)의 부품 규격을 가리켜 조립한다. **`RelationPopover` 는 그대로 재사용한다**(업무 화면 부품이고 드로어가 아니다 — 단일 선택 prop 하나). 회의록 드로어가 받는 것은 `prefill`(= 줄의 `payload`)과 푸터 모드뿐이다(U-9 · U-10). **한 푸터에 「저장」과 「넣기」를 같이 두면 반려**(MF-66). **AI 줄과 사람 줄로 분기하는 코드가 있으면 반려**(MF-65) — 드로어는 `payload` 가 `null` 인지만 본다. **필드는 `payload` 키만** — 제목·유형·설명·시작일을 업무 payload 드로어에 그리거나, 첨부·로그·참고자료를 어느 쪽에든 그리면 반려
- **`payload` 저장(`POST lines` · `PATCH lines`)은 `task_service` 를 부르지 않는다.** 업무를 만들거나 바꾸는 코드는 `…/lines/{id}/task` 두 표면에만 있다
- **`POST lines` 는 줄과 `payload` 를 함께 만든다**(MF-64 정정 — 액션·업무 줄에 한해). 논의·결정 줄의 `payload`·`taskId` 는 스키마 층 422. **`newTask` 갈래가 코드에 있으면 반려**
- **낙관적 갱신을 쓰는 자리**(FE §3-4): 인라인 본문 수정 · 안건 이름 · 줄 추가(U-8 논의·결정) · `payload` 「저장」(칩 진입의 `POST lines` 포함) — 거부할 규칙이 없다. **쓰지 않는 자리**: 줄 삭제(모달을 지나며 응답으로 제거) · 「넣기」(업무 생성 · 갱신) · 일시 · 유형 · 프로젝트
...
- **드로어는 동시에 하나.** U-4 안에서 payload 드로어 · 줄 삭제 모달을 열지 않는다. **칩 → 줄 추가 드로어 → payload 드로어로 드로어가 연달아 둘 뜨는 갈래를 만들지 않는다**(MF-64 정정의 이유). 「삭제」는 드로어를 닫고 모달을 연다(FE §6-2)
...
앱 창에서 사람이 직접 확인하는 문장으로 적는다. **32개.**
...
- [ ] 「+ 결정」 드로어에 근거 구간 입력이 없고, 종류 세그먼트에 **「업무」·「액션」이 없으며**, 「추가」하면 그 안건 맨 아래(마지막 번호 + 1)에 줄이 붙는다
- [ ] 편집 모드 「**+ 액션 아이템**」 → **줄 추가 드로어를 거치지 않고 액션 payload 드로어가 바로 뜬다.** 안건이 고정돼 있고 푸터는 「취소 · 저장」이다. 제목·유형을 채우고 「저장」 → **요청 하나**(`POST …/lines`)로 줄과 `payload` 가 함께 생기고 줄 옆에 점이 켜진다. **내 업무에는 아무것도 안 생긴다.** 「+ 연관 업무」도 같다(업무 payload 드로어 → 헤더에서 업무를 고르고 「저장」)
- [ ] 「+ 액션 아이템」·「+ 연관 업무」 드로어를 **「취소」로 닫으면 줄이 생기지 않는다.** 「+ 논의」·「+ 결정」만 줄 추가 드로어(U-8)를 거치고, 거기서 「추가」가 줄을 만든다
- [ ] AI 액션 줄의 「업무 생성」 → **액션 payload 드로어**가 열리고 맨 위에 안건이 고정돼 있으며, 제목 · 유형 · 프로젝트 · **계획 시작 – 계획 종료** · 설명 · **할일**이 채워져 있다(「회의에서 반영」 표시). **필드는 이 일곱뿐이다** — 「시작 상태」 · 참고자료 · 연관 업무 · 첨부 · 로그 칸이 없다. 보기 모드라 푸터가 「취소 · 넣기」다(「저장」 없음). 「넣기」 → **그 줄이 업무 줄**이 되고 내 업무에 「시작전」 업무가 **할일과 함께** 생기며 캘린더에 기간 바가 있다
- [ ] 편집 모드에서 내가 회의 중 `/액션` 으로 적은 줄의 「업무 생성」 → **같은 액션 payload 드로어**가 **제목 · 프로젝트만** 채워진 채 열리고 푸터는 「취소 · 저장」(「넣기」 없음). 워커 로그에 AI 호출이 없다. 유형·기한을 채우고 **「저장」** → 줄은 액션 줄 그대로 + 점, **내 업무에 아무것도 안 생긴다.** 보기 모드에서 다시 열면 값이 그대로이고 푸터는 「취소 · 넣기」 — AI 줄과 화면이 같다. 「넣기」 → 업무 줄이 된다
- [ ] AI 가 유형을 못 고른 액션 줄(`payload.workTypeId=null`)은 드로어의 유형이 비어 있다 — 편집 모드의 「저장」은 되지만 보기 모드의 「넣기」는 고르기 전엔 비활성이다
- [ ] AI 업무 줄의 「업무 갱신」 → **업무 payload 드로어**가 헤더 제목 자리 **업무 셀렉터**에 그 업무를 물고, 기한 · 진행 메모 · 완료 결과가 **이미 들어간 채로** 열리며(「회의에서 반영」 표시), 상태 셀렉터에 「완료」가 없고, 보기 모드라 푸터가 「취소 · 넣기」다. **필드는 변경분 일곱뿐이다** — 제목 · 유형 · 설명 · 시작일 · 첨부 · 로그 칸이 없다. 헤더 셀렉터를 눌러 **업무 쪽과 같은 후보 고르기**로 다른 업무를 고를 수 있다. 「넣기」 → 「갱신 완료」로 바뀌고 내 업무에 기한 · 메모 새 항목 · 완료 결과가 반영돼 있다. **상태는 바뀌지 않았다**(로그에 완료 전이 없다)
- [ ] 그 업무의 화면에서 「완료 처리」를 누르면 완료 결과가 이미 있어 **게이트 거부 없이** 완료된다
- [ ] 「넣기」 본문에 `status:"done"` 을 실어 보내면 **422** 이고 아무것도 바뀌지 않는다
- [ ] 편집 모드에서 내가 `/업무` 로 적은 줄의 「업무 갱신」 → **같은 업무 payload 드로어**가 헤더 셀렉터 **빈 채**로 열리고 본문이 비활성이며 푸터는 「취소 · 저장」이다. 「업무 연결」 같은 별도 버튼·앞 단계가 없다. 회의 프로젝트에 업무가 0건이면 후보 고르기 기본 칩이 「전체」다. 고르고 기한을 바꿔 「저장」 → 배지가 붙고 툴팁에 변경분, **내 업무는 그대로**. 보기 모드에서 「업무 갱신」 → 「넣기」 → 반영
- [ ] **업무 탭의 새 업무 드로어 · 업무 상세 드로어는 회의록 때문에 달라진 것이 없다**(MF-67) — 업무 탭에서 열면 안건 칸도 「저장/넣기」 푸터도 없고 인라인 자동 저장이 그대로다. 회의록이 여는 것은 **다른 드로어**다
- [ ] 회의록에서 연 payload 드로어 어디에도 **인라인 자동 저장이 없다** — 포커스를 벗어나도 요청이 나가지 않고 편집 모드의 「저장」·보기 모드의 「넣기」만 요청을 낸다. **어느 모드에서도 푸터에 버튼이 둘(취소 + 하나)뿐이다**
- [ ] `PATCH …/lines/{id}` 에 `kind` 를 실으면 **422** 다. `POST …/lines` 에 `newTask` 를 실어도, **논의·결정 줄에 `payload` 나 `taskId` 를 실어도 422** 다. 액션·업무 줄의 `payload` 는 받아들이고 **그때도 내 업무에는 아무것도 안 생긴다**
...
| **MF-65 · 66 · 「payload ≠ 업무 생성」** | **드로어는 액션·업무 각각 하나** — AI 줄/사람 줄 분기 없음, 인라인 자동 저장 없이 **모드별 푸터**(MF-66 — 편집 「취소 · 저장」 / 보기 「취소 · 넣기」). **「저장」 = 줄에 `payload` 만(업무 안 건드림) · 「넣기」 = `…/lines/{id}/task`(업무 생성·갱신).** 업무 payload 드로어 헤더 제목 자리 = 업무 셀렉터(`RelationPopover`) — 「업무 연결」 버튼·후보 고르기 앞 단계 폐지 |
| **MF-64 정정 — 칩은 둘로 갈린다**(2026-09-07 저녁) | U-7 — **「+ 논의」·「+ 결정」은 U-8 줄 추가 드로어**(「추가」에 줄) · **「+ 연관 업무」·「+ 액션 아이템」은 payload 드로어가 바로**(「저장」에 줄 + `payload` **한 요청**). 어느 쪽이든 닫으면 줄이 없다. **U-8 은 논의·결정 전용**이 됐고 「종류 하나로 고정되는 갈래」는 삭제. `POST …/lines` 가 액션·업무 줄에 한해 `payload`(업무 줄은 `taskId` 도)를 받는다 — **`newTask` 는 여전히 없고 업무도 안 생긴다**. 직전 판(「칩 넷이 전부 U-8 로 줄 먼저」)은 폐기 — 액션·업무는 드로어가 연달아 둘 떴다 |
| **MF-67 — payload 드로어는 회의록 것이다**(2026-09-07 저녁 · MF-13 · 14 정정) | **U-9 · U-10 재작성** — 업무 탭 드로어에 안건 슬롯 · 업무 셀렉터 슬롯 · 프리필 · 푸터 모드 prop 을 얹어 재사용하던 계약을 **폐기**하고, `features/meetings` 안에 **액션 payload 드로어 · 업무 payload 드로어 둘**을 둔다. **업무 드로어는 손대지 않는다.** 필드는 **`payload` 키 그대로**(액션 일곱 · 업무 변경분 일곱) — 제목·유형·설명·시작일은 업무 payload 드로어에 없고, 첨부·로그·참고자료는 어느 쪽에도 없다. **시각 참조는 이미 만들어진 SPEC-003 U-1 · U-3(`TaskCreateDrawer` · `TaskDetailDrawer`)의 부품 규격**이고 새 시안은 없다. **`RelationPopover` 는 그대로 재사용**(업무 화면 부품 · 드로어가 아니다). 정합 #1(SPEC-003 에 슬롯 더하기)은 **삭제**됐다 |
| **MF-54 용어 보정** | §4 `context`(①) + `termCorrections`(②) + 서버 치환 규칙(`auto` 만 · 화자 라벨 불변 · 표가 백업). 화면 없음(기획·정책에 없다). `context.terms` 원천은 DEC-003 OQ-10 |
| **MF-13 · 14 드로어 규격**(MF-67 로 정정) | U-9 · U-10 — 옛 회의록 전용 드로어 시안(L2306~2882)은 **`payload` 개념이 없어 참고하지 않는다**. 회의록 payload 드로어 둘이 **SPEC-003 U-1 · U-3 의 부품 규격**을 가리키고, 업무 셀렉터는 SPEC-003 U-8 `RelationPopover`(0건 폴백 「전체」 · 단일 선택)를 그대로 쓴다. **SPEC-003 에 슬롯·푸터를 더하지 않는다**(MF-67) |
...
| **MF-70 웜스타트 실패** | **처리를 만들지 않는다** — 예외 전파 로그뿐. 회의 중 = 배치 미제출(SPEC-007) · 종료 후 = ② `final_failed`(Case Matrix). DEC-003 OQ-8 닫힘 |
| **MF-68 · 69 MCP · 토큰** | 이 spec 이 닿는 것은 **② 종결 시 토큰 폐기 = `auth_session` 행 DELETE**(§5 Flow) 하나다. 서버 배치(`app/mcp/` 별도 컨테이너)·발급은 `backend/README.md` · `system/README.md`. DEC-003 OQ-9 닫힘 |
...
| U-8 줄 추가 드로어 | **논의 · 결정 전용**(MF-64 정정). 종류 세그먼트는 「논의」·「결정」 둘뿐이고 「업무」·「액션」으로 고정되는 갈래가 없다. 액션·업무는 payload 드로어가 바로 뜨고 「저장」이 줄과 `payload` 를 한 요청으로 만든다 — `content` 1~2000 필수는 그대로이고, 그 값은 드로어의 제목(액션)·내용 칸(업무)에서 온다 |
...
| **L2306~2599** | 회의록 전용 「연관 업무 연결」 드로어 | **`payload` 가 없던 시안**이라 참고하지 않는다. 규격은 SPEC-003 U-3 · U-8 을 가리킨다(U-9) | **MF-14 · MF-67** (F-4) |
| **L2600~2882** | 회의록 전용 「업무 생성」 드로어(기한 하나 · 시작 상태 · 메모 · 토글) | **`payload` 가 없던 시안**이라 참고하지 않는다. 규격은 SPEC-003 U-1 을 가리키고 일정은 **시작 ~ 종료 둘**이다 — 시작일이 비던 문제(F-2)가 다시 안 생긴다 | **MF-13 · MF-67** · DEC-002 §5 |
...
이 spec 이 새로 만든 OQ 는 없다. **OQ-8 · OQ-9 는 2026-09-07 저녁에 닫혔다**(MF-68 · 69 · 70). 남은 것은 **DEC-003 OQ-10 · 12** 뿐이고 각각 「어디를 찾았는데 없었나」와 함께 정책서에 있다. 아래는 이 spec 이 취한 자리 —

| OQ | 이 spec 의 처리 |
|---|---|
| ~~OQ-8~~ 웜스타트 실패 | **닫힘(MF-70, 2026-09-07 저녁)** — 별도 기록 · 재시도 · 재웜스타트 갈래를 **만들지 않는다.** 남기는 것은 예외 전파 로그뿐이다. 이미 있는 경로로 흘러간다 — **회의 중**: `ai_session_id` 없음 → 배치 미제출(SPEC-007 검증 0) · **종료 후**: 세션 없음 → ② 가 `final_failed`(§4 Case Matrix ② 행 「세션 없음」) → 「다시 시도」 배너. `/end` 는 `ai_session_id` 없이도 받는다(그대로) |
| ~~OQ-9~~ MCP 배치 · 토큰 인프라 | **닫힘(MF-68 · MF-69, 2026-09-07 저녁)** — MCP 서버는 **별도 컨테이너**(`app/mcp/`, compose 에 back · worker · mcp 셋. FastAPI 안에 두지 않는다). 단명 토큰은 **`auth_session` 의 `kind='meeting'` 행** — `/start` 때 INSERT, MCP 헤더에 싣고, **② 종결 시 행 삭제**(best-effort · MF-4). 무상태 JWT 는 쓰지 않는다(폐기 표면이 없다). 이 spec 이 닿는 곳은 ② 종결 시 **토큰 폐기 = 행 삭제**(§5 Flow)뿐이고 나머지는 `backend/README.md` §4 · §5-2 · `system/README.md` |
...
| # | 무엇 | 어떻게 |
|---|---|---|
| 1 | ~~SPEC-003 U-1 · U-3 에 회의록 슬롯을 더한다~~ → **삭제(MF-67, 2026-09-07 저녁).** 업무 탭 드로어는 손대지 않는다 — 안건 슬롯 · 업무 셀렉터 슬롯 · 프리필 · 푸터 모드 prop 을 SPEC-003 에 더하지 않는다. U-1 · U-3 의 「회의록에서 열 때」 문단은 **지웠다**. **남는 것은 U-8 하나** — 기준 프로젝트가 **0건**일 때도 기본 「전체」 + **단일 선택 모드 prop**(회의록의 업무 payload 드로어 헤더가 `RelationPopover` 를 그대로 쓴다 — MF-14) | SPEC-003 U-8. 이 spec U-9 가 계약이다 |
...
| U-6 줄 버튼 | L1484~1486 · L1508~1510 | [09] L668~680 | BASE-003 #10 · DEC-003 §1 · §4 · §5 · **MF-13 · 14 · 59 · 61 · 65 · 67** | ERD M-14 · M-14-a · M-20 · BE §8-3 |
...
| U-8 줄 추가 드로어(논의 · 결정 전용) | L2042~2305 | [09] L740~741 · [07] L431 | DEC-003 §5 · **MF-60 · MF-64 정정** | FE §6 · SPEC-007 `POST lines` |
| U-9 업무 payload 드로어 | — (시안 L2306~2599 참고 안 함 — `payload` 없음) | [09] L744~745 · **SPEC-003 U-3 부품 규격** | BASE-003 L84 · DEC-003 §4 · §5 · **MF-14 · 59 · 61 · 64 · 65 · 66 · 67** · SPEC-004 §4 전이 그래프 | SPEC-003 U-3(시각 참조) · U-8 `RelationPopover`(재사용) · §4 후보 API · ERD M-14 · M-14-a · FE §2 규칙 8 |
| U-10 액션 payload 드로어 | — (시안 L2600~2882 참고 안 함 — `payload` 없음) | [09] L748~749 · **SPEC-003 U-1 부품 규격** | BASE-003 #10 · DEC-002 §5 · **MF-13 · 21 · 59 · 61 · 64 · 65 · 66 · 67** · DEC-003 OQ-12 | SPEC-003 U-1(시각 참조) · §4 · ERD M-14 · BE §7 · FE §2 규칙 8 |
| §4 종료 파이프라인 · 검증 · 용어 보정 | — | — | DEC-003 §4 · §6 · §7 · §STT · **MF-37 · 52 · 54 · 56 · 57 · 58 · 59** · OQ-10 | `system/README.md` §③ · 불변식 2·3·7·11·12·13·14·15 · ERD M-6 ~ M-9-b · M-15 · M-19 · BE §5-2 · §6 · §7 · §8-1 · §8-3 · SPEC-007 AI 배치 계약 |

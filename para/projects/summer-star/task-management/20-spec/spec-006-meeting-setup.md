...
- Architecture reference: `backend/README.md` §5-1(스트림은 회의당 하나) · §6(장시간 작업) · §7(일정 파생은 같은 트랜잭션) · §8(에러 규약) · §10(회의록 표면) · `frontend/README.md` §1(라우트 `?id=`) · §3-3(무효화 표) · §6(오버레이 3종) · §7(반응형) · §9(V2Gate) · `database/README.md` §0-1(소프트 딜리트) · §3(`schedule` 파생) · `database/domains/meeting.md` M-1~M-18
...
- **반응형**(1280~1439 · ≥1440)
- **AI 한 줄 요약 바의 표시** — 목록 행 셋째 줄 + 우측 미리보기 패널 상단(U-8). 생성은 SPEC-008 종료 파이프라인 ②(DEC-003 §1 표 · §4 — 2026-09-06)
- **후속 SPEC 이 물려받는 계약** — `GET /api/meetings/{id}` 상세 응답 형태(**정본은 이 spec**) · `POST /api/meetings/{id}/start` · 안건·첨부 계약 · 상태 4종

Out of scope:

- **회의 중** — 마이크·WS 스트림·실시간 스크립트·사람 트랙 줄 입력·AI 배치·일시정지 — **SPEC-007**
- **종료·생성중·통합본·편집(줄 삭제 · 안건 이름 수정 포함)·근거 칩·회의 상세 드로어(SPEC-008 U-4)** — **SPEC-008**. 한 줄 요약의 **생성**도 거기다
...
- **상태**
  - *정상*: breadcrumb 「홈 › 회의록」(L47~51, top 38 · 13px `#9EA2AE`) → 타이틀 행(L53~68) → 2패널(L70 · L148, top 140)
...
- **문구**
  ...
  - 행(padding 14/18 · 구분 `#F1F2F5` · hover `#FAFBFC` · **선택 행 `#F8FAFF`**(L84)):
    1. 첫 줄 — 일시 12/`#757575` 「08월 27일 (목) 09:30」(L86) + 우측 **유형 배지**(h20 r4 11/600, **동적 유형의 색 토큰** — L87 의 `#EEF1FE/#4A55B8` 는 그 유형 색의 예시값이다)
       - **상태 표기**(시안 없음 → 디자인 시스템으로 조립. DEC-003 §3 status 4종 + §4 「통합 실패 시 ended + 실패 표시」): `scheduled` → 일시 뒤에 「**예정**」(시작 전 헤더 L576 의 어휘) · `recording` → 「**기록 중**」 + 7px dot `#7181F8` + 3px 광륜([09] L723) · `generating` → 「**생성중**」 · `ended` → 없음 · `ended` + `integrationState=failed` → 「**통합 실패**」 12/`#E2685B`
    2. 둘째 줄 — 제목 15/700(L89)
    3. 셋째 줄 — 한 줄 13/`#5F6470` ellipsis(L90): **`headline`**(AI 한 줄 요약 — DEC-003 §1 표 2026-09-06 · ERD `ai_headline`. 종료 후 통합 ② 에서 통합본과 **같은 응답으로** 받는다 — SPEC-008)이 있으면 그것, 없으면 **사람 트랙 안건 제목을 「 · 」로 이어** 붙인다. 안건도 없으면 「안건 없음」 `#9EA2AE`
  - 정렬 팝오버: 「**최신순**」(기본 — `startAt` 내림차순) · 「오래된순」. 고르면 즉시 반영, 「적용」 버튼 없음
- **CTA**
  - 행 클릭 → **선택** + 우측 미리보기 패널(U-8) 갱신. **상세 페이지로 바로 가지 않는다** — 상세는 U-8 의 「상세보기」 또는 행 더블클릭
...
- **상태**
  ...
  - *정상*: breadcrumb 「홈 › 회의록 › 시작 전」(L565~571) → 헤더(L573~585) → 상태 바(L587~593, top 150 · 1640×56) → 좌 패널 1160(L595) + 우 패널 464(L631, left 1416) — 패널 규격은 [09] L723 「좌측 패널 1160 = 회의록 | AI 요약, 우측 464 = 실시간 스크립트 | 첨부 파일」
  - *없는 항목*: 「없는 회의록입니다」 + 「목록으로」. **리다이렉트하지 않는다**(FE §1-2)
  - *시작 중*: 「회의 시작」 비활성 + 진행 표시. 중복 요청이 나가지 않는다
- **문구·구성**
  - 헤더: 제목 28/700(L575) + 서브 13/`#757575` 「**08월 27일 (목) 09:30 예정 · <유형 이름> · <프로젝트 이름>**」(L576 — 무소속이면 프로젝트 자리를 비운다). 우측 「**회의 시작**」 34px `#7181F8` + 마이크 글리프(L580~584) 와 그 왼쪽 **`⋯` 30px 고스트**(시안 없음 → 디자인 시스템 [06] 패널 헤더 버튼 규격으로 조립. 항목 「**삭제**」 `#E2685B` → U-5. SPEC-004 U-5 「마우스가 없는 경로도 남는다」와 같은 이유)
...
- **CTA**
  - 「**회의 시작**」 → `POST /api/meetings/{id}/start`(§4). `200` 이면 `status='recording'` 이고 화면이 **SPEC-007 U-1 로 바뀐다** — 마이크 권한·WS 연결·웜스타트는 그 시점부터 SPEC-007 의 계약이다
  ...
  - `⋯` → 「삭제」 → U-5
- **기대 결과**: 안건은 **사람 트랙(`track='human'`)** 에 쌓이고 회의 시작 시 웜스타트 컨텍스트로 AI 에 전달된다(DEC-003 §8 「미리 작성된 안건」 · SPEC-007). 자동 저장 실패는 **SPEC-002 U-7 규격**(토스트 + 그 행 실패 표시 + 「다시 저장」, 자동 재시도 없음). 회의를 시작하면 다시 `scheduled` 로 돌아오지 않는다(DEC-003 §4 상태 흐름은 한 방향)
...
| **1280 ~ 1439** | 사이드바 숨김 + 햄버거, 여백 48. **좌 목록 400 고정 + 우 미리보기 유동** | 좌 패널 유동 + **우 패널 400 고정**(FE §7-1 「좌 유동 + 우 400」) | **전체 화면** — 스크림 없음, 헤더 좌측 `←`(SPEC-003 U-11 과 같은 규칙). U-3 의 2열(프로젝트·유형)은 **1열로 쌓인다** |
| **≥ 1440** | 사이드바 200, 여백 80→240 보간. **좌 500 고정 + 우 유동** | 좌 유동 + **우 464 고정** | 840 고정 |
...
- **상태**
  ...
  - *정상*: 패널 유동(L148, r16 `#D9D9D9`) — 헤더 64(L150~158) → 탭 44(L160~163) → 본문 padding 28/32(L165)
  - *로딩*: 본문 스켈레톤
- **문구·구성**
  - 헤더: 제목 16/700(L152) + 일시 12/`#9EA2AE` 「**08월 27일 (목) 09:30 – 10:30**」(L153 — **「· 회의실 A」는 그리지 않는다**, DEC-003 §1 표). 우측 「**상세보기**」 30px `#D9D9D9`(L156)
  - 탭: 「**회의록**」 / 「**원문**」(L161~162). 원문 = 트랜스크립트(DEC-003 §6 「확정 토큰을 타임코드·화자와 함께 보존」)
  - 본문은 **상태로 갈린다**:
    ...
    | `recording` | 「**기록 중입니다**」 + 「상세보기」 안내. **미리보기는 스트림을 열지 않는다** — WS 는 회의당 하나이고 상세 화면이 소유한다(BE §5-1 · FE §8) | 〃 |
    | `generating` | 「**회의록 생성중**」 + 진행 표시(SPEC-008 U-1 의 문구를 그대로 쓴다) | 〃 |
    | `ended` + `integrationState=succeeded` | **AI 한 줄 요약 바**(아래) 아래에 **통합본(`merged`) 트리** — 안건 > 줄. 렌더 규격은 **SPEC-008 이 정본**이고 이 패널은 그것을 **읽기 전용으로 그대로** 그린다. 하단 첨부 행 | 트랜스크립트 — **SPEC-007 스크립트 패널 규격**을 읽기 전용으로 |
    | `ended` + `integrationState=failed` | 요약 바 자리 없이 **사람 원본(`human`) 트리** + 「통합 정리 실패」 캡션. 「다시 생성」은 **여기 없다** — 전체 페이지·상세 드로어(SPEC-008 U-2·U-4)에서 | 〃 |

  - **AI 한 줄 요약 바**(시안 L167~174 의 「요약」 문단 자리 · 규격은 디자인 시스템 **[09] L727~733** — DEC-003 §1 표 「AI 한 줄 요약 바」 2026-09-06): 높이 56 · r12 · 배경 `#EEF1FE` · 테두리 `#C9D1FB` · 배지 「**AI 한 줄 요약**」(22px 흰 배경 · 테두리 `#C9D1FB` · 11/700 `#4A55B8`) + `headline` 한 문장 14/600 `#3D46A6`(**넘치면 말줄임**, 줄바꿈 없음) + 우측 「**안건 n · 결정 n · 액션 n**」 12/`#5F6470`(`mergedSummary` — §4). **한 개만**, 탭 위 본문 첫 요소. `headline` 이 `null` 이면(통합 전·실패) 바를 그리지 않는다 — 빈 바를 두지 않는다. **문단 요약(시안 L173 의 두세 문장)은 그리지 않는다** — 한 줄뿐이다(§7)

  - **「요약 · AI 생성」 문단(L167~174)은 한 줄 요약 바로 대체한다** — 「AI 생성」 배지는 「AI 한 줄 요약」 배지가, 문단은 한 문장이 자리를 잇는다(§7). 안건 상태 배지(「완료」/「**대기**」 L187·L214 — 문구는 SPEC-007 U-2 와 통일, §7)·집계 캡션(L180)은 통합본 렌더의 일부라 SPEC-008 이 정한다
- **CTA**: 「상세보기」 → `/meetings/detail?id=`(상태에 따라 U-4 / SPEC-007 / SPEC-008). 안건·첨부·줄·요약 바를 여기서 누를 수 없다
- **기대 결과**: 행을 바꾸면 패널이 그 회의로 바뀐다. 삭제·생성·상태 변화가 목록에 반영되면 패널도 같이 바뀐다(`['meetings','detail', id]` 캐시를 상세 페이지와 공유한다 — FE §3-3)
...
5. 「상세보기」 → 시작 전 화면. 상태 바에 「기록 대기 00:00:00」.
...
1. 「회의 시작」 → 버튼이 진행 표시로 바뀐다.
2. 상태가 `recording` 이 되고 같은 화면이 회의 중(SPEC-007)으로 바뀐다. 목록의 그 행에는 「기록 중」 dot 이 붙는다.
...
| POST | `/api/meetings/{id}/start` | **상태 전이 `scheduled → recording`** — SPEC-007 의 진입점 | 세션 |
...
- **이 표에 없는 회의 표면** — `POST …/end`(202 + jobId) · 줄 컬렉션 · `WS …/stream` · 트랜스크립트 조회 · `GET /api/jobs/{id}` 는 **SPEC-007·008** 이 정의한다(`backend/README.md` §6·§10). 이 spec 은 그 결과를 U-8 에서 읽기 전용으로 그릴 뿐이다
- **상태 전이는 전용 엔드포인트**다 — `PATCH` 에 `status` 를 실으면 `422`(업무와 같은 규칙 — BE §10). `/start` 외의 전이는 `/end`(SPEC-007) 와 서버 내부(`generating → ended`, SPEC-008)뿐이다
...
| 필드 | 뜻 | 채우는 곳 | 읽는 곳 |
|---|---|---|---|
| `status` · `integrationState` | DEC-003 §3·§4 | 이 spec(`/start`) · SPEC-007(`/end`) · SPEC-008(통합) | 전부 |
| `startAt` · `endAt` · `durationMinutes` | 예정 일시 · 길이(분, **파생** — G-7) | 이 spec | SPEC-008 헤더 「60분」 · SPEC-009 |
| **`recordingStartedAt`** | `/start` 가 성공한 **실제 시각**. 시작 전 `null`. 경과 시간과 `atMs` 오프셋의 기준점(M-11) | 이 spec(`/start`) — **ERD 변경 필요**(`meeting.recording_started_at`, SPEC-007 §7-C) | SPEC-007 상태 바 · 근거 칩 · SPEC-008 |
| **`headline`** | **AI 한 줄 요약** — `meeting.ai_headline`. 통합 전·실패 시 `null` | SPEC-008 종료 파이프라인 ②(통합본과 **같은 응답**, 별도 호출 없음 — DEC-003 §1 표 · §4 L103) | 이 spec 목록 셋째 줄 · U-8 요약 바 · SPEC-008 상단 바 |
| **`mergedSummary`** | `{ agendaCount, decisionCount, actionCount, integratedAt }` — 요약 바 우측 「안건 n · 결정 n · 액션 n」. **파생**, 통합 전 `null` | SPEC-008(정의 — `merged` 안건 수 · `decision` 수 · `action`+`task` 수) | 이 spec U-8 · SPEC-008 |
| `agendas.{human,ai,merged}[]` | 트랙별 「안건 > 줄」 트리. 항목 = `id` · `track` · `title` · `orderIndex` · `state`(`active`\|`done`\|`next`\|`null`) · `sourceAgendaId`(AI·통합 안건이 미러하는 사람 안건, `null`=신설) · `lines[]` | 이 spec(`human` 생성) · SPEC-007(`ai` 증분 · `state`) · SPEC-008(`merged`) | 전부 |
| `agendas[].lines[]` | `id` · `track` · `agendaId` · `kind` · `content` · `detail` · `evidence[]` · `orderIndex` · `taskId` · `pendingChange` · **`sourceHumanLineId` · `sourceAiLineId`**(통합본 계승 — SPEC-008) · **`task`** 요약(`{ id, title, status, dueDate, isDeleted }` — `kind=task` 일 때, SPEC-008) | SPEC-007 · SPEC-008 | SPEC-007 · SPEC-008 · 이 spec U-8(읽기 전용) |
| `attachments[]` | `id` · `kind` · `name` · `documentId` · `folderPath` · `sizeBytes` · **`updatedAt`**(문서면 문서 수정 시각, 링크면 첨부 시각) · `url` · `isDeleted` | 이 spec | 전부 |
| `latestBatchSeq` | 성공한 배치의 최대 `seq`(**파생**, M-6-a). 시작 전 `0` | SPEC-007 | SPEC-007 「배치 n회 반영」 · SPEC-008 |
| **`finalBatchState`** | 최종 배치 결과(`succeeded`\|`failed`\|`null`) — **파생** | SPEC-008 | SPEC-008 AI 탭 안내 바 |
| **`activeJobId`** | 진행 중인 `meeting_finalize` job(**파생**, 없으면 `null`) — `generating` 재진입 폴링 | SPEC-008 | SPEC-008 · 이 spec U-8(`generating` 표시) |

- **트랙별로 갈라서 준다** — `agendas.human` · `ai` · `merged` 각각이 「안건 > 줄」 트리 하나다(BE §10 · M-5-a). **줄은 안건 안에 중첩**한다(트랙 아래 `lines[]` 를 따로 두지 않는다 — SPEC-008 이 이 형태를 따랐다). 시작 전에는 `human` 만 차고 `lines` 는 비어 있다. **`ai`·`merged` 는 항상 키가 있고** 비어 있을 때 `[]` 다 — 화면이 키 유무로 분기하지 않는다. `merged` 는 `integrationState='succeeded'` 일 때만 찬다(SPEC-008)
...
- **이 요청은 상태 전이만 한다.** WS 연결·마이크·웜스타트(AI 세션 생성)는 이 응답 뒤 SPEC-007 이 정하는 순서로 일어난다. `ai_session_id` 를 언제 만드는지는 SPEC-007 의 계약이다
- 전이 시각을 `startAt` 에 덮어쓰지 않는다 — `startAt` 은 **예정 일시**다. 전이 시각은 **`recordingStartedAt`** 에 쓴다(같은 트랜잭션) — 트랜스크립트 `atMs` 오프셋의 기준(M-11)이고 SPEC-007 이 경과 시간·근거 칩 시각에 쓴다
...
sequenceDiagram
    ...
    alt scheduled
        BE-->>FE: 200 MeetingDetail (status=recording)
...
stateDiagram-v2
    ...
    scheduled --> recording: POST /start (이 spec)
    recording --> generating: POST /end → 202 jobId (SPEC-007)
    generating --> ended: 통합 성공 · integrationState=succeeded (SPEC-008)
    generating --> ended: 통합 실패 2회 재시도 후 · integrationState=failed (SPEC-008)
    scheduled --> [*]: 삭제(소프트)
    ended --> [*]: 삭제(소프트)
```
...
- `integrationState` 는 `status` 와 **다른 축**이다(`not_started|running|succeeded|failed` — ERD). 목록의 「통합 실패」 표기와 SPEC-008 의 「다시 생성」 배너는 `ended + failed` 조합에서만(M-4)
...
| 회의록(상세) | 위 + `durationMinutes`(파생) · `recordingStartedAt` · `mergedSummary`(파생 · null 가능) · `agendas: { human[], ai[], merged[] }` · `attachments[]` · `latestBatchSeq`(파생) · `finalBatchState`(파생) · `activeJobId`(파생) · `createdAt` — **필드 소유 표(§4)가 정본** |
| 안건 | `id` · `track` · `title` · `orderIndex` · `state`(`active`\|`done`\|`next`\|null) · `sourceAgendaId` · `lines[]` |
| 줄 | `id` · `track` · `agendaId` · `kind` · `content` · `detail` · `evidence[]` · `orderIndex` · `taskId` · `pendingChange` · `sourceHumanLineId` · `sourceAiLineId` · `task`(요약) — **의미는 SPEC-007·008** |
...
- **`/start` 는 상태 전이만 한다.** 외부 호출(Soniox·codex)을 이 요청 안에서 하지 않는다 — 외부 호출 중에는 트랜잭션을 열어 두지 않는다(BE §7). WS·웜스타트는 SPEC-007
...
- [ ] 「상세보기」로 들어간 시작 전 화면에 「기록 대기 00:00:00」 상태 바와 「회의 시작」 버튼이 있고, **「회의 정보 수정」 버튼과 「내 목소리 등록됨」 푸터가 없다**
...
- [ ] 종료된 회의를 목록에서 고르면 우측 패널 본문 맨 위에 **「AI 한 줄 요약」 바 한 개**가 보이고, 그 문장이 목록 행 **셋째 줄과 같다**. 문단 요약은 없다. 통합 전 회의에는 바가 **없다**
...
| **우측 미리보기 패널의 상태별 본문** | **U-8** — 이 spec 은 프레임과 `scheduled` 본문만 정하고, `ended` 통합본·트랜스크립트 렌더는 SPEC-008·007 규격을 **읽기 전용으로 그대로** 쓴다. 미리보기는 WS 를 열지 않는다 |
...
| **`/start` 의 범위** | 상태 전이 + `recordingStartedAt` 기록만. WS·마이크·웜스타트·`ai_session_id` 생성 시점은 SPEC-007 |
...
| 5 | **SPEC-008 정합 #6** — `MeetingDetail` 에 `durationMinutes` · `activeJobId` · `finalBatchState` · `mergedSummary` · 줄 `sourceHumanLineId` · `sourceAiLineId` · `task` 요약 반영, `PATCH` 의 `recording` 거부 | **반영했다**(§4 필드 소유 표 · 상태별 허용 표) | 확인만 |
...
| AI 한 줄 요약 바 — 표시는 목록 셋째 줄 + 미리보기 패널, 생성은 SPEC-008 | DEC-003 §1 표 「AI 한 줄 요약 바」(2026-09-06) · §4 L103 · [09] L727~733 · ERD `ai_headline` |

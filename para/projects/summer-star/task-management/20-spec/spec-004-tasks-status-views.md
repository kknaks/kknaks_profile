...
- **데이터 범위 — 칸반은 페이지를 쓰지 않는다** (2026-09-06 확정, 구현 중 드러난 공백)
  - **그 달 전체를 한 번에 그린다.** 리스트의 페이지 크기를 물려받으면 **13번째 업무부터 화면에 나타날 길이 없고**(페이지네이션 UI 는 리스트 전용이다) 사용자는 그 달 업무가 12건뿐이라고 읽는다
  - **상한은 500 건.** 개인 한 사람의 한 달 업무가 그걸 넘는 일은 실질적으로 없지만, 넘으면 컬럼이 무한히 길어지므로 선을 둔다. **`total > 500` 이면 보드 하단에 「이 달 업무가 n건이라 500건까지만 그립니다 · 리스트에서 보기」** 를 둔다 — 조용히 자르지 않는다
  - 완료 컬럼의 「8월 12」와 하단 「8월 완료 12건」은 **`statusCounts.done`** 에서 온다(§4). **받아온 카드를 세지 않는다** — 상한에 걸리면 두 수가 갈린다
...
- **상태**
  - *드래그 시작*: 잡은 카드가 **고스트**가 된다 — **브라우저 기본 고스트를 쓴다**(2026-09-06 정정). **회전 효과 없음**만 규격이다
    > 원래는 그림자 `0 16px 40px rgba(0,0,0,0.16)` 을 요구했으나 **HTML5 DnD 의 기본 고스트는 스타일을 온전히 싣지 못한다.** 강제하려면 복제 노드를 만들어 `setDragImage` 로 넘겨야 하는데 **macOS·Windows 에서 다르게 그려진다**(둘 다 배포 대상이다). 화면 하나를 위해 플랫폼별로 갈리는 코드를 두지 않는다.
  - *원래 자리*: **플레이스홀더** — 1px 점선 `#C9D1FB` + 배경 `#F8FAFF`, 카드와 같은 높이
  - *드롭 가능 컬럼*: 컬럼 배경이 `#F1F2FE` 로 바뀌고 1px `#7181F8` 테두리. **삽입 가이드선은 두지 않는다**(2026-09-06 정정)
    > 원래는 「삽입 위치에 2px `#7181F8` 가이드선」이었다. **카드 순서를 저장하지 않기 때문에** 뺐다 — 컬럼 안 정렬은 서버 정렬(기한·수정일)을 그대로 따르므로, 가이드선은 **저장되지 않는 위치를 약속하는 표시**가 된다. 놓고 나면 카드가 다른 자리로 가서 사용자가 「안 먹었다」고 읽는다. **컬럼 하이라이트가 「여기에 놓인다」를 이미 말한다.**
...
- **상태** — **헤더는 뷰와 무관하게 하나다**(2026-09-06 사용자 확정 · 아래 박스)
  - 기간 스테퍼 「‹ 2026년 8월 ›」 — 타이틀에 **붙여서** 좌측 그룹. 항상 노출한다([10] 「스코프 표시 필수」)
  - **라벨을 누르면 날짜 범위를 고른다**(2026-09-06 사용자 확정 · 아래 박스)
  - 유형 탭(전체 + 동적 유형) — **리스트·칸반 공통**
  - 정렬 「정렬 · 종료일」 — **리스트·칸반 공통**. 「기한 빠른 순」(기본) / 「기한 늦은 순」 / 「최근 등록순」
  - 상태 필터 「상태 전체」 — **리스트·칸반 공통**
  - ~~「오늘」 버튼~~ → **없다**(시안에 없다)
  - ~~칸반은 프로젝트 필터~~ → **없다**. 아래 박스 참조

> **헤더는 하나다 — 뷰가 헤더를 바꾸지 않는다** (2026-09-06 사용자 확정)
>
> 정본은 `업무 화면 정의서.dc.html` **리스트 화면(24~88줄)의 헤더**다. 구성은 —
> breadcrumb(「홈」은 링크, 「내 업무」는 지금 여기 — MF-7 · FE §6-3. 목록 화면이라 「←」는 없다) / 좌측 `내 업무 + ‹ 2026년 8월 ›` / 우측 `[리스트|칸반] · 정렬·종료일 · 상태 전체 · + 새 업무` / 그 아래 유형 탭.
>
> **뷰 토글은 「무엇을 보여줄까」만 고른다.** 필터·정렬·유형은 뷰와 무관하게 **같은 자리에 같은 구성**으로 있다.
> 이전 결정(A-17 「칸반 헤더 필터 = 프로젝트」·DEC-002 2026-09-05)은 **이 결정으로 대체된다.**
>
> 알려진 결과 — 칸반은 컬럼 축이 상태라 상태 필터를 걸면 **해당 컬럼 하나만 남는다.** 기능상 문제는 없고,
> 사용자가 이 동작을 알고 통일을 선택했다.

> **「내 업무」는 오늘 업무다 — 기본 단위가 일이다** (2026-09-06 사용자 확정 · DEC-002)
>
> **기본 진입은 「오늘 하루」다.** ~~월 단위~~를 대체한다. 오늘 화면에 뜨는 것은 셋이다 —
> **기한이 오늘인 업무 · 기한 없는 업무(매일) · 지연 업무(미완료인 동안 매일)**.
> 완료·취소는 **그날 것만** 뜬다.
>
> 근거 — 기한 없는 업무가 안 뜨면 **영영 안 보이고 기한을 정할 계기가 생기지 않는다.**
> 지연 업무는 같은 논리가 더 강하게 걸린다: 어제 기한이던 업무가 오늘 범위 밖이면 **가장 봐야 할 것이 사라진다.**
>
> **§4 의 서버 조회 규칙이 바뀐다** — 지금 `from`·`to` 는 기한 범위로만 거른다.
> 「기한 없음」·「지연」을 오늘에 얹는 규칙은 **백엔드 별도 발주**로 처리한다. 프론트가 클라이언트 필터로 흉내내지 않는다.

> **기간 스테퍼는 날짜 범위까지 고른다** (2026-09-06 사용자 확정 — 시안·기존 spec 에 없던 신설)
>
> - **`‹` `›` 는 기간 이동**이다. 기본 범위가 **하루**이므로 **하루씩** 움직인다
> - **라벨을 누르면 달력 팝오버**가 열려 **시작일–종료일**을 직접 고른다. 팝오버 하단에 「**오늘**」·「이번 주」·「이번 달」 바로가기를 둔다
> - `‹` `›` 는 **고른 범위와 같은 길이만큼** 이동한다
> - 라벨은 세 가지다 — 하루면 「**2026년 9월 6일**」 · 범위면 「**09.01 – 09.15**」 · 한 달 경계와 정확히 같으면 「**2026년 9월**」
>   (같은 값을 두 가지로 쓰지 않는다)
>
> **서버 계약은 바뀌지 않는다** — §4 의 `from`·`to` 는 이미 날짜다. 지금까지 프론트가 달 경계로만 채워 보냈을 뿐이다.
> 팝오버 규격은 디자인 시스템 [07] Popover 를 따른다(200~400 · 스크림 없음 · 트리거 아래 8px · 공간 없으면 위로).
>
> **G-3(칸반 데이터 범위 — 그 달 전체, 상한 500, 초과 시 안내)은 「고른 범위 전체」로 읽는다.** 상한 500 과 안내는 그대로다.
  - ~~활성 필터가 있으면 타이틀 옆에 **필터 칩**과 「필터 n ✕」~~ → **삭제**(2026-09-06 사용자 확정)
- **문구**: 위와 같다
- **CTA**: 탭·정렬 모두 팝오버 또는 탭 클릭 즉시 반영. **「적용」 버튼을 두지 않는다**

> **활성 필터 칩을 두지 않는다** (2026-09-06 사용자 확정 · 코디 오적용 교정)
>
> 이 칩은 **내 업무 시안에 없다**(`업무 화면 정의서.dc.html` 「필터」 검색 0건).
> 출처는 **디자인 시스템 [11] CALENDAR** 의 「Type Filter · 범례 겸 필터」이고, 거기 설명 문장이
> 「활성 필터 칩은 타이틀·기간 스테퍼 옆에 두고, **카드 우측 상단**은 필터 n ✕로 해제합니다」다 —
> **유형 분포 카드가 별도 패널이라 화면 위에서 무엇이 걸렸는지 안 보이는 캘린더** 사정이다.
>
> 내 업무는 **유형 탭이 바로 아래에 있고 선택 상태를 밑줄+굵기로 이미 보여준다.** 칩은 같은 정보를
> 두 번 말한다. 코디가 캘린더 전용 패턴을 이 spec 으로 옮겨 적은 것이 원인이다.
...
- **디자인의 「1280 에서 시작일 컬럼 숨김」(P-29)이 도로 성립한다**(2026-09-06 — §A-4 번복으로 `startDate` 가 살아났다). **시작일 컬럼을 숨긴다.** 종료일(기한)은 D-day·지연 판정의 기준이라 남긴다
...
> #### `GET /api/tasks` 의 기간 조회 규칙 — **4종**
> (2026-09-06 사용자 확정 · DEC-002. 이전 「기한이 범위 안 + 기한 없으면 생성일 기준 달」을 **대체**한다)
>
> `from`·`to` 가 온다. **끝 경계는 닫는다** — `from = to = 오늘` 이 오늘 업무를 내야 한다.
>
> | # | 무엇 | 조건 |
> |---|---|---|
> | R-1 | **계획 기간이 범위에 걸침** | `COALESCE(start_date, due_date) ≤ to` **AND** `COALESCE(due_date, start_date) ≥ from` — **구간 겹침**이다 |
...
>
> **「미완료」는 `todo`·`in_progress` 다.** 정의를 코드 한 곳에 둔다.
>
> **R-1 은 「있는 쪽만으로 판단」이 아니다**(2026-09-06 백엔드 워커 지적으로 교정). 그렇게 읽으면
> **`start_date` 없고 기한이 9/20 인 업무가 오늘(9/6) 화면에 뜬다** — 「오늘 업무」가 아니다.
> DEC-002 §「오늘 화면에 뜨는 것 4종」 1번의 **「시작이 없으면 `due_date` = 오늘」** 이 정본이고, 위 COALESCE 겹침이 그것이다.
> **R-4 가 「오늘 완료 처리하면 오늘 완료 칸에 간다」를 성립시킨다** — 기한이 지났든 오늘이 아니든 실적 시각 기준이다.
> `statusCounts`·`unfilteredTotal`·scope별 `total` 도 **같은 규칙으로** 센다.
>
> **끝 경계 주의** — 지금 구현은 `< to`(열림)다. WORK-005 검수 FAIL-① 이 이 어긋남이었고(말일 업무가 통째로 사라짐),
> 일 단위로 바뀌면 **매일** 터진다. 테스트로 고정한다.
  ...
  "items": [
    {
      ...
      "startDate": "2026-08-27", "dueDate": "2026-08-29",
      "startedAt": "2026-08-27T01:12:00Z", "completedAt": null, "cancelledAt": null,
...
- **`typeCounts` 는 그 기간에 업무가 **있는** 유형만 담는다** — 0건 유형은 **행이 아예 없다**(`Task JOIN WorkType` 집계라서). **탭 목록은 화면이 유형 목록에서 그리고, 없는 유형에 `0` 을 표시한다(탭을 감추지 않는다)** — U-1 의 「유형 탭은 동적 유형 전체」와 맞춘다. 서버가 0 행을 만들어 넣지 않는다
- **`statusCounts` · `unfilteredTotal`** — 2026-09-06 신설(구현 중 드러난 공백)

  | 필드 | 무엇 | 규칙 |
  |---|---|---|
  | `statusCounts` | 상태별 총계 `{todo, inProgress, done, cancelled}` | **`typeCounts` 와 같은 결** — 기간·유형·프로젝트 필터는 반영하고 **상태 필터 자신은 반영하지 않는다.** 칸반 완료 컬럼의 「8월 12」가 여기서 온다 |
  | `unfilteredTotal` | **기간만** 적용한 총계 | U-9 「유형·상태 필터를 지우면 **n건이** 보입니다」의 `n` |

  **완료 컬럼의 수는 `items` 를 세서 만들지 않는다.** 세면 「그 달 완료 건수」가 아니라 「지금 받아온 것 중 완료 건수」가 되고, U-2 가 「**월 기준**」이라고 못박은 자리가 조용히 틀린 수를 적는다.
- `isOverdue`·`overdueDays`·`dDay`·`todoProgress` 는 **파생값**이다(T-4 · G-7). 화면이 다시 계산하지 않는다
- **실적 3개(`startedAt`·`completedAt`·`cancelledAt`)를 목록에도 싣는다** — 칸반 완료 카드의 **「계획 대비 차이」**(`dueDate` ↔ `completedAt`)가 이걸 쓴다(DEC-002 §「칸반 카드 하단 행」). 카드마다 상세를 부르지 않게 한다
- **`dueStartTime`·`dueEndTime` 은 제거됐다**(2026-09-06 · DEC-002). 업무는 날짜 단위다
...
- 되돌릴 수 있는 조건: ① 마지막 로그가 상태 전이이고 ② 그 뒤에 **로그를 남기는** 다른 변경이 없으며 ③ 전이 후 **4초 이내**이고 ④ **그 전이가 「취소」가 아니다.** 아니면 거부(§Case Matrix)
  - **④ 실행취소는 「완료」 전용이다** (2026-09-06 추가 — 백엔드 워커가 찾은 공백).
    시안·이 spec 어디에도 **「취소 직후 토스트」가 없다.** 완료 토스트에만 「실행취소」가 붙고(U-6), 거부 토스트에는 붙이지 않는다.
    취소는 **모달 + 사유를 지나는 신중한 조작**이라 4초짜리 되돌리기를 붙이지 않는다 — 되돌리려면 **상태 팝오버에서 「진행중」을 고른다**(S-3 3번).
    **서버가 명시적으로 막는다.** 이전 구현은 「취소 사유 로그가 전이 로그 뒤에 붙어 ①이 깨진다」는 **부작용**으로 막고 있었고,
    그래서 **사유 기록 체크를 끄면 되돌려졌다.** 「사유를 남겼는지」가 「되돌릴 수 있는지」를 정하는 것은 화면에서 설명할 수 없다.

    > **판정 근거는 「지금 취소 상태인가」(`task.status`)다. 「마지막 전이가 취소로 갔는가」가 아니다** (2026-09-06 · 백엔드 워커 교정).
    > 로그로 판정하면 **사유 로그가 전이 로그 뒤에 붙는 경우 ①이 먼저 걸려** 이 규칙에 닿지도 못한다 —
    > 사유 기록을 켜면 `undo_not_available`, 끄면 `cancel_undo_not_allowed` 가 나와 **비일관이 「어떤 코드인가」로 자리만 옮긴다.**
    > **상태는 로그 순서와 무관하다.** 두 판정은 같은 집합을 가리킨다(상태를 바꾸는 경로가 전이뿐이라 「마지막 전이가 취소」 ≡ 「지금 취소」)
    > — 흔들리지 않는 쪽을 쓴다. 검사는 **로그를 읽기 전에** 한다.
    >
    > **취소에서 *나오는* 전이(취소 → 시작전)의 실행취소는 막지 않는다.** 되살린 직후 상태는 취소가 아니므로 이 규칙에 걸리지 않는다 —
    > 취소를 막는다고 취소 근처를 전부 막으면 **잘못 누른 되살림을 무를 길이 없어진다.**
  - ②의 「로그를 남기는」은 **2026-09-06 에 좁힌 문구**다. 판정이 「마지막 로그가 전이인가」이므로 **제목·배경 인라인 편집처럼 로그를 안 남기는 변경은 실행취소를 막지 않는다.** 되돌리는 것이 `status` 하나뿐이라 그게 옳다 — 넓은 문구로 두면 「제목을 고치면 실행취소가 막혀야 하나」를 다음 work 가 다시 묻는다
...
- **같은 상태로의 전이는 그래프 밖이다** — 서버는 `409 invalid_status_transition` 을 낸다. **화면은 그 요청을 애초에 내지 않는다**: 팝오버의 현재 값 항목은 비활성이고, 칸반에서 **같은 컬럼에 다시 놓으면 요청이 0건**이다. 사용자가 아무것도 안 했는데 에러 토스트를 보는 자리이기 때문이다(2026-09-06 확정). **회의록(WORK-008)의 「업무 갱신」도 이 경계를 지켜야 한다** — 이미 완료인 업무를 완료로 보내면 409 가 사용자에게 보인다
...
| `cancel_undo_not_allowed` | `409 {detail:"취소는 실행취소로 되돌릴 수 없습니다", code:"cancel_undo_not_allowed"}` · **상태·실적이 그대로다** | 토스트 같은 문구 + 「상태에서 직접 되돌릴 수 있습니다」 | 화면 하단 토스트 |

> **왜 `undo_not_available` 을 재사용하지 않았나** (2026-09-06 · 백엔드 워커가 흠을 스스로 보고)
>
> 취소 거부는 **시간이 지난 것이 아니다.** 같은 코드를 쓰면 「되돌릴 수 있는 시간이 지났습니다」라는 **사실과 다른 문구**가 나간다.
> 문구를 「지금은 되돌릴 수 없습니다」로 넓히는 안도 있었으나, 그러면 **실제로 자주 보이는 만료 케이스의 안내가 나빠진다** —
> 만료는 「기다렸다가 놓친 것」이라 시간을 말해 주는 편이 낫다.
>
> **화면에 소비자가 없다는 것은 코드를 합칠 이유가 아니다.** 지금 도달 경로는 낡은 클라이언트·API 직접 호출·회의록(WORK-008)뿐이고,
> 그 자리는 **사람이 디버깅하는 자리**다. 거기서 틀린 사유를 읽으면 엉뚱한 데를 판다.
>
> 두 번째 줄(「상태에서 직접 되돌릴 수 있습니다」)은 **두 경우에 다 맞다** — 그건 그대로 쓴다.
...
| 리소스 | 외부에 드러나는 필드 |
|---|---|
| 목록 항목 | `id` · `title` · `status` · `workType`(요약) · `project`(요약·null 가능) · **`startDate`** · `dueDate` · **`startedAt`** · **`completedAt`** · **`cancelledAt`** · `dDay` · `isOverdue` · `overdueDays` · `memoCount` · `todoProgress` · `cancelReason` |
...
| **P-28~31 반응형의 유동 재해석** | **U-13 에서 확정** — 2026-09-06 §A-4 번복으로 시작일이 살아나 **디자인 원본대로 「1280 에서 시작일 컬럼 숨김」** 이다. 칸반은 320 고정 + 가로 스크롤 유지 |

...
- Architecture reference: `system/README.md` §흐름 ③ · 불변식 1~10 · `backend/README.md` §5-1(STT 중계) · §5-2(AI 작업) · §8-2·§8-3(에러) · §10(API 표면) · `frontend/README.md` §3-5 · §4-2(WS 첫 프레임 인증) · §6-2 · §7 · `database/domains/meeting.md` M-1~M-18
...
- **AI 증분은 즉시 반영하고 몇 번째 배치까지 반영됐는지 표시한다** — 「배치 2회 → 3회 → 종결」. 버퍼링하지 않는다(DEC-003 §4 L97).
...
- **수치는 DEC-003 §STT L164 가 확정했다** — 확정 발화 600자 · 안건 전환 즉시(미처리 80자 미만이면 생략) · 상한 180초 · 배치 타임아웃 120초 · 회의당 AI 세션 하나. 실측 전 값이라 조정 가능하나 **계약은 불변**이다.
...
- **사람 트랙**(#5) — `/` 프롬프트로 안건·줄(논의 · 결정 · **업무** · 액션) 직접 입력. 안건 전환 · 완료
- **AI 트랙 배치 요약**(#6) — 웜스타트 컨텍스트 · 배치 트리거 · 출력 검증 · 증분 push · 배치 회차 표시. **회의 중 증분만**
...
- **회의 종료(`/end`) · 「생성중」 · 최종 배치 · 통합본 · **AI 한 줄 요약 바** · 편집(인라인 수정 · 종류 전환 · 드로어 3종 · **줄 삭제 · 안건 이름 수정**) · 업무 생성/갱신 버튼** — SPEC-008. 회의 중 화면의 「회의 종료」 버튼은 **자리만** 두고 동작은 SPEC-008 이 정한다. 줄 삭제·안건 이름 수정은 **종료 후 편집에만** 있다(DEC-003 §1 표 「종료 후 편집 범위」 · §5 수정(U), 2026-09-06)
...
회의 상세 라우트(`/meetings/detail?id=` — FE §1 · F-10 「상태로 갈린다」)가 `status='recording'` 일 때 이 화면이다. 사이드바 200 + breadcrumb 「홈 › 회의록 › 회의 중」(L784~790) + 타이틀 행(L792~807) + 상태 바(L809~831) + 2패널.

```text
+──────────+──────────────────────────────────────────────────────────────+
│ 사이드바  │ 홈 › 회의록 › 회의 중                                        │
│ 200       │ 제목 28/700                          [⏸ 일시정지] [■ 회의 종료]│
│           │ 08월 27일 (목) 09:30 시작 · <유형명>                          │
...
- **상단 바(top 150 · 1640×56 · r12)는 회의 생애 전체에서 한 자리다**([09] L733). 회의 중은 이 spec 의 상태 바(U-1), 종료 후는 SPEC-008 의 **AI 한 줄 요약 바**([09] L727~733 · DEC-003 §1 표 「AI 한 줄 요약 바」, 2026-09-06)가 같은 자리를 쓴다. **회의 중에는 한 줄 요약을 그리지 않는다** — `headline` 은 통합본과 함께 생기므로(DEC-003 §4 종료 파이프라인) 회의 중에는 항상 `null` 이다
...
- **문구**: 타이틀 = 회의 제목(28/700 — L794). 부제 = 「`MM월 DD일 (요일) HH:MM` 시작 · `<유형명>`」(L795 에서 **「회의실 A」를 뺀** 형태 — DEC-003 §1 표 「장소」). breadcrumb 마지막 「회의 중」(L789)
- **CTA**
...
- **상태**
  ...
  - *줄*: 안건 아래 `margin-left:37 · border-left 2px`(활성 안건 `#C9D1FB` · 그 외 `#EBEBEB` — L857·L885) + 줄 행 padding 6/8 r8(L858~871). **라벨 폭 34 · 11/700**, 색은 종류별 — 논의 `#9EA2AE` · 결정 `#1663B5`(본문 600 · 배경 `#F8FAFF`) · **업무 `#5F6470`(본문 600)** · 액션 `#4B52A8`(본문 600)([09] L660~678). 우측 시각 = 줄 `createdAt` `HH:MM`
...
- **문구**: 탭 안내 바(L839~841) 좌측 「입력창에서 **/** 를 입력해 안건 · 논의 · 결정 · **업무** · 액션을 고릅니다」(시안 L840 에 「업무」를 더한다 — DEC-003 §1 표 「업무 줄」), 우측 「자동 저장 · `HH:MM`」(이 세션에서 마지막으로 성공한 줄·안건 쓰기 시각. 아직 없으면 숨김)
...
### U-3. 프롬프트 바 · `/` 종류 팝오버 (L918~972)

- **상태**
  - *기본*: 하단 고정 바 h56 r14, 테두리 `#7181F8`, 그림자(L955). 좌측 **활성 안건 칩**(h26 `#EEF1FE`/`#4A55B8` 「안건 n ▾」 — L959~962) + 입력(15px) + 우측 현재 시각(`HH:MM` `#C9CBD1` — L968) + 전송 버튼(40×40 `#7181F8` ➤ — L969~971). 플레이스홀더 「그대로 입력하면 논의로 추가 · / 로 안건 이동」(L966). **좌측 「+」 버튼(L956~958)은 두지 않는다**(§7-B)
  - *종류 선택 중*: 입력 맨 앞에 `/` 를 치면 팝오버 300 이 바 위에 뜬다(L920~952). 머리 「안건 n 아래에 추가」 + 안건 제목 캡션(L921~924) → 항목 4: 「**논의**」(칩 「논」 `#F1F2F5`/`#757575`) · 「**결정**」(「결」 `#EAF4FF`/`#1663B5`) · 「**업무**」(「업」 `#F1F2F5`/`#5F6470` — 시안 L925~936 에 없는 **누락분을 [09] L668~672 규격으로 추가**) · 「**액션**」(「액」 `#F1F2FE`/`#4B52A8`) → 구분선 → 「**새 안건**」 + 캡션 「다음 주제로」(L938~942) → 구분선 → 「다른 안건으로 이동」 + 활성 아닌 안건 행(「안건 n」 + 제목, h32 — L944~951). 현재 종류 행은 배경 `#F1F2F5`. `↑↓` 이동 · `Enter` 선택 · `Esc` 닫기
  - *종류 고른 뒤*: 칩 옆에 종류 라벨(예 「결정」)이 입력 앞에 붙고 텍스트를 이어 친다. 종류를 안 고르면 **논의**다
  - *새 안건 고른 뒤*: 입력이 안건 제목 입력으로 바뀐다(플레이스홀더 「안건 제목」). `Enter` 로 만든다
  - *안건 없음*: 칩이 「**안건 선택**」(`#F1F2F5`/`#757575`)으로 바뀌고 **전송 버튼 비활성**. 팝오버에는 「새 안건」만 있다 — 줄은 항상 안건에 속한다(DEC-003 §3 · M-5)
  - *전송 중*: 전송 버튼에 진행 표시, 입력 잠금. **중복 전송이 나가지 않는다**
  - *전송 실패*: 입력 내용 유지 + 토스트 「저장하지 못했습니다 · 다시 보내 주세요」. 전송 버튼을 다시 누르면 재요청. **자동 재시도 없음**(§4 Case Matrix)
- **문구**: 위 상태 표의 것 전부. 「업무」 항목 캡션 없음(연결은 종료 후 편집 드로어 — SPEC-008 · M-14)
- **CTA**
  - 전송(➤ 또는 `Enter`) → `POST /api/meetings/{id}/lines { agendaId, kind, content }`. `Shift+Enter` 는 줄바꿈이 아니라 무시한다(줄 본문은 한 줄이다 — §4 Validation)
  - 「새 안건」 → `POST /api/meetings/{id}/agendas { title }`(SPEC-006 표면) → 201 → `PATCH …/agendas/{id} { state:"active" }`. 두 요청이 순서대로 나간다
...
- **상태**
  - *첫 배치 전*: 트리에 **사람 안건이 미러된 AI 안건이 아직 없으므로** 본문 중앙에 「기록이 쌓이면 요약이 생성됩니다」(13 `#C9CBD1` — L1151 문구). 안내 바 우측 「**첫 배치를 기다리는 중**」
  - *배치 반영*: 안내 바 우측 「**배치 `n`회 반영 · `HH:MM`**」(`n` = 서버가 준 `seq`, 시각 = 그 push 를 받은 시각). 트리에 AI 안건 → AI 줄. **안건 배지**: 사람 안건을 미러한 AI 안건(`sourceAgendaId` 있음)은 원본 사람 안건의 `state` 배지를 그대로, **AI 가 새로 만든 안건**(`sourceAgendaId: null`)은 배지 대신 캡션 「AI 안건」(11/700 `#9EA2AE`)
  - *AI 줄*: U-2 와 같은 줄 행 + **우측 끝 펼침 화살표**(22×22, hover 에서만 — [09] L700). 시각 = `createdAt`
  ...
  - *새 증분 도착*: 새 안건·줄이 **즉시** 트리에 붙는다. 사용자가 회의록 탭을 보고 있어도 AI 탭 데이터는 갱신되고 탭 라벨 옆에 **미확인 증분 점**(6px `#338BF6`)이 켜진다 — AI 탭을 열면 꺼진다. 버퍼링하지 않는다(DEC-003 §4 L97 · BE-11)
  - **기존 AI 줄은 회의 중에 바뀌지 않는다**(증분 추가만 — M-7). 사람이 AI 줄을 고칠 수도 없다
- **문구**: 탭 푸터(L1156) 「회의 종료 시 안건별 요약이 회의록에 반영됩니다」 그대로. 안내 바 좌측은 비운다(입력 안내가 없다)
- **CTA**: 펼침 화살표 · 근거 칩(U-6). 그 밖의 버튼 없음 — **회의 중 AI 줄에는 「업무 생성」·「업무 갱신」 버튼을 두지 않는다**(BASE-003 #10 은 종료 후 편집 — SPEC-008)
- **기대 결과**: 배치가 끝날 때마다 안내 바의 회차가 올라가고 트리가 자란다. AI 줄 중 `kind='task'` 는 화이트리스트 안의 `taskId` 를 갖고 라벨 옆에 그 업무의 **유형 배지**([09] L671 — `workType` 이름·색)를 단다. 화이트리스트 밖 참조는 서버가 이미 `action` 으로 강등해 내려주므로 화면에 「없는 업무」가 뜨지 않는다(DEC-003 §7)
...
- **상태**
  ...
  - *이동 후*: 우 패널이 **실시간 스크립트 탭으로 전환**(첨부 탭을 보고 있었어도)되고, `atMs ≥ fromMs` 인 첫 블록으로 스크롤해 **`[fromMs, toMs]` 와 겹치는 블록 전부**를 하이라이트한다(L1782 규격). 하이라이트는 **다른 칩을 누르거나 `Esc`** 를 누를 때까지 남는다
...
1. 프롬프트 바에 그냥 치고 `Enter` → 활성 안건(칩 「안건 2」) 아래 **논의** 줄. 안내 바 「자동 저장 · 09:34」.
2. `/` 를 치면 팝오버. 「결정」을 고르고 문장을 치고 `Enter` → 결정 줄(굵게 · `#F8FAFF`).
3. `/` → 「업무」 → 「소개서 v2 문구 정리」 `Enter` → 업무 줄(라벨 `#5F6470`). 배지도 버튼도 없다 — 연결은 종료 후.
4. `/` → 「새 안건」 → 「가격 표기 처리」 `Enter` → 안건 3 이 만들어지고 **활성**이 된다. 안건 2 배지는 「대기」.
...
1. 확정 발화가 600자를 넘으면 배치가 돈다. 결과가 오면 회의록 탭을 보고 있어도 「AI 요약」 탭 옆에 파란 점.
2. AI 탭을 열면 「배치 1회 반영 · 09:41」. 안건 1(사람 안건 미러, 배지 「완료」) 아래 논의·결정 줄, 그리고 「AI 안건」 캡션이 붙은 새 안건 하나.
3. 3분 뒤 상한(180초)으로 또 한 번 → 「배치 2회 반영 · 09:44」. 기존 줄은 그대로고 새 줄만 늘었다.
...
| POST | `/api/meetings/{id}/start` | 회의 시작 + 웜스타트 | 세션 | SPEC-006(웜스타트 **컨텍스트 내용**은 아래 「AI 배치 계약」) |
...
| AI 증분 | `{"type":"ai.batch","seq":3,"agendas":[AgendaItem…],"lines":[LineItem…]}` | 그 회차에 **새로 생긴** AI 안건·줄만. 배치가 끝나는 즉시(BE-11). 실패·폐기된 배치는 프레임이 없다 |
...
// 201 LineItem
{
  ...
  "taskId": null, "task": null, "pendingChange": null,
  "createdAt": "2026-08-27T00:36:10Z"
}
```

- 사람 줄은 회의 중 **항상** `detail:null · evidence:[] · taskId:null · pendingChange:null` 이다(M-14). 값이 차는 것은 종료 후 편집·통합(SPEC-008)
...
// LineItem (track=ai)
{
  ...
  "orderIndex": 2,
  "taskId": 45, "task": { "id": 45, "title": "소개서 v2 문구 정리", "workType": { "id": 3, "name": "문서·보고", "colorToken": "steel" } },
  "pendingChange": null,
  "createdAt": "2026-08-27T00:41:02Z"
}
```

- AI 안건의 `state` 는 `null` 이다. 배지는 `sourceAgendaId` 가 가리키는 사람 안건의 `state` 로 그린다(U-4). `sourceAgendaId` 는 **ERD 변경 필요**(§7-C)
- AI 줄의 `pendingChange` 는 회의 중 항상 `null` — 업무 갱신 대기분은 편집 드로어(SPEC-008)가 만든다

### AI 배치 계약 — 백엔드 ↔ open-kknaks 워커

외부 계약은 아니지만 **기능명세 #6 의 동작이 이 입출력에 걸리고, 검증 실패 처리(DEC-003 §7)가 이 형태에 묶인다.** 필드 수준만 둔다. 프롬프트 본문·`output_schema` 파일은 구현 몫이다.

**웜스타트**(회의 시작 시 1회 — `/start` 안에서, 결과는 무시 — DEC-003 §STT)

| 항목 | 내용 | 근거 |
|---|---|---|
| 세션 | 새 세션. 돌아온 `session_id` 를 회의에 저장하고 이후 모든 배치가 `resume` 한다 | M-12 · BE §5-2 |
| 컨텍스트 | **프로젝트**(있으면 이름) + **업무 목록**(`id`·제목·상태·기한·유형명) + **사람 안건**(`id`·제목·순서) | DEC-003 §8 L149 |
| 무소속 회의 | 프로젝트가 없으면 **프로젝트 없는 업무**를 준다 | DEC-003 §4 L98 |
| 화이트리스트 | 위 업무 목록의 `id` 집합. 이후 배치의 `taskId` 검증 기준 | DEC-003 §7 · M-15 |

**배치 입력**(회차마다 — `resume` 으로 같은 세션)

| 항목 | 내용 |
|---|---|
| 미처리 구간 | 직전 성공 배치 이후의 확정 발화 블록 전량(실패·폐기 구간 포함 — 커서는 `meeting_batch_run` 의 성공분 `to_transcript_id`) — `{id, speakerLabel, atMs, endMs, content}` |
| 사람 안건 | 현재 사람 안건 전량(`id`·제목·`state`). 회의 중 새로 만든 것이 포함된다 |
| 사람 줄 | 미처리 구간 동안 사람이 적은 줄(`agendaId`·`kind`·`content`) — **읽기 전용 컨텍스트**. AI 는 이것을 고치지도 제안하지도 않는다(BASE-003 L36 「사람 입력 + 트랜스크립트를 워커로」 · §7-D) |
| AI 안건 | 이 회의의 기존 AI 안건 전량(`id`·제목·`sourceAgendaId`) — 새 줄을 어디에 붙일지 고르는 기준 |
| 업무 화이트리스트 | 웜스타트 목록 + **회의 중 새로 생긴 업무**(다음 배치 컨텍스트에 반영 — DEC-003 §7). 회의 중 생성 경로는 v1 회의록에 없으므로 실질적으로 웜스타트 목록과 같다 |

**배치 출력**(`output_schema` 강제 + 백엔드 재검증 — SYS §외부 연동)

```json
{
  "items": [
    {
      "agenda": { "humanAgendaId": 12 },            // 또는 { "aiAgendaId": 41 } 또는 { "newTitle": "…" } — 정확히 하나
      ...
      "taskId": 45                                  // kind=task 일 때만, 없으면 null
    }
  ]
}
```
...
| 2 | **JSON 스키마** — 구조 · 타입 · enum · 길이 · `agenda` 키 정확히 하나 · `evidence` 구간 범위 · 참조한 `humanAgendaId`/`aiAgendaId` 가 이 회의의 것 | **배치 전체 폐기**(`status='discarded'`). 행을 하나도 넣지 않는다. 구간은 다음 배치로 | DEC-003 §7 「JSON 스키마 위반」 · M-16 |
| 3 | `taskId` 가 화이트리스트 안인가 · `kind=task` 인데 `taskId` 가 없는가 | **그 줄만** `taskId` 를 떼고 `kind='action'` 으로 강등. 본문·상세·근거는 살린다 | DEC-003 §7 「없는 업무 참조」 · M-15 |
| 4 | 적재 | `newTitle` 은 AI 안건 신설(`sourceAgendaId:null`). `humanAgendaId` 는 그 사람 안건을 미러하는 AI 안건이 없으면 만든다(`sourceAgendaId` = 그 id, 제목 복사). 줄은 `track='ai'` INSERT 만 — 기존 AI 줄 UPDATE 없음. `status='succeeded'` + `seq` 증가 → 같은 트랜잭션 커밋 후 WS push | M-5-a · M-6 · M-7 · M-6-a |
...
| JSON 스키마 위반 | `status='discarded'`. 행 0건. 구간을 다음 배치로 | **표시 없음** | — | DEC-003 §7 「JSON 스키마 위반 — 전체 폐기, 부분 파싱 금지」 |
| 화이트리스트 밖 `taskId` · `task` 인데 `taskId` 없음 | 그 줄만 `action` 으로 강등, 본문 유지 | 강등된 줄이 액션 줄로 보인다. 표시 없음 | AI 탭 | DEC-003 §7 「없는 업무 참조」 |
...
sequenceDiagram
    ...
    Note over FE,BE: 회의 시작 직후 (status=recording)
    ...
    Note over BE,Q: 배치 — 600자 / 안건 전환 / 180초 상한
    BE->>Q: submit(resume=session) + 미처리 구간 + 사람 안건·줄 + AI 안건 + 화이트리스트
    Q-->>BE: JSON (output_schema)
    alt 스키마 위반
        BE->>BE: 배치 폐기(discarded) — 구간은 다음 배치로
    else 정상
        BE->>BE: taskId 화이트리스트 검증(밖이면 action 강등) → AI 안건·줄 INSERT
        BE-->>FE: {type:ai.batch, seq, agendas, lines}
...
- AI 트랙은 **증분만** 쌓인다. 회의 중에는 어떤 AI 안건·줄도 수정·삭제되지 않는다(M-7). 전체 재정리는 종료 후 최종 배치(SPEC-008)
...
| 경과 시간 | `now − recordingStartedAt` (클라이언트 계산) | `recordingStartedAt` = `/start` 시각. **ERD 변경 필요** — `meeting.recording_started_at`(§7-C). `atMs` 오프셋의 기준점(M-11) |
...
| 안건 우측 시각 | 그 안건 줄의 `min(createdAt)` (클라이언트 파생) | 줄 없으면 숨김 |
| 줄 라벨 · 본문 · 시각 | `meeting_line.kind` · `content` · `created_at` | 라벨 색은 [09] |
| AI 줄 상세 · 근거 칩 | `meeting_line.detail` · `evidence[{fromMs,toMs}]` | 칩 시각 = `recordingStartedAt + ms` |
| AI 업무 줄의 유형 배지 | `meeting_line.task_id` → `task.work_type` (상세·WS 응답의 `task` 요약) | 화이트리스트 통과분만 |
| AI 안건 배지 / 「AI 안건」 캡션 | `meeting_agenda.source_agenda_id` → 사람 안건 `state` / `null` | **ERD 변경 필요**(§7-C) |
| 「배치 n회 반영」 | WS `ai.batch.seq` · 진입 시 `ready.latestBatchSeq` · 상세 응답 `latestBatchSeq`(= `meeting_batch_run` 성공분 최대 `seq` — 파생, 컬럼 없음 M-6-a) | 「종결」 표시는 SPEC-008 |
| AI 탭 미확인 점 | 클라이언트 | — |
...
- **배치 트리거 — 수치는 DEC-003 §STT L164 확정값이다.** 셋 중 먼저 오는 것이 배치를 낸다 ① 미처리 확정 발화 **600자** ② **안건 전환(`state:"active"` 성공)** 즉시 — 단 미처리가 **80자 미만이면 생략** ③ 미처리 구간이 생긴 뒤 **180초** 경과. **회의당 AI 세션 하나 — 동시 실행 금지**: 배치가 도는 중에 트리거가 와도 새로 내지 않고, 끝난 뒤 다음 트리거 때 커진 구간으로 낸다. 배치 하나의 상한은 **120초**(넘으면 실패 처리)
- **배치는 `resume` 으로 같은 세션을 이어 쓴다**(M-12 · BE §5-2). 웜스타트가 세션을 만들고 결과는 버린다. 실행 옵션은 한 빌더에서만 만든다
- **AI 는 `track='ai'` 에만 INSERT 한다.** 사람 안건·줄은 **읽기 전용 컨텍스트**로만 준다(BASE-003 L36·L38 · DEC-003 §4). 줄 제안·사람 줄 수정·사람 안건 생성 어느 것도 없다
- **검증은 두 층이다** — `output_schema` 로 강제하고, 받은 JSON 을 백엔드가 다시 검증한다(SYS §외부 연동). 스키마 위반은 **전체 폐기**, 화이트리스트 위반은 **줄 단위 강등**. 그 밖의 예외(워커 프로토콜 오류 등)는 `except Exception` 없이 전파한다(BE §8-1)
...
- **트랜잭션** — 배치 결과 적재(안건·줄 INSERT + `batch_run` 갱신)는 한 트랜잭션이고, **codex·Soniox 호출 중에는 트랜잭션을 열어 두지 않는다**(BE §7)
...
- [ ] 프롬프트에 그냥 치고 `Enter` 하면 활성 안건 아래 **논의** 줄이 붙고 안내 바에 「자동 저장 · HH:MM」이 뜬다
- [ ] `/` 팝오버에 **논의 · 결정 · 업무 · 액션 4종**이 있고, 「업무」로 적은 줄은 라벨 `#5F6470` 이며 **배지·버튼이 없다**
- [ ] `/` → 「새 안건」으로 만든 안건이 **바로 「논의 중」**이 되고 이전 안건은 「대기」가 된다
- [ ] 안건 체크를 누르면 「완료」 + 취소선, 다음 「대기」 안건이 「논의 중」이 되고 프롬프트 칩이 바뀐다
- [ ] 회의 중 사람 줄을 클릭해도 **편집되지 않는다**
- [ ] 확정 발화가 600자를 넘으면 잠시 뒤 「AI 요약」 탭 옆에 **파란 점**이 켜지고, 열면 「**배치 1회 반영** · HH:MM」과 **안건 > 줄 트리**(카드 아님)가 보인다
- [ ] 회의록 탭에는 **AI 안건·AI 줄이 나타나지 않는다**
- [ ] 두 번째 배치 뒤 「배치 2회 반영」이 되고 **기존 AI 줄은 그대로**, 새 줄만 늘었다
- [ ] AI 줄 hover 화살표 → 펼침에 상세와 「HH:MM – HH:MM」 칩이 있고, 칩을 누르면 우 패널이 스크립트로 바뀌며 해당 블록이 `#F4F5FF` 로 강조된다. `Esc` 로 풀린다
- [ ] 웜스타트 목록에 없는 업무를 AI 가 참조한 줄은 **액션 줄로** 보이고 본문은 남아 있다(서버 로그로 `discarded`/강등 확인)
...
| **배치 트리거의 동시 실행·타임아웃** | DEC-003 §STT L164 수치 그대로 + 「배치 중 트리거는 다음으로 미룬다」 |
...
| `meeting_agenda.source_agenda_id bigint NULL` (self FK) | `track='ai'` 안건이 미러하는 **사람 안건** | 안건이 트랙별로 갈렸으므로(M-5-a) AI 가 사람 안건에 맞춰 채우려면 AI 트랙에 그 안건의 사본이 있어야 하고, 어느 사람 안건인지 연결이 있어야 한다. 배지 표시(U-4)와 종료 후 통합(SPEC-008 「사람 안건 우선」)이 이 컬럼을 쓴다. `NULL` = AI 신설 안건 |
...
| # | 어디 | 무엇 | 근거 |
|---|---|---|---|
| D-1 | **`database/domains/meeting.md` M-6** | 「사람 줄·사람 안건을 **읽지도** 고치지도 않고」 → 「**고치지도 제안하지도** 않는다」로. **읽기(컨텍스트)는 기획·정책이 요구한다** | BASE-003 L36 「백엔드가 **사람 입력 + 트랜스크립트**를 워커로」 · L38 「사람이 적은 안건이 DB 에 있으니 **컨텍스트로 같이 전달**」 · DEC-003 §4 L96 「사람이 만든 안건을 **컨텍스트로 받아**」 · §8 L149 웜스타트 「미리 작성된 안건」. `grep -n "읽지도" 40-architecture/database/domains/meeting.md` → 1건(M-6). `system/README.md` 흐름 ③ L243 도 「미처리 구간 + 안건 + 화이트리스트」라 **사람 줄**이 빠져 있다 — 배치 입력에 넣었다(§4 AI 배치 계약) |
...
| D-6 | **SPEC-008 근거 칩 · AI 탭** | 종료 후 상세의 근거 칩도 `GET …/transcript` 와 U-6 의 스크롤·하이라이트 규칙을 그대로 쓴다. AI 탭의 「배치 n회 반영」은 최종 배치 후 「**종결**」로 바뀐다(DEC-003 §4 L97) — 그 표시는 SPEC-008 | DEC-003 §4 |
...
| AI 는 AI 탭만 · 제안 없음 · AI 안건은 AI 트랙에만 | BASE-003 L37 · DEC-003 §4 L95~96 · M-5-a · M-6 |
...
| 배치 수치(600자 · 80자 · 180초 · 120초 · 세션 1) | DEC-003 §STT L164 · §7 L141 |
| 웜스타트 컨텍스트 · 무소속 회의 · 화이트리스트 | DEC-003 §4 L98 · §8 L149 · §7 L135 · BE §5-2 · M-15·M-15-a |
| 배치 입력에 사람 안건·줄 컨텍스트 | BASE-003 L36·L38 · DEC-003 §4 L96 (→ 7-D D-1) |
| 실패 3종 처리(배치 실패 · 스키마 폐기 · 강등) | DEC-003 §7 L133~135 · BE §8-3 · M-15·M-16 · BE §12 테스트 6·7 |

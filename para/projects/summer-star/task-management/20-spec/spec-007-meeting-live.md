...
- **AI 는 컨텍스트를 받지 않고 도구로 조회한다**(MF-50 · MF-2). 배치가 넘기는 것은 미처리 발화뿐이다. 사람이 회의 중에 계속 고치므로 스냅샷으로 실으면 낡은 걸 보고 판단한다 — 조회하면 그 순간 최신이다. 사람이 적은 줄을 볼 도구가 없어 AI 가 같은 걸 또 만들던 것(F-10)도 `get_agenda(id)` 로 닫힌다.
...
1. 확정 발화가 1000자를 넘으면 배치가 돈다. AI 는 `list_agendas()` · `get_agenda(id)` 로 지금 안건과 사람이 적은 줄을 보고 요약한다. 결과가 오면 회의록 탭을 보고 있어도 「AI 요약」 탭 옆에 파란 점.
...
### AI 배치 계약 — 백엔드 ↔ open-kknaks 워커 (MF-1 · 2 · 3 · 4 · 50 · 51 · 52 · 53 · 55)
...
| 실패 | 회의는 정상 진행. `ai_session_id` 가 `NULL` 인 동안 배치는 제출되지 않는다(트리거만 평가). 기록·재시도는 **DEC-003 OQ-8** | MF-1 |

**AI 도구 7개**(MCP — 백엔드 API 의 얇은 래퍼 · **전부 조회** · 이 회의·이 계정 범위 — MF-2 · 4)

| 도구 | 주는 것 | 근거 |
|---|---|---|
| `get_meeting()` | 회의 정보 — 제목 · 일시 · 유형 · 프로젝트 | MF-2 |
| `get_account()` | 사용자 정보 | MF-2 |
| `list_agendas()` | 안건 목록 — **사람 안건 + AI 안건 둘 다**(`id` · 제목 · `state` · `track` · `sourceAgendaId`). 자기가 앞서 만든 안건을 봐야 또 안 만든다 | **MF-51** |
| `get_agenda(id)` | 안건 상세 — 그 안건에 달린 줄(논의 · 결정 · 액션 · 업무 — `kind` · `content` · `detail` · `taskId`). **사람이 적은 줄을 볼 도구가 없어서 AI 가 같은 걸 또 만들었다**(F-10) — 6개에서 7개가 된 이유 | MF-50 |
| `list_tasks(projectId?)` | 업무 목록(`id` · 제목 · 상태 · 기한 · 유형명 · 프로젝트) | MF-2 |
| `get_task(id)` | 업무 상세 — 할일 · 메모 · 연관 · 일정 | MF-2 |
| `list_work_types()` | 유형 목록 — **이름 · 종류 · 설명**(`work_type.description`, MF-21) | MF-2 · MF-21 |

- 도구 인증은 **회의별 단명 토큰**(codex 실행 옵션의 MCP 헤더 — `system/README.md` §codex 설정). 사용자 세션 JWT 를 주지 않는다. 회의가 끝나면 best-effort 폐기(MF-4). 배치 위치·토큰 인프라는 **DEC-003 OQ-9**
- 못 쓰게 하는 것은 **codex 설정 allow list**(MF-3): `enabled_tools` = 위 7개만 · `features.shell_tool=false` · `web_search="disabled"` · `features.image_generation=false` · `features.apps=false` · `sandbox="read-only"` · 도구별 `approval_mode="approve"`. deny list 는 없다 — 상세는 `system/README.md` §codex 설정
...
| ~~사람 안건 · 사람 줄 · AI 안건 · 화이트리스트~~ | **싣지 않는다.** AI 가 `list_agendas()` → `get_agenda(id)` → (업무를 가리킬 때만) `list_tasks()` · `get_task(id)` · `list_work_types()` 순으로 조회한다 — 조회하면 그 순간 최신이다(MF-50) |
...
```json
{
  ...
  "agendas": [                                      // AI 트랙 전체 — 앞 배치 결과를 포함해 다시 낸다 (MF-53)
    {
      "humanAgendaId": 12,                          // 사람 안건을 미러하면 그 id, AI 신설이면 null
      "title": "개정 대상 섹션 확정",                  // 1~200자 (미러면 사람 안건 제목 그대로)
      "lines": [
        {
...
| 2 | **JSON 스키마** — 구조 · 타입 · enum · 길이 · `evidence` 구간 범위 · 참조한 `humanAgendaId` 가 이 회의의 **사람** 안건(AI 안건 id 를 넣으면 위반 — AI 안건은 매 배치 새로 나온다) | **배치 전체 폐기**(`status='discarded'`). 행을 하나도 넣지 않는다. **AI 트랙은 직전 성공분 그대로.** 구간은 다음 배치로 | DEC-003 §7 「JSON 스키마 위반」 · M-16 · M-7 |
| 3 | **업무 참조 사후 검사** — `taskId` 가 **회의 프로젝트의 업무(무소속 회의면 무소속 업무) 안**인가(검사 시점에 서버가 조회) · `kind=task` 인데 `taskId` 가 없는가 | **그 줄만** `taskId` 를 떼고 `kind='action'` 으로 강등. 본문·상세·근거는 살린다 | DEC-003 §7 「없는 업무 참조」 · M-15 · MF-50 |
| 4 | 회의 중 `payload` · `headline` · `termCorrections` | **버린다**(`null` 로 저장). 폐기 사유가 아니다 | MF-52 |
| 5 | 적재 — **전량 교체** | `track='ai'` 안건·줄을 **한 트랜잭션에서 DELETE 하고** 출력대로 INSERT 한다. `humanAgendaId` 가 있으면 `source_agenda_id` = 그 id(제목은 사람 안건 것을 복사), 없으면 `NULL` 로 신설. `status='succeeded'` + `seq` 증가 → 커밋 후 WS push(AI 트랙 전체) | M-5-a · M-5-b · M-7 · M-6-a · MF-53 |
...
| 웜스타트 미완료·실패(`ai_session_id` 없음) | 배치를 제출하지 않는다. 회의는 정상 진행 | **표시 없음** — AI 탭은 「첫 배치를 기다리는 중」 그대로 | — | MF-1 · DEC-003 §7 「웜스타트 미완료·실패」(기록·재시도는 OQ-8) |
...
sequenceDiagram
    ...
    Q->>M: list_agendas() · get_agenda(id) · list_tasks() · get_task(id) · list_work_types()
    M->>BE: REST (회의별 단명 토큰)
...
| AI 안건 배지 / 「AI 안건」 캡션 | `meeting_agenda.source_agenda_id` → 사람 안건 `state` / `null` | 2026-09-06 ERD 반영 |
...
- **배치는 `resume` 으로 같은 세션을 이어 쓴다**(M-12 · BE §5-2). 세션이 앞 발화를 기억하므로 넘기는 것은 이번 구간뿐이다. 실행 옵션은 한 빌더에서만 만든다 — allow list(MF-3)와 단명 토큰 헤더(MF-4)를 그 빌더가 붙인다
- **AI 는 `track='ai'` 에만 쓰고, 읽기는 도구로 한다.** 사람 안건·줄은 배치 입력에 싣지 않고 AI 가 `list_agendas()` · `get_agenda(id)` 로 조회한다(MF-50 · 51 · M-6). 줄 제안·사람 줄 수정·사람 안건 생성 어느 것도 없다
...
- [ ] 확정 발화가 1000자를 넘으면 잠시 뒤 「AI 요약」 탭 옆에 **파란 점**이 켜지고, 열면 「**배치 1회 반영** · HH:MM」과 **안건 > 줄 트리**(카드 아님)가 보인다. 서버 로그에 배치 입력이 **발화뿐**이고 워커가 `list_agendas` · `get_agenda` 를 부른 흔적이 있다
- [ ] 회의록 탭에는 **AI 안건·AI 줄이 나타나지 않는다**
- [ ] 두 번째 배치 뒤 「배치 2회 반영」이 되고 **트리가 통째로 새 결과로 바뀐다** — 1회 때 사람 안건과 별개로 생겼던 「AI 안건」이 2회 때 사람 안건 아래로 합쳐질 수 있다. DB 에서 1회 배치의 AI 줄 id 가 남아 있지 않다
...
- [ ] codex 설정에 `enabled_tools` 7개 · `features.apps=false` · `sandbox="read-only"` · 도구별 `approval_mode="approve"` 가 있고, 워커 로그에 우리 도구 밖의 도구 호출이 0건이다. 도구 호출 헤더의 토큰이 사용자 세션 토큰과 다르다
...
| **MF-1 `/start` 즉시 응답** | §4 웜스타트 표 · §5 — 웜스타트는 `/start` 밖 백그라운드. 워커가 죽어 있어도 회의는 시작된다. 실패 기록은 DEC-003 OQ-8 |
| **MF-2 · 3 · 4 · 55 AI 도구·잠금·토큰·용어** | §4 「AI 도구 7개」 표 · 웜스타트 「주는 것」 · `system/README.md` §codex 설정. 배치 위치·토큰 인프라는 DEC-003 OQ-9 |
| **MF-50 · 51 배치 입력 = 발화만 · `list_agendas()` 는 둘 다** | §4 배치 입력 표 — 사람 안건·줄·AI 안건·화이트리스트를 싣지 않는다. 사후 검사는 남긴다(검증 3) |
...
| # | 어디 | 무엇 | 근거 |
|---|---|---|---|
| D-1 | **`database/domains/meeting.md` M-6** | ~~「읽지도 고치지도」 → 「고치지도 제안하지도」 + 읽기 컨텍스트~~ → **2026-09-07 재정정(MF-50)**: 읽기는 **컨텍스트가 아니라 도구**로 한다. M-6 · `system/README.md` 흐름 ③ · 불변식 4 가 그렇게 바뀌었다. BASE-003 L36·L38 의 「컨텍스트로 같이 전달」은 「AI 가 볼 수 있어야 한다」는 요구이고 전달 방식은 MF-50 이 도구로 정했다 | MF-50 · MF-51 |
...
**이 spec 이 새로 올린 것은 없다.** 2026-09-07 MF 반영으로 걸린 열린 항목은 **DEC-003 OQ-8**(웜스타트 실패 기록·재시도 — `Meeting flow.md` §1-5 ③ 「확정 안 함」)과 **OQ-9**(MCP 서버 배치·단명 토큰 인프라 — §1-5 ①·②)이고, 정책서에 근거와 함께 올렸다. 이 spec 은 확정된 부분(회의는 시작된다 · 세션 없으면 배치 안 돎 · 도구 7개 · allow list · 헤더 토큰)만 계약에 담았다. (S007-OQ-1 은 아래처럼 닫혔다)
...
| 배치 입력 = 발화 하나 · 안건·줄·업무·유형은 도구 조회 | **MF-50** · M-6 (→ 7-D D-1) |

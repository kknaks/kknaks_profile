...
- **상태**
  - *열림*: 56px 한 줄, 배경 `#FBFCFF`, 컨트롤 36px — [종류 셀렉터 136] [이름 입력(유동, 최소 200)] [**설명 입력(유동, 최소 240)**] [색 트리거 56] … [취소] [추가]. **설명은 2026-09-07 MF-21 로 더한 네 번째 필드**다 — 「어떤 업무인지」. 필드 4개라 여전히 인라인이다([10] 「4개 이하면 드로어를 열지 않는다」)
  - *기본값*: 종류는 **선택 없음**(플레이스홀더 「종류」), 색은 팔레트 첫 값이 미리 잡혀 있다, 설명은 빈 값
  - *제출 가능*: 종류가 선택되고 이름이 1자 이상일 때만 「추가」가 활성. **설명은 선택**이다
  - *실패*: 해당 컨트롤 테두리 실패색 + 행 아래 12px 인라인 문구. 입력값은 남는다
- **문구**: 이름 플레이스홀더 「유형 이름 (예: 외부 미팅)」 · 종류 옵션은 「**미팅**」 / 「**업무**」 두 개 · **설명 플레이스홀더 「어떤 업무인지 (예: 혼자 처리하는 실무)」** · 색 트리거 옆 캡션 「색」. 설명은 회의록 AI 가 새 업무의 유형을 고를 때 읽는다(MF-21 · MF-59 — 없으면 이름만 보고 고른다)
...
- **상태**
  - *커스텀 유형*: 행 64px, 행 hover `#FAFBFC`, 행 구분 `#F1F2F5`. [유형 배지] [이름(인라인 편집)] [**설명(인라인 편집 · 13/`#757575` · 비면 「설명 없음」 `#9EA2AE`)**] … [종류 칩] [색 트리거] [삭제]
  - *기본 유형 3종*: 같은 행에 「**기본**」 칩(`#F4F5F7` + 1px `#E1E3E8` · `#5F6470`)이 붙고, **이름은 읽기 전용**(클릭해도 편집으로 바뀌지 않는다), **종류 칩도 읽기 전용**, **삭제 버튼이 없다**. **색 트리거와 설명만 활성**(DEC-001 §4 · A-4 · MF-21). 시드 설명 — 개인 업무 「혼자 처리하는 실무. 개발·수정·확인 등」 · 문서·보고 「산출물이 문서인 것. 기획서·보고서·회의록 정리」 · 미팅·회의 **빈 값**(DEC-003 OQ-11)
...
**팝오버형 컨트롤(색 트리거 등)** — 2026-09-06 확정(검수 G-12)

입력 상자가 아닌 컨트롤은 **아래에 캡션을 넣을 세로 공간이 없다**(색 트리거 56px 버튼 · 목록 행 64px 고정).
그래서 규격을 하나 더 두지 않고 **행 단위 인라인 자리**로 통일한다.

- **실패한 컨트롤 자신**: 테두리 `#E2685B` + 값 유지 (입력 상자와 같다)
- **캡션·「다시 저장」은 그 행 아래 인라인 자리 하나**에 모은다 — 필드마다 자리를 만들지 않는다.
  문구는 「**〈필드 이름〉이 저장되지 않았습니다**」 + 우측 「**다시 저장**」
- 한 행에서 **여러 필드가 실패하면 줄을 늘려** 각각 적는다(자리는 여전히 행 아래 하나)
- **행 높이(64)를 바꾸지 않는다** — 인라인 자리는 실패가 있을 때만 나타나며 U-8 「두 줄로 접히지 않는다」와 부딪히지 않는다

**「화면에 남는다 · 해제 조건 둘뿐 · 자동 재시도 없음」은 컨트롤 종류와 무관하게 같다.**

> **구현 규약** — 실패 상태를 **컴포넌트 내부 state 로 두지 마라.** `saveFailed`·`onRetry` 를 **prop** 으로
> 받고, 소유자는 **행/패널**이다(`rowFailures[id][field]`). 내부 state 로 두면 두 번째 자동 저장 컨트롤이
> 생기는 순간 규격이 샌다 — 실제로 그렇게 샜다(WORK-003 검수 F-1). FE §3-5 가 요구하는 것도 이것이다.
...
{
  "items": [
    { "id": 1, "kind": "meeting", "name": "미팅·회의", "colorToken": "indigo", "description": null, "isDefault": true },
    { "id": 2, "kind": "task",    "name": "개인 업무", "colorToken": "sky",    "description": "혼자 처리하는 실무. 개발·수정·확인 등", "isDefault": true },
    { "id": 4, "kind": "meeting", "name": "외부 미팅",  "colorToken": "mint",   "description": null, "isDefault": false }
  ]
}
```

**`POST /api/work-types`** — `{ "kind": "meeting", "name": "외부 미팅", "colorToken": "mint", "description": "…" }` → `201` 단건(위 항목 형태). `description` 은 선택

**`PATCH /api/work-types/{id}`** — `{ "name": "외부 미팅(신규)" }` · `{ "colorToken": "sky" }` · `{ "description": "…" }`(`null` 로 지움) → `200` 단건. 기본 유형 3종도 `description` 은 바꿀 수 있다(A-4)
...
| `description`(유형만) | 선택. 앞뒤 공백 제거 후 **0~200자**. 줄바꿈 불가. `null` 허용 — 회의록 AI 의 `list_work_types()` 가 그대로 준다(MF-21) |
| 기본 유형 3종 | `name`·`kind` 를 바꾸는 요청과 삭제 요청은 **거부**(A-4). `colorToken`·`description` 은 바꿀 수 있다 |
...
**유형** — `id`(number) · `kind`(`meeting` | `task`) · `name` · `colorToken` · `description`(string | null — 2026-09-07 MF-21) · `isDefault`(boolean)

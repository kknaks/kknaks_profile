...
### U-2. v2 스코프 표시 — 전 영역 공통 규격 (여기서 확정)

**v2 기능의 UI 는 v1 에서 그대로 그린다. 숨기지 않는다**(DEC-001 §v2). 규격을 여기서 한 번 정하고 이후 spec 은 참조만 한다.

- **상태**: 원래 컴포넌트를 **그대로** 그린 위에 한 겹을 얹는다 — 불투명도 `0.45`, 커서 `not-allowed`, 포커스는 받지만 조작은 막힌다. **레이아웃이 바뀌지 않는다**(자리·크기·문구 그대로)
- **문구**: 조작하면 토스트 「**v2에서 제공됩니다**」 — 규격은 `09-design-tokens` 토스트(400×56, `#1E1E1E`, 하단 중앙 60px 위, 4초). **실행취소 버튼을 붙이지 않는다**(되돌릴 동작이 없다)
- **CTA**: 없다. 클릭·`Enter`·`Space` 어느 쪽으로도 **네트워크 요청이 나가지 않는다**
- **기대 결과**: 화면 상태가 바뀌지 않고 토스트만 뜬다. 같은 요소를 연달아 눌러도 토스트는 **하나**만 유지된다(중복 스택 금지)
- **이 spec 의 적용 대상**: 「회사 계정으로 계속하기」 · 하단 3링크 · 설정 메뉴의 「계정 삭제」 · (다음 배치) 「다른 기기 모두 로그아웃」 토글·목소리 패널·연동 관리
...
- **상태**
  ...
  - *세션 없음*: 로그인 화면으로 보낸다
- **문구**: 사이드바 메뉴 8종은 디자인 그대로(홈·채팅·캘린더·내 업무·회의록·자료함·메시지·설정). **홈·채팅은 U-2 v2 게이트**(라우트를 만들지 않는다 — FE §10)
...
| **DEC-001 §v2 스코프 표시 규격**(§B-1 미설계 · 전 영역 공통) | **U-2 에서 확정** — 원본 그대로 + 불투명도 0.45 + 조작 시 토스트 「v2에서 제공됩니다」, 요청 없음, 토스트 중복 없음. 근거: DEC-001 §v2 · FE §9 · `09-design-tokens` 토스트 규격 |

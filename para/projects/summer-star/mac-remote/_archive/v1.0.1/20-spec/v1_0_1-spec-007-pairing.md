...
- 의존: [[v1_0_1-spec-005-websocket-protocol|MRT-SPEC-005]] (페어링 후 진입하는 WebSocket 연결/재연결 플로우)
- 비즈니스 요구: iOS 앱이 Mac을 원격 제어하려면 먼저 어느 Mac 헬퍼에 붙을지 알아야 한다. 사용자가 IP를 외우지 않고도 QR 한 번으로 1:1 연결을 맺을 수 있어야 한다.
- 관련 워크: [[v1_0_1-work-007-pairing-qr|MRT-WORK-007]] (QR 페어링), [[v1_0_1-work-012-settings-ui|MRT-WORK-012]] (설정/수동 입력 UI)
- 범위
...
4. WebSocket 연결 시도 → 성공 ([[v1_0_1-spec-005-websocket-protocol|MRT-SPEC-005]] 연결 플로우 진입)
...
페어링은 WebSocket 연결 이전에 발생하므로 Mac↔iOS 간 **메시지 계약은 없다**. Mac 헬퍼는 연결 정보를 QR/평문으로 노출하는 책임만 지고, 정보 전달 후 [[v1_0_1-spec-005-websocket-protocol|MRT-SPEC-005]]의 WebSocket 연결 플로우로 진입한다.
...
| Work | 범위 |
|---|---|
| [[v1_0_1-work-007-pairing-qr\|MRT-WORK-007]] | QR 코드 생성(Mac) + QR 스캔/파싱(iOS) + 연결 정보 저장 |
| [[v1_0_1-work-012-settings-ui\|MRT-WORK-012]] | 설정 탭 UI + 수동 IP:포트 입력 fallback |

...
- 의존: [[v1_0_1-spec-001-window-list|MRT-SPEC-001]] (창 목록에서 windowId/PID를 조회)
- 비즈니스 요구: 사용자가 Mac을 직접 만지지 않고 iOS에서 원하는 창을 골라 즉시 전환할 수 있어야 한다. 창 목록(Spec-01)이 보여 주기만 한다면, 이 spec이 "골라서 전환"이라는 핵심 조작을 완성한다.
- 관련 work: [[v1_0_1-work-002-window-focus|MRT-WORK-002]] (창 활성화 구현), [[v1_0_1-work-005-websocket-server|MRT-WORK-005]] (§계약 focus/ack 메시지)
- 범위
  - In: PID로 앱 활성화, AXUIElement로 특정 창 raise, 결과 응답(ack)
  - Out: 창 목록 수집([[v1_0_1-spec-001-window-list|MRT-SPEC-001]]), 창 이동/리사이즈
...
| Work | 범위 |
|---|---|
| [[v1_0_1-work-002-window-focus\|MRT-WORK-002]] | 창 활성화 구현 (PID 활성화 + AXRaise + graceful degradation) |
| [[v1_0_1-work-005-websocket-server\|MRT-WORK-005]] | 계약 focus/ack 메시지 (WS 서버) |

...
flowchart TD
    ...
    G1{"게이트 1<br/>생성 문서 결과"}
    G1 -->|거절| X1[("DB 기록")] --> E1(("종결"))
    G1 -->|승인| S1

    subgraph S1["게이트 1 승인"]
    ...
    G2{"게이트 2<br/>concept 보강안"}
    G2 -->|거절| X2[("DB 기록")] --> E2(("종결"))
    G2 -->|승인| S2

    subgraph S2["게이트 2 승인"]
...
| 모달 생성 | — | `queue` 행 | — |
| 비동기 처리 | — | — | — |
| 게이트 1 승인 | `resources/<종류>/*.md` 생성 | `content` 행 (youtube 만) | `<종류> {name}` |
| 게이트 1 거절 | — | 기록 → 종결 | — |
| 게이트 2 승인 | `areas/concept/<영역>/*.md` 수정·추가 | — | `fix/concept - {name}` |
| 게이트 2 거절 | — | 기록 → 종결 | — |

**md 두 곳, DB 두 곳, 커밋 두 번.**
...
- **md 는 승인 시점에 생긴다.** 게이트마다 커밋·푸시가 따로 나간다

...
- **`title` 의 사실 오류** — 고친다. Day52 의 `MySQL기초(DNL, DQL)` 에서 `DNL` 은
  ...
  **철자 오기도 고친다** — Day64 의 `Componet`(→`Component`). `title` 은 검색에 쓰이므로
  틀린 철자로는 그 노트가 안 잡힌다. **본문의 같은 오기는 그대로 둔다.**

  **단, 틀린 것만이다.** Day54 의 `FK키` 는 `Foreign Key 키` 로 겹치는 표현이지만
  **틀린 말이 아니라 그대로 둔다** — 틀린 것을 고치는 것과 표현을 다듬는 것은 다르다.
  가르는 기준: 그 낱말이 **사전에 없거나 다른 것을 가리키면** 고치고, 어색하기만 하면 둔다.
...
- **제목 층위가 내용과 어긋나면 고친다** — `## resultMap` 아래에 있지만 resultMap 의
  하위가 아닌 절(`### forEach 사용하기`)은 `##` 로 올린다. 번호 재정리와 같은 부류다 —
  목차·개요가 거짓 구조를 보여주기 때문이다.
- **꺾쇠로 감싼 낱말이 HTML 태그로 삼켜지면 괄호로 바꾼다** — `<default>` 는 옵시디언에서
  미지 태그로 처리돼 **렌더에서 사라진다.** `(default)` 로 바꾼다. `++강조++` 와 같은
  논리다 — 두면 작성자가 쓴 말이 없어지고, 표기만 옮기면 남는다.
...
`aliases` 는 SoT 분열을 막는 1차 장치지만, **뒤에 제대로 배울 개념의 이름을 미리 물고
있으면 그때 노트를 못 만든다.** `persistence-framework` 가 `ORM`·`객체 관계 매핑` 을
alias 로 넣었다가 뺀 것이 그 사례다 — 남은 `BackendSchool`·`SpringBoot` 노트 20건에
JPA 가 나오므로 `orm` 은 독립 노트가 될 자리다.

기준: **원본이 한 줄로 스치는 개념의 이름은 청구하지 않는다.** 상위 개념 노트에 담되
그 이름은 비워 두고, 구별은 「경계와 오해」 항목으로 남긴다.

`ls inbox/_restored/*/` 를 훑어 **그 말이 뒤에 또 나오는지** 보면 판단이 쉬워진다.
